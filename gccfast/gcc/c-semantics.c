  ...
  if (ELSE_CLAUSE (t))
    {
      ...
      expand_start_else ();
      /* Can be multiple jumps to here from && conditions */
     ...
     tree case_label;
{
  ...
  /* This counter is fallthrough from previous block; may be
     unreachable. */
     ...
     tree t;
{
  while (t && t != error_mark_node)
	...
	case LABEL_STMT:
	  /* Counter for fallthrough from the previous block,
	     if any.  Possibly unreachable. */
