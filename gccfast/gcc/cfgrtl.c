     ...
     basic_block a, b;
{
  ...
  /* Delete the feedback counter note, if present. */
...
/* Currently, during new feedback loop optimization destroys
   edges, hence their probabilities, but BR_PROB notes are
   still there.  Recompute the edge probabilities from the
   BR_PROB notes. */
