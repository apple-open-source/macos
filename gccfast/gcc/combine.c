...
extern int flag_new_combine; /*M.H. 19.03.03 - flag to control if to used or not the improvement in combine*/
     ...
     rtx insn;
{
  ...
  /* APPLE LOCAL degin reenable some lost combines */
  ...
  /* APPLE LOCAL end reenable some lost combines */
  return 0;
}

/*Given an rtx expression return the memory rtx if it exists NULL_RTX otherwise*/
rtx get_mem_expr(rtx exp)
{
    int i;
    const char *format_ptr;
    if (!exp) return NULL_RTX;
    format_ptr = GET_RTX_FORMAT (GET_CODE (exp));
    for (i=0; i < GET_RTX_LENGTH (GET_CODE (exp)); i++)
      if ((*format_ptr++) == 'e')
      {
        if (GET_CODE(XEXP (exp, i)) == MEM)
          return XEXP (exp, i);
        else
          get_mem_expr(XEXP (exp, i));
     }
     return NULL_RTX;
}
     ...
     int *new_direct_jump_p;
{
  ...
  rtx link;
  int i;
  int move_i2_after_i3 = 0;/*M.H. 13.03.03 - Set when we want to move i2 after i3 instead of making a parallel*/
  ...
  if ((FIND_REG_INC_NOTE (i2, NULL_RTX) != 0
    ...
    /*M.H.      
     Instead of trying to generate a parallel of i2 and i3 (to keap the side effect of i2)
     we can move i2 after i3.
     We want to be conservative when moving i2 after i3 so we do
     several checks:    
          1. do this only in the case we are combining two instructions (i1 == 0).
          2. Make sure i3 and i2 are single sets.
          3. No anit-dependancy between i2 and i3.
          4. No use for i2 dest between i2 and i3.
    */
    if (flag_new_combine && added_sets_2 && !i1 && PREV_INSN(i2) && (i3 != this_basic_block->end)
        && GET_CODE (i3) == INSN && GET_CODE (PATTERN (i3)) == SET
        && GET_CODE (SET_DEST (PATTERN (i3))) == REG && GET_CODE (i2) == INSN 
	&& GET_CODE(PATTERN(i2)) == SET && GET_CODE (SET_DEST (PATTERN (i2))) == REG
        && ! reg_overlap_mentioned_p (SET_DEST (PATTERN (i3)),PATTERN (i2))
        && ! reg_used_between_p(SET_DEST (PATTERN (i2)), i2 , i3) 
        && !get_mem_expr(SET_SRC (PATTERN (i2)))
        && (get_mem_expr(SET_SRC(PATTERN(i3))) || get_mem_expr(SET_DEST(PATTERN(i3)))))
    {
        move_i2_after_i3 = 1;
    } else /*end M.H.*/
 ...
 validate_replacement:
  ...
  /*M.H. If we are sure about the combine, move i2 after i3*/
  if (move_i2_after_i3)
  {
    rtx crr,nexti3 = NEXT_INSN(i3) , nexti2=NEXT_INSN(i2),previ2 = PREV_INSN(i2);
    int tmp = uid_cuid[INSN_UID (i3)];

    /*remove i2 from its place*/
    SUBST(NEXT_INSN(previ2), nexti2);
    SUBST(PREV_INSN(nexti2) ,previ2);
    /*put i2 after i3*/
    SUBST(NEXT_INSN(i3), i2);
    if (nexti3)
      SUBST(PREV_INSN(nexti3),i2);
    SUBST(NEXT_INSN(i2) , nexti3);
    SUBST(PREV_INSN(i2) , i3);
    /*Correct the CUIDs */
    for (crr = nexti2 ; crr != i2; crr = NEXT_INSN(crr))
    {
      int new_cuid = uid_cuid[INSN_UID(crr)] - 1;
      SUBST_INT(uid_cuid[INSN_UID(crr)] , new_cuid);
    }
    SUBST_INT(uid_cuid[INSN_UID (i2)] ,  tmp);
  }/*end M.H. */
...

  {
    ...
    /*M.H.  after moving i2 we want to update the LOG_LINKS, to catch additional combine cases*/
    if (move_i2_after_i3) {
      distribute_links (gen_rtx_INSN_LIST (VOIDmode, i2, NULL_RTX));
    }
    ...
    else if (!move_i2_after_i3) /*M.H. not in case we kept i2 and just moved it*/
      {
	PUT_CODE (i2, NOTE);
    ...
    if (move_i2_after_i3)
    {   /*M.H. in case that i2 is after i3 */
        if (i3notes)
          distribute_notes (i3notes, i3, i2,  NULL_RTX,
                elim_i2, elim_i1);
    } 
    else if (i3notes)
    ...
    {   /*M.H. in case that i2 is after i3 */
        if (i2notes)
          distribute_notes (i2notes, i2, i2,  NULL_RTX,
                elim_i2, elim_i1);
    } 
    else if (i2notes)
    ...
    if (i2dest_in_i2src && !move_i2_after_i3/*M.H. this was alreadycounted */)
      {
    ...
    if (GET_CODE (i2dest) == REG && !move_i2_after_i3 /*M.H. not needeed when we move i2 after i3*/)
      {
	rtx link;
    ...
    if (newi2pat && !move_i2_after_i3/*M.H. not needed when we move i2 after i3*/)
	  ...
	  if (general_operand (true_rtx, VOIDmode)
		...
		x = simplify_gen_unary (NEG, mode,
		       ...
		       && ((reversed = reversed_comparison_code_parts
                           ...
                           != UNKNOWN))
		x = simplify_gen_unary (NEG, mode,
					gen_binary (reversed, mode, cond, cop1),
					mode);
	      else
