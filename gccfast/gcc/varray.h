...
/* Enum indicating what the varray contains.  
   If this is changed, `element_size' and 'uses_ggc' in varray.c 
   need to be updated.  */
...
typedef union varray_data_tag GTY (()) {
  ...
  HOST_WIDEST_INT	  GTY ((length ("%0.num_elements"),
				tag ("VARRAY_DATA_HWINT")))	hwint[1];
  unsigned HOST_WIDEST_INT  GTY ((length ("%0.num_elements"),
				tag ("VARRAY_DATA_UHWINT")))	uhwint[1];
...
#define VARRAY_WIDEST_INT_INIT(va, num, name) \
  va = varray_init (num, VARRAY_DATA_HWINT, name)

#define VARRAY_UWIDEST_INT_INIT(va, num, name) \
  va = varray_init (num, VARRAY_DATA_UHWINT, name)
...
#define VARRAY_WIDEST_INT(VA, N)	VARRAY_CHECK (VA, N, hwint)
#define VARRAY_UWIDEST_INT(VA, N)	VARRAY_CHECK (VA, N, uhwint)
...
#define VARRAY_PUSH_WIDEST_INT(VA, X)	VARRAY_PUSH (VA, hwint, X)
#define VARRAY_PUSH_UWIDEST_INT(VA, X)	VARRAY_PUSH (VA, uhwint, X)
...
#define VARRAY_TOP_WIDEST_INT(VA)	VARRAY_TOP (VA, hwint)
#define VARRAY_TOP_UWIDEST_INT(VA)	VARRAY_TOP (VA, uhwint)
