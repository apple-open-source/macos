...
extern bool x_statement_code_p[MAX_TREE_CODES];

#define STATEMENT_CODE_P(CODE) x_statement_code_p[(int) (CODE)]

#define INIT_STATEMENT_CODES(STMT_CODES)			\
  do {								\
    unsigned int i;						\
    memset (&x_statement_code_p, 0, sizeof (x_statement_code_p));	\
    for (i = 0; i < ARRAY_SIZE (STMT_CODES); i++)		\
      x_statement_code_p[STMT_CODES[i]] = true;			\
  } while (0)
...
extern void genrtl_break_stmt                   PARAMS ((tree));
extern void genrtl_continue_stmt                PARAMS ((tree));
