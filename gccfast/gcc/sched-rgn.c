...
extern int flag_schedule_speculative_cost;
#define MAX_SPECULATIVE_COST (COSTS_N_INSNS(flag_schedule_speculative_cost))

extern int flag_schedule_speculative_profile;
  ...
  if (IS_RGN_ENTRY (bb))
    {
      ...
      prob[bb] = 1.0;
/* APPLE LOCAL begin forbidding over-speculative motion */
      if (flag_schedule_speculative_profile)
      {
        if ((note = find_reg_note (BASIC_BLOCK (BB_TO_BLOCK (bb))->end, REG_BR_PROB, 0)) != NULL)
          prob[bb] = INTVAL (XEXP (note, 0)) * 100.0 / REG_BR_PROB_BASE;
      }
/* APPLE LOCAL end forbidding over-speculative motion */
      return;
    }
     ...
     int trg;
{
  ...
  for (i = trg + 1; i < current_nr_blocks; i++)
      ...
      if (sp->is_valid)
	{
	  ...
	  /* APPLE LOCAL begin forbidding over-speculative motion */
          if (flag_schedule_speculative_profile)
            sp->is_valid = (sp->src_prob >= flag_schedule_speculative_profile);
	  /* APPLE LOCAL end forbidding over-speculative motion */
	}
	...
	for (insn = src_head; insn != src_next_tail; insn = NEXT_INSN (insn))
	  {
	    ...
	    if (!CANT_MOVE (insn)
/* APPLE LOCAL begin forbidding over-speculative motion */
		&& (rtx_cost (PATTERN (insn), 0) <= MAX_SPECULATIVE_COST)
/* APPLE LOCAL end forbidding over-speculative motion */
     ...
     rtx next;
{
  ...
  if (INSN_BB (next) != target_bb
	  ...
	  || CANT_MOVE (next)
/* APPLE LOCAL begin forbidding over-speculative motion */
	  || (rtx_cost (PATTERN (next), 0) > MAX_SPECULATIVE_COST)
/* APPLE LOCAL end forbidding over-speculative motion */
