     ...
     int cond_first_p;
{
  ...
  if (TREE_CODE (lhs) == COND_EXPR)
    clone_rtx_feedback_counter (true_value, lhs);
  if (TREE_CODE (rhs) == COND_EXPR)
    clone_rtx_feedback_counter (false_value, rhs);
  if (TREE_CODE (test) == COND_EXPR)
    clone_rtx_feedback_counter (cond, test);
     ...
     tree expr;
{
  ...
  if (TREE_CODE_CLASS (code) == '1')
      ...
      else if (TREE_CODE (arg0) == COND_EXPR)
	{
	  ...
	  clone_rtx_feedback_counter (arg0, t);
	  return t;
	}
  ...
  switch (code)
    ...
    truth_andor:
	 ...
	 We also do them when feedback is in use; they can change
	 the number of branches emitted, and it is vital that
	 this match in the create and use passes. */
	      ...
	      && TREE_CODE (arg0) != REAL_CST))
	{
      ...
      /* APPLE LOCAL 3233762 */
      /* Treat unsigned >0 as !=0, and unsigned <=0 as ==0, just a little
	 earlier than was done previously.  This allows the foo++ == CONST
	 optimization to work in this case, which is better for some loops.  */
