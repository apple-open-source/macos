		      ...
		      const char *msg)
{
  ...
  expanded_loop = expand_start_loop_continue_elsewhere (!! block, NULL_TREE);
  if (block)
  ...
  if (expr)
    {
      ...
      result = ffecom_make_tempvar ("dowhile", integer_type_node,
				    FFETARGET_charactersizeNONE, -1);
      loop = expand_start_loop (1, NULL_TREE);
    ...
    ffestw_set_do_hook (block, expand_start_loop (1, NULL_TREE));
...
ffeste_R837 (ffelab *labels, int count, ffebld expr)
{
  ...
  expand_start_case (0, texpr, TREE_TYPE (texpr), "computed GOTO statement", 0);
...
ffeste_R1212 (ffebld expr)
{
  ...
  if (labels == NULL)
  ...
  else
    {
      ...
      texpr = ffecom_expr (expr);
      expand_start_case (0, texpr, TREE_TYPE (texpr), "CALL statement", 0);
