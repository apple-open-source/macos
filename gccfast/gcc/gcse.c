...
struct occr
{
  ...
  rtx insn;
  /*Haifa: Added to hold the original expr in the occr, in the hash table we may have the equivelent expression.*/
  rtx expr;
  /*Haifa: the index of the basic block this occurance is in*/
  int bb_index;
...

/*Haifa: From toplev.c */
extern int flag_gcse_after_ra_case1;
extern int flag_gcse_after_ra_case2;
extern int flag_gcse_after_ra_parial;
extern int flag_gcse_after_ra_hot_fraction;
extern int flag_gcse_after_ra_partial_fraction;
extern int flag_gcse_after_ra_debug;
extern int flag_gcse_after_ra_limit;
extern int flag_gcse_mem_alias_after_ra;
extern int flag_load_after_store;
extern int flag_gcse_mem_alias;
extern int flag_gcse_loop_depth;
extern int gcse_max_deletes;
extern char *gcse_mem_alias_func;
/*Haifa: to enalbe alias analysis only in the pre.*/
static int pre_gcse_pass = 0;

/*From alias.c */
int rtx_equal_for_memref_p    PARAMS ((rtx, rtx));
...
static int pre_expr_reaches_here_p PARAMS ((basic_block, struct expr *,
					    basic_block));
static void insert_insn_end_bb	PARAMS ((struct expr *, basic_block, int, struct occr*));
...
static int delete_null_pointer_checks_1 PARAMS ((unsigned int *,
						  sbitmap *, sbitmap *,
						  struct null_pointer_info *));
static rtx process_insert_insn	PARAMS ((struct expr *, struct occr *));
...
/*Haifa load-after-store and after RA gcse*/
static int edge_reaches_here_p_work PARAMS((edge, int, char *));
static int edge_reaches_here_p PARAMS((edge, int bb_to));
static void pre_edge_loop_depth_check PARAMS((struct edge_list *, struct expr **));
static int bb_has_one_predecessor PARAMS((basic_block));
static int bb_is_single_successor PARAMS((basic_block));
static int bb_has_one_ancient_predecessor PARAMS((basic_block));
static struct occr* get_bb_avail_insn PARAMS((basic_block, struct occr *occr));
static rtx eliminate_load_single_successor PARAMS((basic_block, rtx, struct expr*, int*));
static rtx eliminate_load_one_predecessor PARAMS((basic_block, rtx, struct expr*, int*));
static rtx eliminate_load_one_ancient_predecessor PARAMS((basic_block, rtx, struct expr*, int*));
static void gcse_after_ra PARAMS((void));
static void hash_scan_set_after_ra PARAMS((rtx, rtx, struct hash_table *));
static void compute_hash_table_after_ra PARAMS((struct hash_table *));
void gcse_after_ra_main PARAMS((rtx, FILE* ));
static int reg_used_between_after_ra_p PARAMS((rtx, rtx, rtx));
static int reg_set_between_after_ra_p PARAMS((rtx, rtx, rtx));
/*Haifa end*/
  ...
  for (i = 0; i < table->size; i++)
      ...
      for (expr = table->table[i]; expr != NULL; expr = expr->next_same_hash)
	{
	  ...
	  if (transp)
	  {
	    ...
	    /*Haifa: if it is a memory check if there are different occurancies and check them*/
	    if (GET_CODE(expr->expr) == MEM)
	    {
	    	struct occr *occr;
                for (occr = expr->antic_occr; occr != NULL; occr = occr->next)
                    if (!expr_equiv_p(XEXP(occr->expr,0), XEXP(expr->expr,0)))
                        compute_transp(XEXP(occr->expr,0),indx,transp,table->set_p);
...
/*Replace the settin insn*/
static void
replace_one_set (regno, old_insn,new_insn)
     int regno;
     rtx old_insn;
     rtx new_insn;
{
     ...
     int *do_not_record_p;
{
 ...
 repeat:
  ...
  switch (code)
    {
    case REG:
      if (mem_ref && flag_gcse_mem_alias && pre_gcse_pass &&
          (!gcse_mem_alias_func || !strcmp(gcse_mem_alias_func,current_function_name)))
     ...
     rtx x, y;
{
  ...
  switch (code)
	...
	return 0;
      if (flag_gcse_mem_alias && pre_gcse_pass &&
          (!gcse_mem_alias_func || !strcmp(gcse_mem_alias_func,current_function_name)))
     ...
     struct hash_table *table;
{
  ...
  if (antic_p)
      ...
      else
	{
	  ...
	  antic_occr->insn = insn;
          antic_occr->expr = x;
          antic_occr->bb_index = BLOCK_NUM(insn);
  ...
  if (avail_p)
      ...
      else
	{
	  ...
	  avail_occr->insn = insn;
          avail_occr->expr = x;
          avail_occr->bb_index = BLOCK_NUM(insn);
     ...
     int is_libcall_result;
{
 ...
 else if (flag_load_after_store && (GET_CODE(src) == REG) && GET_CODE(dest) == MEM)
    {
     ...
     rtx from, to, insn;
{
  ...
  /* Do not replace a hard reg with a pseudo.  This is made Apple specific
     because only the magic R12 reg requires it, but is probably harmless
     in general. */
  if ( GET_CODE (from) == REG && REGNO (from) < FIRST_PSEUDO_REGISTER
       && GET_CODE (to) == REG && REGNO (to) >= FIRST_PSEUDO_REGISTER)
    return 0;
#endif
  /* APPLE LOCAL end use R12 for indirect calls */
...

static int
edge_reaches_here_p_work(edge_from,bb_to , visited)
     edge edge_from;
     int bb_to;
     char *visited;
{
    edge succ;
    if (!edge_from || edge_from->dest == EXIT_BLOCK_PTR)
        return 0;
    if (edge_from->dest->index == bb_to)
        return 1;
    if (visited[edge_from->dest->index])
        return 0;
    visited[edge_from->dest->index] = 1;
    for (succ = edge_from->dest->succ;succ != NULL; succ = succ->succ_next)
        if (edge_reaches_here_p_work(succ,bb_to,visited))
            return 1;
    return 0;
}
/*
 Check if a BB_TO is reachable from EDGE_FROM
 */
static int
edge_reaches_here_p(edge_from, bb_to)
     edge edge_from;
     int bb_to;
{
  int rval;
  ...
  rval = edge_reaches_here_p_work (edge_from, bb_to, visited);
...
static rtx
process_insert_insn (expr,occr)
     struct expr *expr;
     struct occr *occr;
{
  rtx reg = expr->reaching_reg;
  rtx exp = copy_rtx(occr->expr);
...
static void
insert_insn_end_bb (expr, bb, pre,occr)
     struct expr *expr;
     basic_block bb;
     int pre;
     struct occr *occr;
{
  ...
  pat = process_insert_insn (expr,occr);
...
#define FREQUENCY_THRESHOLD_FACTOR_PRE_EDGE 10
/*
  For all the edge we want to insert copies of expressions
  Check loop depth difference between the edge on which we want to insert
  The copy and the basic block where the reduadant expression is.
  if the loop depth on the edge is higher than the loop depth of basic block
  of the original expression , clear the pre_delete_map , so we don't delete
  the expression in pre_delete , and we don't make a copy inside a higher loop depth.*/
static void
pre_edge_loop_depth_check (edge_list, index_map)
     ...
     struct expr **index_map;
{
  ...
  for (e = 0; e < num_edges; e++)
    {
      int indx;
      basic_block 	bb_e_pred = INDEX_EDGE_PRED_BB (edge_list, e),
      			bb_e_succ = INDEX_EDGE_SUCC_BB (edge_list, e) ;
	    ...
	    if ((insert & 1) != 0 && index_map[j]->reaching_reg != NULL_RTX)
	      {
			...
			for (occr = expr->antic_occr; occr != NULL; occr = occr->next)
			{
				basic_block bb_occr = BLOCK_FOR_INSN (occr->insn);
				if (bb_e_pred->loop_depth > bb_e_succ->loop_depth)
					e_loop_depth = bb_e_succ->loop_depth;
				else
					e_loop_depth = bb_e_pred->loop_depth;
				if (bb_occr->loop_depth < e_loop_depth)
					RESET_BIT (pre_delete_map[bb_occr->index], j);
          	}
          }
     }
	}
		...
		for (occr = expr->antic_occr; occr != NULL; occr = occr->next)
		  {
		    ...
		    if (!TEST_BIT (inserted[e], j) &&
                        edge_reaches_here_p (eg, occr->bb_index))
		      {
			rtx insn;
			...
			if ((eg->flags & EDGE_ABNORMAL) == EDGE_ABNORMAL)
			  insert_insn_end_bb (index_map[j], bb, 0,occr);
			else
			  {
			    insn = process_insert_insn (index_map[j],occr);
...
{
    rtx new_set;
  new_insn = emit_insn_after (gen_move_insn (copy_rtx(SET_DEST (set)),reg), insn);
  new_set = single_set (new_insn);
  if (!new_set)
    abort();
 SET_DEST(set) = copy_rtx(SET_SRC(new_set));

  /* Keep register set table up to date.  */
  if (GET_CODE(SET_DEST(new_set)) == REG)
    replace_one_set(REGNO(SET_DEST(new_set)),insn,new_insn);
  record_one_set (regno, insn);
}
#endif
  gcse_create_count++;
	...
	for (occr = expr->antic_occr; occr != NULL; occr = occr->next)
	  {
	    ...
	    if (TEST_BIT (pre_delete_map[bb->index], indx) &&
                (!gcse_max_deletes || deletes_so_far <= gcse_max_deletes))
	      {
		set = single_set (insn);
		if (! set)
		  abort ();
...
pre_gcse ()
{
  ...
  /*First remove deletes which cause copy of instructions to deaper loop best*/
  if (flag_gcse_loop_depth)
  	pre_edge_loop_depth_check(edge_list, index_map);
...
{
  int changed = 0;
  /*pre_gcse_pass = 1;/*M.H. only for debug*/
  gcse_subst_count = 0;
  gcse_create_count = 0;
/*    init_alias_analysis (); M.H.*/
  alloc_hash_table (max_cuid, &expr_hash_table, 0);
  ...
  pre_gcse_pass = 0 /*Haifa: to enalbe alias analysis only in the pre.*/;
  return changed;
}

	      ...
	      for (j = 0; j < domby_len; j++)
		{
		  ...
		  if (hoist_expr_reaches_here_p (bb, i, dominated, NULL))
			...
			expr->reaching_reg
		      ...
		      if (!insn_inserted_p)
			{
			  insert_insn_end_bb (index_map[i], bb, 0,occr);
			  insn_inserted_p = 1;
			}
		    }
		}
     ...
     struct expr * expr;
{
  ...
  if ((mem_ptr = find_rtx_in_ldst (expr->expr)))
      ...
      for ( ; list != NULL_RTX; list = XEXP (list, 1))
	{
	  ...
	  if (expr->reaching_reg == src || INSN_DELETED_P(insn))
	    continue;
...
/**********
 * Haifa:
 * The following functions implement the after RA gcse,
 * the purpose of this pass is to cleanup redunant loads
 * generated in spilling.
 **********/

/* Nonzero if register REG is set or clobbered in an insn between
   FROM_INSN and TO_INSN (exclusive of those two).  */
static int
reg_set_between_after_ra_p (reg, from_insn, to_insn)
     rtx reg, from_insn, to_insn;
{
	...
	if (GET_CODE(reg) != REG)
		return 1;
	regno = REGNO(reg);
  	if (regno >= FIRST_PSEUDO_REGISTER)/*We are called after register allocation*/
  		abort();
...
/* Nonzero if register REG is used in an insn between
   FROM_INSN and TO_INSN (exclusive of those two).  */
static int
reg_used_between_after_ra_p (reg, from_insn, to_insn)
     rtx reg, from_insn, to_insn;
{
  rtx insn;
  int regno;
...
/*
 * Return  nonzero if the basic block has only one predecessor
 */
static int
bb_has_one_predecessor(bb)
	basic_block bb;
{
	edge pred;
	int pred_count = 0;

  if (!flag_gcse_after_ra_case2)
    return 0;	/* AZ */
  /* return ((pred = bb->pred) && !pred->pred_next); AZ */
	for (pred = bb->pred;pred != NULL; pred = pred->pred_next)
		if (++pred_count > 1)
			return 0;
	if (!pred_count)
...
/*
 * Return nonzero if the basic block BB is a single successor of its predecessors.
 */
static int
bb_is_single_successor(bb)
	basic_block bb;
{
  edge pred;
  if (!flag_gcse_after_ra_case1)
    return 0;
  for (pred = bb->pred;pred != NULL; pred = pred->pred_next)
    if (pred->src->succ->succ_next)
      return 0;
  return 1;
}

/*
 * Return  nonzero if the basic block has only one ancient predecessor, that means
 * 1. It has more than predecessor and,
 * 2. All its perdecessors share the same predecessor.
 */
static int
bb_has_one_ancient_predecessor(bb)
	basic_block bb;
{
	edge pred;
	int pred_index = -1;
     if (bb_has_one_predecessor(bb))
     	return 0;

	for (pred = bb->pred;pred != NULL; pred = pred->pred_next)
	{
		if (!bb_has_one_predecessor(pred->src))
			return 0;
		if ((pred_index != -1) &&(pred_index != pred->src->pred->src->index))
			return 0;
		pred_index = pred->src->pred->src->index;
	}
     return 1;
 ...
 */
static int
bb_has_well_behaved_predecessors(bb)
	basic_block bb;
{
  edge pred;
  if (!bb->pred)
...
/*
 * Search for for the occurance of expression in BB.
 */
static struct occr*
get_bb_avail_insn(bb,occr)
	basic_block bb;
	struct occr *occr;
{

	for (;occr != NULL; occr = occr->next)
          if (occr->bb_index == bb->index)
          	return occr;
	return NULL;
}

static void
recompute_cuid_for_insn(new_insn)
	rtx new_insn;
{
	int i,n;
	rtx insn;
	return ;
  	if (max_uid >= INSN_UID(new_insn))
  		return;
  	max_uid = get_max_uid ();
  	n = (max_uid + 1) * sizeof (int);
  	uid_cuid = (int *) grealloc ((char*)uid_cuid,n);
	for (insn =new_insn , i = uid_cuid[INSN_UID(PREV_INSN(new_insn))] + 1; insn; insn = NEXT_INSN (insn))
	{
		if (INSN_P (insn))
			uid_cuid[INSN_UID (insn)] = i++;
		else
  	...
  	max_cuid = i;
  	n = (max_cuid + 1) * sizeof (rtx);
  	cuid_insn = (rtx *) grealloc ((char*)cuid_insn,n);
	for (insn =new_insn , i = uid_cuid[INSN_UID(PREV_INSN(new_insn))] + 1; insn; insn = NEXT_INSN (insn))
    		if (INSN_P (insn))
      		CUID_INSN (i++) = insn;
}
...
/*
 * Recompute the mapping between UID to CUIDs for a single basic block,
...
recompute_cuids_for()
{
	...
	max_uid = get_max_uid();
  	n = (max_uid + 1) * sizeof (int);
  	uid_cuid = (int *) grealloc (uid_cuid,n);
...
/* Perform partial GCSE pass after array , try to eliminate redundant loads
 * that caused by the reload pass.
 * We try to catch two cases:
 * 1. several stores feeding a load.
 * 2. A single store feeding several loads.
  Algorithm:
	1. For each basic block build the available memory hash table, as follows:
		1. For each load/store instruction in the basic block
			1. If the loaded/stored memory didn't change till basic block end
			   add this memory expression to the hash table.
	(Catching CASE I)
   	2. For each basic block find redundant loads and eliminate them if possible, as follows:
		1. Foreach load instruction int the basic block do:
			1. Check if the loaded memory is not changed since the BB beginning.
			2. Check if the destination register is not used nor killed from the BB start
			   till this instruction.
			3. If both conditions hold do the following:
				1. Foreach predecessor BB do
					1. check if the predecessors have a single successor.
					   if not stop , the load cannot be eliminated.(see NOTE 1 bellow).
					2. Get the hash table entry for the memory expression we are loading.
					3. If the entry is empty (no available loads/stores to that memory ,
					   the load is not fully redundant)stop the search (failure).
					4. If the entry is not empty check if the loaded register is not used
					   nor killed  between the BB end and the available load/store.
						1. If so record the load/store instructions of the memory.
						   At the end will add copies right after it to the loaded register.
						2. If not stop the search.
		2. If the load could be eliminated - the memory was available in registers
		   in all the predecessors, delete the load instruction and add after each
		   recorded loads/stores a move to the loaded register, from the register already have the
		   memory value.

	(Catching CASE II)
	In the previous case we didn't handle available memory when the BB has several susccessors (2.3.1.1).
	Here we handle a special case of it.

	3. For each basic block find (CASE II) redundant loads and eliminate them if possible, as follows:
		1. For each load instruction in the BB do the following:
			1. if the loaded memory is not anticipatable (memory is written before the load)
			   at BB begin, stop.
			2. if the load has more than one predecessor stop.
			3. Get the memory expression from the predecessor hash table, if not found stop.
			4. If found, check if the loaded/stored register is not used or killed tell the
			   predecessor end, if killed/used stop.
			5. Replace the load by a move from the predecessor loaded/stored register to
			   the loaded register in the current BB.

* NOTE 1:
*	if some predecessor block has more than one successor,
*	must check that the loaded register is not used (available?)
*	going down some path through that successor.
*	Easiest for now is to check if all predecessors have a single successor.
*
* Future enhancement:
...
*	and replace load with a copy from this register to the loaded register.
*/


/*
 * CASE I : several stores feeding a load.
 */
static rtx
eliminate_load_single_successor(bb,insn,expr ,deleted)
	basic_block bb;
	rtx insn;
	struct expr *expr;
	int *deleted;
{
	...
	rtx avail_insn=NULL_RTX,
		avail_reg;
	struct occr *a_occr,*occr,*avail_occrs=NULL;
	rtx dest = SET_DEST(PATTERN (insn));
	*deleted = 0;
	/*Check if the loaded register is not used nor killed from the begining of the block*/
     if (reg_used_between_after_ra_p(dest,PREV_INSN (bb->head),insn))
     	return NULL_RTX;

	/*Make sure that all the predecessors has the memory avaialbe*/
	for (pred = bb->pred; pred ; pred = pred->pred_next)
	{
		if (!(a_occr = get_bb_avail_insn(pred->src,expr->avail_occr)))
			return NULL_RTX;/*All the predecessors must have availabe memory*/
		if (reg_used_on_edge(dest,pred))
			return NULL_RTX;/*All the predecessors must have availabe memory*/
		do {
			avail_insn = a_occr->insn;
			avail_reg = GET_AVAIL_LOAD_STORE_REG(avail_insn);
			/*Make sure we can generate a correct move from register to register*/
			extract_insn(gen_move_insn (copy_rtx(dest),copy_rtx(avail_reg)));
			if (!constrain_operands(1))
		    	  avail_insn = NULL_RTX;
			else /*Check if the loaded register is not used */
				if (!reg_used_between_after_ra_p(dest,avail_insn,
				NEXT_INSN (BLOCK_FOR_INSN(avail_insn)->end)))
			{
				occr = (struct occr *) gmalloc (sizeof (struct occr));
		...
		} while ((a_occr = get_bb_avail_insn(pred->src,a_occr->next)));
		...
		*deleted = 1;
		...
		for (occr = avail_occrs;occr; occr = occr->next)
		{
			...
			if (!(avail_reg = GET_AVAIL_LOAD_STORE_REG(avail_insn)))
				abort(); /*Can't be*/
			recompute_cuid_for_insn(emit_insn_after (gen_move_insn (copy_rtx(dest),copy_rtx(avail_reg)),occr->insn));
			if (flag_gcse_after_ra_debug)
			{
				fprintf (stderr, "GCSE CASE1: generating move from %d to %d after %d\n",
					REGNO(avail_reg), REGNO(dest), INSN_UID (occr->insn));
				fprintf (stderr, "CASE1 PRED:\n");
          		debug_rtx_range(BLOCK_FOR_INSN(avail_insn)->head,BLOCK_FOR_INSN(avail_insn)->end);
          	}
          }
		if (flag_gcse_after_ra_debug)
		{
         		fprintf (stderr, "CASE1 BLOCK:\n");
         		debug_rtx_range(bb->head,bb->end);
         	}
...
/*
 * CASE II : a store feeding several loads.
 */
static rtx
eliminate_load_one_predecessor(bb,insn,expr,deleted)
	basic_block bb;
	rtx insn;
	struct expr *expr;
	int *deleted;
{
	...
	rtx dest = SET_DEST(PATTERN (insn));
	*deleted = 0;
	/*Check if the memory available and get the
	  instruction making it so(load/store)*/
	if (!(a_occr = get_bb_avail_insn(bb->pred->src,expr->avail_occr)))
		return NULL_RTX;
	do {
		rtx avail_reg;
		avail_insn = a_occr->insn;
		...
		/*Check if the previously loaded/stored register is not modified
		  between the load/store tell this load.*/
		if (reg_set_between_after_ra_p(
				avail_reg,avail_insn,
				NEXT_INSN (BLOCK_FOR_INSN(avail_insn)->end))
		    || reg_set_between_after_ra_p(avail_reg,PREV_INSN (bb->head),insn)
		    || reg_killed_on_edge(avail_reg, bb->pred))
			continue;
		/*Make sure we can generate a correct move from register to register*/
		extract_insn(gen_move_insn (copy_rtx(dest),copy_rtx(avail_reg)));
		if (!constrain_operands(1))
	    	  continue;
		*deleted = 1;
		/*The laod could be replaced by a move to register*/
		recompute_cuid_for_insn(emit_insn_before(gen_move_insn(copy_rtx(dest),copy_rtx(avail_reg)),insn));
		if (flag_gcse_after_ra_debug)
		{
        		fprintf (stderr, "GCSE CASE2: generating move from %d to %d after %d\n",
          				REGNO(avail_reg), REGNO(dest), INSN_UID (insn));
          	fprintf (stderr, "CASE2 PRED:\n");
          	debug_rtx_range(bb->pred->src->head,bb->pred->src->end);
          	fprintf (stderr, "CASE2 BLOCK:\n");
          	debug_rtx_range(bb->head,bb->end);
		}
		/*Delete the insn if it is not available in this block
		And mark it for deletion if it is available.*/
		...
		if (a_occr)
		{
			...
			return NULL_RTX;
              	...
              	return insn;
		break;
	} while ((a_occr = get_bb_avail_insn(bb->pred->src,a_occr->next)));
	return NULL_RTX;
}

/*
 * CASE II : a store feeding a load , and between them a several basic blocks.
 *                  --------------
 *				| store/load |
 *				--------------
 *					/\
...
static rtx
eliminate_load_one_ancient_predecessor(bb,insn,expr,deleted)
	basic_block bb;
	rtx insn;
	struct expr *expr;
	int *deleted;
{
	...
	rtx avail_insn = NULL_RTX;
	edge pred;
	basic_block ancient_pred = bb->pred->src->pred->src;
	rtx dest = SET_DEST(PATTERN (insn));
	rtx src = SET_SRC(PATTERN (insn));
	*deleted = 0;
	/*Check if the memory available and get the
	  instruction making it so(load/store)*/
	if (!(a_occr = get_bb_avail_insn(ancient_pred,expr->avail_occr)))
		return NULL_RTX;
	do {
		rtx avail_reg;
                int found;
		...
		/*Check if the previously loaded/stored register is not modified
		  between the load/store tell this load. across basic blocks*/
		if (reg_set_between_after_ra_p(
				avail_reg,avail_insn,
				NEXT_INSN (BLOCK_FOR_INSN(avail_insn)->end))||
				reg_set_between_after_ra_p(avail_reg,PREV_INSN (bb->head),insn))
		...
		if (!constrain_operands(1))
	    	  continue;
		/*Now check in bb immediate predecessors*/
                found = 1;
		for (pred = bb->pred; pred ; pred = pred->pred_next)
               if (reg_set_between_after_ra_p(avail_reg,PREV_INSN(pred->src->head),
	       				NEXT_INSN(pred->src->end))
               	   || expr_killed_p(src, pred->src))
                {
                        found = 0;
               		break;
                }
...
/*
 * CASE IV : several stores feeding a partially redundant load.
 */
static rtx
eliminate_partially_redundant_loads(bb, insn, expr, deleted)
	basic_block bb;
	rtx insn;
	struct expr *expr;
	int *deleted;
{
	...
	*deleted = 0;
	...
	/*Check if the loaded register is not used nor killed from the beginning of the block*/
	if (reg_used_between_after_ra_p (dest, PREV_INSN (bb->head), insn))
     	  return NULL_RTX;
	...
	for (pred = bb->pred; pred ; pred = pred->pred_next)
	{
		...
		for (a_occr = get_bb_avail_insn(pred_bb, expr->avail_occr);
		     ...
		     a_occr = get_bb_avail_insn(pred_bb, a_occr->next))
		  {
			...
			if (!(avail_reg = GET_AVAIL_LOAD_STORE_REG(avail_insn)))
			  abort();
			/* Make sure we can generate a move from register avail_reg to dest. */
			extract_insn(gen_move_insn (copy_rtx(dest), copy_rtx(avail_reg)));
			if (constrain_operands(1)
			    && !reg_set_between_after_ra_p(avail_reg, avail_insn, NEXT_INSN(pred_bb->end))
			    && !reg_killed_on_edge (avail_reg, pred)
			    && !reg_used_on_edge (dest, pred))
               		  break;	/* avail_insn remains non-null */
		    	avail_insn = NULL_RTX;
		   }

		if (avail_insn != NULL_RTX)
		  {
		    npred_ok++;	/* later: consider feedback information */
		    ok_count += pred->count;
	...
	if (npred_ok == 0)	/* no load can be replaced by copy */
	  return NULL_RTX;
	if (ok_count < flag_gcse_after_ra_partial_fraction * not_ok_count)
	  {
	    if (flag_gcse_after_ra_debug)
	      fprintf (stderr, "GCSE CASE4 REJECTED: OK = %d  NOT_OK = %d\n",ok_count, not_ok_count);
	    return NULL_RTX;
	  }
	if (flag_gcse_after_ra_debug)
	  {
	    fprintf (stderr, "GCSE CASE4 ACCEPTED: OK= %d  NOT_OK= %d\n",ok_count, not_ok_count);
	    fprintf (stderr, "CASE4 insn=\n");
	    debug_rtx(insn);
	    fprintf (stderr, "CASE4 BLOCK: bb=\n");
	    debug_rtx_range(bb->head, bb->end);
	  }
	...
	for (occr = avail_occrs; occr; occr = occr->next)
	  {
		...
		pred = occr->pred;
	    	...
	    	/* insert a copy after avail_insn */
	    	/* Set avail_reg to be the register having the value of the memory */
	    	avail_reg = GET_AVAIL_LOAD_STORE_REG(avail_insn);
	    	if (!avail_reg)
	      	  abort(); /*Can't be*/

		insert_insn_on_edge(gen_move_insn(copy_rtx(dest), copy_rtx(avail_reg)), pred);

         	if (flag_gcse_after_ra_debug)
		{
			fprintf (stderr, "GCSE CASE4: generating move from %d to %d on edge from %d to %d\n",
				 REGNO(avail_reg), REGNO(dest), pred->src->index, pred->dest->index);
			fprintf (stderr, "CASE4 PRED AV:\n");
			debug_rtx_range(pred->src->head, pred->src->end);
		}
#if 0
	    	recompute_cuid_for_insn(emit_insn_after (gen_move_insn (copy_rtx(dest),copy_rtx(avail_reg)),after));
#endif
	  }
	...
	for (unoccr = unavail_occrs; unoccr; unoccr = unoccr->next)
	  {
	    ...
	    insert_insn_on_edge(copy_insn (PATTERN (insn)), pred);
       	    ...
       	    if (flag_gcse_after_ra_debug)
	      {
	        fprintf (stderr, "GCSE CASE4: generating on edge from %d to %d a copy of load:\n",
		         pred->src->index, pred->dest->index);
	        fprintf (stderr, "CASE4 PRED NAV:\n");
		debug_rtx_range(pred->src->head, pred->src->end);
	      }
	}
...
/*This is a rewrite of the probably_cold_bb_p function from predict.c*/
static int
gcse_probably_cold_bb_p (bb,hot_fraction)
     basic_block bb;
     int hot_fraction;
{
  if (profile_info.count_profiles_merged
	  ...
	  / hot_fraction))
    return 1;
  if (bb->frequency < BB_FREQ_MAX / hot_fraction)
    return 1;
  return 0;
}

/*
 * Catching cases I and II and removing the redundant loads based on the
 * already build hash table containing availabe loads/stores in the end
 * of basic blocks.
 */
static void
gcse_after_ra()
{
	...
	for (i = 0; i < expr_hash_table.size ; i++)
	{
  	...
  	FOR_BB_BETWEEN (bb, ENTRY_BLOCK_PTR->next_bb->next_bb, EXIT_BLOCK_PTR, next_bb)
		...
		int single_successor = bb_is_single_successor(bb); /*CASE I*/
		int one_predecessor = bb_has_one_predecessor(bb); /*CASE II*/
		int one_ancient_predecessor = bb_has_one_ancient_predecessor(bb); /*CASE II*/
		int well_behaved_predecessors = bb_has_well_behaved_predecessors(bb); /* CASE IV */


          /*Do not try this optimization on cold basic blocks*/
          if (	flag_gcse_after_ra_hot_fraction
          	&& gcse_probably_cold_bb_p(bb,flag_gcse_after_ra_hot_fraction)
          	&& (single_successor || well_behaved_predecessors))
          	continue;

		/*Recompute insn CUIDs for the basic block and its successors*/
		recompute_cuids_for();

          /*Compute hash table of available memory expressions for predecessor blocks*/
		/*for (pred = bb->pred; pred ; pred = pred->pred_next)
		{
			compute_hash_table_after_ra_for_bb(expr_hash_table,pred->src);
		}*/
	  		...
	  		if (GET_CODE (insn) == INSN
	      		...
	      		&& GET_CODE (SET_SRC(PATTERN(insn))) == MEM)
	    		{
	      		...
	      		if (general_operand (src, GET_MODE (src))
	  				...
	  				&& !side_effects_p (src)
		  			/*&& oprs_anticipatable_p (src, insn)*/)
		  		{
		  			/*We now have a load (insn) and an available memory at
		  			  its BB start (expr). We have two cases to check if they
		  			  are possible CASE I and CASE II as described before. */
		    	  		deleted = 0;
		    	  		if (single_successor) /*CASE I*/
						prev_delete = eliminate_load_single_successor(bb,insn,expr,&deleted);
					if (!deleted && one_predecessor) /*CASE II*/
						prev_delete = eliminate_load_one_predecessor(bb,insn,expr,&deleted);
					if (!deleted && well_behaved_predecessors && flag_gcse_after_ra_parial) /*CASE IV*/
						prev_delete = eliminate_partially_redundant_loads(bb,insn,expr,&deleted);
					if (flag_gcse_after_ra_limit && deleted)
					  --flag_gcse_after_ra_limit;
	...
	commit_edge_insertions ();
  	...
  	/*Go over the expression hash table and delete insns with occr->delete_p == 1*/
	for (i = 0; i < expr_hash_table.size ; i++)
	{
		for (expr = expr_hash_table.table[i];
			expr != NULL;
 ...
 *  After RA we are interested in loads/stores only.
 */
static void
hash_scan_set_after_ra (pat, insn, table)
     rtx pat, insn;
     struct hash_table *table;
{
  	rtx src = SET_SRC (pat);
...
/*
 * Create hash table of memory expressions available at end of basic blocks.
 */
#if 1

static void
compute_hash_table_after_ra(table)
     struct hash_table *table;
{
  	unsigned int i;
...
/* DN */
      	...
      	for (insn = current_bb->head;
			...
			if (INSN_P (insn) && GET_CODE(PATTERN(insn)) == SET)
	  		{
	    			if (!find_reg_note (insn, REG_LIBCALL, NULL_RTX))
	    				hash_scan_set_after_ra (PATTERN(insn), insn, table);
	  		}
	}
...
#else
static void
compute_hash_table_after_ra_for_bb(table,bb)
	struct hash_table *table;
	basic_block bb;
{
	rtx insn;
...
/*
 * Main entry point of the GCSE after ra - clean some redunant loads
 * Due to spilling.
 */
void
gcse_after_ra_main(f,file)
	rtx f;
	FILE* file;
{
  	gcse_subst_count = 0;
	...
	if (flag_gcse_mem_alias_after_ra)
     		pre_gcse_pass = 1;
        max_gcse_regno = max_reg_num ();
	...
	alloc_reg_set_mem (max_gcse_regno); /*??? Not needed */
  	/*compute_sets (f);*/
	alloc_gcse_mem(f);
  	alloc_hash_table (max_cuid, &expr_hash_table, 0);
  	/*alloc_rd_mem (last_basic_block, max_cuid); Not needed*/
	compute_hash_table_after_ra(&expr_hash_table);
  	...
  	if (expr_hash_table.n_elems > 0)
    	{
      	/* compute_kill_rd (); ??? Not needed I suppose
      	compute_rd ();????  Not needed I suppose
      	alloc_avail_expr_mem (last_basic_block, expr_hash_table.n_elems);???
      	compute_ae_gen (&expr_hash_table);???
      	compute_ae_kill (ae_gen, ae_kill, &expr_hash_table);???
      	compute_available (ae_gen, ae_kill, ae_out, ae_in);???
