	...
	if (i == 4 && GET_CODE (in_rtx) == NOTE)
	  {
		...
		fprintf (outfile, " [");
#if 0
		/* FIXME: We can't easily do this because print-rtl.c
		   is used by the genxxx programs, which don't
		   include feedback.o, and the
		   endianness may be different between host
		   and target, which screws up the dump.
		   Settle for printing the halves separately
		   for now.  Result may be endian dependent. */
		fprintf (outfile, HOST_WIDEST_INT_PRINT_DEC, 
		    get_fdo_note_count (in_rtx));
#endif
		fprintf (outfile, HOST_WIDE_INT_PRINT_DEC, 
		    NOTE_FDO_COUNT_HIGH (in_rtx));
		fprintf (outfile, "/");
		fprintf (outfile, HOST_WIDE_INT_PRINT_DEC, 
		    NOTE_FDO_COUNT (in_rtx));
		fprintf (outfile, "]");
		break;

	      case NOTE_INSN_FDO_COUNT_TABLEJUMP:
		fputs (" [", outfile);
		if (NULL != NOTE_FDO_TABLEJUMP (in_rtx))
		  {
		    indent += 2;
		    if (XVECLEN (in_rtx, 4))
		      sawclose = 1;

		    for (j = 0; j < XVECLEN (in_rtx, 4); j++)
		      print_rtx (XVECEXP (in_rtx, 4, j));
