...
static void emit_case_nodes		PARAMS ((rtx, case_node_ptr, rtx, 
...
struct nesting *
expand_start_loop_continue_elsewhere (exit_flag, t)
     int exit_flag;
     tree t;
{
  struct nesting *thisloop = expand_start_loop (exit_flag, t);
...
expand_end_loop ()
{
  ...
  /* Do this consistently between feedback creation and use passes. */
  if (etc_note
      && (optimize || flag_create_feedback || flag_use_feedback)
      && eh_regions == 0
      && (debug_blocks == 0 || optimize >= 2 
          || flag_create_feedback || flag_use_feedback)
  ...
  emit_rtx_feedback_counter (loop_stack->t, 2, fdo_outgoing);
     ...
     tree cond;
{
  ...
  emit_rtx_feedback_counter (whichloop->t, 1, fdo_outgoing);
  expand_goto_internal (NULL_TREE, whichloop->data.loop.end_label,
			NULL_RTX);
  emit_label (label);
  emit_rtx_feedback_counter (whichloop->t, 0, fdo_block);
     ...
     tree retval;
{
  ...
  /* When using feedback, this must be done the same way in both passes.
     As the Use pass is probably heavily optimized we will do it.
     Don't test frame_offset in this case as it may not match between passes. */
  if (((optimize && frame_offset == 0)
        || flag_create_feedback || flag_use_feedback)
      && retval_rhs != 0
	  ...
	  || TREE_CODE (TREE_OPERAND (retval_rhs, 2)) == CALL_EXPR))
    {
      ...
      emit_rtx_feedback_counter (retval_rhs, 0, fdo_block);      
      ...
      expand_return (expr);
      emit_label (label);
      emit_rtx_feedback_counter (retval_rhs, 1, fdo_block);
  ...
  if (type == error_mark_node)
	...
	x = gen_rtx_MEM (BLKmode, gen_reg_rtx (Pmode));
      ...
      SET_DECL_RTL (decl, x);
	   ...
	   /* This decision needs to be made the same way
	      when creating & using feedback because it can
	      affect when trees get cloned.  It would be better
	      if the cloning mechanism wasn't sensitive to this... */
...
void
expand_start_case (exit_flag, expr, type, printname, t)
     int exit_flag;
     tree expr;
     tree type;
     const char *printname;
...
check_seenlabel ()
{
  /* Feedback creation puts unexecutable code here sometimes; disable
...
void
expand_end_case_type (orig_index, orig_type, t)
     tree orig_index, orig_type, t;
{
  tree minval = NULL_TREE, maxval = NULL_TREE, range = NULL_TREE;
  rtx default_label = 0;
  rtx new_default_label = 0;
  struct case_node *n;
  unsigned int count;
  rtx index;
  rtx table_label;
  ...
  if (index_type != error_mark_node)
		   ...
		   && TREE_CODE (TREE_OPERAND (index_expr, 1)) == INTEGER_CST))
	{
	  ...
	  if (GET_CODE (index) == CONST_INT
	    ...
	    {
	      unsigned int feedback_label_counter = 0;
      ...
      else
	{
	  if (flag_create_feedback)
	    {
	      /* default_label is jumped to from the out-of-range check.
		 There is no counter for this case; it can be deduced
		 by subtracting the counters before & after the check. */
	      /* new_default is jumped to from any default table entries. */
	      new_default_label = gen_label_rtx ();
	    }
          if (flag_create_feedback || flag_use_feedback)
	    emit_rtx_feedback_counter (t, 0, fdo_block);
	  /*** FIXME: casesi does not handle new feedback ***/
	  ...
	  memset ((char *) labelvec, 0, ncases * sizeof (rtx));
          /* For feedback generation, replace each label with a
	     new one in the table.  We generate jumps to the new 
	     labels, each of which will contain a counter followed 
	     by a jump to the old label.  This avoids problems
	     with other ways the old labels can be reached, such
	     as fallthroughs.
	     For feedback use, we do not do the control flow
	     transformation described above; labelvec2 holds a
	     vector of execution counts, one per table element. */
	  if (flag_create_feedback)
	    {
	      labelvec2 = (rtx *) alloca (ncases * sizeof (rtx));
	      memset ((char *) labelvec2, 0, ncases * sizeof (rtx));
	    }
	  if (flag_use_feedback)
	    {
	      labelvec2 = (rtx *) alloca ((ncases + 1) * 2 * sizeof (rtx));
	      memset ((char *) labelvec2, 0, (ncases + 1) * 2 * sizeof (rtx));
	    }
	  for (n = thiscase->data.case_stmt.case_list; n; n = n->right)
	    {
		...
		= tree_low_cst (fold (build (MINUS_EXPR, index_type,
	      ...
	      if (i_low <= i_high && flag_create_feedback)
		labelvec2[i_low] = gen_label_rtx ();
	      for (i = i_low; i <= i_high; i ++)
		{
		  if (flag_create_feedback)
		    {
		      labelvec2[i] = labelvec2[i_low];
		      labelvec[i] = gen_rtx_LABEL_REF (Pmode, labelvec2[i]);
		    }
		  else
		    labelvec[i] = gen_rtx_LABEL_REF (
			    Pmode, label_rtx (n->code_label));
		}
	    }
	  ...
	  for (i = 0; i < ncases; i++)
		...
		if (flag_create_feedback)
		  {
		    labelvec2[i] = new_default_label;
		    labelvec[i] = gen_rtx_LABEL_REF (Pmode, 
						     new_default_label);
		  }
		else
	  ...
	  if (flag_use_feedback)
	    {
	      ...
	      for (n = thiscase->data.case_stmt.case_list; n; n = n->right)
		{
		  /* We build a table of (label, value) pairs for the
		     tablejump count NOTE.  Some duplicates may survive;
		     this is OK. */
		  HOST_WIDE_INT i_low
		    = tree_low_cst (fold (build (MINUS_EXPR, index_type,
						 n->low, minval)), 1);
		  labelvec2[k++] = labelvec[i_low];
		  labelvec2[k++] = gen_rtx_CONST_INT (DImode,
					    times_arc_executed (t, j++));
		}
	      labelvec2[k++] = gen_rtx_LABEL_REF (Pmode, default_label);
	      labelvec2[k++] =  gen_rtx_CONST_INT (DImode,
				    times_arc_executed (t, j));
	      /* Note goes before the actual tablejump insn so it's in
		 that block. */
	      note = emit_note_before (NOTE_INSN_FDO_COUNT_TABLEJUMP, 
			    get_last_insn ());
	      NOTE_FDO_TABLEJUMP (note) = gen_rtvec_v (k, labelvec2);
	    }
	  ...
	  emit_jump (flag_create_feedback 
	  ...
	  if (flag_create_feedback)
	      ...
	      for (n = thiscase->data.case_stmt.case_list; n; n = n->right)
		{
		  ...
		  if (i_low <= i_high)
		    {
		      emit_label (labelvec2[i_low]);
		      if (labelvec2[i_low] == new_default_label)
			default_emitted = 1;
		      emit_rtx_feedback_counter (t, j++, fdo_none);
		      emit_jump (label_rtx (n->code_label));
		    }
		}
	      /* counter for jumps to default from table */
	      if (!default_emitted)
		{
		  emit_label (new_default_label);
		  emit_rtx_feedback_counter (t, j++, fdo_none);
		  emit_jump (default_label);
		}
	    }
...
emit_case_nodes (index, node, default_label, index_type,
     ...
     rtx default_label;
     tree index_type;
     unsigned int *feedback_label_counter;
     tree t;
{
  ...
  if (node_is_bounded (node, index_type))
    {
      emit_rtx_feedback_counter (t, (*feedback_label_counter)++, fdo_outgoing);
  ...
  else if (tree_int_cst_equal (node->low, node->high))
    {
	 ...
	 this node and then check our children, if any.  */
      ...
      emit_rtx_feedback_counter (t, (*feedback_label_counter)++, fdo_incoming);
	  ...
	  if (node_is_bounded (node->right, index_type))
	    {
	      ...
	      emit_rtx_feedback_counter (t, (*feedback_label_counter)++, 
					fdo_incoming);
