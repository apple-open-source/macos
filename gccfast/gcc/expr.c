     ...
     int *pvolatilep;
{
  ...
  while (1)
	       ...
	       || TREE_CODE (exp) == ARRAY_RANGE_REF)
	{
	  ...
	  if (TREE_CODE (offset) == COND_EXPR)
	    {
	      /* We need a place to attach the counter(s) for this
		 newly generated RTL.  If index was a COND and 
		 old_offset was not, index works.  If old_offset was
		 a COND and index is not, old_offset works.  If both
		 are CONDs, there are now 3 CONDs in the tree instead
		 of the original 2.  We allocated a counter for the
		 parent ARRAY_REF that's used only for this case. 
		 Bleah.  (old_offset will typically itself be a clone
		 in the gnarly cases, but that doesn't matter.) */
		  ...
		  && TREE_CODE (index) == COND_EXPR) 
		{
		  ...
		  /* If we duplicated a counter-generating operand
		     (typically comparison) we need to clone it here
		     and make it point at the parent COND (the
		     counters will come out in the same block). 
		     We can't be sure it's duplicated; it might be
		     far down the other branch.
		     There just is NOT anywhere reasonable to attach this
		     node in the original tree.  */
		  /* Assignment hidden in second line is intentional. */
		  ...
		  /* This could happen in either leg.  FIXME: duplication. */
     ...
     enum expand_modifier modifier;
{
  ...
  switch (code)
	...
	emit_0_to_1_insn (target);
      ...
      /* This is now emitted inside jumpifnot */
      /*emit_rtx_feedback_counter (exp, 0, fdo_outgoing);*/
      emit_label (op1);
	...
	/* Disable this one with feedback; we must take the same path 
	   through here in both passes, and if a call down one of the
	   arms gets inlined, the result of safe_from_p might be
	   different in the second pass.  (It would be desirable to
	   check whether there is such a call.  An attempt at a more general
	   fix, around CALL_EXPR in safe_from_p, broke badly.)  */
	else if (temp
  ...
  for (i = 0; i < nwords; i++)
    {
      ...
      /* FIXME: this doesn't work for sizes bigger than 2 words. */
      if (i==0)
	emit_rtx_feedback_counter (exp, 0, fdo_incoming);
    }
...
/* Like do_compare_and_jump_1 (which it replaces), but insert a
   feedback counter after the first branch in the case where there
   are 2 labels.  This counter will be attached to offset 0 of exp;
   we must call this function only if that exists. */
static void
     ...
     rtx if_false_label, if_true_label;
{
  if (if_true_label && if_false_label)
    ...
    {
      /* One label or the other is null; we don't care which. */
      do_compare_and_jump_1 (exp, signed_code, unsigned_code, 
			  if_false_label, if_true_label);
      emit_rtx_feedback_counter (exp, 0, fdo_block);
    } 
...
try_casesi (index_type, index_expr, minval, range,
     ...
     rtx default_label;
     tree t ATTRIBUTE_UNUSED;	/* for new feedback someday */
{
