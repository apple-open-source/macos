...
/* APPLE LOCAL end 64bit registers, ABI32bit */
      ...
      /* Behavior must match between feedback creation and use. */
...
void
expand_function_end (filename, line, end_bindings, fndecl)
     const char *filename;
     int line;
     int end_bindings;
     tree fndecl;
{
  ...
  if (return_label)
    {
      emit_rtx_feedback_counter (fndecl, 2, fdo_outgoing);
