     ...
     rtx f;
{
  ...
  /* Current execution counter from feedback info.  This is -not-
     cleared after each block, but inherited.  This covers the case
     where one pass jammed two adjacent blocks together and the next
     pass recreates them as two blocks. */
  gcov_type count = ~0;
	...
	case NOTE:
	  {
	    ...
	    else if (kind == NOTE_INSN_FDO_COUNT_BLOCK)
	      count = get_fdo_note_count (insn);
	    break;
	  }
