...
typedef struct inline_data
{
  ...
  tree decl;
  /* Adjust execution counts as inlining progresses. */
  double scale;
  /* Synthesized execution count for the RETURN_LABEL.  */
...
/* APPLE LOCAL moved two decls to tree-inline.h */
/* APPLE LOCAL end callgraph inlining */
     ...
     void *data;
{
  ...
  /* Propagate scaled profiling information.  */
  if (flag_use_feedback)
    {
      int i;
      HOST_WIDEST_INT N;
      /* If we have a profile count for this node, assign a scaled
	 profile count to the duplicate we just created.
	 FUNCTION_DECLs get visited from the CONTEXT field of
	 SAVE_EXPRs (and possibly other places) but these don't
	 represent executable code and should not be scaled. */
      ...
      else
	{
	  switch (TREE_CODE (orig_tree))
	    {
	      /* Caution: Java side guessed-at and wholly untested.  */
#ifdef INLINER_FOR_JAVA
	    case RETURN_EXPR:
#else
	    case RETURN_STMT:
	      ...
	      if (N != -1)
		{
		  /* If walk_tree will duplicate this GOTO, copy the
		     unscaled execution count now and let it be scaled
		     normally during the (useless) duplicaton; if there
		     is no returned expression, walk_tree will not
		     duplicate the GOTO, so we set the scaled execution
		     count now.  */
		  /* Probable best fix: instead of chaining the GOTO
		     off the MODIFY, hang them both under a chain-less
		     COMPOUND_STMT, scale the GOTO exec count
		     normally, invoke walk_tree() on the MODIFY
		     directly, and clear *walk_subtrees so walk_tree
		     won't look at the GOTO.  Uses the same number of
		     tree nodes, but avoids this kludgery.  */
		  if (TREE_CODE (*tp) == GOTO_STMT)
		    /* This is the no-value-returned case, scale now. */
		  ...
		  /* Subtract scaled count of this Return off the count
		     of the exit label, which represents the number of
		     times the label is reached by fallthrough, NOT the
		     number of times the code following it is executed. */
		  if (id->ret_label)
	    ...
	    case PARM_DECL:
	    case RESULT_DECL:
	    case LABEL_DECL:	/* id->ret_label only */
	      break;
	    default:
	      abort();
	    }
...
/* APPLE LOCAL begin callgraph inlining */
/* If *TP is a CALL_EXPR, replace it with its inline expansion.  Don't
   seek or perform any other inlining opportunities, and skip the
   usual inlining candidacy testing.  KLUDGE: this is largely
   duplicated from expand_call_inline(); needs re-factoring.  */
     ...
     void *data;
{
  ...
  /* Expand any inlined calls in the initializers.  Do this before we
     ...
     parameters.  */
  /* expand_calls_inline (&arg_inits, id); *//* KLUDGE */
  ...
  /* Execution count of the RET_LABEL is the number of times it is
     reached from fallthrough, NOT the number of times the code
     following the label is executed.  We start by setting it to
     the count of the callee's epilog, and subtract off the
     count of each Return as we see them.  It is scaled later. */
  if (flag_use_feedback)
    set_times_arc_executed (id->ret_label, 0, 
  ...
  *inlined_body = copy_body (id);
...
#else /* INLINER_FOR_JAVA */
  {
   ...
   /* Sometimes the ret_label is reexamined later and sometimes
      it is not.  To outwit this, scale its count now and do
      not scale if encountered later. */
   set_times_arc_executed (id->ret_label, 0, 
...
/* APPLE LOCAL begin callgraph inlining */
/* Inline this call, and don't look for any other inlining
   opportunities.  Invoked from the callgraph logic.  */
tree *
     ...
     double scale;
{
  ...
  id.tree_pruner = htab_create (37, htab_hash_pointer,
				htab_eq_pointer, NULL);
  /* Replace this CALL_EXPR with the body of its callee.  If
     funit_at_a_time is on, don't seek or perform any other inlining
     opportunities.  */
...
/* Remap anything in the given splay_tree, and copy everything
   else.  */
...
/* APPLE LOCAL begin callgraph inlining */
/* src is an arbitrary tree (not necessarily a complete body); return a
   cloned tree, mapping argument declarations according to the ARG_MAP
   splay_tree.  fn is the surrounding FUNCTION_DECL for context.  */
     ...
     void *arg_map;
{
  ...
  memset (&id, 0, sizeof (id));
  VARRAY_TREE_INIT (id.fns, 2, "fns");
  /* VARRAY_PUSH_TREE (id.fns, clone); *//* KLUDGE */
  VARRAY_PUSH_TREE (id.fns, fn);
  id.decl_map = (splay_tree)arg_map;
