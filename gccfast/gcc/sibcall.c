...
replace_call_placeholder (insn, use)
     ...
     sibcall_use_t use;
{
  rtx old_prev_insn = PREV_INSN (insn);
  rtx i;
  int changed = 0;
  ...
  if (flag_use_feedback)
    {
    /* If multiple basic blocks came in, as with ?: arguments, fix up
       the flow graph.  */
    for (i = NEXT_INSN (old_prev_insn); i != insn; i = NEXT_INSN (i))
      {
	BLOCK_FOR_INSN (i) = bb;
	if (GET_CODE (i) == BARRIER)
	  {
	    edge e = split_block (bb, i);
	    /* The barrier is outside of any block. */
	    bb->end = PREV_INSN (i);
	    bb = e->dest;
	    /* following label already has a block */
	    if (GET_CODE (NEXT_INSN (i)) == CODE_LABEL)
		i = NEXT_INSN (i);
	    bb->count = -1;
	    changed = 1;
	  }
	else if (GET_CODE (i) == JUMP_INSN && any_condjump_p (i))
	  {
	    edge e = split_block (bb, i);
	    bb = e->dest;
	    bb->count = -1;
	...
	else if (GET_CODE (i) == CODE_LABEL)
	  {
	    edge e = split_block (bb, PREV_INSN (i));
	    bb = e->dest;
	    bb->count = -1;
	    changed = 1;
	  }
	/* FIXME: do more checks with INCOMING and OUTGOING. */
	else if (GET_CODE (i) == NOTE 
		 && (NOTE_LINE_NUMBER (i) == NOTE_INSN_FDO_COUNT_BLOCK
		     || NOTE_LINE_NUMBER (i) == NOTE_INSN_FDO_COUNT_INCOMING
		     || NOTE_LINE_NUMBER (i) == NOTE_INSN_FDO_COUNT_OUTGOING))
	  bb->count = get_fdo_note_count (i);
	/* Unconditional branches are expected to have a following
	   barrier, so we don't look for them here. */
      }
    if (changed)
      {
	/* Original block has been broken into a sequential sequence
	   of new blocks.  Edges are all wrong, fix them.  We assume
	   the incoming info is right for the first block and the
	   outgoing info is right for the last block. */
	basic_block last_bb = bb, next_bb;
	for (bb = BLOCK_FOR_INSN (NEXT_INSN (old_prev_insn)); 
	     bb != last_bb; bb = next_bb)
	  {
	    edge e;
	    next_bb = bb->succ->dest;
	    if (bb->count == -1)
	      {
		/* Incoming edges should be complete, sum them. */
		gcov_type sum = 0;
	    ...
	    if (any_condjump_p (bb->end))
	      {
		if (bb->count != -1 && next_bb->count != -1)
		  bb->succ->count = next_bb->count;
		e = make_edge (bb, BLOCK_FOR_INSN (JUMP_LABEL (bb->end)), 0);
		if (bb->count != -1 && next_bb->count != -1)
		  e->count = bb->count - next_bb->count;
	      }
	    else if (any_uncondjump_p (bb->end))
	      {
		remove_edge (bb->succ);
		e = make_edge (bb, BLOCK_FOR_INSN (JUMP_LABEL (bb->end)), 0);
		if (bb->count != -1)
	    ...
	    else if (GET_CODE (bb->end) == CALL_INSN
		     && GET_CODE (NEXT_INSN (bb->end)) == BARRIER)
	      {
		/* The barrier means this call does not return, so we
		   don't want the fallthrough edge. */
		remove_edge (bb->succ);
	      }
	    /* else the block falls through, and edge info is correct */
	    else if (bb->succ->count == -1)
	...
	if (last_bb->count == -1)
	  {
	    ...
	    gcov_type sum = 0;
	    edge e;
	    for (e = last_bb->pred; e; e = e->pred_next)
	      sum += e->count;
	    last_bb->count = sum;
	  }
      }
