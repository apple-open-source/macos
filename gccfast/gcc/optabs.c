...
expand_float (to, from, unsignedp)
     ...
     int unsignedp;
{
  expand_float_tree (to, from, unsignedp, NULL_TREE);
}

/* This version takes the original FLOAT_EXPR as an argument 
   as well (feedback friendly).  There may not always be one;
   the form above is historical and is called from several
   back ends. */

void
expand_float_tree (to, from, unsignedp, t)
     rtx to, from;
     int unsignedp;
     tree t;
{
	  ...
	  if ((significand_size (fmode) + 1)
		...
		target = gen_reg_rtx (fmode);
	      ...
	      emit_cmp_and_jump_insns (from, const0_rtx, LT, NULL_RTX, imode,
				       0, neglabel);
	      if (t)
		emit_rtx_feedback_counter (t, 0, fdo_incoming);
...
void expand_fix (to, from, unsignedp)
     rtx to, from;
     int unsignedp;
{
  expand_fix_tree (to, from, unsignedp, NULL_TREE);
}

void
expand_fix_tree (to, from, unsignedp, t)
     rtx to, from;
     int unsignedp;
     tree t;
{
