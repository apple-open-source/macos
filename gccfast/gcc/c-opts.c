...
#define COMMAND_LINE_OPTIONS						     \
  ...
  OPT("W#warnings",		CL_ALL,   OPT_Wpound_warnings)		     \
  ...
  OPT("Wextra-tokens",		CL_ALL,   OPT_Wextra_tokens)		     \
  ...
  OPT("Wfour-char-constants",	CL_ALL,   OPT_Wfour_char_constants)	     \
  ...
  OPT("Wpragma-once",		CL_ALL,   OPT_Wpragma_once)		     \
  ...
  OPT("fasm-blocks",		CL_ALL,   OPT_fasm_blocks)		     \
     ...
     enum c_language_kind lang;
{
...
#if ENABLE_CHECKING
 {
  ...
  cpp_get_callbacks (parse_in)->set_working_directory = gcc_setpwd;
}
...
/* END APPLE LOCAL IMI */
  ...
  switch (code = option->opt_code)
    ...
    case OPT__save_repository:
      dbg_dir = arg;
      cpp_opts->making_ss = on;
      break;
    /* APPLE LOCAL end Symbol Separation */

    case OPT__output_pch:
      /* APPLE LOCAL begin Symbol Separation */
      cpp_opts->making_pch = on;
      pch_file = arg;
      break;
    ...
    case OPT_Waltivec_long_deprecated:
      warn_altivec_long_deprecated = on;
      break;
    ...
    case OPT_Wnewline_eof:
      cpp_opts->warn_newline_at_eof = on;
      break;
    ...
    case OPT_Wpointer_arith:
      ...
      /* APPLE LOCAL begin -Wpragma-once */
    case OPT_Wpragma_once:
      cpp_opts->warn_pragma_once = on;
      break;
      /* APPLE LOCAL end -Wpragma-once */
    ...
    case OPT_fdisable_typechecking_for_spec:
      disable_typechecking_for_spec_flag = on;
      break;
    ...
    case OPT_fpch_preprocess:
      flag_pch_preprocess = on;
      cpp_opts->pch_preprocess = on;
      break;
...
/* END APPLE LOCAL IMI */
  ...
  if (c_language==clk_cplusplus 
    ...
    {
      flag_create_feedback = flag_use_feedback = 0;
      warning("New profiler NYI for C++");
    }
  /* APPLE LOCAL end */
...
/* BEGIN APPLE LOCAL IMI */
    cpp_preprocess_file (parse_in, in_fnames[0], out_stream);
/* END APPLE LOCAL IMI */
}
     ...
     const char *filename;
{
  ...
  /* Do this before initializing pragmas, as then cpplib's hash table
     ...
     the one supplied.  */
/* BEGIN APPLE LOCAL IMI */
  filename = init_c_lex (in_fnames[0]);
/* END APPLE LOCAL IMI */
