...
/* APPLE LOCAL lookup_tag improvement */
#include "hashtab.h"
#include "feedback.h"
...
struct binding_level GTY(())
  {
     ...
     */
    /* APPLE LOCAL begin lookup_tag speedup */
    tree tags_list;   /* Keep tags_list for lookup_tag_reverse */
    htab_t GTY ((param_is (union tree_node)))  tags_ht;
    /* APPLE LOCAL end lookup_tag speedup */
...
/* APPLE LOCAL lookup_tag speedup */
/* Add one more NULL for new binding level member tags_ht.  */
...
static void c_expand_body_1               PARAMS ((tree, int, int));
...
static bool while_loop_is_memset_p       PARAMS ((tree, tree, tree, enum tree_code *));
static bool for_loop_is_memset_p         PARAMS ((tree));
static tree perform_memset_loop_transformation PARAMS ((tree *, int *, void *));
void memset_loop_transformation          PARAMS ((tree));
...
/* Returns non-zero if TREE_PURPOSE (P1) and P2 are equal.  */
static int
...
kept_level_p ()
{
  /* APPLEL LOCAL lookup_tag speedup */
  /* Check tags_ht instead of tags */
     ...
     int functionbody;
{
  ...
  tree decls;
  /* APPLE LOCAL lookup_tag speedup */
  /* Remove tree tags = current_binding_level->tags;  */
...
/* BEGIN APPLE LOCAL IMI */
	  if (current_binding_level == global_binding_level)
	    IDENTIFIER_GLOBAL_VALUE (DECL_NAME (link)) = 0;
/* END APPLE LOCAL IMI */
...
/* END APPLE LOCAL IMI */
	...
	{
	  warning_with_decl (newdecl, "conflicting types for `%s'");
...
/* BEGIN APPLE LOCAL IMI */
      if (t != 0 && duplicate_decls (x, t, different_binding_level, false))
/* END APPLE LOCAL IMI */
	{
...
/* BEGIN APPLE LOCAL IMI */
	  if (decl && ! comptypes (TREE_TYPE (x), TREE_TYPE (decl), false)
/* END APPLE LOCAL IMI */
...
/* BEGIN APPLE LOCAL IMI */
			  TREE_TYPE (IDENTIFIER_IMPLICIT_DECL (name)), false))
/* END APPLE LOCAL IMI */
	{
...
/* Used with htab_traverse_1 () to perform reverse lookup.  */
static PTR
...
/* Similar to htab_traverse.
   This function scans over the entire hash table calling
   CALLBACK for each live entry.  If CALLBACK returns zero,
   the iteration continues.  If CALLBACK returns non-zero,
   the interation stops and non-zero returned value is returned.
   INFO is passed as CALLBACK's second argument.  */
...
/* BEGIN APPLE LOCAL IMI */
      && (!DECL_CONTEXT (tem)
	  || TREE_CODE (DECL_CONTEXT (tem)) == TRANSLATION_UNIT_DECL))
/* END APPLE LOCAL IMI */
    {
...
/* BEGIN APPLE LOCAL IMI */
  if (current_function_decl)
    ...
    DECL_CONTEXT (decl) = current_file_decl;
/* END APPLE LOCAL IMI */
  TREE_USED (decl) = 1;
  TREE_TYPE (decl) = type;
...
parmlist_tags_warning ()
{
  ...
  /* APPLE LOCAL lookup_tag speedup */
  /* Directly use current_function_parm_tags list */
     ...
     int can_defer_p;
{
  ...
  /* APPLE LOCAL begin memset loop */
  /* Transform appropriate loops into library calls to memset, but do not
     do it if syntax only is requested. */
  if (!flag_syntax_only && flag_loop_to_memset)
    memset_loop_transformation (fndecl);
  /* APPLE LOCAL end memset loop */
  ...
  /* APPLE LOCAL begin callgraph inlining */
  if (flag_unit_at_a_time && can_defer_p && !flag_create_feedback)
    {
  ...
  /* APPLE LOCAL begin moved here from 'c_expand_body_1' - callgraph inlining */
  if (!nested)
    {
      /* APPLE LOCAL begin callgraph inlining */
      /* Function is parsed.
      ...
      if (flag_inline_trees)
	{
	  ...
	  uninlinable = ! tree_inlinable_function_p (fndecl, /* nolimit */0);
	  /* Disable all inlining for create-profile. */
      ...
      /* APPLE LOCAL end moved here from 'c_expand_body_1' - callgraph inlining */

      /* Generate RTL for the body of this function.  */
      c_expand_body_1 (fndecl, nested, can_defer_p);

      /* APPLE LOCAL callgraph inlining */
      if (uninlinable && !flag_callgraph_inlining)
	{
     ...
     tree fndecl;
{
  ...
  if (DECL_INLINE (fndecl) && DECL_RESULT (fndecl))
      ...
      if (flag_inline_trees)
	{
      ...
      /* APPLE LOCAL end callgraph inlining */
      c_expand_body_1 (fndecl, 0, 0);
      current_function_decl = NULL;
    }
}

/* Callgraph langhook interface to c_expand_body_1.  */
void
c_expand_body (fndecl)
     tree fndecl;
{
  c_expand_body_1 (fndecl, /* nested_p */0, /* can_defer_p */0);
}
...
static void
c_expand_body_1 (fndecl, nested_p, can_defer_p)
     tree fndecl;
     int nested_p, can_defer_p;
{
  ...
  /* APPLE LOCAL move 26 lines to 'finish_function' above - callgraph inlining */
...
/* END APPLE LOCAL IMI */
  ...
  emit_rtx_feedback_counter (fndecl, 0, fdo_incoming);
  ...
  emit_rtx_feedback_counter (fndecl, 1, fdo_outgoing);
...
/* APPLE LOCAL begin lookup_tag speedup */
/* Check tree code for input tree.
   If it is RECORD_TYPE, or UNION_TYPE or enum, rasie error.
   This function is used by check_for_loop_decls ().
*/
static int
...
check_for_loop_decls ()
{
  ...
  /* APPLE LOCAL lookup_tag speedup */
  /* Use htab_traverse instead of for loop to walk tags list */
  if (current_binding_level->tags_ht)
     ...
     tree t;
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL
    ...
    c_expand_body_1 (decl, /*nested_p=*/1, /*can_defer_p=*/0);
}
...
static bool
while_loop_is_memset_p (init, cond, body, incr_code)
     tree init;
     tree cond;
     tree body;
     enum tree_code *incr_code;
{
  ...
  /* This function takes a tree representing a while loop, and determines if
     the while loop is a good candidate for being transformed into a memset
     function call. */

  bool is_ok_for_memset = true;

  /* Verify that loop condition is a simple LE, LT, GE, or GT */
  ...
  /* Verify that loop index is either a pointer or not unsigned. */

  if ((TREE_UNSIGNED (TREE_TYPE (EXPR_STMT_EXPR (init))) == 1)
      && (POINTER_TYPE_P (TREE_TYPE (EXPR_STMT_EXPR (init))) == 0))
    is_ok_for_memset = false;
  ...
  if (is_ok_for_memset) 
    {
      if (body)
	{
	  tree stmt1 = NULL_TREE;
	  tree stmt2 = NULL_TREE;
	  tree incr = NULL_TREE;

	  /* Currently exactly two types of while loop bodies are okay for
	     conversion to memset calls:  A single statement such as
	     "*a++ = 0;" or two statements such as "a[i] = 0; i++;". (Of course,
	     the postincrement could also be a preincrement, postdecrement, or
	     predecrement).
	  */


	  if (TREE_CODE (body) == COMPOUND_STMT)
	    stmt1 = COMPOUND_BODY (body);
	  else if (TREE_CODE (body) == EXPR_STMT)
	    stmt1 = EXPR_STMT_EXPR (body);

	  if (stmt1 && TREE_CHAIN (stmt1))
	    {
	      stmt2 = TREE_CHAIN (stmt1);

	      /* Verify there are at most two statements in the loop body. */

	      if ((TREE_CHAIN (stmt2))
		  || (TREE_CODE (stmt2) != EXPR_STMT))
		is_ok_for_memset = false;
	      else
		{

		  /* Verify the loop increment statement. */

		  incr = TREE_OPERAND (stmt2, 0);
	  ...
	  if (stmt1 && is_ok_for_memset)
	    {

	      /* Verify the first statement is an assignment of the constant 
		 zero to an array or indirect ref (memory). */

	      if (TREE_CODE(stmt1) == MODIFY_EXPR)
		{
		  tree lhs = TREE_OPERAND (stmt1, 0);
		  ...
		  if (is_ok_for_memset) 
		    {
		      /* For right now, exactly two kinds of assignment statements
			 will be considered ok: A[i++] = 0 (also may use i--, ++i,
			  or --i); and *A++ = 0. */
		      
		      if (!stmt2) 
			{

			  /* Since the  loop body contains only one statement,
			     make sure the statement contains the loop
			     increment. */

			  tree operand_0 = TREE_OPERAND (lhs, 0);
			  if ((TREE_CODE (operand_0) != PLUS_EXPR)
			      && (TREE_CODE (operand_0) != POSTINCREMENT_EXPR)
			      && (TREE_CODE (operand_0) != PREINCREMENT_EXPR)
			      && (TREE_CODE (operand_0) != POSTDECREMENT_EXPR)
			      && (TREE_CODE (operand_0) != PREDECREMENT_EXPR))
			    is_ok_for_memset = 0;
			  else
			    *incr_code = TREE_CODE (operand_0);

			  if (is_ok_for_memset)
			    {
			      if (TREE_CODE(operand_0) == PLUS_EXPR)
				{
				  if (TREE_OPERAND (operand_0, 1) != 
				      TREE_OPERAND (cond, 0))
				    is_ok_for_memset = false;
				}
			      else
				{
				  if (TREE_OPERAND (operand_0, 0) !=
				      TREE_OPERAND (cond, 0))
				    is_ok_for_memset = false;
				}
			    }
	    ...
	    is_ok_for_memset = false;
	} /* if the loop has a body */
      else
	is_ok_for_memset = false;
    }

  return is_ok_for_memset;
}


static bool
for_loop_is_memset_p (loop)
     tree loop;
{

  /* This function takes a tree representing a for loop, and determines if
     the for loop is a good candidate for being transformed into a 
     memset function call. */

  bool is_ok_for_memset = true;
  tree init = FOR_INIT_STMT (loop);
  tree cond = FOR_COND (loop);
  tree incr = FOR_EXPR (loop);
  tree body = FOR_BODY (loop);
  
  /* Make sure body contains exactly one statement, and make sure that
     statement assigns zero to some indirect reference. */
  ...
  if (!(TREE_OPERAND (init, 0)))
    is_ok_for_memset = false;

  /* Verify that the loop index is either a pointer or not unsigned. */

  if (init && (EXPR_STMT_EXPR (init))
      && ((TREE_UNSIGNED (TREE_TYPE (EXPR_STMT_EXPR (init))) == 1)
	  && (POINTER_TYPE_P (TREE_TYPE (EXPR_STMT_EXPR (init))) == 0)))
    is_ok_for_memset = false;


  /* Verify that the loop "step" is an increment or decrement operator. */
  ...
  if (is_ok_for_memset)
    {
      /* Verify that the three parts of the for loop header all use the
	 same index variable */

      if ((TREE_CODE (TREE_OPERAND (init, 0)) != MODIFY_EXPR)
	  || (TREE_CODE (TREE_OPERAND (TREE_OPERAND (init, 0), 0)) != VAR_DECL)
	  || (TREE_CODE (TREE_OPERAND (incr, 0)) != VAR_DECL)
	  || (TREE_CODE (TREE_OPERAND (cond, 0)) != VAR_DECL)
	  || (TREE_OPERAND (incr, 0) != TREE_OPERAND (cond, 0))
	  || (TREE_OPERAND (incr, 0) != TREE_OPERAND (TREE_OPERAND (init, 0), 0)))
	is_ok_for_memset = false;
      
      /* Verify that loop condition is an inequality comparison. */
...
/* Global variable needed to hold prospective while loop initialization 
   statements. */

tree while_loop_init_arg = NULL_TREE;

/* The following function is applied by the function walk_tree, to find
   the initialization statement for a while loop.  The while loop in
   question is passed in the "data" parameter; the "tp" parameter is the
   current tree node in the walk.  The idea is to start at the beginning of
   the function, looking for assignment statements that modify the variable
   used in the while loop condition.  Whenever such an assignment is found
   it is stored in the global variable "while_loop_init_arg" (overwriting
   the previous value of the variable).  When the loop itself is encountered
   in the tree walk, the walk is halted.  Thus the global variable should
   contain the last assignment to the variable before the loop. 
*/

static tree
find_while_loop_init (tp, walk_subtrees, data)
     tree *tp;
     int *walk_subtrees;
     void *data;
{
  ...
  while_loop = *((tree *) data);

  init_var = TREE_OPERAND (WHILE_COND (while_loop), 0);

  if (*tp == while_loop)
    return while_loop_init_arg;
  else if (tp
	   && *tp
	   && (TREE_CODE (*tp) == EXPR_STMT)
	   && (EXPR_STMT_EXPR (*tp) != NULL_TREE)
	   && (TREE_CODE (EXPR_STMT_EXPR (*tp)) == MODIFY_EXPR)
	   && (TREE_OPERAND (EXPR_STMT_EXPR (*tp), 0) == init_var))
    while_loop_init_arg = *tp;

  return NULL_TREE;
}

static tree
build_while_loop_memset_tree (loop, stmt1, stmt2, init_stmt, increment_type)
     tree loop;
     tree stmt1;
     tree stmt2;
     tree init_stmt;
     int increment_type;
{
  /* Given the pieces of a while loop that qualifies for conversion to
     a memset call, this function returns a tree for the memset call. */
  ...
  /* Find the memory address to be updated. */

  base_address = TREE_OPERAND (TREE_OPERAND (stmt1, 0), 0);

  if (TREE_CODE (base_address) == PLUS_EXPR)
    {
      /* The loop body is of the form "a[i] = 0;".  Get 'a', and
	 add the loop initialization value to it. */
	...
	loc_arg = build_binary_op (PLUS_EXPR, loc_arg, init_arg, 1);
    }
  else
    {
      /* The loop body is of the form '*a++ = 0'. Get 'a'. */

      loc_arg = TREE_OPERAND (base_address, 0);
    }


  /* Size_arg is the amount of memory to be set by memset.  It should
     be the right-hand size of the loop condition +/- the loop initialization
     value.  If the loop comparison involved equality, add one to the size. */

  if (!stmt2)
    {
      size_arg = build_binary_op (MINUS_EXPR, TREE_OPERAND (WHILE_COND (loop), 1),
				  loc_arg, 1);
      size_arg = build_binary_op (MULT_EXPR, size_arg, build_int_2_wide (4, 0), 
				  1);
    }
  else
    {
      if ((TREE_CODE (WHILE_COND (loop)) == LT)
	  || (TREE_CODE (WHILE_COND (loop)) == LE))
	size_arg = build_binary_op (MINUS_EXPR, 
				    TREE_OPERAND (WHILE_COND (loop), 1),
				    init_arg, 1);
      else
	size_arg = build_binary_op (MINUS_EXPR, init_arg,
				    TREE_OPERAND (WHILE_COND (loop), 1), 1);

      if ((TREE_CODE (WHILE_COND (loop)) == LE)
	  || (TREE_CODE (WHILE_COND (loop)) == GE))
	size_arg = build_binary_op (PLUS_EXPR, size_arg, build_int_2_wide (1, 0),
				    1);
  ...
  /* Get the value which memset is going to put in memory. */
  ...
  arglist = tree_cons (NULL_TREE, value_arg, arglist);
  arglist = tree_cons (NULL_TREE, loc_arg, arglist);

  /* Make the memset call. */

  memset_call = build_function_call (IDENTIFIER_GLOBAL_VALUE 
				                    (get_identifier ("memset")),
				     arglist);

  /* Make the call into a statement. */

  new_stmt1 = build_stmt (EXPR_STMT, memset_call);

  return new_stmt1;
}

static tree
perform_memset_loop_transformation (tp, walk_subtrees, data)
     tree *tp;
     int *walk_subtrees;
     void *data ATTRIBUTE_UNUSED;
{
  ...
  if (TREE_CODE (*tp) == FOR_STMT)
      ...
      if (for_loop_is_memset_p (*tp)) 
	{
	  ...
	  if ((TREE_CODE (mem_loc_arg) != INDIRECT_REF)
	    ...
	    {
	      operand_0 = TREE_OPERAND (mem_loc_arg, 0);
	      if (TREE_CODE (operand_0) == PLUS_EXPR)
		{
		  base_address = TREE_OPERAND (operand_0, 0);
		  if ((TREE_CODE (base_address) == VAR_DECL) 
		      || (TREE_CODE (base_address) == PARM_DECL))
		    {
	  ...
	  if (base_address && offset)
		...
		value_arg = EXPR_STMT_EXPR (TREE_CHAIN (value_arg));
		 ...
		 of the difference between the loop initialization value and
		 the conditional comparison value.  If the conditional comparison
		 is LE or GE, then add one to the size argument. */
	      
	      cond_arg = TREE_OPERAND (FOR_COND (*tp), 1);
	      init_arg = TREE_OPERAND (TREE_OPERAND (FOR_INIT_STMT (*tp), 0), 1);
	  
	      if ((TREE_CODE (FOR_EXPR (*tp)) == PREINCREMENT_EXPR)
		  || (TREE_CODE (FOR_EXPR (*tp)) == POSTINCREMENT_EXPR))
		size_arg = build_binary_op (MINUS_EXPR, cond_arg, init_arg, 1);
	      else
		size_arg = build_binary_op (MINUS_EXPR, init_arg, cond_arg, 1);
	      
	      if ((TREE_CODE (FOR_COND (*tp)) == LE_EXPR)
		  || (TREE_CODE (FOR_COND (*tp)) == GE_EXPR))
		size_arg = build_binary_op (PLUS_EXPR, size_arg, 
					    build_int_2_wide (1, 0), 1);
	      
	     
	      size_arg = build_binary_op (MULT_EXPR, size_arg, offset, 1);
		...
		value_arg = integer_zero_node;
	      ...
	      arglist = build_tree_list (NULL_TREE, size_arg);
	      arglist = tree_cons (NULL_TREE, value_arg, arglist);
	      arglist = tree_cons (NULL_TREE, mem_loc_arg, arglist);
	      
	      memset_tree = build_function_call (IDENTIFIER_GLOBAL_VALUE (get_identifier ("memset")),
						 arglist);

		      
	      /* Make memset_tree into a statement. */
	      
	      new_stmt = build_stmt (EXPR_STMT, memset_tree);
	      
	      /* Replace for-loop with memset tree. */
  ...
  else if (TREE_CODE (*tp) == WHILE_STMT)
    {

      tree cond_arg = WHILE_COND (*tp);
      tree body = WHILE_BODY (*tp);
      tree init_var = NULL_TREE;
      tree init_arg = NULL_TREE;
      tree while_loop = *tp;
      enum tree_code  incr_code = 0;


      if (TREE_CODE (WHILE_COND (*tp)) != INTEGER_CST)
	init_var = TREE_OPERAND (cond_arg, 0);

      if (init_var)
	{
	  if ((TREE_CODE (init_var) == VAR_DECL)
	      || (TREE_CODE (init_var) == PARM_DECL))
	    {

	      /* Find statement preceding the while loop that initializes the
		 condition variable. */
	      
	      walk_tree (&DECL_SAVED_TREE (current_function_decl), 
			 find_while_loop_init, tp, NULL);

	      init_arg = while_loop_init_arg;

	      /* init_arg should now contain the last assignment to the condition
		 variable before encountering the while loop (i.e. the while loop
		 initialization). */
	  ...
	  && cond_arg 
	  && body
	  && while_loop_is_memset_p (init_arg, cond_arg, body, &incr_code))
	{
	  tree chain;
	  tree stmt1;
	  tree new_stmt = NULL_TREE;
	  tree stmt2 = NULL_TREE;
	  tree base_address = NULL_TREE;
	  tree new_stmt2 = NULL_TREE;
	  tree lhs = NULL_TREE;
	  tree rhs = NULL_TREE;

	  *tp = while_loop;

	  /* Get the value argument (constant 0). */

	  if (TREE_CODE (body) == EXPR_STMT)
	    stmt1 = EXPR_STMT_EXPR (body);
	  else if (TREE_CODE (body) == COMPOUND_STMT)
	    stmt1 = COMPOUND_BODY (body);
	  ...
	  new_stmt = build_while_loop_memset_tree (*tp, stmt1, stmt2,
						   while_loop_init_arg,
						   incr_code);

	  /* For while loops that look like this:

	     a = x;
	     while (a < e)
	        *a++ = 0;
	     
	    After converting it to a memset call, we need to add
	    an assignment statment, "a = e;".
	  */
	  ...
	  if (!stmt2)
	    {
	      ...
	      /* Make the necessary assignment statement. */

	      lhs = TREE_OPERAND (WHILE_COND (while_loop), 0);

	      rhs = TREE_OPERAND (WHILE_COND (while_loop), 1);

	      new_stmt2 = build (MODIFY_EXPR, TREE_TYPE (lhs), lhs, rhs);

	      new_stmt2 = build_stmt (EXPR_STMT, new_stmt2);
	    }
	  else
	    new_stmt2 = NULL_TREE;
	  ...
	  if (new_stmt)
	    {
	      ...
	      *tp = new_stmt;
	      if (new_stmt2)
		{
		  TREE_CHAIN (*tp) = new_stmt2;
		  TREE_CHAIN (new_stmt2) = chain;
		}
	      else
		TREE_CHAIN (*tp) = chain;
	    }
  ...
  else if (TREE_CODE (*tp) == DO_STMT)
    {

      /* In the future, do something similar with do loops. */
    }
...
/* The main entry point for tranforming appropriate loops to library
   calls to memset. */
void
memset_loop_transformation (fn)
     tree fn;
{
  timevar_push (TV_LOOP_MEMSET);
  walk_tree (&DECL_SAVED_TREE (fn), perform_memset_loop_transformation, NULL, 
	     NULL);
  timevar_pop (TV_LOOP_MEMSET);
}
/* APPLE LOCAL end memset loop */
