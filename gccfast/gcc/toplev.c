...
/* APPLE LOCAL -O5 */
/* Used by various targets to initialize target specific switches when -O5 is used.  */
...
/* APPLE LOCAL begin new feedback */
/* Nonzero if new feedback info is to be generated. */

int flag_create_feedback = 0;

/* Nonzero if new feedback info is to be read back and used. */

int flag_use_feedback = 0;
/* APPLE LOCAL end new feedback */
...
/* Nonzero if blocks should be partitions into hot and cold sections in .o */
...
/* Nonzero if calls to floor should be inlined. */

int flag_inline_floor = 0;
...
/* APPLE LOCAL begin enable branch predictions */
/* Zero to disable predicting branches */

int flag_branch_predictions = 1;

/* The following flag takes an integer value;  the integer indicates what
   percent the branch needs to be taken before it is predicted "taken". It
   must be at least 50 */

int branch_predictions_threshold = 98;

/* APPLE LOCAL end enable branch predictions */
...
/*Haifa: Nonzero means perform global cse after register allocation*/
int flag_gcse_after_ra =0;
int flag_gcse_after_ra_case1 =1;
int flag_gcse_after_ra_case2 =1;
int flag_gcse_after_ra_parial =0;
int flag_gcse_after_ra_hot_fraction = 0;
int flag_gcse_after_ra_partial_fraction = 1;
int flag_gcse_mem_alias_after_ra = 1;
int flag_gcse_after_ra_debug=0;
int flag_gcse_after_ra_limit=0;
...
/* APPLE LOCAL begin forbidding over-speculative motion */
int flag_schedule_speculative_cost = 10;      /* allow all by default */
int flag_schedule_speculative_profile = 0;    /* disabled by default */
/* APPLE LOCAL end forbidding over-speculative motion */

/* DN begin */
int sched_stalled_insns = 0;
int sched_costly_dep = 0;
int insert_sched_nops = 0;
int use_sched_prioritize_hook = 0;
int flag_sched_queue_cost = 0;
int flag_sched_allow_deps_in_group = 0;
int tune_sched_dep_insns_in_group = 0;
int sched_load_balance = 0;
int sched_load_balance_scale = 1;
int insert_sched_nops_for_ldb = 0;
int flag_sched_ldb_global = 1;
int flag_align_first_group = 0;
int flag_mpowerpc64fix = 0;
/* DN end */
...
/* APPLE LOCAL begin forbidding over-speculative motion */
/* If nonzero, use the improved instruction comination.  */
int flag_new_combine = 0;

/* If nonzero, use the early coalescing pass */
int flag_early_coalesce = 0;
/* APPLE LOCAL end forbidding over-speculative motion */
...
/* If nonzero, generate code suitable for fast turn around debugging.
   This flag must be applied to any translation unit that we would
   like to overlay with a new replacement.  */
int flag_fix_and_continue;

/* If nonzero, generate code suitable for fast turn around debugging.
   This flag must be applied to any translation unit that we would
   like to use as a replacement.  */
...
/* Nonzero if inlining choices are derived from the callgraph.  */
int flag_callgraph_inlining = 0;

/* Nonzero to throttle inlining with percentage growth.  */
/* Defaulted to 25 for tuning. */
int flag_callgraph_inlining_limit = 25;

/* Nonzero to throttle inlining with call counts.  */
HOST_WIDEST_INT flag_callgraph_inlining_count = 0;

/* haifa begin */
/* inline only if call is executed at least ratio% of hotest call */
HOST_WIDEST_INT flag_callgraph_inlining_ratio = 0;

/* inline only if call is executed at least ratio% of total callee */
HOST_WIDEST_INT flag_callgraph_inlining_callee_ratio = 0;

/* inline only if call is executed at least ratio% of total caller */
HOST_WIDEST_INT flag_callgraph_inlining_caller_ratio = 0;

/* inline always if callee has at most flag_imline_small stmts */
HOST_WIDEST_INT flag_callgraph_inline_small = 0;
/* haifa end */

/* APPLE LOCAL end callgraph inlining */

int flag_load_after_store = 0;
int flag_gcse_mem_alias = 0;
int flag_gcse_loop_depth = 0;
int gcse_max_deletes = 0;
int flag_new_delete_dead = 1;
char *gcse_mem_alias_func = 0;

/* APPLE LOCAL begin memset loop */
/* Nonzero if we transform appropriate loops into memset library calls */
int flag_loop_to_memset = 0;
/* APPLE LOCAL end memset loop */
...
/*Haifa - don't align calls by default*/
int align_calls = 0;
int align_calls_log;
int align_calls_max_skip = 8;
/*Haifa end*/
...
static const lang_independent_options f_options[] =
{
  ...
  {"callgraph-inlining", &flag_callgraph_inlining, 1,
   N_("Use callgraph to choose inlining opportunities") },
  /* APPLE LOCAL end callgraph inlining */
  ...
  /* APPLE LOCAL end loop transposition */
  /* APPLE LOCAL begin memset loop */
  {"loop-to-memset", &flag_loop_to_memset, 1,
   N_("Transform appropriate loops to memset library calls") },
  /* APPLE LOCAL end memset loop */ 
  ...
  /* APPLE LOCAL begin forbidding over-speculative motion */
  {"if-cost-conversion", &flag_if_cost_conversion, 1,
   N_("Perform conversion of conditional jumps to conditional execution with cost threshold") },
  /* APPLE LOCAL end forbidding over-speculative motion */
  /* DN begin */
  {"sched-costly-dep", &sched_costly_dep, 1,
   N_("Define what is considered a costly dependence") },
  {"insert-sched-nops", &insert_sched_nops, 1,
   N_("Fill empty slots with nops") },
  {"use-sched-prioritize-hook", &use_sched_prioritize_hook, 1,
   N_("Prioritize insns that are restricted to certain issue slots") },
  {"sched-queue-cost", &flag_sched_queue_cost, 1,
   N_("Reschedule instructions without queueing after register allocation") },
  {"sched-stalled-insns", &sched_stalled_insns, 1,
   N_("Allow scheduling stalled instructions ahead of time") },
  {"sched-dep-insns-in-group", &flag_sched_allow_deps_in_group, 1,
   N_("Allow scheduling dependent instructions in the same group after register allocation") },
  {"tune-sched-dep-insns-in-group", &tune_sched_dep_insns_in_group, 1,
   N_("Control scheduling dependent instructions in the same group after register allocation") },
  {"sched-ldb", &sched_load_balance, 1,
   N_("Schedule for better FU load balance") },
  {"sched-ldb-scale", &sched_load_balance_scale, 1,
   N_("Schedule for better FU load balance, biased for more frequently used FUs") },
  {"sched-ldb-global", &flag_sched_ldb_global, 1,
   N_("Schedule for better FU load balance across dispatch groups") },
  {"insert-sched-nops-for-ldb", &insert_sched_nops_for_ldb, 1,
   N_("Fill empty slots with nops during load balancing") },
  {"align-first-group", &flag_align_first_group, 1,
   N_("align so that first group is in fetch block") },
  {"mpowerpc64fix", &flag_mpowerpc64fix, 1,
   N_("DJE's fix to mpowerpc64") },
  /* DN end */
  ...
  /* APPLE LOCAL begin forbidding over-speculative motion */
  {"sched-spec-cost",&flag_schedule_speculative_cost, 1,
   N_("Allow speculative motion of non-loads with small rtx_cost") },
  /* APPLE LOCAL end forbidding over-speculative motion */
  ...
  /* APPLE LOCAL begin forbidding over-speculative motion */
  {"sched-spec-prof",&flag_schedule_speculative_profile, 1,
   N_("consider profiling weights in spec scheduler") },
  /* APPLE LOCAL end forbidding over-speculative motion */
...
/* APPLE LOCAL begin new feedback */
  ...
  {"relax-aliasing", &flag_relax_aliasing, 1,
   N_("Assume relax aliasing rules apply") },
...
/*Haifa - align calls to reduce miss-aligment of returns*/   
  {"align-calls", &align_calls, 32,
   N_("Align calls ") },
  {"align-calls-max-skip", &align_calls_max_skip, 8,
   N_("Maximum number of bytes to be added wehn aligning calls") },
  {"gcse-after-ra", &flag_gcse_after_ra, 1,
   N_("Perform redundant load elimination after register allocation") },
  {"gcse-after-ra-case1", &flag_gcse_after_ra_case1, 1,
   N_("Perform redundant load elimination after register allocation case 1: single successor") },
  {"gcse-after-ra-case2", &flag_gcse_after_ra_case2, 1,
   N_("Perform redundant load elimination after register allocation case 2: single predecessor ") },
  {"gcse-after-ra-partial", &flag_gcse_after_ra_parial, 1,
   N_("Perform Partially redundant load elimination after register allocation. ") },
  {"gcse-after-ra-hot-fraction", &flag_gcse_after_ra_hot_fraction, 1,
   N_("Perform redundant load elimination after register allocation only for hot basic blocks") },
  {"gcse-mem-alias-after-ra",&flag_gcse_mem_alias_after_ra,1,
   N_("Consider memory aliassing when recognizing equivilent memory expressions in GCSE after ra")},
   {"gcse-after-ra-debug",&flag_gcse_after_ra_debug,1,
   N_("Print Debug information in GCSE after ra")},
/*Haifa end*/
  ...
  /* APPLE LOCAL begin forbidding over-speculative motion */
  {"new-combine", &flag_new_combine, 1,
   N_("Improved instruction combination phase.") },
  {"early-coalesce", &flag_early_coalesce, 1,
   N_("Early register coalescing phase.") },
  /* APPLE LOCAL end forbidding over-speculative motion */
  ...
  /* APPLE LOCAL begin allow disabling branch predictions */
  { "branch-predictions", &flag_branch_predictions, 1,
   N_("Enable generation of branch prediction bits") },
  /* APPLE LOCAL end allow disabling branch predictions */
  /* APPLE LOCAL begin set branch predictions threshold */
  { "branch-predictions-threshold", &branch_predictions_threshold, 0,
   N_("Set threshold for generation of branch prediction bits") },
  /* APPLE LOCAL end   set branch predictions threshold */
  /* APPLE LOCAL begin testsuite OS flush bug workaround */
  ...
  /* APPLE LOCAL end - rarely executed bb optimization */
  /* APPLE LOCAL begin inline floor */
  { "inline-floor", &flag_inline_floor, 1,
    N_("Inline fast code for floor library calls") },
  /* APPLE LOCAL end inline floor */
  ...
  {"load-after-store",&flag_load_after_store,1,
  N_("Perform load after store optimization")},
    {"gcse-mem-alias",&flag_gcse_mem_alias,1,
  N_("Consider memory aliassing when recognizing equivilent memory expressions in GCSE")},
    {"gcse-loop-depth",&flag_gcse_loop_depth,1,
  N_("Consider loop depth when deciding to eliminate parially redundant expressions")},
  {"new-delete-dead",&flag_new_delete_dead,1,
  N_("Improved delete trivially dead instructions")}
};
...
compile_file ()
{
  ...
  /* APPLE LOCAL new feedback */
  init_feedback (aux_base_name);
     ...
     tree decl;
{
  ...
  /* Turn NOTE_INSN_FDO_COUNTs into branch probabilities. */
...
#ifdef INLINE_FLOOR_CALLS
  if (flag_inline_floor)
    INLINE_FLOOR_CALLS;
#endif
  ...
  if (optimize)
    {
      ...
      if (flag_use_feedback)
	/** this should be in ENABLE_CHECKING eventually. **/
        verify_flow_info ();
      if (!flag_use_feedback)
        find_basic_blocks (insns, max_reg_num (), rtl_dump_file);
    }
  ...
  /* APPLE LOCAL begin forbidding over-speculative motion */
  /*Do early coalescing pass before scheduling*/
  if (flag_early_coalesce)
  {
  ...
  close_dump_file (DFI_postreload, print_rtl_with_bb, insns);
     ...
     if (optimize > 0 && flag_gcse_after_ra)
     {
       int save_csb, save_cfj;
       int tem2 = 0;
       ...
       tem = gcse_after_ra_main (insns, rtl_dump_file);
       rebuild_jump_labels (insns);
       delete_trivially_dead_insns (insns, max_reg_num ());
     ...
     const char *arg;
{
  ...
  /* APPLE LOCAL begin -fast or -fastf */
  if (!strcmp (arg, "ast"))
    {
      flag_fast = 1;
      return 1;
    }
  else if (!strcmp (arg, "astf")) 
    {
      flag_fastf = 1;
      return 1;
    }
  ...
  /* APPLE LOCAL begin callgraph inlining */
  else if ((option_value = skip_leading_substring (arg, "callgraph-inlining-limit-"))
	   || (option_value = skip_leading_substring (arg, "callgraph-inlining-limit=")))
    {
      flag_callgraph_inlining_limit =
	read_integral_parameter (option_value, arg - 2, 0);
    }
  else if ((option_value = skip_leading_substring (arg, "callgraph-inlining-count-"))
	   || (option_value = skip_leading_substring (arg, "callgraph-inlining-count=")))
    {
...
/* haifa begin */
  else if ((option_value = skip_leading_substring (arg, "callgraph-inlining-ratio-"))
	   || (option_value = skip_leading_substring (arg, "callgraph-inlining-ratio=")))
    {
      flag_callgraph_inlining_ratio =
	read_integral_parameter (option_value, arg - 2, 0);
    }
  else if ((option_value = skip_leading_substring (arg, "callgraph-inlining-callee-ratio-"))
	   || (option_value = skip_leading_substring (arg, "callgraph-inlining-callee-ratio=")))
    {
      flag_callgraph_inlining_callee_ratio =
	read_integral_parameter (option_value, arg - 2, 0);
    }
  else if ((option_value = skip_leading_substring (arg, "callgraph-inlining-caller-ratio-"))
	   || (option_value = skip_leading_substring (arg, "callgraph-inlining-caller-ratio=")))
    {
      flag_callgraph_inlining_caller_ratio =
	read_integral_parameter (option_value, arg - 2, 0);
    }
  else if ((option_value = skip_leading_substring (arg, "callgraph-inline-small-"))
	   || (option_value = skip_leading_substring (arg, "callgraph-inline-small=")))
    {
...
#endif
/* DN begin */
  else if ((option_value = skip_leading_substring (arg, "use-sched-prioritize-hook=")))
    use_sched_prioritize_hook = read_integral_parameter (option_value, arg - 2, use_sched_prioritize_hook);
  else if ((option_value = skip_leading_substring (arg, "sched-queue-cost=")))
    flag_sched_queue_cost = read_integral_parameter (option_value, arg - 2, flag_sched_queue_cost);
  else if ((option_value = skip_leading_substring (arg, "sched-stalled-insns=")))
    sched_stalled_insns = read_integral_parameter (option_value, arg - 2, sched_stalled_insns);
  else if ((option_value = skip_leading_substring (arg, "sched-costly-dep=")))
    sched_costly_dep = read_integral_parameter (option_value, arg - 2, sched_costly_dep);
  else if ((option_value = skip_leading_substring (arg, "tune-sched-dep-insns-in-group=")))
    tune_sched_dep_insns_in_group = read_integral_parameter (option_value, arg - 2, tune_sched_dep_insns_in_group);
  else if ((option_value = skip_leading_substring (arg, "sched-ldb=")))
    sched_load_balance = read_integral_parameter (option_value, arg - 2, sched_load_balance);
  else if ((option_value = skip_leading_substring (arg, "insert-sched-nops=")))
    insert_sched_nops = read_integral_parameter (option_value, arg - 2, insert_sched_nops);
/* DN end */
  ...
  /* APPLE LOCAL begin forbidding over-speculative motion */
  else if ((option_value = skip_leading_substring (arg, "sched-spec-cost=")))
    flag_schedule_speculative_cost = read_integral_parameter (option_value, arg - 2, flag_schedule_speculative_cost);
  /* APPLE LOCAL end forbidding over-speculative motion */
...
/* APPLE LOCAL end -f[align-loops|align-jumps]-max-skip */
/* APPLE LOCAL begin set branch predictions threshold */
  else if ((option_value = skip_leading_substring (arg, 
						"branch-predictions-threshold=")))
    branch_predictions_threshold = read_integral_parameter (option_value, arg - 2,
...
/*Haifa the amount of alignment in call return*/
  else if ((option_value = skip_leading_substring (arg, "align-calls=")))
    align_calls
      = read_integral_parameter (option_value, arg - 2, align_calls);
  else if ((option_value = skip_leading_substring (arg, "align-calls-max-skip=")))
    align_calls_max_skip
      = read_integral_parameter (option_value, arg - 2, align_calls_max_skip);
  else if ((option_value = skip_leading_substring (arg,"gcse-after-ra-limit=")))
  	 flag_gcse_after_ra_limit = read_integral_parameter (option_value, arg - 2,flag_gcse_after_ra_limit );
  else if ((option_value = skip_leading_substring (arg,"gcse-after-ra-hot-fraction=")))
  	 flag_gcse_after_ra_hot_fraction = read_integral_parameter (option_value, arg - 2,flag_gcse_after_ra_hot_fraction );
  else if ((option_value = skip_leading_substring (arg,"gcse-after-ra-partial-fraction=")))
    flag_gcse_after_ra_partial_fraction = read_integral_parameter (option_value, arg - 2,flag_gcse_after_ra_partial_fraction );
/*Haifa end*/

  /* APPLE LOCAL begin forbidding over-speculative motion */
  else if ((option_value = skip_leading_substring (arg, "sched-spec-prof=")))
    flag_schedule_speculative_profile = read_integral_parameter (option_value, arg - 2, flag_schedule_speculative_profile);
  /* APPLE LOCAL end forbidding over-speculative motion */
...
/* Even though DWARF2_DEBUGGING_INFO is defined, use stabs for debugging symbols
   with -ggdb.  Remove this local patch when we switch to dwarf.  */
                  type = DBX_DEBUG;
     ...
     char **argv;
{
  ...
  switch (*arg)
    ...
    /* Need this flag to disable indexing during preprocessing.  */
    case 'E':
...
process_options ()
{
  ...
  if (flag_create_feedback)
    {
      /* To get the most accurate data, disable all optimization 
	 in this case.  This is probably too draconian; the 
	 ones that need to be disabled are the ones that change
	 control flow. */
      optimize = 0;
      optimize_size = 0;
      flag_merge_constants = 0;
  ...
  /* Currently feedback use pass doesn't work with non-callgraph
     inlining.  We'll fall back on guessing in this case. */
  if (flag_use_feedback && !flag_no_inline && !flag_callgraph_inlining)
    flag_use_feedback = 0;
  /* Currently the two feedback passes are incompatible, and
     if we have profile info we shouldn't be guessing.  */
  if (flag_use_feedback || flag_create_feedback)
    {
      flag_branch_probabilities = 0;
      profile_arc_flag = 0;
      flag_guess_branch_prob = 0;
      /* info is not good enough to be guessing */
      if (flag_use_feedback)
	flag_branch_predictions = 0;
    }
  ...
  /*Haifa - calls alignment*/
  align_calls_log = floor_log2(align_calls*2 -1 );
  /*End Haifa */
