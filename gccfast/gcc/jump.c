...
/* LOOP_START is a NOTE_INSN_LOOP_BEG note that is followed by an unconditional
   ...
   If flag_use_feedback, the control flow graph is expected to be meaningful
   on entry (including counts) and is maintained so.  If not, not. */
     ...
     rtx loop_start;
{
  ...
  /* LOOP BEG may appear either within a block or between two blocks.
     Look for the latter case and transform it into the former, for
     simplicity farther down. */
  ...
  /* We need to do something reasonable about the feedback counts.
     If the loop body is executed more often than the header,
     assume that it is executed whenever its header is.  If the loop
     body is executed less often than the header, assume it's
     executed one iteration each time it's executed.  These guesses
     will not always be right.  They should suffice for branch
     prediction, but are maybe not so good for unrolling.  The right
     thing is to figure out some way to get a counter in here on
     pass 1... */
  ...
  lastexit = insn;  /* the LOOP END note */
  ...
  /* Find label at top of loop.   LOOP BEGIN is followed by 
     unconditional branch, then barrier, then label we want. */
  loop_top_label = next_nonnote_insn (next_nonnote_insn 
		    (next_nonnote_insn (loop_start)));
  loop_top_bb = BLOCK_FOR_INSN (loop_top_label);

  /* New label and basic block which falls into the loop. */
	...
	case JUMP_INSN:
	  ...
	  if (any_condjump_p (copy))
	      ...
	      if (label)
		{
		  if (label == loop_top_label)
		      ...
		      if (flag_use_feedback)
			{
			  ...
			  /* Assume we get into the loop as often as possible. */
			  e->count = test_copy_count;
			  e->probability = (e->count * REG_BR_PROB_BASE 
						+ test_copy_count / 2) 
					    / test_copy_count;
		      ...
		      if (flag_use_feedback)
			{
			  ...
			  /* Assume we stay in the loop, i.e. do not branch out. */
	      ...
	      if (flag_use_feedback && label)
		{
	      ...
	      /* If more code is to be duplicated the following iterations
		 will screw up the edges.  However this should not happen
		 because there are no labels being copied and no other way
		 to reach such code. */
	    }
          if (flag_use_feedback)
	    {
  ...
  if (! copy || GET_CODE (copy) != BARRIER)
      ...
      if (flag_use_feedback)
	{
	  ...
	  if (oldlabel_after_loop == 0)
	    {
	      /* we just added a new label and block after the loop (at end
		 of function).  the block before lastexit used to drop
		 through (presumably to EXIT, but we don't care) and 
		 drops through to the new block instead. */
  ...
  /* Mark the exit code as the virtual top of the converted loop.  
     We place the note before the label, which is about to be deleted;
     this keeps the note out of any block. */
...
/* As above, but also insert label as head of basic block that
   contains the insn originally following INSN.  If there is
   no such block (at end of function) one is created and its
   successor is set to EXIT_BLOCK.  */
  ...
  if (oldlabel == 0 || GET_CODE (oldlabel) != CODE_LABEL)
    {
      ...
      if (oldlabel)
	{
	  /* Do not pick up anything (such as deleted insns) that are
	     outside all blocks. */
...
/* Like above but remove appropriate edges after deleting a branch.
   Does not currently do anything for tablejumps, but probably will.
   It *should* be safe to just make this change in delete_related_insns,
   but I have no way to test all of it, notably reorg.c.  */
