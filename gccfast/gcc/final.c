...
struct function_list
{
  ...
  long count_edges;		/* number of instrumented edges in this function */
  long count_calls;		/* number of instrumented calls in this function */
};
     ...
     const char *filename;
{
  if ((profile_arc_flag || flag_create_feedback)
      && profile_info.count_instrumented_edges)
    {
      ...
      /* For the new feedback, the main header has 13 lucky words:
	 0:  1 if initialized, else 0
	 1:  address of file name (LC0)
	 2:  address of table of arc counts (LFDO0)
	 3:  number of counts in LFDO0
	 4:  address of table of call counts (LFDO1)
	 5:  number of counts in LFDO1
	 6:  address of table of arc indices (LFDO2)
	 7:  number of indices in LFDO2 (not necessarily the
	     same as word 3, this table can have holes)
	 8:  address of table of call indices (LFDO3)
	 9:  number of indicies in LFDO3 (may always be the
	     same as word 5)
	10:  always 0, used by libgcc2 as a pointer
        11:  Number of bytes in this header.
        12:  address of table of function checksums (LPBX7).  */


       /* New feedback files are suffixed .db not .da, so the
	  different format should not cause confusion. */
...
/* Return boolean indicating if this instruction is part of a 'jump table'
   basic block (which means it must go in the same section as the 
