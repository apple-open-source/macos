/* APPLE LOCAL begin - 3.4 scheduler update: THIS WHOLE FILE:  DON'T CHANGE IF YOU
   CAN HELP IT */
...
/* DN begin */
extern int sched_stalled_insns;
extern int use_sched_prioritize_hook;
extern int flag_sched_queue_cost;
extern int flag_sched_allow_deps_in_group;
extern int tune_sched_dep_insns_in_group;
/* DN end */
...
/* DN begin */
static void early_queue_to_ready PARAMS ((state_t, struct ready_list *, int));
/* DN end */
...
/* DN begin */
  int target_hook_priority = 0;
  int dep_on_last_insn_priority = 0;
  int dep_on_last_insn_val = 0;
  rtx link2;
  int cost, cost2;
/* DN end */
  ...
  if (last_scheduled_insn)     
    {
      ...
      if(link)
	cost = insn_cost (last_scheduled_insn, link, tmp) ;
      ...
      link2 = find_insn_list (tmp2, INSN_DEPEND (last_scheduled_insn));
      if(link2)
	cost2 = insn_cost (last_scheduled_insn, link2, tmp2) ;
      if (link2 == 0 || insn_cost (last_scheduled_insn, link2, tmp2) == 1)
        tmp2_class = 3;
      ...
      dep_on_last_insn_val = tmp2_class - tmp_class;
#if 0
      if(targetm.sched.is_costly_dependence &&
	 ( (link2 && targetm.sched.is_costly_dependence(last_scheduled_insn, tmp2, link2, cost2)) ||
	   (link && targetm.sched.is_costly_dependence(last_scheduled_insn, tmp, link, cost)) )){ 
        dep_on_last_insn_priority = dep_on_last_insn_val; 
      }
#else
     dep_on_last_insn_priority = dep_on_last_insn_val; 
#endif
...
/* DN begin */
  if(use_sched_prioritize_hook
     && targetm.sched.priority_evaluation_hook)
    target_hook_priority = (*targetm.sched.priority_evaluation_hook)(tmp2,tmp);

  if(dep_on_last_insn_priority && tune_sched_dep_insns_in_group == 2) 
    priority_val = dep_on_last_insn_priority;
  else if(target_hook_priority && use_sched_prioritize_hook == 1)
    priority_val = target_hook_priority;
/* DN end */
...
/* DN begin */
  if (target_hook_priority && use_sched_prioritize_hook == 2)
    return target_hook_priority;
/* DN end */
...
/* DN begin */
  if (target_hook_priority && use_sched_prioritize_hook == 3)
    return target_hook_priority;
/* DN end */
...
/* DN begin */
  if (target_hook_priority && use_sched_prioritize_hook == 4)
    return target_hook_priority;
/* DN end */
...
#else
  if(dep_on_last_insn_val)
    return dep_on_last_insn_val;
#endif /* DN end */

/* DN begin */
  if (target_hook_priority && use_sched_prioritize_hook == 5)
    return target_hook_priority;
/* DN end */
...
/* DN begin */
  if (target_hook_priority && use_sched_prioritize_hook == 6)
    return target_hook_priority;
/* DN end */
...
#if 1 /* DN begin */
	      if (reload_completed 
		 &&  flag_sched_allow_deps_in_group
		 &&  targetm.sched.is_costly_dependence
		 &&  ((tune_sched_dep_insns_in_group != 1) || (ready->n_ready == 0))){

   	        int queue_cost = effective_cost;

	        if (flag_sched_queue_cost){
                  if (flag_sched_queue_cost == 1)
		    queue_cost = 0;
		  else if (flag_sched_queue_cost > 1)
		    queue_cost = flag_sched_queue_cost;
		}

		if (targetm.sched.is_costly_dependence(insn, next, link, effective_cost))
		  queue_insn (next, queue_cost);
		else
		  ready_add(ready, next);
	      }
              else{
...
/* DN begin */
/* Remove insns from the queue, even before they became ready.  */
static int
ok_for_early_queue_removal(insn)
	rtx insn;
{
  int n_groups = sched_stalled_insns;
...
static void
early_queue_to_ready (state, ready, first_cycle_insn_p)
     state_t state;
     struct ready_list *ready;
     int first_cycle_insn_p;
{
  ...
  if (sched_stalled_insns && reload_completed) 
    {
      int stalls;
...
#if 1 /* DN begin */
/* Called from backends from targetm.sched.finish to emit stuff into
   the instruction stream.  */

rtx
sched_emit_insn_after (insn, after, prev)
     rtx insn;
     rtx after;
     rtx prev; 
{
  ...
  if(prev){    
    if(last_scheduled_insn == insn)
      last_scheduled_insn = prev;
    next_insn = move_insn1 (insn, after);
  }
  else{ /* nops... */
    ...
    next_insn = emit_insn_after(insn, after);

    if(end_bb && bb->end != end_bb)
      bb->end = end_bb;
  }

  if(after == last_scheduled_insn){
    last_scheduled_insn = next_insn;
  }
  ...
  while ((*current_sched_info->schedule_more_p) ())
      ...
      for (;;)
	{
	  ...
	  if (!targetm.sched.use_dfa_pipeline_interface
		...
		/* DN begin */
		if (ready.n_ready == 0 && sched_stalled_insns ) {
		  if(sched_verbose >= 6)
			fprintf(sched_dump,";;\t\tSecond chance\n");
		  memcpy (temp_state, curr_state, dfa_state_size); 
		  early_queue_to_ready(temp_state, &ready, first_cycle_insn_p); 
		  if (ready.n_ready == 0){
		    if(sched_verbose >= 6)
			fprintf(sched_dump,";;\t\tStill empty ready list\n");
		    break;
		  }
                }
		else{
