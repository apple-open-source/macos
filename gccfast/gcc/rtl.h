...
/* The next two fields should be referenced only via 
   get_fdo_note_count () and set_fdo_note_count () */
#define NOTE_FDO_COUNT(INSN)	XCINT (INSN, 4, NOTE)
#define NOTE_FDO_COUNT_HIGH(INSN)	XCINT (INSN, 6, NOTE)
#define NOTE_FDO_TABLEJUMP(INSN)    XCVEC (INSN, 4, NOTE)
#define NOTE_FDO_COUNT_RTX(INSN)    XCEXP (INSN, 4, NOTE)
...
enum insn_note
{
  ...
  /* Boolean indicating that the basic block is unlikely to be executed and
     should be moved to the UNLIKELY_EXECUTED_TEXT_SECTION. */
  NOTE_INSN_UNLIKELY_EXECUTED_CODE,

  /* Annotation for jump instructions indicating they should not be
     shortened during shorten_branches optimization; this is necessary for
     optimization that moves rarely executed code to separate section of
     .o file. */
  ...
  /* APPLE LOCAL begin new feedback */
  /* Creation pass placeholder for the actual incrementation RTL.
     We squirrel it away here until after RTL generation to prevent
     it from interfering with what the RTL looks like, for example
     so that group_case_nodes will not break. */
  NOTE_INSN_FDO_COUNT_RTX,
  /* Count of times an arc was executed.  INCOMING notes refer to
     the arc leading to the basic block in which the note appears;
     there should be only 1.  OUTGOING notes refer to the arc
     leaving the basic block; there should be only 1.  BLOCK notes
     refer to a basic block.  TABLEJUMP notes have a list of counts 
     for the immediately following tablejump instruction (blocks can 
     have both multiple predecessors and successors in this case).
     Not all possible notes will appear, but there should be sufficient
     information to deduce all block and arc counts. */
  NOTE_INSN_FDO_COUNT_INCOMING,
  NOTE_INSN_FDO_COUNT_OUTGOING,
  NOTE_INSN_FDO_COUNT_BLOCK,
  NOTE_INSN_FDO_COUNT_TABLEJUMP,
  /* APPLE LOCAL end new feedback */
...
extern rtx get_label_after_fix_bb	PARAMS ((rtx));
