...
typedef struct {
    double remove_time; // When record was removed
    const char *file;   // Source file where remove call originated
    int line;           // Source line where remove call originated
    int err;            // Error returned by DNSServiceRemoveRecord
} srp_record_remove_time_t;

static void
srp_mdns_stale_record_callback(DNSServiceRef sdref, DNSRecordRef rref,
                               DNSServiceFlags UNUSED flags, DNSServiceErrorType error, void *context)
{
    srp_record_remove_time_t *remove_time = context;
    if (remove_time != NULL) {
        FAULT("stale record callback %lf seconds later for sdref %p rref %p error %d (now %d) at " PUB_S_SRP ":%d",
              remove_time->remove_time, sdref, rref, remove_time->err, error, remove_time->file, remove_time->line);
    } else {
        FAULT("stale record callback for sdref %p rref %p with null context pointer, error %d.", sdref, rref, error);
    }
}

void
srp_mdns_shared_record_remove_(srp_server_t *server_state, adv_record_t *record, bool remove_previous,
                               const char *file, int line)
{
    RETAIN_HERE_LOG(record, adv_record);
    ...
    if (rref_to_remove != NULL) {
        if (shared_txn != 0 && shared_txn == (intptr_t)server_state->shared_registration_txn) {
            const char *valid = (dns_service_record_validate(server_state, server_state->shared_registration_txn->sdref,
                                                             rref_to_remove)
                                 ? "valid" : "invalid");
            INFO("removing " PUB_S_SRP " " PUB_S_SRP " %p %p", valid, which, rref_to_remove, record);
            int err = dns_service_remove_record(server_state,
                                                server_state->shared_registration_txn->sdref, rref_to_remove, 0);
            // Should not validate after removing
            if (dns_service_record_validate(server_state, server_state->shared_registration_txn->sdref, rref_to_remove))
            {
                FAULT("record %p (%p %p) validates after removal with error %d",
                      record, rref_to_remove, (void *)shared_txn, err);
                srp_record_remove_time_t *remove_time = srp_strict_calloc(1, sizeof(remove_time));
                if (remove_time != NULL) {
                    remove_time->remove_time = srp_fractional_time();
                    remove_time->file = file;
                    remove_time->line = line;
                    remove_time->err = err;
                }
                dns_service_record_set_callback(server_state, server_state->shared_registration_txn->sdref,
                                                rref_to_remove, srp_mdns_stale_record_callback, remove_time);
            }
            ...
            if (err == kDNSServiceErr_NoError) {
                RELEASE_HERE_LOG(record, adv_record); // Release the DNSService callback's reference
            } else {
            ...
            // Should not validate here--this should never ever actually happen
            if (dns_service_record_validate(server_state, (DNSServiceRef)shared_txn, rref_to_remove))
            {
                FAULT("record %p (%p %p) validates when shared_txn != server_state->shared_txn %p",
                      record, rref_to_remove, (void *)shared_txn, server_state->shared_registration_txn);
                srp_record_remove_time_t *remove_time = srp_strict_calloc(1, sizeof(remove_time));
                if (remove_time != NULL) {
                    remove_time->remove_time = srp_fractional_time();
                    remove_time->file = file;
                    remove_time->line = line;
                    // DNSServiceRemoveRecord should never return this, so we can use it to flag this scenario.
                    remove_time->err = kDNSServiceErr_NATPortMappingUnsupported;
                }
                dns_service_record_set_callback(server_state, server_state->shared_registration_txn->sdref,
                                                rref_to_remove, srp_mdns_stale_record_callback, remove_time);
            }
    ...
    RELEASE_HERE_LOG(record, adv_record);
}
...
adv_record_finalize(adv_record_t *record)
{
    ...
    srp_strict_free(&record->rdata);
    srp_strict_free(&record);
}
...
adv_instance_finalize(adv_instance_t *instance)
{
    ...
    srp_strict_free(&instance->txt_data);
    srp_strict_free(&instance->instance_name);
    srp_strict_free(&instance->service_type);
...
{                                                                  \
    type ## _vec_t *vec;                                           \
    vec = srp_strict_calloc(1, sizeof(*vec));                      \
    if (vec != NULL) {                                             \
        if (size == 0) {                                           \
            size = 1;                                              \
        }                                                          \
        vec->vec = srp_strict_calloc(size, sizeof (*(vec->vec)));  \
        if (vec->vec == NULL) {                                    \
            srp_strict_free(&vec);                                 \
        } else {                                                   \
...
#define DECLARE_VEC_COPY(type, retain)                  \
static type ## _vec_t *                                 \
...
#define DECLARE_VEC_FINALIZE(type, release)                     \
static void                                                     \
...
{                                                               \
    ...
    for (i = 0; i < vec->num; i++) {                            \
        if (vec->vec[i] != NULL) {                              \
            release(vec->vec[i], type);                         \
            vec->vec[i] = NULL;                                 \
        }                                                       \
    }                                                           \
    srp_strict_free(&vec->vec);                                 \
    srp_strict_free(&vec);                                      \
}


DECLARE_VEC_CREATE(adv_instance);
DECLARE_VEC_COPY(adv_instance, RETAIN_HERE);
DECLARE_VEC_FINALIZE(adv_instance, RELEASE_HERE);

DECLARE_VEC_CREATE(adv_record);
DECLARE_VEC_COPY(adv_record, RETAIN_HERE_LOG);
DECLARE_VEC_FINALIZE(adv_record, RELEASE_HERE_LOG);
...
srp_dump_server_stats(srp_server_t *server_state, bool full, bool periodic)
{
    ...
    for (adv_host_t *hp = server_state->hosts; hp != NULL; hp = hp->next) {
        ...
        if (full) {
            INFO("host %p " PRI_S_SRP " key_id %xu stable %" PRIx64 " lease %d key_lease %d expiry %d" PUB_S_SRP PUB_S_SRP,
                 hp, hp->name, hp->key_id, hp->server_stable_id, hp->lease_interval, hp->key_lease, expiry,
                ...
                if (hp->addresses->vec[i] != NULL) {
                    ...
                    if (record->rrtype == dns_rrtype_a) {
                        if (full) {
                            ...
                            INFO("%p  IN    A " PRI_IPv4_ADDR_SRP PRI_S_SRP, record,
                                 IPv4_ADDR_PARAM_SRP(record->rdata, addr_buf),
                        ...
                        if (full) {
                            ...
                            INFO("%p  IN AAAA " PRI_IPv6_ADDR_SRP PRI_S_SRP, record,
                                 IPv6_ADDR_PARAM_SRP(record->rdata, addr_buf),
                                 record->shared_txn == (intptr_t)server_state->shared_registration_txn ? " live" : "");
                        }
                        aaaa_record_count++;
                    }
        ...
        if (hp->key_record != NULL) {
            INFO("%p  IN KEY %xu " PUB_S_SRP, hp->key_record, hp->key_id,
                 hp->key_record->shared_txn == (intptr_t)server_state->shared_registration_txn ? " live" : "");
        }
                                   ...
                                   srpl_connection_t *srpl_connection, comm_t *connection, int rcode, bool last)
{
	if (server_state->srp_replication_enabled) {
        ...
        if (host != NULL) {
            if (rcode == dns_rcode_noerror) {
                memcpy(&host->server_stable_id, &host->server_state->srp_server_stable_id, sizeof(host->server_stable_id));
            }
...
srp_mdns_shared_registration_txn_setup(srp_server_t *server_state)
{
    if (server_state->shared_registration_txn == NULL) {
        ...
        if (err != kDNSServiceErr_NoError) {
            ERROR("unable to create shared connection for registration: %d", err);
            return false;
        }
        server_state->shared_registration_txn = ioloop_dnssd_txn_add(sdref, server_state, NULL, shared_registration_fail);
        if (server_state->shared_registration_txn == NULL) {
            ERROR("unable to create shared connection transaction object for registration.");
                  ...
                  const char *parent_type, const void *parent_pointer, const char *hostname)
{
    ...
    if (record->rref != NULL && record->shared_txn == affected_service_pointer) {
        INFO("forgetting record %p rref %p on " PUB_S_SRP " %p " PRI_S_SRP, record, record->rref, parent_type, parent_pointer, hostname);
        record->rref = NULL;
    }
}
...
record_vec_txns_forget(adv_record_vec_t *records, intptr_t affected_service_pointer,
                       const char *parent_type, const void *parent_pointer, const char *hostname, adv_record_t *expected)
{
    bool seen = false;
    if (records == NULL) {
        return seen;
    ...
    for (int i = 0; i < records->num; i++) {
        if (records->vec[i] != NULL && records->vec[i] == expected) {
            seen = true;
        }
...
static bool
host_txns_forget(adv_host_t *host, intptr_t affected_service_pointer, adv_record_t *expected)
{
    bool seen = false;
    ...
    // service pointer.
    if (record_vec_txns_forget(host->addresses, affected_service_pointer, "host", host, host->name, expected)) {
        seen = true;
    }
    instance_vec_txns_forget(host->instances, affected_service_pointer, "host", host, host->name);
    if (host->key_record == expected) {
        seen = true;
    }
    record_txn_forget(host->key_record, affected_service_pointer, "host key", host, host->name);
    if (host->update != NULL) {
        if (record_vec_txns_forget(host->update->remove_addresses, affected_service_pointer,
                                   "host update remove addresses", host->update, host->name, expected))
        {
            seen = true;
        }
        if (record_vec_txns_forget(host->update->add_addresses, affected_service_pointer,
                                   "host update add addresses", host->update, host->name, expected))
        {
            seen = true;
        }
        if (host->update->key == expected) {
            seen = true;
        }
...
static bool
service_disconnected(srp_server_t *server_state, intptr_t service_pointer, adv_record_t *expected)
{
    bool seen = false;
        ...
        server_state->shared_registration_txn != NULL)
    {
        ...
        for (adv_host_t *host = server_state->hosts; host != NULL; host = host->next) {
            if (host_txns_forget(host, service_pointer, expected)) {
                seen = true;
            }
        }
        dns_service_op_not_to_be_freed = NULL;
    ...
    // We don't want to indicate that we saw a record if the expected record is NULL.
    if (expected == NULL) {
        seen = false;
    }
    return seen;
...
static void
adv_update_free_instance_vectors(adv_update_t *NULLABLE update)
{
    if (update == NULL) {
        return;
    }
...
static void
adv_update_finalize(adv_update_t *NULLABLE update)
{
    if (update == NULL) {
        return;
    }
    ...
    if (update->key != NULL) {
        RELEASE_HERE_LOG(update->key, adv_record);
    }
    ...
    if (host->addresses != NULL) {
        for (i = 0; i < host->addresses->num; i++) {
            if (host->addresses->vec[i] != NULL) {
                ...
                RELEASE_HERE_LOG(host->addresses->vec[i], adv_record);
...
host_invalidate(adv_host_t *host)
{
    ...
    if (host->key_record != NULL) {
        ...
        RELEASE_HERE_LOG(host->key_record, adv_record);
...
adv_host_finalize(adv_host_t *host)
{
    ...
    srp_strict_free(&host->key_rdata);
    if (host->key_record != NULL) {
        RELEASE_HERE_LOG(host->key_record, adv_record);
        host->key_record = NULL;
    }
    ...
    srp_strict_free(&host->name);
    srp_strict_free(&host->registered_name);
    srp_strict_free(&host);
}
...
host_ready(adv_host_t *host)
{
    ...
    if (*p_hosts == NULL) {
        ERROR("called with nonexistent host %p.", host);
        return NULL;
    }
...
srp_mdns_update_finished(adv_update_t *update)
{
        ...
        if (host->addresses != NULL) {
            for (i = 0; i < host->addresses->num; i++) {
                ...
                if (rec != NULL && (update->remove_addresses == NULL || update->remove_addresses->vec[i] == NULL))
                {
                    ...
                    addresses->vec[j] = rec;
                    RETAIN_HERE_LOG(addresses->vec[j], adv_record);
                    j++;
                }
            }
        }
        if (update->add_addresses != NULL) {
            for (i = 0; i < update->add_addresses->num; i++) {
                ...
                if (rec != NULL) {
                    ...
                    RELEASE_HERE_LOG(update->add_addresses->vec[i], adv_record);
    ...
    if (host->key_record == NULL && update->key != NULL) {
        ...
        RETAIN_HERE_LOG(host->key_record, adv_record);
        if (update->key->update != NULL) {
            RELEASE_HERE(update->key->update, adv_update);
            update->key->update = NULL;
        }
            ...
            if (update->host != NULL) {
                if (update->host->server_state != NULL) {
                    service_disconnected(update->host->server_state,
                                         (intptr_t)update->host->server_state->shared_registration_txn, NULL);
                }
                             ...
                             const char *name, const char *regtype, const char *domain, void *context)
{
    ...
    if (error_code == kDNSServiceErr_ServiceNotRunning || error_code == kDNSServiceErr_DefunctConnection) {
        bool freed = !srp_log_ref_check(instance, "adv_instance", __FILE__, __LINE__, true);
        INFO("instance defunct: sdref %p flags %x error %d context %p freed " PUB_S_SRP ".",
             sdref, flags, error_code, context, freed ? "yes" : "indeterminate");
        return;
    }
    ...
    if (error_code == kDNSServiceErr_NoError) {
        ...
        if (error_code == kDNSServiceErr_ServiceNotRunning || error_code == kDNSServiceErr_DefunctConnection) {
            service_disconnected(host->server_state, instance->shared_txn, NULL);
...
register_instance(adv_instance_t *instance)
{
    ...
    if (tsr_attribute == NULL) {
        ...
        uint32_t flags = kDNSServiceFlagsShareConnection | kDNSServiceFlagsNoAutoRename | kDNSServiceFlagsKnownUnique;
    ...
    if (err != kDNSServiceErr_NoError) {
            ...
            err == kDNSServiceErr_BadParam || err == kDNSServiceErr_BadReference || err == 1)
        {
            ...
            service_disconnected(server_state, (intptr_t)server_state->shared_registration_txn, NULL);
        } else {
...
update_from_host(adv_host_t *host)
{
    ...
    update = srp_strict_calloc(1, sizeof *update);
    if (update == NULL) {
...
static void
srp_mdns_record_disconnect(void *context)
{
    srp_servers->disconnect_scheduled = false;
    bool seen = service_disconnected(srp_servers, (intptr_t)srp_servers->shared_registration_txn, context);
    if (context != NULL && !seen) {
        FAULT("record %p was not seen", context);
        abort();
    }
    INFO("successfully disconnected services: %p", context);
}
                                ...
                                DNSServiceFlags flags, DNSServiceErrorType error_code, void *context)
{
    ...
    if (error_code == kDNSServiceErr_ServiceNotRunning || error_code == kDNSServiceErr_DefunctConnection) {
        bool freed = !srp_log_ref_check(record, "adv_record", __FILE__, __LINE__, true);
        INFO("service defunct: sdref %p rref %p flags %x error %d context %p freed " PUB_S_SRP ".",
             sdref, rref, flags, error_code, context, freed ? "yes" : "indeterminate");
        if (!srp_servers->disconnect_scheduled) {
            srp_servers->disconnect_scheduled = true;
            ioloop_run_async(srp_mdns_record_disconnect, context, NULL);
        }
        return;
    }
    ...
    if (error_code == kDNSServiceErr_NoError) {
        ...
        // want to retry later on in the hopes that mDNSResponder will come back.
        if (error_code == kDNSServiceErr_ServiceNotRunning || error_code == kDNSServiceErr_DefunctConnection) {
            service_disconnected(host->server_state, record->shared_txn, NULL);
            if (update != NULL) {
                host->registered = false;
                wait_retry(host);
            }
...
out:
    RELEASE_HERE_LOG(record, adv_record); // Release the reference to the record that we retained at the beginning
}
...
adv_instance_create(service_instance_t *raw, adv_host_t *host, adv_update_t *update)
{
    ...
    adv_instance_t *instance = srp_strict_calloc(1, sizeof *instance);
    if (instance == NULL) {
    ...
    instance->instance_name = srp_strict_strdup(instance_name);
    if (instance->instance_name == NULL) {
        ERROR("adv_instance:create: unable to allocate instance name.");
        RELEASE_HERE(instance, adv_instance);
        return NULL;
    }
    instance->service_type = srp_strict_strdup(service_type);
    if (instance->service_type == NULL) {
    ...
    if (raw->txt != NULL) {
        if (raw->txt->data.txt.len > 0) {
            txt_data = srp_strict_malloc(raw->txt->data.txt.len);
            if (txt_data == NULL) {
                RELEASE_HERE(instance, adv_instance);
...
adv_record_create_(uint16_t rrtype, uint16_t rdlen, uint8_t *rdata, adv_host_t *host, const char *file, int line)
{
    ...
    adv_record_t *new_record = srp_strict_calloc(1, sizeof(*new_record) + rdlen - 1);
    if (new_record == NULL) {
        ERROR("no memory for new_record");
        return NULL;
    }
    if (rdlen > 0) {
        new_record->rdata = srp_strict_malloc(rdlen);
        if (new_record->rdata == NULL) {
            ERROR("no memory for new_record->rdata");
            srp_strict_free(&new_record);
            return NULL;
        }
    ...
    RETAIN_LOG(new_record, adv_record);
    return new_record;
}
...
register_host_record(adv_host_t *host, adv_record_t *record, bool skipping)
{
    ...
    if (err != kDNSServiceErr_NoError) {
            ...
            err == kDNSServiceErr_BadParam || err == kDNSServiceErr_BadReference || err == 1)
        {
            ...
            service_disconnected(host->server_state, (intptr_t)host->server_state->shared_registration_txn, NULL);
        } else {
    ...
    RETAIN_HERE_LOG(record, adv_record); // for the callback
    if (host->update != NULL) {
...
start_host_update(adv_host_t *host)
{
         ...
         host->key_record->rref == NULL))
    {
        ...
        RETAIN_HERE_LOG(update->key, adv_record);
        RELEASE_HERE_LOG(host->key_record, adv_record);
        host->key_record = NULL;
    ...
    if (num_remove_addrs != 0) {
        for (i = 0; i < num_remove_addrs; i++) {
            if (host->addresses != NULL && host->addresses->vec[i] != NULL) {
                ...
                RETAIN_HERE_LOG(remove_addrs->vec[i], adv_record);
            }
            for (j = 0; j < num_add_addrs; j++) {
                ...
                {
                    RELEASE_HERE_LOG(remove_addrs->vec[i], adv_record);
                    remove_addrs->vec[i] = NULL;
                    RELEASE_HERE_LOG(add_addrs->vec[j], adv_record);
...
found_something:
    ...
    if (outcome == missed) {
        ...
        host = srp_strict_calloc(1, sizeof *host);
        if (host == NULL) {
        ...
        host->name = srp_strict_strdup(pres_name);
        if (host->name == NULL) {
            RELEASE_HERE(host, adv_host);
            host = NULL;
        ...
        host->key_rdata = srp_strict_malloc(host->key_rdlen);
        if (host->key_rdata == NULL) {
            RELEASE_HERE(host, adv_host);
            host = NULL;
...
{
    srp_server_t *server_state = srp_strict_calloc(1, sizeof(*server_state));
    if (server_state == NULL || (server_state->name = srp_strict_strdup(name)) == NULL) {
        ERROR("no memory for server state");
        srp_strict_free(&server_state);
        return NULL;
    }
...
#  endif
    ...
    INFO("--------------------------------"
         ...
         "--------------------------------", __DATE__, __TIME__);
#endif
    srp_servers->start_time = srp_time();
    ...
    if (0) {
        ...
        srp_servers->srp_server_stable_id = srp_random64();

        // Start srp listener
        srp_proxy_listen(NULL, 0, NULL, NULL, NULL, NULL, NULL, srp_servers);

#  if SRP_FEATURE_REPLICATION
        if (srp_servers->srp_replication_enabled) {
            INFO("start srp replication.");
            srpl_startup(srp_servers);
        }
#  endif
