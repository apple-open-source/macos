...
- (id) initWithCCache: (cc_ccache_t) newCCache
{
    if ((self = [super init])) {
	...
	ccache = newCCache;
        ...
        if ([self update] != ccNoError) {
            ccache = NULL; // Caller won't expect us to free this
            [self release];
            self = NULL;
        }
    }
...
- (int) update
{
    ...
    if (!err) {
	err = cc_ccache_get_change_time (ccache, &newLastChangeTime);
    }
    
    if (!err) {
        err = cc_ccache_get_last_default_time (ccache, &newLastDefaultTime);
	...
	if (!err) {
        ...
        if (!err) {
            err = cc_ccache_get_principal (ccache, cc_credentials_v5, &credsPrincipal);
        }
            ...
            if (!err) {
                if (creds->data->version == cc_credentials_v5) {
                    ...
                    if (credential) { 
                        ...
                        if (!credential) { err = ccErrInvalidCredentials; }
                    }
