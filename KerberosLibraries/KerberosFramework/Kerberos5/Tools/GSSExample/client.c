/*
 * Copyright 2004-2008 Massachusetts Institute of Technology.
 * All Rights Reserved.
 *
                           ...
                           const char            **outClientName)
{
    ...
    if (!err) {
        /* Using your bundle ID is fine here */
        err = kim_selection_hints_create (&hints, kGSSExampleID);
    }
    
    if (!err) {
        /* If it's obvious why your application needs Kerberos, 
         * you can skip this. */
        err = kim_selection_hints_set_explanation (hints, 
                                                   kGSSExampleExplanantion);
    }
    
    /* The selection hints exist so that if your application is
     * document based and multiple documents can be simultaneously 
     * using Kerberos, you can associate a different identity with
     * each document.  If your documents connect to different user
     * accounts, change the kim_hint_key_user hint. In this case
     * each connection is to a different host and service so we set 
     * kim_hint_key_server and kim_hint_key_service_identity. */
    if (!err) {
    ...
    if (!err) {
        *outSelectionHints = hints;
        hints = NULL; /* caller takes ownership */
        
        *outClientName = clientName;
        clientName = NULL; /* caller takes ownership */
    } else { 
                         ...
                         gss_ctx_id_t *outGSSContext)
{
    ...
    /*
     * Here is where the client sets up the client principal it wants to use.  
...
int main (int argc, const char *argv[]) 
{
    ...
    if (!err && !clientName) {
        /* If the user didn't pass in a client name, try to select one */
    ...
    if (!err && selectedClientName) {
        /* Remember the client identity we selected */
