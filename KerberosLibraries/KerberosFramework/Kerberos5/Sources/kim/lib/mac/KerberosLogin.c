...
static KLStatus kl_check_error_ (kim_error inError, const char *function, const char *file, int line)
{
	...
	case KIM_KRB5_INIT_FAILED_ERR:
	    err = klV5InitializationFailedErr;
	    break;
	case KIM_NO_REALMS_ERR:
	    err = klNoRealmsErr;
	    break;
	case KIM_NO_SUCH_REALM_ERR:
	    err = klRealmDoesNotExistErr;
	    break;
	case KIM_UNSUPPORTED_HINT_ERR:
	    err = klBadLoginOptionsErr;
	    break;
	case KIM_CLOCK_SKEW_ERR:
	    err = KRB5_KDCREP_SKEW;
	    break;
	case KIM_BAD_PRINCIPAL_STRING_ERR:
	case KIM_BAD_COMPONENT_INDEX_ERR:
	    err = klBadPrincipalErr;
	    break;
	case KIM_PASSWORD_MISMATCH_ERR:
	    err = KRB5KRB_AP_ERR_BAD_INTEGRITY;
	    break;
	case KIM_INSECURE_PASSWORD_ERR:
	    err = klInsecurePasswordErr;
	    break;
	case KIM_PASSWORD_CHANGE_FAILED_ERR:
	    err = klPasswordChangeFailedErr;
	    break;
	case KIM_PREAUTH_FAILED_ERR:
	    err = KRB5KRB_AP_ERR_BAD_INTEGRITY;
	    break;
	case KIM_BAD_OPTIONS_ERR:
	case KIM_BAD_OPTIONS_VALUE_ERR:
	    err = klBadLoginOptionsErr;
	    break;
	case KIM_CAPS_LOCK_ERR:
	    err = klCapsLockErr;
	    break;
	case KIM_NO_SERVER_ERR:
	    err = klPrincipalDoesNotExistErr;
	    break;
	case KIM_NO_UI_ERR:
	case KIM_DUPLICATE_UI_REQUEST_ERR:
	    err = klCantDisplayUIErr;
	    break;
	case KIM_PREFERENCES_READ_ERR:
	    err = klPreferencesReadErr;
	    break;
	case KIM_PREFERENCES_WRITE_ERR:
	    err = klPreferencesWriteErr;
	    break;
	case KIM_IDENTITY_NOT_IN_LIST_ERR:
	case KIM_IDENTITY_ALREADY_IN_LIST_ERR:
	case KIM_BAD_IDENTITY_INDEX_ERR:
	    err = klPrincipalDoesNotExistErr;
	    break;
	case KIM_NO_SUCH_PRINCIPAL_ERR:
	    err = klPrincipalDoesNotExistErr;
	    break;
	case KIM_CANT_BECOME_DEFAULT_ERR:
	    err = klPrincipalDoesNotExistErr;
	    break;
	case KIM_CREDENTIALS_EXPIRED_ERR:
	    err = klCredentialsExpiredErr;
	    break;
	case KIM_NO_CREDENTIALS_ERR:
	    err = klNoCredentialsErr;
	    break;
	case KIM_BAD_IP_ADDRESS_ERR:
	    err = klCredentialsBadAddressErr;
	    break;
	case KIM_NO_SUCH_CCACHE_ERR:
	    err = klCacheDoesNotExistErr;
	    break;
	case KIM_BAD_HOST_CONFIGURATION_ERR:
	    err = klPreferencesReadErr;
	    break;
	case KIM_NEEDS_VALIDATION_ERR:
	    err = klCredentialsNeedValidationErr;
	    break;
    }
                                              ...
                                              char           **outCredCacheName)
{
    kim_error err = KIM_NO_ERROR;
    kim_ccache ccache = NULL;
    kim_boolean allow_prompt;
    KLLoginOptions  internalOptions = NULL;

    if (inLoginOptions == NULL) {
	KLCreateLoginOptions(&internalOptions);
	inLoginOptions = internalOptions;
    }

    allow_prompt = kim_options_get_allow_prompt(inLoginOptions);
    kim_options_set_allow_prompt(inLoginOptions, FALSE);
    ...
    kim_options_set_allow_prompt(inLoginOptions, allow_prompt);
    ...
    if (internalOptions)
	KLDisposeLoginOptions(internalOptions);
