...
kim_error kim_options_set_allow_prompt (kim_options  io_options, kim_boolean allow_prompt)
{
    kim_error err = KIM_NO_ERROR;
    ...
    if (!err) {
	io_options->allow_prompt = allow_prompt;
    }
    
    return check_error (err);
}

kim_boolean kim_options_get_allow_prompt (kim_options  io_options)
{
    if (!io_options) { return kim_default_allow_prompt; }
    
    return io_options->allow_prompt;
}
