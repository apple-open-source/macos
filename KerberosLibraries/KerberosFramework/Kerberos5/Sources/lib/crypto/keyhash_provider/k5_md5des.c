	       ...
	       const krb5_data *input, krb5_data *output)
{
    ...
    if (output->length != (CONFLENGTH+CC_MD5_DIGEST_LENGTH))
	return(KRB5_CRYPTO_INTERNAL);
    ...
    CC_MD5_Init(&ctx);
    CC_MD5_Update(&ctx, conf, CONFLENGTH);
    CC_MD5_Update(&ctx, (unsigned char *) input->data, 
		   (unsigned int) input->length);
    CC_MD5_Final(output->data+CONFLENGTH, &ctx);
...
{
    CC_MD5_CTX ctx;
    unsigned char plaintext[CONFLENGTH+CC_MD5_DIGEST_LENGTH];
    unsigned char xorkey[8];
    ...
    if (ivec)
	return(KRB5_CRYPTO_INTERNAL);
    if (hash->length != (CONFLENGTH+CC_MD5_DIGEST_LENGTH)) {
#ifdef KRB5_MD5DES_BETA5_COMPAT
	if (hash->length != CC_MD5_DIGEST_LENGTH)
	    return(KRB5_CRYPTO_INTERNAL);
	else
	    compathash = 1;
#else
    ...
    CC_MD5_Init(&ctx);
    if (!compathash) {
	CC_MD5_Update(&ctx, plaintext, CONFLENGTH);
    }
    CC_MD5_Update(&ctx, (unsigned char *) input->data, 
    ...
    if (!compathash) {
	*valid =
	    (memcmp(plaintext+CONFLENGTH, digest, CC_MD5_DIGEST_LENGTH)
	     == 0);
    } else {
	*valid = 
	    (memcmp(plaintext, digest, CC_MD5_DIGEST_LENGTH) == 0);
    }
...
const struct krb5_keyhash_provider krb5int_keyhash_md5des = {
    CONFLENGTH+CC_MD5_DIGEST_LENGTH,
    k5_md5des_hash,
    k5_md5des_verify
};
