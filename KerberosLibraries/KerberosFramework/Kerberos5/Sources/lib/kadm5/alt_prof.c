...
#ifdef __APPLE__
	hierarchy[2] = "pws_enabled";

	params.pws_enabled = FALSE;
	params.mask |= KADM5_CONFIG_PWS_ENABLED;

	if (params_in->mask & KADM5_CONFIG_PWS_ENABLED) {
		params.mask |= KADM5_CONFIG_PWS_ENABLED;
		params.pws_enabled = params_in->pws_enabled;
	} else {
		krb5_boolean bvalue;
		if (aprofile &&
		    !krb5_aprof_get_boolean(aprofile, hierarchy, TRUE, &bvalue)) {
		    params.pws_enabled = bvalue;
		    params.mask |= KADM5_CONFIG_PWS_ENABLED;
		}
	}
#endif
    ...
    krb5_realm_params	**rparamp;
{
    ...
    hierarchy[2] = "pws_enabled";
    if (!krb5_aprof_get_boolean(aprofile, hierarchy, TRUE, &bvalue))
	rparams->realm_pws_enabled = !!bvalue;
#endif
