...
{
    return
         kadm5_chpass_principal_4(server_handle, principal,
...
kadm5_ret_t
kadm5_chpass_principal_4(void *server_handle,
			 krb5_principal principal, krb5_boolean keepold,
			 int n_ks_tuple, krb5_key_salt_tuple *ks_tuple,
			 char *password, int is_admin)
{
    krb5_int32			now;
...
#ifdef USE_PASSWORD_SERVER
    if ((kadm5_use_password_server () || handle->params.pws_enabled) &&
        ...
        char *argv[] = { "mkpassdb", NULL, NULL, NULL };
        char *pstring = NULL;

	if (is_admin)
	  argv[1] = "-setpassword";
	else
	  argv[1] = "-setuserpassword";
