...

{
	...
	if (major == GSS_S_COMPLETE) {
	    ...
	    if ((cred_usage & GSS_C_OPTION_MASK) == GSS_C_ACCEPT) {
		if (outTime > acceptTimeOut)
		    outTime = acceptTimeOut;
	    } else if ((cred_usage & GSS_C_OPTION_MASK) == GSS_C_INITIATE) {
    ...
    OM_uint32 *acceptor_time_rec)
{
	...
	*initiator_time_rec = 0;
    ...
    if ((cred_usage & GSS_C_OPTION_MASK) != GSS_C_ACCEPT
	&& (cred_usage & GSS_C_OPTION_MASK) != GSS_C_INITIATE
	&& (cred_usage & GSS_C_OPTION_MASK) != GSS_C_BOTH) {
	if (minor_status) {
	    *minor_status = EINVAL;
	    map_errcode(minor_status);
	}
    ...
    OM_uint32		*acceptor_time_rec;
{
    ...
    else if ((cred_usage & GSS_C_OPTION_MASK) == GSS_C_BOTH)
	...
	union_cred->auxinfo.cred_usage = (cred_usage & GSS_C_OPTION_MASK);
	...
	if ((cred_usage & GSS_C_OPTION_MASK) == GSS_C_ACCEPT || (cred_usage & GSS_C_OPTION_MASK) == GSS_C_BOTH)
	    *acceptor_time_rec = time_rec;
    if (initiator_time_rec)
	if ((cred_usage & GSS_C_OPTION_MASK) == GSS_C_INITIATE || (cred_usage & GSS_C_OPTION_MASK) == GSS_C_BOTH)
