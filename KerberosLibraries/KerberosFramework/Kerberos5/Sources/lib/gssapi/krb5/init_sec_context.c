...
/*
 * $Id: init_sec_context.c 20876 2008-10-15 21:58:43Z tlyu $
 */
                   ...
                   void *cksum_data, krb5_data **out)
{
    ...
    if (data->ctx->gss_flags & GSS_C_DELEG_FLAG) {
        ...
        code = krb5_cc_get_config(context, data->cred->ccache,
                                  data->ctx->there, "gss-forward-cache", &credmsg);
        if (code) {
            ...
            if (code == NULL) {
                 ...
                 * ignore it
                 */
                krb5_cc_set_config(context, data->cred->ccache,
                                   data->ctx->there, "gss-forward-cache",
                                   &credmsg);
            }
        }
        ...
        if (code) {
            /* don't fail here; just don't accept/do the delegation
               request */
	    data->ctx->gss_flags &= ~(GSS_C_DELEG_FLAG|GSS_C_DELEG_POLICY_FLAG);
    ...
    int default_mech)
{
   ...
   if ((req_flags & GSS_C_DELEG_POLICY_FLAG) && (k_cred->ticket_flags & TKT_FLG_OK_AS_DELEGATE))
...
k5_mutex_t kg_nc_mutex = K5_MUTEX_PARTIAL_INITIALIZER;
...
static OM_uint32
check_neg_cache(OM_uint32 *minor_status, krb5_context context,
                gss_cred_id_t gss_cred, gss_name_t target_name)
{
    krb5_principal server = (krb5_principal)target_name;
    krb5_gss_cred_id_t cred = (krb5_gss_cred_id_t) gss_cred;
    OM_uint32 major;
    krb5_error_code ret;
    int check, i;
    
    ret = k5_mutex_lock(&kg_nc_mutex);
    if (ret)
        return GSS_S_COMPLETE;
    ...
    if (!nc.inited) {
        ...
        status = notify_register_check("com.apple.Kerberos.cache.changed",
                                       &nc.token);
        if (status != NOTIFY_STATUS_OK) {
            k5_mutex_unlock(&kg_nc_mutex);
            return GSS_S_COMPLETE;
        }
        nc.inited = 1;
    }
                 ...
                 gss_name_t target_name)
{
    ...
    if (cred && cred->princ) {
        krb5_copy_principal(context, cred->princ,
    ...
    OM_uint32 *time_rec;
{
    ...
    if (*context_handle == GSS_C_NO_CONTEXT) {
        ...
        major_status = check_neg_cache(minor_status, context, claimant_cred_handle, target_name);
    ...
    if (claimant_cred_handle == GSS_C_NO_CREDENTIAL) {
        major_status = kg_get_defcred(minor_status, (req_flags & GSS_C_NO_UI_FLAG), (gss_cred_id_t *)&cred);
    ...
    if (*context_handle == GSS_C_NO_CONTEXT) {
        ...
        if (GSS_ERROR(major_status))
            update_neg_cache(major_status, *minor_status,
                             context, claimant_cred_handle, target_name);
        k5_mutex_unlock(&cred->lock);
        if (*context_handle == GSS_C_NO_CONTEXT) {
            save_error_info (*minor_status, context);
            krb5_free_context(context);
