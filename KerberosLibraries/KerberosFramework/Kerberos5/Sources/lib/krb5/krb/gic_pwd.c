...
cleanup:
   ...
   if (ret == 0) {
      ...
      if (as_reply->enc_part2 && as_reply->enc_part2->last_req) {
	 last_req = as_reply->enc_part2->last_req;
	 if (*last_req && opte->opt_private && opte->opt_private->lr.func)
	     (*opte->opt_private->lr.func)(context, last_req,
					   opte->opt_private->lr.ctx);
      }
