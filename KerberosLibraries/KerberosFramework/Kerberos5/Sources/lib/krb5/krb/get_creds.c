		     ...
		     krb5_creds **out_creds)
{
	...
	*out_creds = ncreds;
    ...
    /* if its a request for a exact enctype, ignore negative cache, ie let nfs suffer */
    if (!not_ktype) {
	krb5_data neg;
	retval = krb5_cc_get_config(context, ccache, in_creds->server, "negative-cache", &neg);
	if (retval == 0) {
	    int32_t t32 = time(NULL); /* if entry less then 4, its a negativ entry */
	    ...
	    if (abs(time(NULL) - t32) < 7200) { /* negative entry not expired, fail */
		char *str = NULL;
