...
static const SecAsn1Template KRB5_PA_PK_AS_REP_IETF_DH_Template[] = {
    { SEC_ASN1_CONTEXT_SPECIFIC | SEC_ASN1_CONSTRUCTED | SEC_ASN1_EXPLICIT | 0,
      0,
      kSecAsn1PointerToAnyTemplate }
};

static const SecAsn1Template KRB5_PA_PK_AS_REP_IETF_RSA_Template[] = {
    ...
    krb5_data       *pa_pk_as_rep)      /* mallocd and RETURNED */
{
	...
	CSSM_DATA data;
	if (dh_signed_data) {
	    PKI_KRB_TO_CSSM_DATA(dh_signed_data, &data);
	    ortn = SecAsn1EncodeItem(coder, &data, KRB5_PA_PK_AS_REP_IETF_DH_Template, &der);
	} else if(enc_key_pack) {
	    PKI_KRB_TO_CSSM_DATA(enc_key_pack, &data);
	    ortn = SecAsn1EncodeItem(coder, &data, KRB5_PA_PK_AS_REP_IETF_RSA_Template, &der);
	} else {
	    ourRtn = EINVAL;
	    goto errOut;
	}
    ...
    krb5_data *enc_key_pack)
{
    ...
    memset(&asRep, 0, sizeof(asRep));
    if(SecAsn1DecodeData(coder, &der, KRB5_PA_PK_AS_REPTemplate, &asRep) == 0) {
	if(asRep.dhSignedData && (ourRtn = pkiCssmDataToKrb5Data(asRep.dhSignedData, dh_signed_data)))
	    goto errOut;
	if(asRep.encKeyPack && (ourRtn = pkiCssmDataToKrb5Data(asRep.encKeyPack, enc_key_pack)))
	    goto errOut;
    } else {
	CSSM_DATA data;
	memset(&data, 0, sizeof(data));
	if(SecAsn1DecodeData(coder, &der, KRB5_PA_PK_AS_REP_IETF_RSA_Template, &data) == 0) {
	    ourRtn = pkiCssmDataToKrb5Data(&data, enc_key_pack);
	    goto errOut;
	}
	memset(&data, 0, sizeof(data));
	if(SecAsn1DecodeData(coder, &der, KRB5_PA_PK_AS_REP_IETF_DH_Template, &data) == 0) {
	    ourRtn = pkiCssmDataToKrb5Data(&data, dh_signed_data);
	    goto errOut;
	}
	ourRtn = ASN1_BAD_FORMAT;
	goto errOut;
