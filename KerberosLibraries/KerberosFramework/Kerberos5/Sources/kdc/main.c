 ...
 */
kdc_realm_t *
find_realm_data(const char *rname, krb5_ui_4 rsize)
{
    int i;
...
init_realm(const char *progname, kdc_realm_t *rdp, const char *realm, 
	   const char *def_mpname, krb5_enctype def_enctype, char *def_udp_ports,
	   char *def_tcp_ports, krb5_boolean def_manual, char **db_args)
{
	...
	com_err(progname, ENOMEM, "out of memory");
	goto whoops;
    }
...
#ifdef __APPLE__
    /* Handle pws_enabled flag */
    if (rparams && rparams->realm_pws_enabled)
	rdp->realm_pws_enabled = 1;
    else
	rdp->realm_pws_enabled = 0;
#endif
...
static void
add_realm(const char *progname, const char *realm, const char *mkey_name, 
	  krb5_enctype menctype, int manual, char **db_args,
	  char *udp_ports, char	*tcp_ports)
{
    ...
    /* find dups */
    {
	const kdc_realm_t *r2;
	r2 = find_realm_data(realm, (krb5_ui_4) strlen(realm));
	if (r2)
	    return;
    }
    r = malloc(sizeof(*r));
    ...
    retval = init_realm(progname, r, realm, mkey_name, menctype,
			udp_ports, tcp_ports, manual, db_args);
    if (retval) {
    ...
    ptr = realloc(kdc_realmlist, sizeof(kdc_realmlist[0]) * (kdc_numrealms + 1));
    if (ptr == NULL) {
	fprintf(stderr,"%s: cannot allocate space for realm %s\n",
		progname, realm);
	exit(1);
    }
    kdc_realmlist = ptr;

    kdc_realmlist[kdc_numrealms] = r;
    kdc_numrealms++;
}
...
#define APPLE_KDC_MODS
	...
	case 'r':			/* realm name for db */
	    add_realm(argv[0], optarg, mkey_name, menctype, manual, db_args,
    ...
    if (kdc_numrealms == 0) {
#ifdef __APPLE__
	/* no realm specified, use all configured */
	char **realms, **cpp, *str;
        const char *names[2];
	profile_t profile;
	...
	retval = krb5_get_profile(kcontext, &profile);
	if (retval) {
	    fprintf(stderr, "Can't get profile to fetch realms\n");
	    exit(1);
	}

	retval = profile_get_subsection_names(profile, names, &realms);
	if (retval) {
	    fprintf (stderr, "%s: %s, attempting to retrieve realm list\n",
	...
	for (cpp = realms; *cpp; cpp++) {
	    /*
	     * Use the existance of the key "database_name" as
	     * litmus-test that we have a realm configured.
	     */
	    retval = profile_get_string(profile, "realms", *cpp,
					"database_name", NULL, &str);
	    if (retval || str == NULL)
		continue;
	    profile_release_string(str);

	    add_realm(argv[0], *cpp, mkey_name, menctype, manual, db_args,
		      default_udp_ports, default_tcp_ports);
	}
	...
	if (kdc_numrealms == 0) {
	    fprintf(stderr, "%s: didn't find any realms when starting\n", argv[0]);
	    exit(1);
	}
#else
	char *lrealm;
	...
	add_realm(argv[0], lrealm, mkey_name, menctype, manual, db_args,
		  default_udp_ports, default_tcp_ports);
#endif
    }
...
int main(int argc, char **argv)
{
    ...
    load_authdata_plugins(kcontext);
     ...
     pidfile = pidfile_open("/var/run/krb5kdc.pid", 0600, NULL);
     if (pidfile == NULL) {
 	printf("%s already running ?", argv[0]);
