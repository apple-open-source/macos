...
static void
dns_read_callback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info)
{
    DNSServiceErrorType ret;

    ret = DNSServiceProcessResult((DNSServiceRef)info);
...
register_srv_realms(CFStringRef host)
{
	...
	register_srv(kdc_realmlist[i]->realm_name, hostname, 88);
...
update_all(SCDynamicStoreRef store, CFArrayRef changedKeys, void *info)
{
    ...
    for (i = 0; i < kdc_numrealms; i++)
	update_entries(store, kdc_realmlist[i]->realm_name, flags);
