...
static krb5_pa_data *
find_pa_type(krb5_kdc_req *request, int type)
{
    krb5_pa_data **padata;
    
    for (padata = request->padata; *padata; padata++)
	if ((*padata)->pa_type == type)
	    return *padata;
    return NULL;
}
    ...
    void **pa_request_context)
{
     ...
     * Pick the key from the avaible keys in the client and configured keys in the kdb.
     */

    enctype = 0;
    for(i = 0; i < request->nktypes && enctype == 0; i++) {
	int j;
	if (!krb5_c_valid_enctype(request->ktype[i]))
	    continue;
	for(j = 0; j < client->n_key_data && enctype == 0; j++) {
	    krb5_key_data *key_data = &client->key_data[j];
	    if (key_data->key_data_type[0] == request->ktype[i] &&
		key_data->key_data_type[1] != KRB5_KDB_SALTTYPE_CERTHASH)
		enctype = request->ktype[i];
	}
    }
    if (enctype == 0) {
	krtn = KRB5KDC_ERR_ETYPE_NOSUPP;
	goto cleanup;
    }
    ...
    /*
     * If this is an IETF request, send the enctype assisated
     * checksumtype for the reply. Default to the old type that the
     * BTMM LKDC used.
     */
    cksum_type = context->kdc_req_sumtype;

    if (find_pa_type(request, KRB5_PADATA_PK_AS_09_BINDING)) {
	krb5_cksumtype *cksum_types;
	unsigned int num_types;

	krtn = krb5_c_keyed_checksum_types(context, encrypting_key->enctype,
					   &num_types, &cksum_types);
	if (krtn)
	    goto cleanup;
	if (num_types > 0) {
	    cksum_type = cksum_types[0];
	    ietf = 1;
	}
	free(cksum_types);
    }

    krtn = krb5_c_make_checksum(context, cksum_type, 
	encrypting_key, KRB5_KEYUSAGE_TGS_REQ_AUTH_CKSUM, 
	req_pkt, &as_req_checksum);
    if(krtn) {
	goto cleanup;
    }
     ...
     */
    krtn = krb5int_pkinit_as_rep_create(context, ietf,
