...
struct ccs_ccache_d ccs_ccache_initializer = { NULL, 0, NULL, 0, NULL, NULL, NULL, 0, 0, 0, 0, 0, 0, NULL, NULL };

/* ------------------------------------------------------------------------ */

cc_int32 ccs_ccache_new (ccs_ccache_t      *out_ccache,
			 au_asid_t	    in_session_id,
                         ...
                         ccs_ccache_list_t  io_ccache_list)
{
    ...
    if (!err) {
	ccache->session_id = in_session_id;
	ccs_session_add(in_session_id);
    }
...
css_ccache_valid(ccs_ccache_t ccache, cc_time_t now, int *valid)
{
    ...
    err = ccs_credentials_list_new_iterator (ccache->credentials,
					     CCS_PIPE_NULL,
					     &credentials_iterator);
	...
	err = ccs_credentials_list_iterator_next (credentials_iterator,
						  &credentials);
	if (!err)
	    err = ccs_credentials_valid(credentials, now, valid);
    }
    if (err == ccIteratorEnd) { err = ccNoError; }

    if (credentials_iterator)
	ccs_credentials_list_iterator_release (credentials_iterator);
...
ccs_ccache_kill_session(ccs_ccache_t ccache, 
			...
			au_asid_t session_id)
{
    if (ccache->session_id == session_id)
	ccs_cache_collection_destroy_ccache(io_cache_collection,
					    ccache->identifier);
}
