...
cc_int32
ccs_credentials_valid(ccs_credentials_t creds, cc_time_t now, int *valid)
{
    cc_int32 err = ccNoError;
    
    if (creds->cred_union->version == cc_credentials_v5) {
	if (creds->cred_union->credentials.credentials_v5->endtime > now)
	    *valid = 1;
    } else
