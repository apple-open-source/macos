...
@DateTime::Locale::pt_BR::ISA = qw(DateTime::Locale::pt);
...
sub full_time_format               { "\%Hh\%Mmin\%Ss\ \%\{time_zone_long_name\}" }
sub long_time_format               { "\%\{hour\}h\%\{minute\}min\%\{second\}s\ \%\{time_zone_long_name\}" }
