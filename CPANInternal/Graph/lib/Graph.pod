...
By default, the vertex names of the connected graph are formed from
...
use the C<supercomponent>) option:

    $cg = $g->connected_graph(super_component => sub { ... });

The subroutine reference gets the 'subcomponents' (the original
vertices) as arguments, and it is supposed to return the new
supercomponent vertex.
...
graph.  The vertex attribute C<subvertices> is also used to store the
list (as an array reference) of the original vertices.
