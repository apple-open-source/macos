...
{
	if (bit)
		SetLE16(ee_p, EEPROM_CONTROL_EEDI);
	else
		ClearLE16(ee_p, EEPROM_CONTROL_EEDI);

	SetLE16(ee_p, EEPROM_CONTROL_EESK);
    IODelay(MIN_SK_HIGH);
    ClearLE16(ee_p, EEPROM_CONTROL_EESK);
    IODelay(MIN_SK_LOW);
}
...
bool EEPROMReadBit(volatile eeprom_control_t * ee_p)
{
	...
	SetLE16(ee_p, EEPROM_CONTROL_EESK);
    IODelay(MIN_SK_HIGH);
	bit = (ReadLE16(ee_p) & EEPROM_CONTROL_EEDO) ? 1 : 0;
	ClearLE16(ee_p, EEPROM_CONTROL_EESK);
    IODelay(MIN_SK_LOW);
    return (bit);
}
...
{
	SetLE16(ee_p, EEPROM_CONTROL_EECS);
    return;
}
...
{
    ClearLE16(ee_p, EEPROM_CONTROL_EECS);
    return;
}
