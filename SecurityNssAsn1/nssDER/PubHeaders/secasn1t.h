...
#ifdef	__APPLE__
/* 
 * This eliminates the need for specification of 
 * SECItem.type = siUnsignedInteger. The default is unsigned,
 * which causes a leading zero to be encoded if the MS bit of the
 * source data is 1.
 * As far as I know we *always* use the default of unsigned, at least'
 * in the PKI context. 
 */
...
** "arg" is a pointer to the structure being encoded/decoded, generally
**       a SECItem
...
#ifdef	__APPLE__
/*
** "buf", on decode, points to the start of the decoded data. Callee
**        can use the tag at this location to infer the returned
**        template. Not used on encode. "Dest" points to the specifc
**		  item (typically a SECItem) being decoded TO. (This is as 
**		  opposed to arg, which points to the start of the struct
**		  associated with the current array of templates). 
*/
#endif	/* __APPLE__ */

typedef const SEC_ASN1Template * SEC_ASN1TemplateChooser(
	void *arg, 
	PRBool enc,
	const char *buf,	/* __APPLE__ addenda */
	void *dest);		/* __APPLE__ addenda */
