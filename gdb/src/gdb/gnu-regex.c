...
# define REALLOC(p,s) xrealloc ((p), (size_t) (s))
#else
...
#define FREE_STACK_RETURN(value)		\
  return (xfree (compile_stack.stack), value)
...
int
gdb_regcomp (preg, pattern, cflags)
    regex_t *preg;
    const char *pattern;
    int cflags;
{
...
int
gdb_regexec (preg, string, nmatch, pmatch, eflags)
    const regex_t *preg;
    const char *string;
    size_t nmatch;
    regmatch_t pmatch[];
