  ...
  if (funname == NULL)
    {
      ...
      if (NULL != esymbol)
 	{
 	  ...
 	  func = NULL;
 	  args = 0;		/* at least for AIX traceback tables there is no argument info */
 	}
    }
#endif /* TARGET_HAS_EMBEDDED_SYMBOLS */
  ...
  if (fi->saved_regs != NULL)
    {
      ...
      for (i = 0; i < numregs; i++)
	if (fi->saved_regs[i] && (i != SP_REGNUM) && (i != PC_REGNUM))
	  {
	    if (count == 0)
