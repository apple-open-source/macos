...
/* APPLE LOCAL: See comments in frame_info (). */

struct frame_info *deprecated_frame_next_hack (struct frame_info *frame)
{
  return frame->next;
}

void *deprecated_frame_cache_hack (struct frame_info *frame)
{
  return & (frame->prologue_cache);
}

void default_print_extra_frame_info (struct frame_info *next_frame, void **this_cache)
{
  return;
}
...
frame_unwind_address_in_block (struct frame_info *next_frame)
{
  ...
  /* APPLE LOCAL: If, for whatever reason, we have ended up with a
     bogus value of $pc as 0, subtracting 1 will give us rather
     confusing output. */
