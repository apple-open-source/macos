...
kdp_set_trace_bit (int step)
{
  ...
  default:
    error ("kdp_set_trace_bit: unknown host type 0x%lx", (unsigned long) kdp_host_type);
  }
}
...
kdp_wait (ptid_t pid, struct target_waitstatus *status, gdb_client_data client_data)
{
  ...
  if (PIDGET (pid) != KDP_REMOTE_ID) {
    error ("kdp: unable to switch to process-id %s", target_pid_to_str (pid));
  }
...
kdp_store_registers (int regno)
{
  ...
  default:
    error ("kdp_store_registers: unknown host type 0x%lx", (unsigned long) kdp_host_type);
  }
}
...
kdp_fetch_registers (int regno)
{
  ...
  default:
    error ("kdp_fetch_registers: unknown host type 0x%lx", (unsigned long) kdp_host_type);
  }
}
...
static void
update_kdp_default_host_type (char *args, int from_tty, struct cmd_list_element *c)
{
  int htype;
