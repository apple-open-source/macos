...
all: "$(BUILT_PRODUCTS_DIR)"/$(JAMBASE_NAME)
	$(CP) -p $(JAMBASE_RESOURCES) "$(BUILT_PRODUCTS_DIR)"

"$(BUILT_PRODUCTS_DIR)"/$(JAMBASE_NAME): $(JAMBASE_SOURCES)
	$(MKDIRS) "$(BUILT_PRODUCTS_DIR)"
	$(CAT) $(JAMBASE_SOURCES) > "$(BUILT_PRODUCTS_DIR)/$(JAMBASE_NAME)"

install: "$(BUILT_PRODUCTS_DIR)"/$(JAMBASE_NAME) $(JAMBASE_RESOURCES)
	$(MKDIRS) $(DSTROOT)$(INSTALL_PATH)
	$(CP) -p  "$(BUILT_PRODUCTS_DIR)/$(JAMBASE_NAME)" $(DSTROOT)$(INSTALL_PATH)
...
clean:
	$(RM) -f "$(BUILT_PRODUCTS_DIR)/$(JAMBASE_NAME)"
