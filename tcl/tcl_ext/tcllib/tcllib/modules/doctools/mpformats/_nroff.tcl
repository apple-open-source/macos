...
proc nroff_postprocess {nroff} {
    ...
    set nroff [string map $finalMap [join $lines "\n"]]
    if {[llength [info commands get_nr_include]] &&
	    [regexp -line {^\.so (.+)$} $nroff -> inc]} {
	catch {regsub -line {^\.so (.+)$} $nroff [get_nr_include $inc] nroff}
    }
    return $nroff
}
