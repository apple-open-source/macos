...
AC_DEFUN(SC_PATH_XOTCLCONFIG, [
	...
	AC_CACHE_VAL(ac_cv_c_xotclconfig,[
		...
		for i in \
			...
			`ls -dr ../../../../xotcl-* 2>/dev/null` \
			../../../../../xotcl \
			`ls -dr ../../../../../xotcl-* 2>/dev/null` ; do
		    if test -f "$i/unix/xotclConfig.sh" ; then
			ac_cv_c_xotclconfig=`(cd $i/unix; pwd)`
			break
		    fi
		done
	    fi
		...
		for i in \
			${srcdir}/../xotcl \
			`ls -dr ${srcdir}/../xotcl[[8-9]].[[0-9]]* 2>/dev/null` ; do
		    if test -f "$i/unix/xotclConfig.sh" ; then
		    ac_cv_c_xotclconfig=`(cd $i/unix; pwd)`
		    break
		fi
