...
void *XML_GetBuffer(XML_Parser parser, size_t len)
{
  if (len > bufferLim - bufferEnd) {
    ...
    if (neededSize  <= bufferLim - buffer) {
      memmove(buffer, bufferPtr, (size_t)(bufferEnd - bufferPtr));
	...
	return 0;
      ...
      if (bufferPtr) {
	memcpy(newBuf, bufferPtr, (size_t)(bufferEnd - bufferPtr));
	free(buffer);
      }
	...
	*nextPtr = s;
	...
	if (MUST_CONVERT(enc, s)) {
	  ...
	  characterDataHandler(handlerArg, dataBuf, 
			       (size_t)(dataPtr - (ICHAR *)dataBuf));
	}
	else
	  characterDataHandler(handlerArg,
		  	       (XML_Char *)s,
			       (size_t)((XML_Char *)end - (XML_Char *)s));
      }
      else if (defaultHandler)
	reportDefault(parser, enc, s, end);
      if (startTagLevel == 0) {
	...
	*eventPP = end;
	...
	if (MUST_CONVERT(enc, s)) {
	  for (;;) {
	    ...
	    *eventEndPP = s;
	    characterDataHandler(handlerArg, dataBuf, (size_t)(dataPtr - (ICHAR *)dataBuf));
	    if (s == next)
	      break;
	    *eventPP = s;
	  }
	  ...
	  characterDataHandler(handlerArg,
			       (XML_Char *)s,
			       (size_t)((XML_Char *)next - (XML_Char *)s));
      }
      else if (defaultHandler)
	reportDefault(parser, enc, s, next);
      break;
...
handleUnknownEncoding(XML_Parser parser, const XML_Char *encodingName)
{
  if (unknownEncodingHandler) {
    ...
    if (unknownEncodingHandler(unknownEncodingHandlerData, encodingName, &info)) {
      ENCODING *enc;
      unknownEncodingMem = malloc((size_t)XmlSizeOfUnknownEncoding());
