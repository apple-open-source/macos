...
#  $Id: manager.tcl,v 1.3 2009/09/08 21:22:09 oberdorfer Exp $
# ----------------------------------------------------------------------------
#
...
proc DemoManager::_mainframe { parent } {
    ...
    BWidget::wrap checkbutton $subf.chk1 -text "View toolbar 1" -variable Demo::toolbar1 \
        -command {$Demo::mainframe showtoolbar 0 $Demo::toolbar1}
    BWidget::wrap checkbutton $subf.chk2 -text "View toolbar 2" -variable Demo::toolbar2 \
        -command {$Demo::mainframe showtoolbar 1 $Demo::toolbar2}
    pack $subf.chk1 $subf.chk2 -anchor w -fill x -padx 5
    pack $labf1 -fill both

    set labf2 [LabelFrame $parent.labf2 -text "Status bar" -side top -anchor w \
                   -relief sunken -borderwidth 2]
    set subf  [$labf2 getframe]
    BWidget::wrap checkbutton $subf.chk1 -text "Show Progress\nindicator" \
        -variable DemoManager::_progress \
        -command  {DemoManager::_show_progress}
    pack $subf.chk1 -anchor w -fill x -padx 5
...
proc DemoManager::_notebook { parent } {
    BWidget::wrap checkbutton $parent.chk1 -text "Homogeneous label" \
        -variable DemoManager::_homogeneous \
        -command  {$Demo::notebook configure -homogeneous $DemoManager::_homogeneous}
    pack $parent.chk1 -side left -anchor n -fill x -padx 5
}
...
proc DemoManager::_paned { parent } {
    ...
    foreach pane [list $pane1 $pane2] {
        ...
        set lb    [ListBox $sw.lb \
	               -height 8 -width 20 -highlightthickness 0 \
		       -selectmode multiple -selectfill 1]

        for {set i 1} {$i <= 8} {incr i} {
            $lb insert end item:$i \
	        -text "Value $i" -image [Bitmap::get file]
        }
        $sw setwidget $lb
    ...
    set chk [BWidget::wrap checkbutton $subf.chk -text "Constrained width" \
