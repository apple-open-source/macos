...
#
# $Id: install.tcl,v 1.9 2003/03/05 22:16:15 balls Exp $
...
proc Slave_Require {slave name {number 1.0}} {
...
proc Installer::UpdateTemplatedCopy { src dest size perm macroArray} {
	...
	while { [ gets $srcId line ] >= 0 && ! [eof $srcId] } {
	    ...
	    if { [regsub -all ${starttag}(\[A-Za-z0-9_\-\]+)${endtag} \
		      $line {[DoSubst ::TEA::\1]} line ] } {
		catch { 
		    set newline [ subst $line ] 
		} result
	    }	    
	    regsub -all {\\(.)} $newline {\1} line
	    puts $destId $line
	}
        close $srcId
	close $destId
...
# Installer::DoSubst --
#
#	Resolves a (scalar) variable, taking care of undefined variables.
#
# Arguments:
#	what	variable name
#
# Results:
#	Value of variable, or "UNSPECIFIED" if variable undefined

proc Installer::DoSubst what {
    set result UNSPECIFIED

    catch {set result [set $what]}
...
#	destroy .warnings
    ...
    after idle { 
	::Installer::CenterPanel .warnings 
	grab set .warnings
    } 
