...
typedef struct content_particle {
    struct content_particle *next;
    int tokenType;
    const XML_Char *name;
} CONTENT_PARTICLE;
...
/* ericm@scriptics.com; the following vars are for storing the contentspec
   of the element */
    XML_Char **contentSpec;
    int nContentParticles;
    int maxContentParticles;
    XML_Char **attributes;
    int nAttributes;
    int maxAttributes;
} ELEMENT_TYPE;
...
/* ericm@scriptics.com */
/* Append a given content particle to the content specification string of
 *  the given element.
 */
enum XML_Error appendContent(ELEMENT_TYPE *el, int type, const XML_Char *name);
enum XML_Error appendAttribute(ELEMENT_TYPE *el, XML_Char *name);
...
void XML_SetDefaultExpandInternalEntities(XML_Parser parser, 
					  int expandEntities)
{
  if (expandEntities != 0) {
    defaultExpandInternalEntities = 1;
...
const XML_LChar *XML_ErrorString(int code)
{
  static const XML_LChar *message[] = {
    ...
    XML_T("duplicate element declaration")
  };
...
/* ericm@scriptics.com, 1999.09.10.  Additions made to allow user access to
   information read from the DTD, including the names and content
   specifications of every element encountered.
*/
	case XML_ROLE_ELEMENT_NAME:
	{
	    ...
	    /* Find (or create) the element type structure in the element
	     * types hashtable
	     */
	    declElementType = (ELEMENT_TYPE *)lookup(&dtd.elementTypes,
		    name, sizeof(ELEMENT_TYPE));

	    if (!declElementType)
		return XML_ERROR_NO_MEMORY;

	    /* Per the XML Spec, it is illegal to have multiple declarations
	     * of a given element type.  Therefore, check to see if this
	     * element has already been defined, by seeing if contentSpec
	     * has been set.  If the element type structure was created by a
	     * previous element decl, then contentSpec will be non-NULL.  If
	     * it was created by being referenced in an attlist decl, then
	     * contentSpec will be NULL.
	     * NOTE:  This should probably only bail out if we are in
	     * validating mode.
	     */
	    if (declElementType->contentSpec != NULL) {
		return XML_ERROR_DUPLICATE_ELEMENT;
	    }
	...
	case XML_ROLE_ELEMENT_DECL_CLOSE:
	{
	    if (elementDeclHandler) {
		elementDeclHandler(handlerArg,
			declElementType->name, &declElementType->contentSpec);
	    }
	    break;
	}
	       

	case XML_ROLE_ATTLIST_DECL_CLOSE:
	{
	    if (attlistDeclHandler) {
		attlistDeclHandler(handlerArg,
			declElementType->name, &declElementType->attributes);
	    }
	    break;
	}
...
/* ericm@scriptics.com */
	...
	result = appendAttribute(declElementType, declAttributeId->name);
	if (result) {
	    return result;
	}
	break;
	...
	*eventEndPP = s;
	...
	result = appendContent(declElementType, XML_ROLE_GROUP_OPEN, NULL);
	if (result) {
	    return result;
	}
	break;
	...
	result = appendContent(declElementType, XML_ROLE_GROUP_SEQUENCE, NULL);
	if (result) {
	    return result;
	}
	break;
	...
	result = appendContent(declElementType, XML_ROLE_GROUP_CHOICE, NULL);
	if (result) {
	    return result;
	}
	break;
...
/* ericm@scriptics.com, 1999.09.10; additions enable tracking contentspec's */
	...
	{
	    enum XML_Error result;
	    result = appendContent(declElementType, role, NULL);
	    if (result) {
		return result;
	    }
	    break;
	...
	case XML_ROLE_CONTENT_ELEMENT_PLUS:
	{
	    ...
	    if (!name)
		return XML_ERROR_NO_MEMORY;
	    result = appendContent(declElementType, role, name);
	    if (result) {
		return result;
	    }
	    poolFinish(&dtd.pool);
...
/* ericm@scriptics.com */	
	...
	break;
      default:
        /* SRB: 20020814: XML_ROLE_IGNORE_SECT is not defined... */
 ...
 * appendContent --
 *
 *	Add a content particle to the content spec for a given element.
 *      el         pointer to the element to modify
 *      type       type of content particle to add, one of the XML_ROLE_*
 *                 enumerations defined in xmlrole.h
 *      name       pointer to the name of the element indicated by the content
 *                 particle (ie, if the content particle is an "element"
 *                 particle of some sort); for non-element content particles
 *                 (group, or's, comma's, etc), it is NULL.
 *
 * Results:
 *	enum XML_Error;  XML_ERROR_NO_MEMORY if there is insufficient free
 *      memory; XML_ERROR_NONE if there was no problem.
 *
 * Side effects:
 *	May grow the contentSpec array of the element pointed to by el.
 *
 *----------------------------------------------------------------------
 */

enum XML_Error
appendContent(ELEMENT_TYPE *el, int type, const XML_Char *name)
{
    int i;
    XML_Char **result;
    /* Grow the content spec array, if necessary */
    if (el->nContentParticles + 1 >= el->maxContentParticles) {
	if (el->contentSpec) {
	    el->maxContentParticles *= 2;
	    result =
		realloc(el->contentSpec,
			(el->maxContentParticles * sizeof(XML_Char *)));
	    if (!result) {
		return XML_ERROR_NO_MEMORY;
	    }
	    el->contentSpec = result;
	    for (i = el->nContentParticles; i < el->maxContentParticles; i++) {
		el->contentSpec[i] = NULL;
	    }
	} else {
	    el->maxContentParticles = 16;
	    el->contentSpec =
		calloc(el->maxContentParticles, sizeof(XML_Char *));
	    if (!el->contentSpec) {
		return XML_ERROR_NO_MEMORY;
	    }
	}
    ...
    switch (type) {
	case XML_ROLE_CONTENT_ANY:
	    el->contentSpec[el->nContentParticles++] = "ANY";
	    break;
	case XML_ROLE_CONTENT_EMPTY:
	    el->contentSpec[el->nContentParticles++] = "EMPTY";
	    break;
	case XML_ROLE_CONTENT_PCDATA:
	    el->contentSpec[el->nContentParticles++] = "#PCDATA";
	    break;
	case XML_ROLE_GROUP_OPEN:
	    el->contentSpec[el->nContentParticles++] = "(";
	    break;
	case XML_ROLE_GROUP_CLOSE:
	    el->contentSpec[el->nContentParticles++] = ")";
	    break;
	...
	case XML_ROLE_CONTENT_ELEMENT_OPT:
	case XML_ROLE_CONTENT_ELEMENT_PLUS:
	    el->contentSpec[el->nContentParticles++] = (XML_Char *)name;
	    break;
    }
    return XML_ERROR_NONE;
}
 ...
 * appendAttribute --
 *
 *	Add an attribute to the list of attributes for a given element.
 *
 * Results:
 *	enum XML_Error; XML_ERROR_NO_MEMORY if there is insufficient memory for
 *      the operation; XML_ERROR_NONE if there was no problem.
 *
 * Side effects:
 *	May grow the attribute array of the element pointed to by el.
 *
 *----------------------------------------------------------------------
 */

enum XML_Error
appendAttribute(ELEMENT_TYPE *el, XML_Char *name)
{
    int i;
    XML_Char **result;

    /* First see if this attribute is already part of the list */
    /* TODO This is *NOT* the best way to avoid duplicate attributes */
    for (i = 0; i < el->nAttributes; i++) {
	if (el->attributes[i] == name) {
	    return XML_ERROR_NONE;
	}
    }
	
    /* Grow the attribute array, if necessary */
    if (el->nAttributes + 1 >= el->maxAttributes) {
	if (el->attributes) {
	    el->maxAttributes *= 2;
	    
	    result =
		realloc(el->attributes,
			(el->maxAttributes * sizeof(XML_Char *)));
	    if (!result) {
		return XML_ERROR_NO_MEMORY;
	    }
	    el->attributes = result;
	    for (i = el->nAttributes; i < el->maxAttributes; i++) {
		el->attributes[i] = NULL;
	    }
	} else {
	    el->maxAttributes = 8;
	    el->attributes =
		calloc(el->maxAttributes, sizeof(XML_Char *));
	    if (!el->attributes) {
		return XML_ERROR_NO_MEMORY;
	    }
	}
    ...
    el->attributes[el->nAttributes++] = name;
    return XML_ERROR_NONE;
}
