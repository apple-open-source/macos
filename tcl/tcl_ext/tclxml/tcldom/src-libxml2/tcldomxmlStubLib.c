 ...
 *
 * RCS: @(#) $Id: tcldomxmlStubLib.c,v 1.1 2002/10/31 23:40:23 andreas_kupries Exp $
 */
...
TcldomxmlStubs *tcldomxmlStubsPtr;
...
#ifdef Tcldomxml_InitStubs
#undef Tcldomxml_InitStubs
#endif

CONST char *
Tcldomxml_InitStubs(interp, version, exact)
    Tcl_Interp *interp;
    CONST char *version;
    int exact;
{
...
#define UNCONST 
    ...
    result = Tcl_PkgRequireEx(interp, "dom::libxml2", UNCONST version, exact,
		(ClientData *) &tcldomxmlStubsPtr);
    if (!result || !tcldomxmlStubsPtr) {
        return (char *) NULL;
    }
