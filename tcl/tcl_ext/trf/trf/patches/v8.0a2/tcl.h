 ...
 *
 * SCCS: @(#) tcl.h 1.302 97/01/22 08:47:54
 */
...
typedef struct Tcl_Obj {
    ...
    char *bytes;		/* This points to the first byte of the
				 ...
				 * Clients should use
				 * Tcl_GetStringFromObj to get a pointer
				 * to the byte array as a readonly
				 * value. As a special case, if both
				 * bytes and typePtr are NULL, the
...
#  define Tcl_NewIntObj(val) \
     Tcl_DbNewIntObj(val, __FILE__, __LINE__)
...
EXTERN Tcl_Obj *	Tcl_DbNewDoubleObj _ANSI_ARGS_((double doubleValue,
                            char *file, int line));
EXTERN Tcl_Obj *	Tcl_DbNewIntObj _ANSI_ARGS_((long intValue,
                            char *file, int line));
...
EXTERN int		Tcl_StringMatch _ANSI_ARGS_((char *string,
			    char *pattern));
EXTERN void		Tcl_StringObjAppend _ANSI_ARGS_((Tcl_Obj *objPtr,
			    char *bytes, int length));
EXTERN void		Tcl_StringObjAppendObj _ANSI_ARGS_((Tcl_Obj *objPtr,
			    Tcl_Obj *srcPtr));
