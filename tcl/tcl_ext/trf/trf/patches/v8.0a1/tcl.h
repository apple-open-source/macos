 ...
 *
 * SCCS: @(#) tcl.h 1.296 96/12/17 15:30:07
 */
...
typedef void (Tcl_UpdateStringProc) _ANSI_ARGS_((Tcl_Interp *interp,
	struct Tcl_Obj *objPtr));
typedef char *(Tcl_VarTraceProc) _ANSI_ARGS_((ClientData clientData,
	Tcl_Interp *interp, char *part1, char *part2, int flags));

/*
 * Tcl_Objs provide a general-purpose mechanism for storing and exchanging
 * Tcl values. A Tcl_Obj has two representations: a string and an internal
 * representation such as an integer or bytecode structure. The two
 * representations are a cache of each other and are regenerated lazily:
 * changing one invalidates the other, which will only be recomputed when
 * necessary. As an example, a Tcl program doing integer calculations can
 ...
 * strings. Only when it needs a string representing the variable's value,
 * say to print it, will the program regenerate the string representation
 * from the integer.
 *
 * The internal representation is created and manipulated by
 * Tcl_ConvertToType and the various type-specific procedures such as
 * Tcl_StringObjAppend and Tcl_GetBooleanFromObj. Whenever the internal
 * representation is changed, the string rep is invalidated by setting its
 * char * byte pointer NULL. The string rep will be regenerated, if needed
 * later, by procedures such as Tcl_GetStringFromObj. Similarly, whenever
 * the string is changed, the internal form is invalidated by setting its
 * type pointer NULL. Because of this invalidation and regeneration, it is
 * dangerous for clients to access Tcl_Obj fields directly; they should only
 * use procedures like Tcl_GetStringFromObj to access Tcl_Obj information.
 * 
 * The string representation is not the traditional NULL-terminated C
 * string. Instead, it is a counted string: a pointer to an array of bytes
 * allocated on the heap and a length. This allows a Tcl object's string to
 * contain binary data with embedded NULLs. We require the byte array to
 * have a NULL after the last byte. This makes it possible to pass the byte
 * pointer directly to procedures that expect a NULL-terminated C string
 * without copying the byte array. Of course this can only be done when the
 * byte array does not contain an embedded NULL. Note that this requirement
 * forces programs to allocate enough storage for the string's bytes and
 * the terminating NULL.
 *
 * By convention, an object representing an empty string is represented by
 * an object with a NULL string rep "bytes" pointer and a NULL type pointer.
 */
...
typedef struct Tcl_Obj {
    ...
    char *bytes;		/* This points to the first byte of the
				 * object's string rep. The "length" member
				 * gives the number of bytes in the byte
				 * array. The array must be followed by a
				 * NULL byte (i.e., at offset length). The
				 * array's storage is allocated by ckalloc.
				 * NULL indicates the string rep is empty or
				 * invalid and must be regenerated from the
				 * internal rep. Clients should use
				 * Tcl_GetStringFromObj to get a
				 * pointer to the byte array as a readonly
				 * value. As a special case, if both bytes
				 * and typePtr are NULL, the object
				 * represents an empty string. */
    int length;			/* The number of bytes in the string rep's
				 * byte array. */
    Tcl_ObjType *typePtr;	/* Denotes the object's type. Always
				 * corresponds to the type of the object's
				 * internal rep. NULL indicates the object
				 * has no internal rep (has no type). If
				 * both string and typePtr are NULL, the
				 * Tcl_Obj represents an empty string. */
    union {			/* The internal representation: */
	long intValue;		/*   - an integer value */
 ...
 * to Tcl_SetCmdInfo. proc is a traditional Tcl argc/argv string-based
 * command procedure while objProc is an objc/objv object-based procedure.
 * objProc is called directly by the bytecode interpreter whenever the
 * command is invoked. proc is only called by the compatibility routine
 * TclInvokeStringCommand when no object-based procedure was otherwise
 * defined for a command. Tcl_CreateCommand and Tcl_CreateObjCommand
 * ensure that both proc and objProc are non-NULL. 
 */
...
#define TCL_PART1_NOT_PARSED	0x400
...
/*
 * Macros to free up result of interpreter.
 */
...
EXTERN Tcl_Command	Tcl_CreateObjCommand _ANSI_ARGS_((
			    Tcl_Interp *interp, char *name, int nameLength,
...
EXTERN int		Tcl_ExprString _ANSI_ARGS_((Tcl_Interp *interp,
			    char *string));
EXTERN int		Tcl_ExprStringObj _ANSI_ARGS_((Tcl_Interp *interp,
			    Tcl_Obj *objPtr));
...
EXTERN int		Tcl_GetAlias _ANSI_ARGS_((Tcl_Interp *interp,
			    ...
			    char ***argvPtr));
EXTERN int		Tcl_GetAllObjTypes _ANSI_ARGS_((Tcl_Interp *interp,
			    Tcl_Obj **objPtrPtr));
...
EXTERN Tcl_ByteOrder	Tcl_GetChannelByteorder _ANSI_ARGS_((
    			    Tcl_Channel chan));
EXTERN Tcl_ByteOrder	Tcl_GetHostByteorder _ANSI_ARGS_(());
...
EXTERN char *		Tcl_GetStringFromObj _ANSI_ARGS_((
			    Tcl_Interp *interp, Tcl_Obj *objPtr,
			    int *lengthPtr));
...
EXTERN Tcl_Obj *	Tcl_NewIntObj _ANSI_ARGS_((long intValue));
EXTERN Tcl_Obj *	Tcl_NewListObj _ANSI_ARGS_((int objc, Tcl_Obj **objv));
...
EXTERN void		Tcl_ResetObjResult _ANSI_ARGS_((Tcl_Interp *interp));
...
EXTERN int		Tcl_StringMatch _ANSI_ARGS_((char *string,
			    char *pattern));
EXTERN void		Tcl_StringObjAppend _ANSI_ARGS_((
			    Tcl_Interp *interp, Tcl_Obj *objPtr,
			    char *bytes, int length));
EXTERN void		Tcl_StringObjAppendObj _ANSI_ARGS_((
			    Tcl_Interp *interp, Tcl_Obj *objPtr,
			    Tcl_Obj *srcPtr));
