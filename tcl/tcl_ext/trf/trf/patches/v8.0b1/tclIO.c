 ...
 *
 * SCCS: @(#) tclIO.c 1.258 97/05/20 18:07:26
 */
    ...
    char *newValue;		/* New value for option. */
{
	...
	if (interp) {
	    Tcl_AppendResult(interp,
		    "unable to set channel options: invalid channel",
		    (char *) NULL);
	}
        return TCL_ERROR;
    }
    ...
    ClientData clientData;
{
    ...
    /*
     * Check to see if data is still available to be read.  If it is,
     * notify the channel, otherwise just update the notifier interest.
     */
    ...
    int mask;			/* Current channel event flags. */
{
	...
	if (errObj) {
	    ...
	    newObj = TclConcatObj(2, concatObjv);

	    if (Tcl_EvalObj(interp, newObj) != TCL_OK) {
		Tcl_BackgroundError(interp);
		result = TCL_ERROR;
	    }
	    Tcl_DecrRefCount(newObj);
	    Tcl_DecrRefCount(concatObjv[1]);
	    Tcl_Release((ClientData) interp);
	}
