...
cacheFileSystemMatchingArray()
{
	if (!matchingArray) {
		...
		matchingArray = CFArrayCreateMutable(NULL, 0, &kCFTypeArrayCallBacks);
		...
		for (n = 0; n < nfs; n++) {
			...
			if (mediaTypeDict != NULL) {
				CFIndex             j = CFDictionaryGetCount(mediaTypeDict);
...
repairPathForFileSystem(char *fsname)
{
	...
	{
		CFIndex persCount = CFDictionaryGetCount(personalities);
		      ...
		      struct statfs * stat_p, int stat_number, UInt64 ioSize)
{
    ...
    CFIndex			count = CFArrayGetCount(matchingArray);
...
{
    int i;
    CFIndex count = CFArrayGetCount(diskList->list);
    if(!diskList)
        return;
...
DiskVolumesPtr DiskVolumes_do_volumes(DiskVolumesPtr diskList)
{
	...
	for (diskPtr = g.Disks; diskPtr != NULL; diskPtr = diskPtr->next )
	{
		...
		isWritable = ( diskPtr->flags & kDiskLockedMask ) == 0;
		isRemovable = ( diskPtr->flags & kDiskEjectableMask ) != 0;
		isInternal = ( diskPtr->flags & kDiskInternal ) != 0;

                if ((diskPtr->flags & kDiskNoSizeMask) != 0) {
...
boolean_t
supported_fstype(const char * fs_type)
{
    static const char * list[] = {
	"hfs",
	"apfs"
    };
    static int		list_count = sizeof(list) / sizeof(list[0]);

    for (int i = 0; i < list_count; i++) {
	if (strcmp(fs_type, list[i]) == 0) {
	    return (TRUE);
	}
    }
    return (FALSE);
		     ...
		     const char * volume_name)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (vol->removable
		    ...
		    || vol->dirty == TRUE
		    || vol->fs_type == NULL
		    || supported_fstype(vol->fs_type) == FALSE) {
			continue;
		}
		if (volume_name != NULL
...
CFIndex 	DiskVolumes_count(DiskVolumesPtr diskList)
{
...
void 
GetDisksFromRegistry(io_iterator_t iter, int initialRun)
{
	kern_return_t   kr;
	...
	while ((entry = IOIteratorNext(iter))) {
		...
		if (CFDictionaryContainsKey(properties, CFSTR(kAPFSRoleKey))) {
		    dwarning(("Role set, skipping\n"));
		    	goto Next;
		}
		//Writable
		...
		if (!ioWritable)
			flags |= kDiskLockedMask;

		if (ioEjectable)
			flags |= kDiskEjectableMask;

		if (!ioSize)
			flags |= kDiskNoSizeMask;
		//blank media
