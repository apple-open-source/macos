...
@interface SecTrustStoreServerTests : TrustDaemonTestCase
@end

@implementation SecTrustStoreServerTests

- (void)testCopyAll_SystemConstrained {
#if TARGET_OS_BRIDGE
    /* bridgeOS doesn't use trust store */
    XCTSkip();
#endif
    SecTrustStoreRef ts = SecTrustStoreForDomainName(CFSTR("system"), NULL);

    CFArrayRef trustStoreContents = NULL;
    CFErrorRef error = NULL;
    XCTAssert(_SecTrustStoreCopyAll(ts, NULL, &trustStoreContents, &error));
    XCTAssertNotEqual(trustStoreContents, NULL);
    XCTAssertEqual(error, NULL);
    XCTAssertGreaterThan(CFArrayGetCount(trustStoreContents), 10);
    CFReleaseNull(trustStoreContents);

    XCTAssert(_SecTrustStoreCopyAll(ts, kSecPolicyAppleSSLServer, &trustStoreContents, &error));
    XCTAssertNotEqual(trustStoreContents, NULL);
    XCTAssertEqual(error, NULL);
    XCTAssertGreaterThan(CFArrayGetCount(trustStoreContents), 10);
    CFReleaseNull(trustStoreContents);

    XCTAssert(_SecTrustStoreCopyAll(ts, kSecPolicyAppleMobileAsset, &trustStoreContents, &error));
    XCTAssertNotEqual(trustStoreContents, NULL);
    XCTAssertEqual(error, NULL);
    ...
    XCTAssert(_SecTrustStoreCopyAll(ts, kSecPolicyAppleVerifiedMark, &trustStoreContents, &error));
    XCTAssertNotEqual(trustStoreContents, NULL);
    XCTAssertEqual(error, NULL);
    XCTAssertGreaterThan(CFArrayGetCount(trustStoreContents), 0);
    CFReleaseNull(trustStoreContents);

    XCTAssert(_SecTrustStoreCopyAll(ts, kSecPolicyAppleMDLTerminalAuth, &trustStoreContents, &error));
    XCTAssertNotEqual(trustStoreContents, NULL);
    XCTAssertEqual(error, NULL);
    XCTAssertGreaterThan(CFArrayGetCount(trustStoreContents), 0);
    CFReleaseNull(trustStoreContents);
