...
- (void)testPinningDisable
{
    ...
    __Require_Action(baltimoreRoot = (__bridge SecCertificateRef)[self SecCertificateCreateFromResource:@"BaltimoreCyberTrustRoot"
                                                                                         subdirectory:(NSString *)kSecTrustTestPinningPolicyResources],
                   errOut, fail("failed to create geotrust root"));
    __Require_Action(appleISTCA2 = (__bridge SecCertificateRef)[self SecCertificateCreateFromResource:@"AppleISTCA2G1-Baltimore"
                                                                                       subdirectory:(NSString *)kSecTrustTestPinningPolicyResources],
                   errOut, fail("failed to create apple IST CA"));
    __Require_Action(pinnedNonCT = (__bridge SecCertificateRef)[self SecCertificateCreateFromResource:@"ckdatabasews"
                                                                                       subdirectory:(NSString *)kSecTrustTestPinningPolicyResources],
    ...
    __Require_noErr_Action(SecTrustSetPinningPolicyName(trust, kSecPolicyNameAppleAMPService), errOut, fail("failed to set policy name"));
#if !TARGET_OS_BRIDGE
...
static SecTrustResultType test_with_policy_exception(SecPolicyRef CF_CONSUMED policy, bool set_exception)
{
    ...
    __Require_noErr_Quiet(SecTrustCreateWithCertificates((__bridge CFArrayRef)certs, policy, &trust), cleanup);
    __Require_noErr_Quiet(SecTrustSetAnchorCertificates(trust, (__bridge CFArrayRef)root), cleanup);
    __Require_noErr_Quiet(SecTrustSetVerifyDate(trust, (__bridge CFDateRef)verifyDate), cleanup);
    if (set_exception) {
        SecTrustSetPinningException(trust);
    }
    __Require_noErr_Quiet(SecTrustGetTrustResult(trust, &trustResult), cleanup);
