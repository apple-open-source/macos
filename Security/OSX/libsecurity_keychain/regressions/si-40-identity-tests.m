...
static NSArray *createIdentities(void) {
    SecKeyRef rsa_key = NULL, ec_key = NULL;
    SecKeyRef publicKey = NULL;
    SecCertificateRef rsa_cert = NULL, ec_cert = NULL;
    SecIdentityRef rsa_identity = NULL, ec_identity = NULL;
    ...
    ok_status(SecKeyGeneratePair((__bridge CFDictionaryRef)rsa_parameters, &publicKey, &rsa_key),
    ...
    ok_status(SecKeyGeneratePair((__bridge CFDictionaryRef)ec_parameters, &publicKey, &ec_key),
              "Failed to generate EC key");
    CFReleaseNull(publicKey);

    /* Generate a self-signed rsa cert */
    NSArray *ca_rdns = @[
         @[@[(__bridge NSString*)kSecOidCommonName, @"RSA Preferred Cert"]]
     ];
    ...
    rsa_cert = SecGenerateSelfSignedCertificate((__bridge CFArrayRef)ca_rdns,
                                               (__bridge CFDictionaryRef)ca_parameters,
                                               NULL, rsa_key);
    rsa_identity = SecIdentityCreate(NULL, rsa_cert, rsa_key);
    CFReleaseNull(rsa_cert);
    CFReleaseNull(rsa_key);

    /* Generate a self-signed ec cert */
    ca_rdns = @[
         @[@[(__bridge NSString*)kSecOidCommonName, @"EC Preferred Cert"]]
     ];
    ec_cert = SecGenerateSelfSignedCertificate((__bridge CFArrayRef)ca_rdns,
                                               (__bridge CFDictionaryRef)ca_parameters,
                                               NULL, ec_key);
    ec_identity = SecIdentityCreate(NULL, ec_cert, ec_key);
    CFReleaseNull(ec_cert);
    CFReleaseNull(ec_key);

    return @[CFBridgingRelease(rsa_identity), CFBridgingRelease(ec_identity)];
}

static void test_modern_identity(void) {
    OSStatus status = errSecParam;
    NSArray *identities = createIdentities();
    ...
    for (id item in identities) {
        ...
        isnt(foundIdentity, NULL, "plain name identity preference should not be deleted");
        CFReleaseNull(foundIdentity);

        // clear preference
        status = SecIdentitySetPreferred(NULL, (__bridge CFStringRef)name, NULL);
        is(status, errSecSuccess, "clear preferred identity with plain name");

        // verify cleared
        foundIdentity = SecIdentityCopyPreferred((__bridge CFStringRef)name, NULL, NULL);
        is(foundIdentity, NULL, "plain name identity preference found after being cleared");
        CFReleaseNull(foundIdentity);
