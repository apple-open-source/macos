...
PR_IMPLEMENT(void)
PL_InitArenaPool(PLArenaPool* pool, const char* name, PRUint32 size, PRUint32 align)
{
#if !defined(__GNUC__)
#pragma unused(name)
#endif
 ...
 */
PR_IMPLEMENT(void*)
PL_ArenaGrow(PLArenaPool* pool, void* p, PRUint32 origSize, PRUint32 incr)
{
    void* newp;
    PLArena* thisArena;
    PLArena* lastArena;
...
PR_IMPLEMENT(void) PL_FinishArenaPool(PLArenaPool* pool)
{
...
#ifdef PL_ARENAMETER
    {
        ...
        for (statsp = &arena_stats_list; (stats = *statsp) != 0; statsp = &stats->next) {
            if (stats == &pool->stats) {
                *statsp = stats->next;
                return;
            }
...
PR_IMPLEMENT(void) PL_ArenaCountGrowth(PLArenaPool* pool, PRUint32 size, PRUint32 incr)
{
    pool->stats.ngrows++;
