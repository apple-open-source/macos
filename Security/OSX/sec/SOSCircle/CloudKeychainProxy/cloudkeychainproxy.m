...
static void cloudkeychainproxy_peer_dictionary_handler(const xpc_connection_t peer, xpc_object_t event)
{
    ...
    else if (operation && !strcmp(operation, kOperationSynchronizeAndWait))
    {
        ...
        NSArray *keysToGet = (__bridge_transfer NSArray *)(_CFXPCCreateCFObjectFromXPCObject(xkeys));
        ...
        [[UbiqitousKVSProxy sharedKVSProxy] waitForSynchronization:keysToGet
                                                           handler:^(NSDictionary *values, NSError *err) {
                                                               if (replyMessage)   // Caller wanted an ACK, so give one
                                                               {
                                                                   secnotice(PROXYXPCSCOPE, "%s Result from [[UbiqitousKVSProxy sharedKVSProxy] waitForSynchronization:]: %@", kWAIT2MINID, err);
                                                                   ...
                                                                   if (err)
                                                                   {
                                                                       xpc_object_t xerrobj = SecCreateXPCObjectWithCFError((__bridge CFErrorRef)(err));
                                                                       xpc_dictionary_set_value(replyMessage, kMessageKeyError, xerrobj);
                                                                   }
                                                                   xpc_connection_send_message(peer, replyMessage);
                                                               }
                                                               else {
                                                                   secerror("%s XPC request: %s - No replyMessage: %@", kWAIT2MINID, kOperationSynchronizeAndWait, err);
                                                               }
                                                           }];
    }
...
{
    [[UbiqitousKVSProxy sharedKVSProxy] setItemsChangedBlock:^CFArrayRef(CFDictionaryRef values)
        {
            secdebug(PROXYXPCSCOPE, "UbiqitousKVSProxy called back");
