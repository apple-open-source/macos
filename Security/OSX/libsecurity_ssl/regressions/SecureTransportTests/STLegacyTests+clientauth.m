...
ssl_client_handle_create(bool break_on_req, int comm, CFArrayRef certs, CFArrayRef trustedCA, int auth)
{
    ...
    __Require_noErr(SSLCopyTrustedRoots(ctx, &trustedRoots), out);
#if (TARGET_OS_MAC && !TARGET_OS_IPHONE)
    __Require(trustedRoots, out);
#endif
...
static void *securetransport_ssl_client_thread(void *arg)
{
    ...
    do {
        ...
        if (ortn == errSSLClientCertRequested) {
            __Require_noErr(SSLGetNumberOfSignatureAlgorithms(ctx, &numSigAlgs), out);
            sigAlgs = malloc(numSigAlgs * sizeof(SSLSignatureAndHashAlgorithm));
            __Require_noErr(SSLGetSignatureAlgorithms(ctx, sigAlgs, &numSigAlgs), out);
            __Require_noErr(SSLGetNumberOfClientAuthTypes(ctx, &numTypes), out);
            authTypes = malloc(numTypes * sizeof(SSLClientAuthenticationType));
            __Require_noErr(SSLGetClientAuthTypes(ctx, authTypes, &numTypes), out);
            __Require_String(ssl->auth, out, "cert req not expected");
            __Require_String(ssl_state == kSSLHandshake, out, "wrong client handshake state after errSSLClientCertRequested");
            ...
            __Require_String(clientState == kSSLClientCertRequested, out, "Wrong client cert state after cert request");

            __Require_String(ssl->break_on_req, out, "errSSLClientCertRequested in run not testing that");
            if(ssl->certs) {
    ...
    __Require_String((got_client_cert_req || !ssl->auth || !ssl->break_on_req), out, "didn't get client cert req as expected");
...
ssl_server_handle_create(SSLAuthenticate client_auth, int comm, CFArrayRef certs, CFArrayRef trustedCA)
{
    ...
    __Require_noErr(SSLSetClientSideAuthenticate(ctx, client_auth), out);
    __Require_noErr(SSLGetClientSideAuthenticate(ctx, &auth), out);
    __Require(auth == client_auth, out);
