...
static SecCmsRecipientInfoRef nss_cmsrecipientinfo_create(SecCmsMessageRef cmsg,
 ...
 */
SecCmsRecipientInfoRef SecCmsRecipientInfoCreate(SecCmsMessageRef cmsg, SecCertificateRef cert)
{
    return nss_cmsrecipientinfo_create(cmsg, SecCmsRecipientIDIssuerSN, cert, NULL, NULL);
}

SecCmsRecipientInfoRef SecCmsRecipientInfoCreateWithSubjKeyID(SecCmsMessageRef cmsg,
                                                              CSSM_DATA_PTR subjKeyID,
                                                              SecPublicKeyRef pubKey)
{
    return nss_cmsrecipientinfo_create(cmsg, SecCmsRecipientIDSubjectKeyID, NULL, pubKey, subjKeyID);
}
...
SecCmsRecipientInfoRef SecCmsRecipientInfoCreateWithSubjKeyIDFromCert(SecCmsMessageRef cmsg,
                                                                      SecCertificateRef cert)
{
    ...
    switch (certalgtag) {
        case SEC_OID_PKCS1_RSA_ENCRYPTION:
            ...
            if (cert) {
                ...
                rv = SecCmsUtilEncryptSymKeyRSAPubKey(
                    poolp, extra->pubKey, bulkkey, &ri->ri.keyTransRecipientInfo.encKey);
                if (rv != SECSuccess)
                    break;
            }
