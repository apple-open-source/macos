...
#import <ctkloginhelper.h>
#import <EndpointSecuritySystem/ESSubmitSPI.h>
#import <mach/message.h>
#import <sys/stat.h>

#import <SoftLinking/SoftLinking.h>
#import <LocalAuthentication/LAUser.h>
#import <LocalAuthentication/LAUserStore.h>
#import <membershipPriv.h>

SOFT_LINK_FRAMEWORK(Frameworks, LocalAuthentication)
SOFT_LINK_CLASS(LocalAuthentication, LAUserStore)
...
OSStatus engine_authorize(engine_t engine, auth_rights_t rights, auth_items_t environment, AuthorizationFlags flags)
{
    ...
    if (rights_count) {
        ...
        auth_rights_iterate(rights, ^bool(const char *key) {
            ...
            *current_right = key;
            current_right++;
            if (strstr(key, "system.login.") == key) {
                // we are authorizing login or screensaver, keybag check required
	...
	if (engine->auth && !auth_token_apple_signed(engine->auth) && (localFlags & kAuthorizationFlagSheet)) {
#ifdef NDEBUG
    ...
    // revert keybag to the original state if login authorization failed
    // login authorization is determin upon auth start by setting the immutable hint
    // asking authorizationhost to unlock keybag during auth
    if (status != errAuthorizationSuccess && auth_items_exist(engine->context, AGENT_CONTEXT_OTIKBG_STATE)) {
        // auth failed and stored keybag state is stored => return keybag to the original state
        ...
        if (auth_items_exist(engine->context, AGENT_USERNAME)) {
            userName = auth_items_get_string(engine->context, AGENT_USERNAME);
        ...
        if (auth_items_get_bool(engine->context, AGENT_CONTEXT_OTIKBG_STATE)) {
            if ([laUser logOut:&error]) {
                os_log_error(AUTHD_LOG, "engine %llu: Failed to logout OTI: %{public}@", engine->engine_index, error);
            } else {
                os_log_debug(AUTHD_LOG,"engine %llu: OTI logout completed", engine->engine_index);
            }
        } else {
            if ([laUser lock:&error]) {
                os_log_error(AUTHD_LOG, "engine %llu: Failed to lock OTI: %{public}@", engine->engine_index, error);
            } else {
                os_log_debug(AUTHD_LOG,"engine %llu: OTI lock completed", engine->engine_index);
            }
        }
    revertKeyBagEnd:
    }
