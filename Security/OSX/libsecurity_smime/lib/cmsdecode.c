...
static void nss_cms_decoder_update_filter(void* arg, const char* data, size_t len, int depth, SEC_ASN1EncodingPart data_kind);
...
static void nss_cms_decoder_work_data(SecCmsDecoderRef p7dcx, const unsigned char* data, size_t len, Boolean final);
 ...
 *  further processing.
 */
static void nss_cms_decoder_notify(void* arg, Boolean before, void* dest, int depth)
{
        ...
        case SEC_OID_OTHER:
            ...
            if (before && dest == &(rootcinfo->content)) {
                ...
                SEC_ASN1DecoderSetFilterProc(
                    p7dcx->dcx, nss_cms_decoder_update_filter, p7dcx, (Boolean)(p7dcx->cb != NULL));
                break;
            }
...
static void nss_cms_decoder_work_data(SecCmsDecoderRef p7dcx, const unsigned char* data, size_t len, Boolean final)
{
    ...
    if (cinfo->ciphcx != NULL) {
         ...
         */
        rv = SecCmsCipherContextDecrypt(cinfo->ciphcx, buf, &outlen, buflen, data, len, final);
        if (rv != SECSuccess) {
            p7dcx->error = PORT_GetError();
            PORT_SetError(0);  // Clean the thread error since we've returned the error
            goto loser;
            ...
            case SEC_OID_OTHER:
                ...
                if (storage->Length == 0) {
                    ...
                    dest = (unsigned char*)PORT_ArenaGrow(
                        p7dcx->cmsg->poolp, storage->Data, storage->Length, storage->Length + len);
                }
                if (dest == NULL) {
                    p7dcx->error = SEC_ERROR_NO_MEMORY;
                    goto loser;
 ...
 */
OSStatus SecCmsDecoderCreate(SecArenaPoolRef pool,
                             SecCmsContentCallback cb,
                             void* cb_arg,
                             ...
                             SecCmsDecoderRef* outDecoder)
{
    ...
    SecCmsMessageSetEncodingParams(cmsg, pwfn, pwfn_arg, decrypt_key_cb, decrypt_key_cb_arg, NULL, NULL);
 ...
 */
OSStatus SecCmsDecoderUpdate(SecCmsDecoderRef p7dcx, const void* buf, CFIndex len)
{
    if (!p7dcx || len < 0) {
        return errSecParam;
    }
    ...
    if (p7dcx->dcx != NULL && p7dcx->error == 0) { /* if error is set already, don't bother */
        if (SEC_ASN1DecoderUpdate(p7dcx->dcx, buf, (size_t)len) != SECSuccess) {
            p7dcx->error = PORT_GetError();
 ...
 */
void SecCmsDecoderDestroy(SecCmsDecoderRef p7dcx)
{
 ...
 */
OSStatus SecCmsDecoderFinish(SecCmsDecoderRef p7dcx, SecCmsMessageRef* outMessage)
{
    SecCmsMessageRef cmsg;
    OSStatus result;
...
OSStatus SecCmsMessageDecode(const CSSM_DATA* encodedMessage,
                             SecCmsContentCallback cb,
                             void* cb_arg,
                             PK11PasswordFunc pwfn,
                             void* pwfn_arg,
                             ...
                             SecCmsMessageRef* outMessage)
{
    ...
    result = SecCmsDecoderCreate(
        NULL, cb, cb_arg, pwfn, pwfn_arg, decrypt_key_cb, decrypt_key_cb_arg, &decoder);
    if (result) {
        return result;
    }
    if (encodedMessage->Length > LONG_MAX) {
        return errSecParam;
    }
    result = SecCmsDecoderUpdate(decoder, encodedMessage->Data, (CFIndex)encodedMessage->Length);
    if (result) {
        SecCmsDecoderDestroy(decoder);
        return result;
    }

    return SecCmsDecoderFinish(decoder, outMessage);
}
