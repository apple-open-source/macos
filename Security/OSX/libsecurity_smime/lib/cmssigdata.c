...
static OSStatus validateTSAResponseAndAddTimeStamp(SecCmsSignerInfoRef signerinfo,
                                                   ...
                                                   uint64_t expectedNonce)
{
    ...
    tsaWriteFileX("/tmp/tsa-timeStampToken.der",
                  respDER.timeStampTokenDER.Data,
                  respDER.timeStampTokenDER.Length);
#endif
...
OSStatus SecCmsSignedDataEncodeAfterData(SecCmsSignedDataRef sigd)
{
    ...
    if (cinfo->digcx) {
        ...
        rv = SecCmsDigestContextFinishMultiple(cinfo->digcx, (SecArenaPoolRef)poolp, &digests);
        cinfo->digcx = NULL;
        if (rv != SECSuccess) {
            goto loser;		/* error has been set by SecCmsDigestContextFinishMultiple */
        }
        if (digests) {
            rv = SecCmsSignedDataSetDigests(sigd, sigd->digestAlgorithms, digests);
            if (rv != SECSuccess) {
    ...
    /* prepare all the SignerInfos (there may be none) */
    for (i = 0; i < SecCmsSignedDataSignerInfoCount(sigd); i++) {
        SecCmsSignerInfoRef signerinfo = SecCmsSignedDataGetSignerInfo(sigd, i);
    ...
    if (certcount <= 0 || certcount >= (long)((LONG_MAX / sizeof(CSSM_DATA_PTR)) - 1)) {
        sigd->rawCerts = NULL;
    } else {
        /*
	 ...
	 */
        sigd->rawCerts =
            (CSSM_DATA_PTR*)PORT_ArenaAlloc(poolp, (size_t)(certcount + 1) * sizeof(CSSM_DATA_PTR));
	 ...
	 */
        ...
        if (signerinfos != NULL) {
            for (si = 0; signerinfos[si] != NULL; si++) {
                SecCmsSignerInfoRef signerinfo = signerinfos[si];
...
OSStatus SecCmsSignedDataDecodeAfterData(SecCmsSignedDataRef sigd)
{
    ...
    if (sigd->contentInfo.digcx) {
        ...
        if (sigd->contentInfo.content.data && sigd->contentInfo.content.data->Length) {
            rv = SecCmsDigestContextFinishMultiple(sigd->contentInfo.digcx, (SecArenaPoolRef)sigd->cmsg->poolp, &(sigd->digests));
            sigd->contentInfo.digcx = NULL;
            if (rv != SECSuccess) {
                goto loser;        /* error has been set by SecCmsDigestContextFinishMultiple */
            }
        } else {
            SecCmsDigestContextCancel(sigd->contentInfo.digcx);
            sigd->contentInfo.digcx = NULL;
        }
    }
loser:
 ...
 */
CSSM_DATA_PTR* SecCmsSignedDataGetCertificateList(SecCmsSignedDataRef sigd)
{
                                     ...
                                     Boolean keepcerts)
{
    ...
    rv = CERT_ImportCerts(
        keychain, certusage, certcount, sigd->rawCerts, NULL, keepcerts, PR_FALSE, NULL);
                                          ...
                                          SecTrustRef* trustRef)
{
    ...
    /* Create digests for empty content */
    if (!sigd->digests) {
        ...
        SecCmsDigestContextFinishMultiple(digcx, (SecArenaPoolRef)sigd->cmsg->poolp, &(sigd->digests));
    }
...
static OSStatus SecCmsSignedDataVerifyCertsOnly_internal(SecCmsSignedDataRef sigd, SecKeychainRef keychainOrArray, CFTypeRef policies)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        rv |= CERT_VerifyCert(
            keychainOrArray, cert, sigd->rawCerts, policies, CFAbsoluteTimeGetCurrent(), NULL);
        CFReleaseNull(cert);
    }
 ...
 */
OSStatus SecCmsSignedDataSetDigests(SecCmsSignedDataRef sigd, SECAlgorithmID** digestalgs, CSSM_DATA_PTR* digests)
{
    int cnt, i, idx;

    /* Check input structure and items in structure */
    if (sigd == NULL || sigd->digestAlgorithms == NULL || sigd->cmsg == NULL ||
        sigd->cmsg->poolp == NULL) {
        PORT_SetError(SEC_ERROR_INVALID_ARGS);
        return SECFailure;
    }
    ...
    if (cnt <= 0 || cnt >= (int)((INT_MAX / sizeof(CSSM_DATA_PTR)) - 1)) {
        PORT_SetError(SEC_ERROR_NO_MEMORY);
        return SECFailure;
    }
    sigd->digests = PORT_ArenaZAlloc(sigd->cmsg->poolp, (size_t)(cnt + 1) * sizeof(CSSM_DATA_PTR));
...
OSStatus SecCmsSignedDataSetDigestValue(SecCmsSignedDataRef sigd, SECOidTag digestalgtag, CSSM_DATA_PTR digestdata)
{
...
OSStatus SecCmsSignedDataAddDigest(SecArenaPoolRef pool, SecCmsSignedDataRef sigd, SECOidTag digestalgtag, CSSM_DATA_PTR digest)
{
 ...
 * of pointers.
 */
extern OSStatus SecCmsSignedDataRawCerts(SecCmsSignedDataRef sigd, CSSM_DATA_PTR** rawCerts)
{
