...
void CL_nssGeneralNameToCssm(
	...
	CssmAllocator &alloc)			// destination 
{
	...
	switch(nssObj.tag) {
		...
		case NGT_DirectoryName:	// ASN_ANY --> NSS_Name
		{
			...
			prtn = coder.decodeItem(nssObj.item, NSS_NameTemplate, nssName);
			if(prtn) {
				clErrorLog("CL_nssGeneralNameToCssm: error decoding "
						"NSS_Name\n");
				CssmError::throwMe(CSSMERR_CL_UNKNOWN_FORMAT);
	...
	CssmAllocator &alloc)			// destination 
{
	...
	for(unsigned dex=0; dex<numNames; dex++) {
		if(coder.decodeItem(*nssObj.names[dex], NSS_GeneralNameTemplate,
				&names[dex])) {
	...
	switch(cdsaObj.nameType) {
		case GNT_OtherName:	
			...
			prtn = coder.encodeItem(cdsaObj.name.Data,
				NSS_OtherNameTemplate, nssObj.item);
			if(prtn) {
	...
	SecNssCoder &coder)
{
	...
	for(unsigned dex=0; dex<cdsaObj.numNames; dex++) {
		...
		CL_cssmGeneralNameToNss(cdsaObj.generalName[dex],
			names[dex], coder);
		if(coder.encodeItem(&names[dex], NSS_GeneralNameTemplate,
				*nssObj.names[dex])) {
	...
	CssmAllocator 					&alloc)
{
	...
	for(unsigned i=0; i<cdsaObj->numNames; i++) {
		 ...
		 */
		CE_GeneralName *genName = &cdsaObj->generalName[i];
