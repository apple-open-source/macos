...
#define SERVER_NAME "SecServer v" STR(LATEST_SERVER_VERSION)
...
#define BEGIN_IPC(name)	BEGIN_IPCN RefPointer<Connection> connRef(&Server::connection(replyPort, auditToken)); \
        ...
        secinfo(SERVER_NAME, "request entry " #name " (pid:%d session:%d)", connection.process().pid(), connection.session().sessionId());

#define END_IPC(base)	END_IPCN(base) Server::requestComplete(*rcode); return KERN_SUCCESS;
#define END_IPCN(base) 	secinfo(SERVER_NAME, "request return: %d", *(rcode)); \
	} \
	...
	/* deallocate replyPort send right generated by MIG in ucsp.defs */ \
...
kern_return_t ucsp_server_queryServerVersion(UCSP_ARGS, uint32_t *version)
{
	BEGIN_IPCN
	secinfo("SecServer", "request entry: queryServerVersion");
...
kern_return_t ucsp_server_changeKeybagPassphrase(UCSP_ARGS, DbHandle db,
    DATA_IN(oldPassphrase), DATA_IN(newPassphrase))
{
    BEGIN_IPC(changeKeybagPassphrase)
    secnotice("dp_login", "changingKeybagPassphrase");
    Server::keychain(db)->changeKeybagPassphrase(DATA(oldPassphrase), DATA(newPassphrase));
    END_IPC(DL)
}
...
static void check_entitlement(Process & proc, CFStringRef entitlement)
{
    ...
    if (CFDictionaryGetValueIfPresent(entitlements, entitlement, &value)) {
...
{
    check_entitlement(proc, kSecEntitlementPrivateStash);
}

static void check_derivedentropy_entitlement(Process & proc)
{
    check_entitlement(proc, kSecEntitlementPrivateDerivedEntropy);
}
...
{
    BEGIN_IPC(unlockKeybagWithPassphrase)
    Server::keychain(db)->unlockKeybag(DATA(passphrase));
    END_IPC(DL)
}
...
static KeyHandle storeHandleData(const CssmData& data, uint64_t timeoutSeconds = 5) {
...
static bool retrieveHandleData(KeyHandle handle, CssmDataContainer& data) {
    initializeAndCleanupHandleStorage();
...
static void removeHandle(KeyHandle handle) {
    initializeAndCleanupHandleStorage();
...
kern_return_t ucsp_server_unlockDbWithHandle(UCSP_ARGS, DbHandle db, KeyHandle kh)
{
    BEGIN_IPC(unlockDbWithHandle)
    secnotice("dp_login", "unlocking with handle %u", kh);
    CssmDataContainer handleData;
...
kern_return_t ucsp_server_changeDbPassphraseWithHandle(UCSP_ARGS, DbHandle db, KeyHandle kh)
{
    BEGIN_IPC(changeDbPassphraseWithHandle)
    secnotice("dp_login", "changing passphrase with handle %u", kh);
    CssmDataContainer handleData;
    if (!retrieveHandleData(kh, handleData)) {
        CssmError::throwMe(CSSMERR_CSP_INVALID_KEY_REFERENCE);
    }
    Server::keychain(db)->changePassphraseTo(handleData);
    END_IPC(DL)
}
...
static CFDataRef CopyProtectedEntropyForSalt(CssmData& salt) {
...
kern_return_t ucsp_server_generateDerivedEntropy(UCSP_ARGS, KeyHandle *kh, DATA_IN(salt), DATA_IN(passphrase))
{
    ...
    secnotice("dp_login", "generating derived entropy for %s", DATA(salt).toHex().c_str());
    ...
    CFDataRef protectedEntropy = CopyProtectedEntropyForSalt(cssmSalt);
    ...
    CCHmacUpdate(&ctxHMAC, DATA(passphrase).Data, DATA(passphrase).Length);
    ...
    *kh = storeHandleData(entropyContainer);
...
kern_return_t ucsp_server_releaseHandle(UCSP_ARGS, KeyHandle kh)
{
    BEGIN_IPC(releaseHandle)
    secnotice("dp_login", "releasing handle %u", kh);
    removeHandle(kh);
    END_IPC(DL)
}

kern_return_t ucsp_server_getDerivedEntropy(UCSP_ARGS, KeyHandle kh, DATA_OUT(entropy))
{
    BEGIN_IPC(getDerivedEntropy)
    check_derivedentropy_entitlement(connection.process());
    secnotice("dp_login", "retrieving entropy for handle %u", kh);
    CssmDataContainer handleData;
    if (!retrieveHandleData(kh, handleData)) {
        CssmError::throwMe(CSSMERR_CSP_INVALID_KEY_REFERENCE);
    }
    *entropy = handleData.Data;
    *entropyLength = (mach_msg_type_number_t)handleData.Length;
    // Let the Mach server free the memory when it's done,not when handleData goes out of scope
    handleData.invalidate();
    Server::releaseWhenDone(*entropy);
