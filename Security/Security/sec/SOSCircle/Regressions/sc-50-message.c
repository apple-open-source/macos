...
__unused static void testFlaggedMessage(uint64_t msgid, SOSMessageFlags flags)
{
    SOSMessageRef sentMessage = NULL;
    SOSMessageRef rcvdMessage = NULL;
    SOSManifestRef sender = NULL;
...
__unused static void testDeltaManifestMessage(uint64_t msgid)
{
    SOSMessageRef sentMessage = NULL;
    SOSMessageRef rcvdMessage = NULL;
    SOSManifestRef sender = NULL;
...
static CFDataRef testCopyAddedObject(SOSMessageRef message, CFPropertyListRef plist)
{
    CFErrorRef error = NULL;
    CFDataRef der;
    ok(der = kc_plist_copy_der(plist, &error), "copy der: %@", error);
    CFReleaseNull(error);
...
__unused static void testObjectsMessage(uint64_t msgid)
{
    SOSMessageRef sentMessage = NULL;
    SOSMessageRef rcvdMessage = NULL;
    SOSManifestRef sender = NULL;
    ...
    f0 = f1 = f2 = f3 = false;
    SOSMessageWithObjects(rcvdMessage, &error,  ^(CFDataRef object, bool *stop) {
...
static void tests(void)
{
    ...
    testFlaggedMessage(++msgid, 0x865);
    testFlaggedMessage(++msgid, 0xdeadbeef);
    testDeltaManifestMessage(0);
    testDeltaManifestMessage(++msgid);
    testObjectsMessage(0);
    testObjectsMessage(++msgid);
    }
#endif
}

int sc_50_message(int argc, char *const *argv)
{
