/*
 * Copyright (c) 2000-2001,2005-2008,2010-2014 Apple Inc. All Rights Reserved.
 *
...
{   OSStatus            err;
	...
	sessionDataLen = offsetof(ResumableSession, certs);
#ifdef USE_SSLCERTIFICATE
	cert = ctx->peerCert;
	certCount = 0;
	while (cert)
    ...
    session->cipherSuite = tls_handshake_get_cipherspec(ctx->hdsk);
    memcpy(session->masterSecret, tls_handshake_get_master_secret(ctx->hdsk), 48);
...
{   OSStatus            err;
    ...
    memcpy(tls_handshake_get->masterSecret, session->masterSecret, 48);
