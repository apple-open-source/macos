...
#define CIPHER(cipher, dh_anonymous, dtls) { cipher, #cipher, dh_anonymous, dtls},

static const CipherSuiteName ciphers[] = {
    //SSL_NULL_WITH_NULL_NULL, unsupported
    CIPHER(SSL_RSA_WITH_NULL_SHA, false, true)
    CIPHER(SSL_RSA_WITH_NULL_MD5, false, true)
    CIPHER(TLS_RSA_WITH_NULL_SHA256, false, true)

    CIPHER(SSL_RSA_WITH_3DES_EDE_CBC_SHA, false, true)

    CIPHER(SSL_DHE_RSA_WITH_3DES_EDE_CBC_SHA, false, true)
    CIPHER(SSL_DH_anon_WITH_3DES_EDE_CBC_SHA, true, true)
    CIPHER(TLS_DHE_RSA_WITH_AES_128_CBC_SHA, false, true)
    CIPHER(TLS_DH_anon_WITH_AES_128_CBC_SHA, true, true)
    CIPHER(TLS_DHE_RSA_WITH_AES_256_CBC_SHA, false, true)
    CIPHER(TLS_DH_anon_WITH_AES_256_CBC_SHA, true, true)

    CIPHER(TLS_RSA_WITH_AES_128_CBC_SHA, false, true)
    CIPHER(TLS_RSA_WITH_AES_256_CBC_SHA, false, true)

#if 1
    CIPHER(TLS_PSK_WITH_AES_128_CBC_SHA, true, true)
    CIPHER(TLS_PSK_WITH_AES_256_CBC_SHA384, true, true)
    CIPHER(TLS_PSK_WITH_AES_128_CBC_SHA256, true, true)
    CIPHER(TLS_PSK_WITH_AES_256_CBC_SHA, true, true)
    CIPHER(TLS_PSK_WITH_AES_128_CBC_SHA, true, true)
    CIPHER(TLS_PSK_WITH_3DES_EDE_CBC_SHA, true, true)
    CIPHER(TLS_PSK_WITH_NULL_SHA384, true, true)
    CIPHER(TLS_PSK_WITH_NULL_SHA256, true, true)
    CIPHER(TLS_PSK_WITH_NULL_SHA, true, true)
#endif

    // Put all the RC4 ciphers at the end: no DTLS for them.
    CIPHER(SSL_RSA_WITH_RC4_128_MD5, false, false)
    CIPHER(SSL_RSA_WITH_RC4_128_SHA, false, false)
    CIPHER(SSL_DH_anon_WITH_RC4_128_MD5, true, false)
    CIPHER(TLS_PSK_WITH_RC4_128_SHA, true, false)


#if 0
    // ECDH and ECDHE not supported on server yet
...
#endif
            SKIP:{
                skip("This ciphersuite is not supported for DTLS", 1, !d || ciphers[i].dtls);
