	...
	Allocator &alloc)
{
	...
	OSStatus status = SecPolicyGetValue(revPolicy, &policyValue);
	if (!status && policyValue.Data) {
	...
	if (policyFlags & kSecRevocationOCSPMethod) {
		...
		if (policyFlags & kSecRevocationRequirePositiveResponse) {
			// FIXME: possibly set CSSM_TP_ACTION_REQUIRE_REV_PER_CERT in actionFlags,
			// but verify whether that only applies to certs which specify a revocation method
		}
