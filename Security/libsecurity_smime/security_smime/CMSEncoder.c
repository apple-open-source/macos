 ...
 */
typedef enum { EO_Sign, EO_Encrypt, EO_SignEncrypt } CMSEncoderOp;
 ...
 */
static int cfStringToNumber(CFStringRef inStr)
{
    int max = 32;
    char buf[max];
 ...
 */
static unsigned encodeNumber(int num,
 ...
 */
static int encodeOid(const unsigned char* inStr, unsigned char** outOid, unsigned int* outLen)
{
...
static int convertOid(CFTypeRef inRef, SecAsn1Oid* outOid)
{
    if (!inRef || !outOid) {
        return errSecParam;
    }
...
{
    cmsEncoderTypeID = _CFRuntimeRegisterClass((const CFRuntimeClass* const) & cmsEncoderRuntimeClass);
}
...
static OSStatus cmsSetupEncoder(CMSEncoderRef cmsEncoder)
{
    OSStatus ortn;
    ...
    ortn = SecCmsEncoderCreate(cmsEncoder->cmsMsg,
                               ...
                               NULL,
                               NULL,  // no password callback (right?)
                               NULL,
                               NULL,  // decrypt key callback
                               &cmsEncoder->encoder);
    if (ortn) {
 ...
 */
static OSStatus cmsSetupForSignedData(CMSEncoderRef cmsEncoder)
{
    ...
    for (dex = 0; dex < numSigners; dex++) {
        ...
        SecIdentityRef ourId = (SecIdentityRef)CFArrayGetValueAtIndex(cmsEncoder->signers, dex);
        ...
        ortn = SecCmsSignerInfoIncludeCerts(signerInfo, chainMode, certUsageEmailSigner);
        if (ortn) {
            ortn = cmsRtnToOSStatus(ortn);
        ...
        if (cmsEncoder->signedAttributes & kCMSAttrSmimeCapabilities) {
            ...
            if (ortn) {
                ortn = cmsRtnToOSStatus(ortn);
                CSSM_PERROR("SecCmsSignerInfoAddSMIMECap", ortn);
                break;
            }
        }
        ...
        if (cmsEncoder->signedAttributes & kCMSAttrAppleCodesigningHashAgilityV2) {
            ortn = SecCmsSignerInfoAddAppleCodesigningHashAgilityV2(
                signerInfo, cmsEncoder->hashAgilityV2AttrValues);
            /* libsecurity_smime made a copy of the attribute value. We don't need it anymore. */
 ...
 */
static OSStatus cmsSetupForEnvelopedData(CMSEncoderRef cmsEncoder)
{
    ...
    CFIndex dex;
    SecCertificateRef* certArray =
        (SecCertificateRef*)malloc((size_t)(numCerts + 1) * sizeof(SecCertificateRef));

    for (dex = 0; dex < numCerts; dex++) {
        certArray[dex] = (SecCertificateRef)CFArrayGetValueAtIndex(cmsEncoder->recipients, dex);
    }
    ...
    envelopedData = SecCmsEnvelopedDataCreate(cmsEncoder->cmsMsg, algorithmTag, keySize);
    ...
    if (cmsEncoder->eContentType.Data != NULL) {
        ...
        ortn = SecCmsContentInfoSetContentData(
            contentInfo, NULL /* data - provided to encoder, not here */, cmsEncoder->detachedContent);
    }
    if (ortn) {
        ortn = cmsRtnToOSStatus(ortn);
    ...
    for (dex = 0; dex < numCerts; dex++) {
        ...
        SecCertificateRef thisRecip =
            (SecCertificateRef)CFArrayGetValueAtIndex(cmsEncoder->recipients, dex);
 ...
 */
static OSStatus cmsSetupCmsMsg(CMSEncoderRef cmsEncoder)
{
 ...
 */
static OSStatus cmsContentInfoContent(SecAsn1CoderRef asn1Coder,
                                      ...
                                      SecAsn1Item* content) /* RETURNED */
{
    ...
    ortn = SecAsn1DecodeData(asn1Coder, contentInfo, cmsSimpleContentInfoTemplate, &decodedInfo);
    if (ortn) {
        return ortn;
    }
 ...
 */
OSStatus CMSEncoderCreate(CMSEncoderRef* cmsEncoderOut) /* RETURNED */
{
    CMSEncoderRef cmsEncoder = NULL;

    CFIndex extra = sizeof(*cmsEncoder) - sizeof(cmsEncoder->base);
    cmsEncoder = (CMSEncoderRef)_CFRuntimeCreateInstance(NULL, CMSEncoderGetTypeID(), extra, NULL);
    if (cmsEncoder == NULL) {
        return errSecAllocate;
    }
 ...
 */
OSStatus CMSEncoderCopySigners(CMSEncoderRef cmsEncoder, CFArrayRef* signers)
{
...
OSStatus CMSEncoderAddRecipients(CMSEncoderRef cmsEncoder, CFTypeRef recipientOrArray)
{
    ...
    return cmsAppendToArray(recipientOrArray, &cmsEncoder->recipients, SecCertificateGetTypeID());
}

/*
 * Obtain an array of recipients as specified in CMSEncoderSetRecipients().
 */
OSStatus CMSEncoderCopyRecipients(CMSEncoderRef cmsEncoder, CFArrayRef* recipients)
{
...
OSStatus CMSEncoderAddSupportingCerts(CMSEncoderRef cmsEncoder, CFTypeRef certOrArray)
{
    ...
    return cmsAppendToArray(certOrArray, &cmsEncoder->otherCerts, SecCertificateGetTypeID());
}

/*
 * Obtain the SecCertificates provided in CMSEncoderAddSupportingCerts().
 */
OSStatus CMSEncoderCopySupportingCerts(CMSEncoderRef cmsEncoder, CFArrayRef* certs) /* RETURNED */
{
...
OSStatus CMSEncoderGetHasDetachedContent(CMSEncoderRef cmsEncoder, Boolean* detachedContent) /* RETURNED */
{
 ...
 * called, is id-data.
 */
static OSStatus CMSEncoderSetEncapsulatedContentType(CMSEncoderRef cmsEncoder, const SecAsn1Oid* eContentType)
{
 ...
 */
OSStatus CMSEncoderCopyEncapsulatedContentType(CMSEncoderRef cmsEncoder, CFDataRef* eContentType)
{
    if ((cmsEncoder == NULL) || (eContentType == NULL)) {
        return errSecParam;
    }
 ...
 */
OSStatus CMSEncoderSetSigningTime(CMSEncoderRef cmsEncoder, CFAbsoluteTime time)
{
    if (cmsEncoder == NULL) {
        return errSecParam;
    }
 ...
 * is included.
 */
OSStatus CMSEncoderSetAppleCodesigningHashAgilityV2(CMSEncoderRef cmsEncoder, CFDictionaryRef hashAgilityV2AttrValues)
{
    if (cmsEncoder == NULL || cmsEncoder->encState != ES_Init) {
        return errSecParam;
    }
 ...
 */
OSStatus CMSEncoderSetAppleExpirationTime(CMSEncoderRef cmsEncoder, CFAbsoluteTime time)
{
    if (cmsEncoder == NULL) {
        return errSecParam;
    }
...
void CmsMessageSetTSAContext(CMSEncoderRef cmsEncoder, CFTypeRef tsaContext)
{
    if (cmsEncoder->cmsMsg)
...
/* forward declaration */
static OSStatus CMSEncode(CFTypeRef signers,
 ...
 */
OSStatus CMSEncoderCopyEncodedContent(CMSEncoderRef cmsEncoder, CFDataRef* encodedContent)
{
    ...
    switch (cmsEncoder->encState) {
        ...
        case ES_Init:
             ...
             */
            if ((cmsEncoder->signers != NULL) || (cmsEncoder->recipients != NULL) ||
    ...
    if (CFDataGetLength(cmsEncoder->encoderOut) < 0) {
        return errSecAllocate;
    }
    SecAsn1Item encoderOut = {(size_t)CFDataGetLength(cmsEncoder->encoderOut),
...
OSStatus CMSEncodeContent(CFTypeRef signers,
                          CFTypeRef recipients,
                          ...
                          CFDataRef* encodedContentOut) /* RETURNED */
{
    ...
    OSStatus result = CMSEncode(
        signers, recipients, contentTypePtr, detachedContent, signedAttributes, content, contentLen, encodedContentOut);
    if (contentType.Data) {
        free(contentType.Data);
    }
    return result;
 ...
 */
OSStatus CMSEncoderGetCmsMessage(CMSEncoderRef cmsEncoder, SecCmsMessageRef* cmsMessage) /* RETURNED */
{
 ...
 */
OSStatus CMSEncoderSetEncoder(CMSEncoderRef cmsEncoder, SecCmsEncoderRef encoder)
{
 ...
 */
OSStatus CMSEncoderGetEncoder(CMSEncoderRef cmsEncoder, SecCmsEncoderRef* encoder) /* RETURNED */
{
    if ((cmsEncoder == NULL) || (encoder == NULL)) {
        return errSecParam;
    }
 ...
 */
OSStatus CMSEncoderCopySignerTimestamp(CMSEncoderRef cmsEncoder,
                                       size_t signerIndex,        /* usually 0 */
                                       CFAbsoluteTime* timestamp) /* RETURNED */
{
    return CMSEncoderCopySignerTimestampWithPolicy(cmsEncoder, NULL, signerIndex, timestamp);
}

OSStatus CMSEncoderCopySignerTimestampWithPolicy(CMSEncoderRef cmsEncoder,
