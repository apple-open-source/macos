...
static const SECOidData oids[] = {
    ...
    OD(ecPublicKey, SEC_OID_EC_PUBLIC_KEY, "ECDSA Public Key", CSSM_ALGID_NONE, INVALID_CERT_EXTENSION),
    OD(ecdsaWithSHA1, SEC_OID_ECDSA_WithSHA1, "SHA-1 With ECDSA", CSSM_ALGID_NONE, INVALID_CERT_EXTENSION),
    OD(dhSinglePassStdDHsha1kdf, SEC_OID_DH_SINGLE_STD_SHA1KDF, "ECDH With SHA1 KDF", CSSM_ALGID_NONE, INVALID_CERT_EXTENSION),
    ...
    OD(appleHashAgilityV2, SEC_OID_APPLE_HASH_AGILITY_V2, "appleCodesigningHashAgilityAttributeV2", CSSM_ALGID_NONE, INVALID_CERT_EXTENSION),
...
static SECStatus InitOIDHash(void)
{
    ...
    for (ix = 0; ix < (sizeof(oids) / sizeof(SECOidData)); ix++) {
        ...
        if (oid->cssmAlgorithm.algorithm.Length /*CSSM_ALGID_NONE*/) {
            entry = PL_HashTableAdd(oidmechhash, (void*)&(oid->cssmAlgorithm), (void*)oid);
            if (entry == NULL) {
                PORT_SetError(SEC_ERROR_LIBRARY_FAILURE);
                PORT_Assert(0); /* This function should never fail. */
                return (SECFailure);
...
SECOidData* SECOID_FindOIDByCssmAlgorithm(SecAsn1AlgId cssmAlgorithm)
{
    SECOidData* ret;
...
/* TODO: appears to be the public entry point */
SECOidData* SECOID_FindOID(const SecAsn1Item* oid)
{
    SECOidData* ret;
...
SECOidTag SECOID_FindOIDTag(const SecAsn1Item* oid)
{
    SECOidData* oiddata;
