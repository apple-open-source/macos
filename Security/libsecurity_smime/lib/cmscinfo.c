 ...
 */
void SecCmsContentInfoDestroy(SecCmsContentInfoRef cinfo)
{
    SECOidTag kind;
 ...
 */
SecCmsContentInfoRef SecCmsContentInfoGetChildContentInfo(SecCmsContentInfoRef cinfo)
{
    void* ptr = NULL;
 ...
 */
OSStatus SecCmsContentInfoSetContent(SecCmsContentInfoRef cinfo, SECOidTag type, void* ptr)
{
    OSStatus rv;
    ...
    rv = SECITEM_CopyItem(
        cinfo->cmsg->poolp, &(cinfo->contentType), &(cinfo->contentTypeTag->oid));
    if (rv != SECSuccess)
 ...
 */
OSStatus SecCmsContentInfoSetContentData(SecCmsContentInfoRef cinfo, CFDataRef dataRef, Boolean detached)
{
    ...
    if (dataRef) {
	   ...
	   always copying it for performance. */
        ...
        data->Length = (size_t)CFDataGetLength(dataRef);
        if (data->Length) {
    ...
    cinfo->rawContent =
        (detached) ? NULL : (data) ? data : SECITEM_AllocItem(cinfo->cmsg->poolp, NULL, 1);
    return errSecSuccess;
}

OSStatus SecCmsContentInfoSetContentSignedData(SecCmsContentInfoRef cinfo, SecCmsSignedDataRef sigd)
{
    return SecCmsContentInfoSetContent(cinfo, SEC_OID_PKCS7_SIGNED_DATA, (void*)sigd);
}

OSStatus SecCmsContentInfoSetContentEnvelopedData(SecCmsContentInfoRef cinfo, SecCmsEnvelopedDataRef envd)
{
    return SecCmsContentInfoSetContent(cinfo, SEC_OID_PKCS7_ENVELOPED_DATA, (void*)envd);
}

OSStatus SecCmsContentInfoSetContentDigestedData(SecCmsContentInfoRef cinfo, SecCmsDigestedDataRef digd)
{
    return SecCmsContentInfoSetContent(cinfo, SEC_OID_PKCS7_DIGESTED_DATA, (void*)digd);
}

OSStatus SecCmsContentInfoSetContentEncryptedData(SecCmsContentInfoRef cinfo, SecCmsEncryptedDataRef encd)
{
    return SecCmsContentInfoSetContent(cinfo, SEC_OID_PKCS7_ENCRYPTED_DATA, (void*)encd);
}

OSStatus SecCmsContentInfoSetContentOther(SecCmsContentInfoRef cinfo,
                                          SecAsn1Item* data,
                                          Boolean detached,
 ...
 * needs to be casted...
 */
void* SecCmsContentInfoGetContent(SecCmsContentInfoRef cinfo)
{
    SECOidTag tag = (cinfo && cinfo->contentTypeTag) ? cinfo->contentTypeTag->offset : SEC_OID_UNKNOWN;
    switch (tag) {
        case SEC_OID_PKCS7_DATA:
        case SEC_OID_PKCS7_SIGNED_DATA:
        case SEC_OID_PKCS7_ENVELOPED_DATA:
 ...
 */
const SecAsn1Item* SecCmsContentInfoGetInnerContent(SecCmsContentInfoRef cinfo)
{
 ...
 */
SECOidTag SecCmsContentInfoGetContentTypeTag(SecCmsContentInfoRef cinfo)
{
    if (cinfo->contentTypeTag == NULL)
...
SecAsn1Oid* SecCmsContentInfoGetContentTypeOID(SecCmsContentInfoRef cinfo)
{
    if (cinfo->contentTypeTag == NULL)
        cinfo->contentTypeTag = SECOID_FindOID(&(cinfo->contentType));
 ...
 */
SECOidTag SecCmsContentInfoGetContentEncAlgTag(SecCmsContentInfoRef cinfo)
{
 ...
 */
SECAlgorithmID* SecCmsContentInfoGetContentEncAlg(SecCmsContentInfoRef cinfo)
{
    return &(cinfo->contentEncAlg);
}

OSStatus SecCmsContentInfoSetContentEncAlg(SecCmsContentInfoRef cinfo,
                                           SECOidTag bulkalgtag,
                                           const SecAsn1Item* parameters,
                                           int keysize)
{
...
OSStatus SecCmsContentInfoSetContentEncAlgID(SecCmsContentInfoRef cinfo, SECAlgorithmID* algid, int keysize)
{
    PLArenaPool* poolp = cinfo->cmsg->poolp;
    OSStatus rv;
...
void SecCmsContentInfoSetBulkKey(SecCmsContentInfoRef cinfo, SecSymmetricKeyRef bulkkey)
{
    CFRetainSafe(bulkkey);
    if (!bulkkey || !cinfo) {
        return;
    }
...
SecSymmetricKeyRef SecCmsContentInfoGetBulkKey(SecCmsContentInfoRef cinfo)
{
    if (cinfo == NULL) {
        return NULL;
    }

    return CFRetainSafe(cinfo->bulkkey);
}

int SecCmsContentInfoGetBulkKeySize(SecCmsContentInfoRef cinfo)
{
