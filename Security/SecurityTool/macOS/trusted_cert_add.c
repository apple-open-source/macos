...
#define MAX_POLICIES    16  /* upper limit on policies that can be set in one invocation */
#define MAX_STRINGS     128 /* upper limit on per-policy strings */
#define MAX_ERRORS      128 /* upper limit on per-policy allowed errors */
	...
	CFMutableDictionaryRef *dict)	/* result RETURNED here, created if necessary */
{
	...
	if(policy != NULL) {
		bool tls = false;
		oid = policyStringToOid(policy, &tls);
		if(oid == NULL) {
...
//	char *policyString = NULL;
	...
	char **policyNames = (char **)malloc(MAX_POLICIES * sizeof(char *));
	char **policyStrings = (char **)malloc(MAX_STRINGS * sizeof(char *));
	int *allowedErrors = (int *)malloc(MAX_ERRORS * sizeof(int));
	int policyNameCount = 0, policyStringCount = 0, allowedErrorCount = 0;
	if (!policyNames || !policyStrings || !allowedErrors) {
		...
		switch (arg) {
			...
			case 'r':
				if(!strcmp(optarg, "trustRoot")) {
					...
					ourRtn = SHOW_USAGE_MESSAGE;
					goto errOut;
				}
				haveConstraints = 1;
				break;
			case 'p':
				if (policyNameCount < MAX_POLICIES) {
			...
			case 'a':
				...
				break;
			case 's':
				if (policyStringCount < MAX_STRINGS) {
				...
				break;
			case 'e':
				if (allowedErrorCount < MAX_ERRORS) {
