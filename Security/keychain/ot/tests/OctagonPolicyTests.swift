...
class OctagonPolicyTests: XCTestCase {
    func prevailingPolicy() throws -> TPPolicy {
        let policyDocument = builtInPolicyDocuments.first { $0.version.versionNumber == prevailingPolicyVersion.versionNumber }
    ...
    func testPrevailingPolicyOutranksKnownPolicies() throws {
        ...
        let builtinPoliciesWithHigherVersionNumbers = builtInPolicyDocuments.filter { $0.version.versionNumber > prevailingPolicy.version.versionNumber }
