...
#include "keychain/SecureObjectSync/ViewList.list"
    if(setKind == kViewSetAlwaysOn && sTestViewSet != NULL) {
        CFReleaseNull(result);
        result = CFSetCreateMutableForCFTypes(kCFAllocatorDefault);
    }
    if(setKind == kViewSetAll && sTestViewSet != NULL) {
        CFSetUnion(result, sTestViewSet);
    }
    return result;
}
...
static CFMutableSetRef sosAllViews = NULL;
...
void SOSViewsSetTestViewsSet(CFSetRef testViewNames) {
    ...
    sosAllViews = SOSViewCopyViewSet(kViewSetAll);
    CFSetAddValue(sosAllViews, kSOSViewKeychainV0);
    if(sTestViewSet) CFSetUnion(sosAllViews, sTestViewSet);
}
