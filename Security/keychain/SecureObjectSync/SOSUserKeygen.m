...
SecKeyRef SOSUserKeygen(CFDataRef password, CFDataRef parameters, CFErrorRef *error)
{
    ...
    ccpbkdf2_hmac(ccsha256_di(), password_length, password_bytes,
                  saltlen, salt,
                  iterations,
    ...
    int rngError = 0;
    int keyError = -1;
    struct ccrng_state *rng = ccrng(&rngError);
    if(rng) {
        keyError=ccec_generate_key_deterministic(cp, drbg_output_size, drbg_output, rng, CCEC_GENKEY_DETERMINISTIC_LEGACY, tmpkey);
        cc_clear(drbg_output_size, drbg_output);
    }
...
{
    CFStringRef keyparm = UserParametersDescription(parameters);
    if (keyparm == NULL) {
        secnotice("circleOps", "failed to decode pbkdf2 params");
        return;
    }
    secnotice("circleOps", "%@ %@]", message, keyparm);
    CFReleaseNull(keyparm);
}
