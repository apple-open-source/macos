...
let prevailingPolicyVersion = TPPolicyVersion(version: 19, hash: "SHA256:jeVSNUOuQg7ENqP1vicvqGJdGWILI8g8jHeEag9YbC8=")
...
var builtInPolicyDocuments: [TPPolicyDocument] = {
    ...
    return rawPolicies.map { raw in
        assert(!reservedVersions.contains(raw.version.versionNumber))
        ...
        if !doc.isEqual(to: raw.plaintextPolicy) {
            ...
            logger.info("raw policy doesn't match encoded bytes, new hash would be: \(hash, privacy: .public) new data: \(bodyBase64, privacy: .public)")
        }
