...
class SFAnalyticsUploadTests: SupdTests {
    ...
    func testSFAnalyticsTopicGenerator() async throws {
        ...
        let uuid = UUID()
        let testPath = FileManager.default.temporaryDirectory.appending(path: uuid.uuidString)
        ...
        guard let c2 = try? fileManager.contentsOfDirectory(at: testPath, includingPropertiesForKeys: []) else {
            XCTFail("contentsOfDirectory")
            return
        }
        ...
        try uploadFiles.forEach { file in
            ...
            do {
                ...
                guard let j else {
                    XCTFail("j should be an JSON")
                    return
                }
                XCTAssertNotNil(j["postTime"], "should have posttime")
                XCTAssertNotNil(j["events"], "should have events")
                guard let events = j["events"] as? [NSDictionary] else {
                    XCTFail("events should be an array of NSDictionary")
                    return
                }
