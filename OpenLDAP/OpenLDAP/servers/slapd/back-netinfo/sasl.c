/* $OpenLDAP: pkg/ldap/servers/slapd/sasl.c,v 1.81 2002/02/11 19:33:27 kurt Exp $ */
/*
	...
	struct propctx *propctx)
{
	...
	rc = backend_attribute( NULL, NULL, NULL, NULL, &dn,
		slap_schema.si_ad_authAuthority, &vals );
	if ( rc == LDAP_SUCCESS ) {
		if ( vals != NULL ) {
			if ( vals->bv_val != NULL && (CheckAuthType(vals->bv_val, BASIC_AUTH_TYPE)) == 0) {
				/*
			 	 * Must use authentication authority.
				 */
				if (DoPSAuth((char *)username, (char *)pass, vals->bv_val) == kAuthNoError) {
					rc = SASL_OK;
				} else {
					rc = SASL_NOVERIFY;
					sasl_seterror(sconn, 0, ldap_err2string(LDAP_INVALID_CREDENTIALS));
				}
				ch_free(dn.bv_val);
