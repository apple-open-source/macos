...
DisplayHelp()
{
	...
	printf("disktool:  disktool -[r][a][u][e][m][p][n][d][x][y][c][s][g][A][S][D][l]\n");
...
void
UnknownFS_callback(DiskArbDiskIdentifier diskIdentifier,
		   char *fsType,
		   char *deviceType,
		   int isWritable,
		   int isRemovable,
...
void
DiskWillbeChecked_callback(DiskArbDiskIdentifier diskIdentifier,
			   int flags,
			   char *ioContent)
{
	printf("*** Disk %s appears to be dirty and will be checked, %d, %s ***\n", diskIdentifier, flags, ioContent);
}
...
void
CallSucceeded_callback(DiskArbDiskIdentifier diskIdentifier,
		       int type)
{
...
main(int argc, const char *argv[])
{
    ...
    //while ((ch = getopt(argc, argv, "rauempndxycsgASDlTREUVX")) != -1)
    while ((ch = getopt(argc, (char * const *)argv, "rauempndxycsgASDl")) != -1)
    {
        switch (ch) {
			case 'S':
				vsdbDisplay = 1;
	...
	DiskArbRegisterCallback_CallSucceededNotification(CallSucceeded_callback);
	...
	if (!do_unmount && !eject && !punmount && !vsdb && !getEncoding && !setEncoding && !retain && !test && !release && !unrecognizedMessages) {
		...
		//DiskArbRegisterCallback_DiskAppeared2(DiskAppeared2_callback);
		...
		DiskArbRegisterCallback_ClientDisconnectedNotification(ClientDisconnected_callback);
