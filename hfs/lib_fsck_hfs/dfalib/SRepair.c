...
static int MRepair( SGlobPtr GPtr )
{
	...
	{
		fsckPrintFormat(GPtr->context, hfsRebuildExtentBTree);
	...
	if ( GPtr->CBTStat & S_RebuildBTree )
	{
		...
		fsckPrintFormat(GPtr->context, hfsRebuildCatalogBTree);
	...
	{
		fsckPrintFormat(GPtr->context, hfsRebuildAttrBTree);
	...
	case kHFSPlusFolderRecord:	// directory hard link
		...
		if (retval) {
			...
			if ((GPtr->calculatedAttributesFCB->fcbPhysicalSize == 0) &&
			    ...
			    (fsck_get_verbosity_level() >= kDebugLog)) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tFixBadLinkChainFirst: Attribute btree does not exists.\n");
			}
		}
		break;
...
{ 
	...
	if (msg_display == 0) {
		fsckPrintFormat(GPtr->context, fsckLostFoundDirectory, "lost+found");
		msg_display = 1;
	}
...
static OSErr FixBadFlags(SGlobPtr GPtr, RepairOrderPtr p)
{
	...
	if (p->type == E_DirInodeBadFlags) {
		if ((rec.hfsPlusFolder.flags != p->incorrect) && (fsck_get_verbosity_level() >= kDebugLog)) {
			fsck_print(ctx, LOG_TYPE_STDERR, "\tFixBadFlags (folder):  old = %#x, incorrect = %#x, correct = %#x\n", rec.hfsPlusFolder.flags, (int)p->incorrect, (int)p->correct);
		}
		rec.hfsPlusFolder.flags = p->correct;
	} else if (p->type == E_DirLinkAncestorFlags) {
		if ((rec.hfsPlusFolder.flags != p->incorrect) && (fsck_get_verbosity_level() >= kDebugLog)) {
			fsck_print(ctx, LOG_TYPE_STDERR, "\tFixBadFlags (parent folder):  old = %#x, incorrect = %#x, correct = %#x\n", rec.hfsPlusFolder.flags, (int)p->incorrect, (int)p->correct);
		}
		rec.hfsPlusFolder.flags = p->correct;
	} else {
		if ((rec.hfsPlusFolder.flags != p->incorrect) && (fsck_get_verbosity_level() >= kDebugLog)) {
			fsck_print(ctx, LOG_TYPE_STDERR, "\tFixBadFlags (file):  old = %#x, incorrect = %#x, correct = %#x\n", rec.hfsPlusFolder.flags, (int)p->incorrect, (int)p->correct);
		}
...
#define DPRINT(where, fmt, ...) \
	if (fsck_get_verbosity_level() >= kDebugLog) \
		fsck_print(ctx, LOG_TYPE_INFO, fmt, ## __VA_ARGS__);
	...
	while( (p = cur) && (err == noErr) )	//	loop over each repair order
	{
		...
		switch( p->type )									//	branch on repair type
		{
			...
			default:										//	unknown repair type
				if (fsck_get_verbosity_level() >= kDebugLog) {
						fsck_print(ctx, LOG_TYPE_INFO, "\tUnknown repair order found (type = %d)\n", p->type);
				}
				err = IntError( GPtr, R_IntErr );			//	treat as an internal error
				break;
		}

		if ((err != 0) && (fsck_get_verbosity_level() >= kDebugLog)) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tDoMinorRepair: Repair for type=%d failed (err=%d).\n", p->type, err);
		}	
...
FixIllegalNames( SGlobPtr GPtr, RepairOrderPtr roPtr )
{
	...
	if ( result == noErr ) {
		if ( fsck_get_verbosity_level() >= kDebugLog ) {
        	fsck_print(ctx, LOG_TYPE_INFO, "\treplacement name already exists \n" );
			fsck_print(ctx, LOG_TYPE_INFO, "\tduplicate name is 0x" );
...
ErrorExit:
    ...
    if ( fsck_get_verbosity_level() >= kDebugLog )
       	fsck_print(ctx, LOG_TYPE_INFO, "\t%s - repair failed for type 0x%02X %d \n", __FUNCTION__, roPtr->type, roPtr->type );
...
FixBSDInfo(SGlobPtr GPtr, RepairOrderPtr p)
{
	...
	if (p->type == E_InvalidPermissions &&
		...
		if (fsck_get_verbosity_level() >= kDebugLog)
		   	fsck_print(ctx, LOG_TYPE_INFO, "\t\"%s\": fixing mode from %07o to %07o\n",
...
static OSErr FixAttrSize(SGlobPtr GPtr, RepairOrderPtr p)
{
	...
	if (result) {
		fsck_debug_print(ctx, d_error|d_xattr, "%s: Cannot find attribute record (err = %d)\n", __FUNCTION__, result);
		goto out;
	}

	/* We should only get record of type kHFSPlusAttrForkData */
	if (record.recordType != kHFSPlusAttrForkData) {
		fsck_debug_print(ctx, d_error|d_xattr, "%s: Record found is not attribute fork data\n", __FUNCTION__);
		result = btNotFound;
		goto out;
	}
	...
	if (doReplace == true) {
		result = BTReplaceRecord(GPtr->calculatedAttributesFCB, &iterator,
					&btRecord, recSize);
		if (result) {
			fsck_debug_print(ctx, d_error|d_xattr, "%s: Cannot replace attribute record (err=%d)\n", __FUNCTION__, result);
			goto out;
		}
	}
...
{	
	...
	if (extentStartBlock == 0) {
		...
		if (err) {
			if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "%s: Could not get catalog record for fileID %u\n", __FUNCTION__, fileID);
			goto out;
		}
		...
		if (err) {
			if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "%s: Could not replace catalog record for fileID %u\n", __FUNCTION__, fileID);
			goto out;
		}
		didRepair = true;
		...
		if (err) {
			if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "%s: Could not get overflow extents record for fileID %u, fork %u, start block %u\n", __FUNCTION__, fileID, forkType,
                            extentStartBlock);
			goto out;
		}
		...
		if (err) {
			if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "%s: Could not replace overflow extents record for fileID %u, fork %u, start block %u\n", __FUNCTION__, fileID,
...
del_overflow_extents:
	...
	while (err == noErr) {
		...
		fsck_debug_print(ctx, d_info, "%s: Deleting extent overflow for fileID=%u, forkType=%u, startBlock=%u\n", __FUNCTION__, fileID, forkType, foundStartBlock);
		if (err) {
			goto create_symlink;
		}
		didRepair = true;
		...
		switch( recordType )
		{
			...
			case kHFSPlusFileRecord:
					...
					if (isHFSPlus) {
						...
						if (isDirectory == true) {
							if (foundRecType != kHFSPlusFolderThreadRecord) {
								err = btNotFound;
								if (fsck_get_verbosity_level() >= kDebugLog) {
									fsck_print(ctx, LOG_TYPE_INFO, "\t%s: Folder thread recordType mismatch for id=%u (found=%u)\n", __FUNCTION__, cNodeID, foundRecType);
								}
							} 
						} else {
							if (foundRecType != kHFSPlusFileThreadRecord) {
								err = btNotFound;
								if (fsck_get_verbosity_level() >= kDebugLog) {
									fsck_print(ctx, LOG_TYPE_INFO, "\t%s: File thread recordType mismatch for id=%u (found=%u)\n", __FUNCTION__, cNodeID, foundRecType);
								}
							} 
						}
						...
						if (parentID != threadRecord.hfsPlusThread.parentID) {
							err = btNotFound;
							if (fsck_get_verbosity_level() >= kDebugLog) {
								fsck_print(ctx, LOG_TYPE_INFO, "\t%s: parentID for id=%u do not match (fileKey=%u threadRecord=%u)\n", __FUNCTION__, cNodeID, parentID, threadRecord.hfsPlusThread.parentID);
							}
						}
				...
				if ( err == btNotFound )
				{
					...
					if (fsck_get_verbosity_level() >= kDebugLog) {
						fsck_print(ctx, LOG_TYPE_INFO, "\t%s: Created thread record for id=%u (err=%u)\n", __FUNCTION__, cNodeID, err);
					}
				}
			...
			case kHFSPlusFileThreadRecord:
					...
					if (isHFSPlus) {
						...
						if (isDirectory == true) {
							if (foundRecType != kHFSPlusFolderRecord) {
								err = btNotFound;
								if (fsck_get_verbosity_level() >= kDebugLog) {
									fsck_print(ctx, LOG_TYPE_INFO, "\t%s: Folder recordType mismatch for id=%u (found=%u)\n", __FUNCTION__, cNodeID, foundRecType);
								}
							} 
						} else {
							if (foundRecType != kHFSPlusFileRecord) {
								err = btNotFound;
								if (fsck_get_verbosity_level() >= kDebugLog) {
									fsck_print(ctx, LOG_TYPE_INFO, "\t%s: File recordType mismatch for id=%u (found=%u)\n", __FUNCTION__, cNodeID, foundRecType);
								}
							} 
						}
				...
				if ( err != noErr )
				{
					...
					if (fsck_get_verbosity_level() >= kDebugLog) {
						if (isHFSPlus) {
							fsck_print(ctx, LOG_TYPE_INFO, "\t%s: Deleted thread record for id=%d (err=%d)\n", __FUNCTION__, foundKey.hfsPlus.parentID, err);
						} else {
			...
			default:
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\t%s: Unknown record type.\n", __FUNCTION__);
				}
				break;
...
static OSErr GetCatalogRecord(SGlobPtr GPtr, UInt32 fileID, Boolean isHFSPlus, CatalogKey *catKey, CatalogRecord *catRecord, UInt16 *recordSize) 
{
	...
	if (err) {
		if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "%s: No matching catalog thread record found\n", __FUNCTION__);
		goto out;
	}

#if DEBUG_XATTR
	fsck_print(ctx, LOG_TYPE_INFO, "%s(%s,%d):1 recordType=%x, flags=%x\n", __FUNCTION__, __FILE__, __LINE__,
	...
	if (err) {
		if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "%s: No matching catalog record found\n", __FUNCTION__);
		if (state.cur_debug_level & d_dump_record)
		{
			fsck_print(ctx, LOG_TYPE_INFO, "Searching for key:\n");
...
#if DEBUG_XATTR
	fsck_print(ctx, LOG_TYPE_INFO, "%s(%s,%d):2 recordType=%x, flags=%x\n", __FUNCTION__, __FILE__, __LINE__,
				catRecord->hfsPlusFile.recordType, 
				catRecord->hfsPlusFile.flags);
#endif
	...
	if (thread_key_parentID != catRecord->hfsPlusFile.fileID) {
		...
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\t%s: fileID=%u, thread.key.parentID=%u, record.fileID=%u\n",
...
static OSErr RepairAttributesCheckABT(SGlobPtr GPtr, Boolean isHFSPlus) 
{
	...
	do {
		...
		attrName[len] = '\0';
		fsck_print(ctx, LOG_TYPE_INFO, "%s(%s,%d): Found attrName=%s for fileID=%d\n", __FUNCTION__, __FILE__, __LINE__, attrName, attrKey.fileID);
#endif
		...
		if (attrKey.fileID != lastID.fileID) {
			...
			if (didRecordChange == true) {
				...
				if (err) {
					if (fsck_get_verbosity_level() >= kDebugLog) {
						fsck_print(ctx, LOG_TYPE_INFO, "\t%s: Error in replacing catalog record for id=%u\n", __FUNCTION__, lastID.fileID);
					}
					goto out;
				}
			}
			...
			if (err) {
				...
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\t%s: No matching catalog record found for id=%u\n", __FUNCTION__, attrKey.fileID);
				}
...
#if DEBUG_XATTR
				fsck_print(ctx, LOG_TYPE_INFO, "%s: Ignore catalog check for fileID=%d for attribute=%s\n", __FUNCTION__, attrKey.fileID, attrName);
#endif
					goto getnext;
				}
				...
				if (err) {
					if (fsck_get_verbosity_level() >= kDebugLog) {
						fsck_print(ctx, LOG_TYPE_INFO, "\t%s: Error in deleting attribute record for id=%u\n", __FUNCTION__, attrKey.fileID);
					}
					goto out;
				}
				goto getnext;
...
getnext:
	...
	if (didRecordChange == true) {
		...
		if (err) {
#if DEBUG_XATTR
			fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in replacing Catalog Record\n", __FUNCTION__);
#endif		
			goto out;
		}
	}
...
static OSErr RepairAttributesCheckCBT(SGlobPtr GPtr, Boolean isHFSPlus)
{
	...
	do {
...
#if DEBUG_XATTR
			fsck_print(ctx, LOG_TYPE_INFO, "%s: No matching attribute record found\n", __FUNCTION__);
#endif
			goto out;
		}
		...
		if (didRecordChange == true) {
			...
			if (err) {
#if DEBUG_XATTR
				fsck_print(ctx, LOG_TYPE_INFO, "%s: Error writing catalog record\n", __FUNCTION__);
#endif
				goto out;
			}
		}
...
#define	S_MOVEEXTENT		0x02	/* moving extent succeeded */
	...
	for (i=0; i<numOverlapExtents; i++) {
		...
		fsck_print(ctx, LOG_TYPE_INFO, "%d: fileID = %d, startBlock = %d, blockCount = %d\n", i, extentInfo->fileID, extentInfo->startBlock, extentInfo->blockCount);
	}
#endif
	...
	for (i=0; i<numOverlapExtents; i++) {
		...
		if ((err != noErr)) {
			...
			status |= S_DISKFULL;
#if DEBUG_OVERLAP
			fsck_print(ctx, LOG_TYPE_INFO, "%s: Not enough disk space to allocate extent for fileID = %d (start=%d, count=%d)\n", __FUNCTION__, extentInfo->fileID, extentInfo->startBlock, extentInfo->blockCount);
#endif
		}
	}
	...
	for (i=0; i<numOverlapExtents; i++) {
		...
		if (err != noErr) {
#if DEBUG_OVERLAP
			fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in creating symlink for fileID = %d (err=%d)\n", __FUNCTION__, extentInfo->fileID, err);
#endif
		} else {
#if DEBUG_OVERLAP
			fsck_print(ctx, LOG_TYPE_INFO, "%s: Created symlink for fileID = %u (old=%u, new=%u, count=%u)\n", __FUNCTION__, extentInfo->fileID, extentInfo->startBlock, extentInfo->newStartBlock, extentInfo->blockCount);
#endif
		}
	}
...
out:
	...
	if (status & S_DISKFULL) {
		fsckPrintFormat(GPtr->context, E_DiskFull);
	} 
...
static OSErr MoveExtent(SGlobPtr GPtr, ExtentInfo *extentInfo)
{
	...
	if (extentInfo->forkType == kEAData) {
		...
		if (err != noErr) {
			if (noMoreExtents == false) { 
				...
				if (err != noErr) {
                    fsck_debug_print(ctx, d_error|d_overlap, "%s: No matching extent record found in extents btree for fileID = %d (err=%d)\n", __FUNCTION__, extentInfo->fileID, err);
					goto out;
				}
			} else {
				/* No more extents exist for this file */
                fsck_debug_print(ctx, d_error|d_overlap, "%s: No matching extent record found for fileID = %d\n", __FUNCTION__, extentInfo->fileID);
				goto out;
			}
		}
	}
	...
	if (err != noErr) {
        fsck_debug_print(ctx, d_error|d_overlap, "%s: Error in copying disk blocks for fileID = %d (err=%d)\n", __FUNCTION__, extentInfo->fileID, err);
		goto out;
	}
	...
	if (err != noErr) {
        fsck_debug_print(ctx, d_error|d_overlap, "%s: Error in updating extent record for fileID = %d (err=%d)\n", __FUNCTION__, extentInfo->fileID, err);
		goto out;
	}
...
static OSErr CreateCorruptFileSymlink(SGlobPtr GPtr, UInt32 fileID)
{
	...
	if (fileID >= kHFSFirstUserCatalogNodeID) {
		...
		if (err != noErr) {
#if DEBUG_OVERLAP
			fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in getting name/path for fileID = %d (err=%d)\n", __FUNCTION__, fileID, err);
#endif
			goto out;
		}
	...
	if (err != noErr) {
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in creating fileType = %d for fileID = %d (err=%d)\n", __FUNCTION__, fileType, fileID, err);
#endif
		goto out;
	}
...
out:
	if (err) {
		if ((GPtr->PrintStat & S_SymlinkCreate) == 0) {
			fsckPrintFormat(GPtr->context, E_SymlinkCreate);
			GPtr->PrintStat|= S_SymlinkCreate;
		}
	} else {
		if ((GPtr->PrintStat & S_DamagedDir) == 0) {
			fsckPrintFormat(GPtr->context, fsckCorruptFilesDirectory, "DamagedFiles");
				...
				UInt16 *recordSize, UInt32 *foundExtentIndex)
{
	...
	if (result) {
        fsck_debug_print(ctx, d_error|d_overlap, "%s: Error finding attribute record (err=%d) for fileID = %d, attrname = %d\n", __FUNCTION__, result, extentInfo->fileID, extentInfo->attrname);
		goto out;	
	}
...
{							 
	...
	if (err != noErr) {
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: No matching catalog record found for fileID = %d (err=%d)\n", __FUNCTION__, extentInfo->fileID, err);
#endif
		goto out;
	}
...
static OSErr UpdateExtentInCatalogBT (SGlobPtr GPtr, ExtentInfo *extentInfo, CatalogKey *catKey, CatalogRecord *catRecord, UInt16 *recordSize, UInt32 foundInExtentIndex)
{
	...
	if (err != noErr) {
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in replacing catalog record for fileID = %d (err=%d)\n", __FUNCTION__, extentInfo->fileID, err);
#endif
	}
	return err;
...
static OSErr SearchExtentInExtentBT(SGlobPtr GPtr, ExtentInfo *extentInfo, HFSPlusExtentKey *extentKey, HFSPlusExtentRecord *extentRecord, UInt16 *recordSize, UInt32 *foundExtentIndex) 
{
	...
	if ((err != noErr) && (err != btNotFound)) {
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: Error on searching first record for fileID = %d in Extents Btree (err=%d)\n", __FUNCTION__, extentInfo->fileID, err);
#endif
		goto out;
	}
...
OSErr GetFileNamePathByID(SGlobPtr GPtr, UInt32 fileID, char *fullPath, unsigned int *fullPathLen, char *fileName, unsigned int *fileNameLen, u_int16_t *status)
{
	...
	if (!filename) {
		err = memFullErr;
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: Not enough memory (err=%d)\n", __FUNCTION__, err);
#endif
		goto out;
	}
	...
	while (fileID != kHFSRootFolderID) {
		...
		if (err) {
#if DEBUG_OVERLAP
			fsck_print(ctx, LOG_TYPE_INFO, "%s: Error finding thread record for fileID = %d (err=%d)\n", __FUNCTION__, fileID, err);
#endif
			goto out;
		}
		...
		if ((catRecord.hfsPlusThread.recordType != kHFSPlusFileThreadRecord) &&
			...
			err = paramErr;
#if DEBUG_OVERLAP
			fsck_print(ctx, LOG_TYPE_INFO, "%s: Error finding valid thread record for fileID = %d\n", __FUNCTION__, fileID);
#endif
			goto out;
		}
...
FixMissingThreadRecords( SGlob *GPtr )
{
	...
	for (mtp = GPtr->missingThreadList; mtp != NULL; mtp = mtp->link) {
		...
		if ( mtp->thread.parentID == 0 ) {
			...
			if ( lostAndFoundDirID == 0 ) {
				if ( fsck_get_verbosity_level() >= kDebugLog )
					fsck_print(ctx, LOG_TYPE_INFO, "\tCould not create lost+found directory \n" );
				return( R_RFail );
			}
			fsckPrintFormat(GPtr->context, E_NoDir, mtp->threadID);
			result = FixMissingDirectory( GPtr, mtp->threadID, lostAndFoundDirID );
			if ( result != 0 ) {
				if ( fsck_get_verbosity_level() >= kDebugLog )
					fsck_print(ctx, LOG_TYPE_INFO, "\tCould not recreate a missing directory (error %d)\n", result );
				return( R_RFail );
			}
			else
				headsUp = true;
...
{	
...
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: %s probably exists in dirID = %d (err=%d)\n", __FUNCTION__, fileName, parentID, err);
#endif
		err = EEXIST;
		goto out;
	}
	...
	if (data) {
		...
		if (blockCount) {
			...
			if (err != noErr) {
#if DEBUG_OVERLAP
				fsck_print(ctx, LOG_TYPE_INFO, "%s: Not enough disk space (err=%d)\n", __FUNCTION__, err);
#endif
				goto out;
			}
			...
			if (err != noErr) {
#if DEBUG_OVERLAP
				fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in writing data of %s to disk (err=%d)\n", __FUNCTION__, fileName, err);
#endif
				goto out;
			}
		}
	...
	if (err != noErr) {
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: Error inserting thread record for file = %s (err=%d)\n", __FUNCTION__, fileName, err);
#endif
		goto out;
	}
	...
	if (recordSize == 0) {
		...
		if (err != noErr) {
#if DEBUG_OVERLAP
			fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in removing thread record\n", __FUNCTION__);
#endif
		}
		err = paramErr;
		goto out;
...
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "Created \"%s\" with ID = %d startBlock = %d, blockCount = %d, dataLen = %d\n", fileName, nextCNID, startBlock, blockCount, dataLen);
#endif
	} else {
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in inserting file record for file = %s (err=%d)\n", __FUNCTION__, fileName, err);
#endif
	...
	if (err != noErr) {
#if DEBUG_OVERLAP
		fsck_print(ctx, LOG_TYPE_INFO, "%s: Error in updating parent folder count (err=%d)\n", __FUNCTION__, err);
#endif
		goto out;
	}
