...
OSErr	RebuildBTree( SGlobPtr theSGlobPtr, int FileID )
{
	...
	if (kHFSCatalogFileID == FileID) {
		...
		if (oldFCBPtr->fcbLogicalSize == 0) {
			if (state.debug)
				fsck_print(ctx, LOG_TYPE_INFO, "Requested attributes btree rebuild, but attributes file size is 0\n");
			myErr = 0;
	...
	if ( noErr != myErr ) {
#if DEBUG_REBUILD
		fsck_print(ctx, LOG_TYPE_INFO, "CreateNewBTree returned %d\n", myErr);
#endif
		if (myErr == dskFulErr) {
			fsckPrintFormat(theSGlobPtr->context, E_DiskFull);
		}
		goto ExitThisRoutine;
	}
	...
	if (state.debug) {
		...
		fsck_print(ctx, LOG_TYPE_INFO, "Extent records for rebuilt file %u:\n", FileID);
		for (i = 0; i < kHFSPlusExtentDensity; i++) {
			fsck_print(ctx, LOG_TYPE_INFO, "\t[ %u, %u ]\n", te[i].startBlock, te[i].blockCount);
		}
	}
#endif
	...
	while ( true )
	{
...
#if DEBUG_REBUILD 
			{
				fsck_print(ctx, LOG_TYPE_INFO, "%s - Record length validation (file %d) failed! \n", __FUNCTION__, FileID );
				fsck_print(ctx, LOG_TYPE_INFO, "%s - record %d in node %d is not recoverable. \n",
...
#if DEBUG_REBUILD 
			{
				fsck_print(ctx, LOG_TYPE_INFO, "%s - InsertBTreeRecord failed with err %d 0x%02X \n",
					__FUNCTION__, myErr, myErr );
				fsck_print(ctx, LOG_TYPE_INFO, "%s - numRecords = %d\n", __FUNCTION__, numRecords);
				fsck_print(ctx, LOG_TYPE_INFO, "%s - record %d in node %d is not recoverable. \n",
						__FUNCTION__, (theSGlobPtr->scanState.recordNum - 1), 
						theSGlobPtr->scanState.nodeNum );
		...
		numRecords++;
#if DEBUG_REBUILD
		if (state.debug && ((numRecords % 1000) == 0))
			fsck_print(ctx, LOG_TYPE_INFO, "btree file %d:  %u records\n", FileID, numRecords);
#endif
	}
	...
	if (FileID == kHFSCatalogFileID) {
		...
		if (theSGlobPtr->calculatedAttributesFCB == NULL) {
			if (state.debug) {
				fsck_print(ctx, LOG_TYPE_INFO, "Can't rebuilt attributes btree when there is no attributes file\n");
			}
			myErr = noErr;
			goto ExitThisRoutine;
		}
...
static OSErr ValidateAttributeRecordLength (SGlobPtr s, HFSPlusAttrRecord * theRecPtr, UInt32 theRecSize)
{
	...
	case kHFSPlusAttrInlineData:
		if (theRecSize > maxInlineSize) {
			if (state.debug)
				fsck_print(ctx, LOG_TYPE_INFO, "Inline Attribute size %u is larger than maxsize %u\n", theRecSize, maxInlineSize);
			retval = -1;
		}
		break;
	case kHFSPlusAttrForkData:
		if (theRecSize != sizeof(HFSPlusAttrForkData)) {
			if (state.debug)
				fsck_print(ctx, LOG_TYPE_INFO, "Fork Data attribute size %u is larger then HFSPlusAttrForkData size %u\n", theRecSize, sizeof(HFSPlusAttrForkData));
			retval = -1;
		}
		break;
	case kHFSPlusAttrExtents:
		if (theRecSize != sizeof(HFSPlusAttrExtents)) {
			if (state.debug)
				fsck_print(ctx, LOG_TYPE_INFO, "Extents Data attribute size %u is larger than HFSPlusAttrExtents size %u\n", theRecSize, sizeof(HFSPlusAttrExtents));
			retval = -1;
		}
		break;
	default:
		...
		if (state.debug)
			fsck_print(ctx, LOG_TYPE_INFO, "Unknown attribute type %u\n", theRecPtr->recordType);
		retval = -1;
		break;
	}
	return retval;
...
static void PrintNodeDescriptor( NodeDescPtr thePtr )
{
	...
	fsck_print(ctx, LOG_TYPE_INFO, "   fLink %d \n", thePtr->fLink );
	fsck_print(ctx, LOG_TYPE_INFO, "   bLink %d \n", thePtr->bLink );
	fsck_print(ctx, LOG_TYPE_INFO, "   kind %d ", thePtr->kind );
	if ( thePtr->kind == kBTLeafNode )
		...
		fsck_print(ctx, LOG_TYPE_INFO, "do not know?? \n" );
	fsck_print(ctx, LOG_TYPE_INFO, "   height %d \n", thePtr->height );
	fsck_print(ctx, LOG_TYPE_INFO, "   numRecords %d \n", thePtr->numRecords );
...
static void PrintBTHeaderRec( BTHeaderRec * thePtr )
{
	...
	fsck_print(ctx, LOG_TYPE_INFO, "   nodeSize %d \n", thePtr->nodeSize );
	fsck_print(ctx, LOG_TYPE_INFO, "   maxKeyLength %d \n", thePtr->maxKeyLength );
	fsck_print(ctx, LOG_TYPE_INFO, "   totalNodes %d \n", thePtr->totalNodes );
	fsck_print(ctx, LOG_TYPE_INFO, "   freeNodes %d \n", thePtr->freeNodes );
	fsck_print(ctx, LOG_TYPE_INFO, "   clumpSize %d \n", thePtr->clumpSize );
	fsck_print(ctx, LOG_TYPE_INFO, "   btreeType 0x%02X \n", thePtr->btreeType );
	fsck_print(ctx, LOG_TYPE_INFO, "   attributes 0x%02X \n", thePtr->attributes );
...
static void PrintIndexNodeRec( UInt32 theNodeNum )
{
	...
	fsck_print(ctx, LOG_TYPE_INFO, "   node number %d \n", theNodeNum );
...
static void PrintLeafNodeRec( HFSPlusCatalogFolder * thePtr )
{
	...
	fsck_print(ctx, LOG_TYPE_INFO, "   recordType %d ", thePtr->recordType );
