    ...
    Boolean  expectUpToDate)
{
    ...
    if (!CFURLGetFileSystemRepresentation(volumeURL,
        /* resolveToBase */ true, (UInt8 *)volRoot,
        sizeof(volRoot))) {
    ...
    // a longer window during which the file can be updated.  Also, 
    // being locked means owners are enabled so it's okay for
    // readCaches to make [yes, SIC] the bootstamps directory.

    // kextcache -U doesn't lock (and kextd holds off for 5 minutes)
    if (expectUpToDate == false) {
        if (takeVolumeForPaths(volRoot)) { // lock (logs)
            goto finish;
        }
    ...
    if (!(up.caches = readCaches(dadisk))) {
        // using WarningLevel for now; only emitted by kextcache -u
        OSKextLog(/* kext */ NULL,
            kOSKextLogWarningLevel | kOSKextLogFileAccessFlag,
    ...
    if (check_mkext(up.caches)) {
        ...
        OSKextLog(/* kext */ NULL, kOSKextLogProgressLevel,
    ...
    if (!hasBootRootBoots(up.caches, &up.boots, &up.onAPM)) {
        result = 0;     // no boots -> nothing to do; byebye
            OSKextLog(/* kext */ NULL,
            kOSKextLogWarningLevel | kOSKextLogFileAccessFlag,
...
bootfail:
   ...
   /* We're here if we successfully updated the helpers.  If weren't expecting
    * to make updates, return EX_OSFILE, otherwise return success.  */
    if (expectUpToDate) {
        result = EX_OSFILE;
    } else {
        result = EX_OK;
...
finish:
    ...
    // notify kextd that we're done by sending it a a "fake" lock request.
    if (expectUpToDate && result != EX_OSFILE && geteuid() == 0) {
        // PR-5519500 - Call overloaded takeVolumeForPath to signal kextd
        // that -U is done and it can start watching volumes.  If the 
        // lock succeeds b/c this isn't early boot, an unlock occurs below.
    ...
    putVolumeForPath(volRoot, result);          // handles not locked (& logs)
...
static int unmountBoot(struct updatingVol *up)
{
    ...
    OSKextLog(/* kext */ NULL, kOSKextLogDetailLevel | kOSKextLogFileAccessFlag,
