 ...
 */
#ifndef __OPEN_SOURCE__      // BLGenerateLabelData() uses CG
static int
writeLabels(struct updatingVol *up, char *labelpath)
{
    int opres, rval = ELAST + 1;
    ...
    // create label text (format can depend on presence of recovery)
    makebootpath(path, "/" kRecoveryBootDir);
    if (!up->detectedRecovery) {
        up->detectedRecovery = (stat(path, &sb) == 0);
    }
    if (up->bootIdx == 0 || up->detectedRecovery) {
        fmt = "%s";
    } else {
    ...
    if (snprintf(bootname, NAME_MAX, fmt, up->caches->defLabel,
                 up->bootIdx + 1) >= NAME_MAX) {
        rval = EOVERFLOW; goto finish;
    }

    // create the parent directory
    pathcpy(path, dirname(labelpath));
    if ((opres = sdeepmkdir(up->curbootfd, path, kCacheDirMode))) {
        rval = opres; goto finish;
    }

    // write .root_uuid while we have the parent directory handy
    if (up->bootIdx == 0 || up->detectedRecovery) {
        pathcat(path, "/" kBRRootUUIDFile);
        // in case code is later added above
        if (fd != -1)   close(fd);
        ...
        len = NCHARSUUID;
        if (write(fd, up->caches->fsys_uuid, len) != len ||
                write(fd, "\n", 1) != 1) {
            rval = errno; goto finish;
        }
    }

    // generate and write .disk_label (nukeBRLabels() removed old one)
    opres = BLGenerateLabelData(NULL, bootname, kBitmapScale_1x, &lData);
    if (opres) {
        // if BLGenerateLabelData() returns non-zero, you can look at
        // its source to see how far it got. :P
    ...
    if (write(fd, CFDataGetBytePtr(lData), len) != len) {
        rval = errno; goto finish;
    }

    // generate and write .disk_label_2x
    CFRelease(lData);   lData = NULL;
    opres = BLGenerateLabelData(NULL, bootname, kBitmapScale_2x, &lData);
    if (opres) {
        OSKextLog(NULL, up->errLogSpec, "%s - BLGenerateLabelData(): %d",
                  __FUNCTION__, opres);     // see above re: interpretation
        rval = opres; goto finish;
    }
