...
)
{
    ...
    if ( ldap_locate_path( path, SUFFIX_TEMPLATEFILE ) == NULL ) {
    ...
    if ( tmpllist == NULL ) {
	if (( err = ldap_init_templates( path, &tmpllist )) != 0 ) {
...
searchaction( LDAP *ld, char *buf, char *base, LDAPMessage *entry, char *dn,
	...
	writeptype writeproc, void *writeparm, char *eol, char *urlprefix )
{
    int			err = LDAP_LOCAL_ERROR, lderr, i, count, html;
    char		**vals, **members;
