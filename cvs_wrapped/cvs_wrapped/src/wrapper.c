...
void wrap_setup()
{
    ...
    {
	char *file;
	int max_wrapper = (sizeof(CVSDOTWRAPPER) > sizeof(CVSDOTWRAPPER_FALLBACK))
	    ? sizeof(CVSDOTWRAPPER) : sizeof(CVSDOTWRAPPER_FALLBACK);

	file = xmalloc (strlen (homedir) + max_wrapper + 10);
	(void) sprintf (file, "%s/%s", homedir, CVSDOTWRAPPER);
	if (isfile (file))
	{
	    wrap_add_file (file, 0);
	} else {
	    (void) sprintf (file, "%s/%s", homedir, CVSDOTWRAPPER_FALLBACK);
	    if (isfile (file))
	    {
		wrap_add_file (file, 0);
	    }
