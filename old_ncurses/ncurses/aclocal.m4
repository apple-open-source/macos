...
dnl
dnl $Id: aclocal.m4,v 1.8 2002/02/15 22:13:26 jevans Exp $
	...
	*)
	...
	darwin*)
		for i in $RC_ARCHS ; do
			ARCH_CFLAGS="$ARCH_CFLAGS -arch $i"
		done
		EXTRA_CFLAGS="-no-cpp-precomp"
		EXTRA_LDFLAGS="$ARCH_CFLAGS"
		CC_SHARED_OPTS="-dynamic"
		mk_shared_lib_prefix='$(CC)'
		mk_shared_lib_suffix='-dynamiclib -install_name $(DESTDIR)$(libdir)/`basename $[@]` -compatibility_version $(ABI_VERSION) -current_version $(ABI_VERSION) -o $[@]'
		MK_SHARED_LIB="$mk_shared_lib_prefix $ARCH_CFLAGS $mk_shared_lib_suffix"
