...
SWIFT_DISPATCH_SOURCE_TYPE(EXCLAVES_NOTIFICATION)
#endif
...
/*
 * WARNING: DO NOT MOVE.
 * Dispatch and Swift Concurrency have a contract about the job struct layout.
 * Consult include/swift/ABI/Task.h file from swift repo before modifying this.
 *
 * The scheduler_private field holds two pointers :
 * DispatchLinkageIndex : To be used by dispatch when enqueueing this swift job
 * onto a given queue.
 * DispatchQueueIndex : Should point to the executor queue for this job.
 *
 * The Dispatch object header is one pointer and two ints, followed by
 * DispatchLinkageIndex. This is equivalent to three pointers on 32-bit and
 * two pointers 64-bit. We set the DISPATCH_QUEUE_INDEX accordingly so that
 * DispatchLinkageIndex points to where it is expected.
 * In future, we should really fix this by rdar://111466204 (We should figure
 * out the correct split dispatch and the concurrency runtime for managing job struct)
 */
#define JOB_PRIORITY_MASK     0xFF00
#define JOB_PRIORITY_FIRSTBIT  0x8

#if __LP64__
#define DISPATCH_QUEUE_INDEX 1
#else
#define DISPATCH_QUEUE_INDEX 0
#endif

static struct _swift_minimal_job_header {
	/* Opaque reserves space for HeapObject which is 2 words. */
    void *opaque[2];
	void *scheduler_private[2];
    uint32_t flags;
};

static inline void _swift_job_set_executor_queue(void *job, dispatch_queue_t queue) {
	struct _swift_minimal_job_header *minimum_job = (struct _swift_minimal_job_header *)job;
	/* We do not want to take another ref here so __bridge should be okay. */
	minimum_job->scheduler_private[DISPATCH_QUEUE_INDEX] = (__bridge void*)queue;
}

static inline dispatch_qos_class_t _swift_job_priority(void *job) {
	struct _swift_minimal_job_header *minimum_job = (struct _swift_minimal_job_header *)job;
	return (minimum_job->flags & JOB_PRIORITY_MASK) >> JOB_PRIORITY_FIRSTBIT;
}
...
static inline void _swift_dispatch_async_and_wait(
    dispatch_queue_t queue,
    __swift_shims_dispatch_block_t block) {
...
static inline bool
_swift_dispatch_verify_current_queue_4swiftonly(dispatch_queue_t q)
{
  return dispatch_verify_current_queue_4swiftonly(q);
}
