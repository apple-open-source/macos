...
// It isn't possible to import Combine on 32-bit iOS targets.
// FIXME: The DispatchTimeInterval APIs below don't depend on Combine, and
...
extension DispatchTime /* : Strideable */ {
	...
	public func distance(to other: DispatchTime) -> DispatchTimeInterval {
		...
		if lhs >= rhs, let diff = Int64(exactly: lhs - rhs) {
			return DispatchTimeInterval(_nanoseconds: diff)
		} else if lhs < rhs, let diff = Int64(exactly: rhs - lhs) {
			return DispatchTimeInterval(_nanoseconds: -diff)
		} else {
			return .never
		}
...
extension DispatchQueue: Scheduler {
    ...
    public struct SchedulerTimeType: Strideable, Codable, Hashable, Sendable {
        ...
        public static func <(_ lhs: DispatchQueue.SchedulerTimeType, _ rhs: DispatchQueue.SchedulerTimeType) -> Bool {
        ...
        public struct Stride: SchedulerTimeIntervalConvertible, Comparable, SignedNumeric, ExpressibleByFloatLiteral, Hashable, Codable {
			...
			internal init(_nanoseconds: Int64) {
				self._nanoseconds = _nanoseconds
			}

			internal init(_seconds: Double) {
				if let ns = Int64(exactly: (_seconds * 1_000_000_000).rounded(.towardZero)) {
					self._nanoseconds = ns
				} else {
					self._nanoseconds = _seconds > 0 ? Int64.max : Int64.min
				}
			}
			...
			public var magnitude: Int {
				get {
					// FIXME: On arm64_32 and other 32-bit archs, this overflows after ~2.14 seconds.
					Int(_nanoseconds)
				}
				set {
					// FIXME: This setter only exists for ABI compatibility with the original version of this type in Jazzkon.
					// See rdar://79954211 (DispatchQueue.SchedulerTimeType.Stride.magnitude has a nonsensical public setter)
            ...
            public var timeInterval: DispatchTimeInterval {
                DispatchTimeInterval(_nanoseconds: _nanoseconds)
            }
            ...
            public init(_ timeInterval: DispatchTimeInterval) {
				self.init(_nanoseconds: timeInterval.rawValue)
            }
            ...
            public init(integerLiteral value: Int) {
				self.init(_nanoseconds: clampedInt64Product(Int64(value), 1_000_000_000))
            }
            ...
            public init?<T>(exactly source: T) where T: BinaryInteger {
				...
				let (result, overflow) = v.multipliedReportingOverflow(by: 1_000_000_000)
				guard !overflow else { return nil }
				self.init(_nanoseconds: result)
            }
            ...
            public static func * (lhs: Stride, rhs: Stride) -> Stride {
                let seconds = (Double(lhs._nanoseconds) / 1_000_000_000) * (Double(rhs._nanoseconds) / 1_000_000_000)
				return Stride(_seconds: seconds)
            }

            public static func + (lhs: Stride, rhs: Stride) -> Stride {
				return Stride(_nanoseconds: lhs._nanoseconds + rhs._nanoseconds)
            }
