...
UTextTest::runIndexedTest(int32_t index, UBool exec,
    ...
    TESTCASE_AUTO(AccessChangesChunkSize); // rdar://78720690 
    TESTCASE_AUTO_END;
}
...
// AccessChangesChunkSize rdar://78720690 test utext_current32 using access with changing chunk sizes

static const UChar testAccessText[] = { // text with surrogates at chunk boundaries
...
// modified UTextAccess function for UChar string; a cross between
// UText ucstrTextAccess and CFStringUText doAccess
// 1. assumes native length is known and in ut->a
// 2. assumes that most fields may be 0 or NULL, will fill out if index not in range
...
ustrTextAccessModChunks(UText *ut, int64_t index, UBool forward) {
    ...
    if (ut->chunkContents != NULL && ut->chunkLength != 0) {
...
u32Access(UText *ut, int64_t index, UBool forward) {
    const UChar32 *str = (const UChar32 *)ut->context;
    int64_t length = ut->a;
    const int64_t* offsetMap = (const int64_t*)ut->p;
    const UChar *u16 = (const UChar *)ut->q;
...
static UText *
utext_openUChar32s(UText *ut, const UChar32 *s, int64_t length, const UChar *q, UErrorCode *status) {
    if (U_FAILURE(*status)) {
        return NULL;
    }
    if (s==NULL || length < 0) {
