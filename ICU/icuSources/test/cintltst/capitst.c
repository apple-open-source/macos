...
// rdar://
    UCollator *zhCol = ucol_open("zh", &status);
    if (U_FAILURE(status)) {
        log_err_status(status, "zh collator creation failed.: %s\n", myErrorName(status));
        ucol_close(col);
        return;
    }
...
// rdar://35946337 Rewrite urbtok_tokenize & other urbtok_ interfaces to work with new RBBI but be fast enough
      log_info("ucol_getVersion() en_US result: %02x.%02x.%02x.%02x\n",
                  versionArray[0], versionArray[1], versionArray[2], versionArray[3]);
      ucol_getVersion(zhCol, versionArray);
      log_info("ucol_getVersion() zh result: %02x.%02x.%02x.%02x\n",
                  versionArray[0], versionArray[1], versionArray[2], versionArray[3]);
      ucol_close(zhCol);
#else
...
#if APPLE_ICU_CHANGES
//  rdar://97937093 (ICU 72 integration)
    log_info("ucol_getUCAVersion() en_US result: %02x.%02x.%02x.%02x\n",
        versionUCAArray[0], versionUCAArray[1], versionUCAArray[2], versionUCAArray[3]);
    log_info("u_getUnicodeVersion() result: %02x.%02x.%02x.%02x\n",
        versionUCDArray[0], versionUCDArray[1], versionUCDArray[2], versionUCDArray[3]);
//  rdar:
