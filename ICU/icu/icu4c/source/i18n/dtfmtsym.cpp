...
DateFormatSymbols::initializeData(const Locale& locale, const char *type, UErrorCode& status, UBool useLastResortData)
{
    ...
    // rdar://166595417
    // As of ICU 78 & CLDR 48, `iso8601` does not have era rules.
    // This code catches that case (and any others like it) and
    // falls back to using `gregorian` for the era rules instead.
    // It also uses a separate `eraStatus` to avoid corrupting
    // the main `status` when era rules aren't available, as
    // that was preventing other resources from loading.
    // This is the CLDR ticket for adding the `iso8601` data:
    // CLDR-18562 Missing data in calendarData
    UErrorCode eraStatus = U_ZERO_ERROR;
    LocalPointer<EraRules> eraRules(EraRules::createInstance(type, false, eraStatus));
    if (eraStatus == U_MISSING_RESOURCE_ERROR) {
        type = "gregorian";
        eraStatus = U_ZERO_ERROR;
        eraRules.adoptInstead(EraRules::createInstance(type, false, eraStatus));
    }
    int32_t maxEra = (U_SUCCESS(eraStatus))? eraRules->getMaxEraCode(): 0;
#else
