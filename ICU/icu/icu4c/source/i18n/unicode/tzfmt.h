...
#if APPLE_ICU_CHANGES && U_PLATFORM_IS_DARWIN_BASED // rdar://162810290
    // the compiler adds 7 uninitialized padding bytes here,
    // so let's initialize them
    // (swapping the order of fAbuttingOffsetHoursAndMinutes and
    // fTZDBTimeZoneNames did not help, it just moved the padding around)
    uint8_t padding_bytes2[7] = {};
#endif
