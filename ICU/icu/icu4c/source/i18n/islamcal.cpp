...
{
    setLocation(aLocale.getCountry(), success);
 ...
 * @internal
 */
void IslamicCalendar::setLocation(double latitude, double longitude, UErrorCode &status) {
    static UMutex astroLock;      // pod bay door lock
    umtx_lock(&astroLock);
    if(gIslamicCalendarAstro == nullptr) {
        gIslamicCalendarAstro = new CalendarAstronomer();
    ...
    gIslamicCalendarAstro->setLocation(latitude, longitude);
    umtx_unlock(&astroLock);
}
...
/**
  ...
  * @internal
  */
void IslamicCalendar::setLocation(const char* region, UErrorCode &status) {
    static UMutex astroLock;
    umtx_lock(&astroLock);
    if(gIslamicCalendarAstro == nullptr) {
        gIslamicCalendarAstro = new CalendarAstronomer();
        if (gIslamicCalendarAstro == nullptr) {
    ...
    gIslamicCalendarAstro->setLocation(region);
    umtx_unlock(&astroLock);
}
 ...
 * @internal
 */
double IslamicCalendar::getLocationLatitude(UErrorCode &status){
    static UMutex astroLock;
    umtx_lock(&astroLock);
    if (gIslamicCalendarAstro == nullptr) {
        setLocation(0.0, 0.0, status);
    }
    umtx_unlock(&astroLock);
    return gIslamicCalendarAstro->getLocationLatitude();
}
 ...
 * @internal
 */
double IslamicCalendar::getLocationLongitude(UErrorCode &status){
    static UMutex astroLock;
    umtx_lock(&astroLock);
    if (gIslamicCalendarAstro == nullptr) {
        setLocation(0.0, 0.0, status);
    ...
    umtx_unlock(&astroLock);
    return gIslamicCalendarAstro->getLocationLongitude();
}
