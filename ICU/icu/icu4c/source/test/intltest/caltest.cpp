...
void CalendarTest::TestIslamicLocation() {
    ...
    if (fabs(iCal->getLocationLatitude(status) - saLatitude) > DBL_EPSILON) {
        errln("Wrong default Latitude for Islamic Calendar for Saudi Arabia; expected: %f, get: %f", saLatitude, iCal->getLocationLatitude(status));
    }
    
    if (fabs(iCal->getLocationLongitude(status) - saLongitude) > DBL_EPSILON) {
        errln("Wrong default Longitude for Islamic Calendar for Saudi Arabia; expected: %f, get: %f", saLongitude, iCal->getLocationLongitude(status));
    }
    
    iCal->setLocation(48.3, 21.4, status);
    
    if (fabs(iCal->getLocationLatitude(status) - 48.3) > DBL_EPSILON) {
        errln("Wrong Latitude for Islamic Calendar set with setLocation()");
    }
    
    if (fabs(iCal->getLocationLongitude(status) - 21.4) > DBL_EPSILON) {
        errln("Wrong Longitude for Islamic Calendar set with setLocation()");
    }
 
    // { "CK",    -21.2367,    -159.7776 },
    iCal->setLocation("CK", status);
    double ckLatitude = -21.2367;
    double ckLongitude = -159.7776;
    
    if (fabs(iCal->getLocationLatitude(status) - ckLatitude) > DBL_EPSILON) {
        errln("Wrong Latitude for Islamic Calendar set with setLocation() for CK; expected: %f, get: %f", ckLatitude, iCal->getLocationLatitude(status));
    }
    
    if (fabs(iCal->getLocationLongitude(status) -  ckLongitude) > DBL_EPSILON) {
        errln("Wrong Longitude for Islamic Calendar set with setLocation() for CK; expected: %f, get: %f", saLongitude, iCal->getLocationLongitude(status));
    }
    
    // Set location 0.0, 0.0 for non-existing country code
    iCal->setLocation("XX", status);
    
    if (fabs(iCal->getLocationLatitude(status)) > DBL_EPSILON) {
        errln("Wrong Latitude for Islamic Calendar set with setLocation() for CK; expected: %f, get: %f", ckLatitude, iCal->getLocationLatitude(status));
    }
    
    if (fabs(iCal->getLocationLongitude(status)) > DBL_EPSILON) {
        errln("Wrong Longitude for Islamic Calendar set with setLocation() for CK; expected: %f, get: %f", saLongitude, iCal->getLocationLongitude(status));
    }
    
    // Set location 0.0, 0.0 for null
    iCal->setLocation(nullptr, status);
