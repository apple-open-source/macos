...
#if APPLE_ICU_CHANGES
void RegexTest::TestForHang() {
    // rdar://131705700
    // https://unicode-org.atlassian.net/browse/ICU-23047

    // If the open source community comes up with a more elegant
    // solution for ICU-23047, we can replace this with their test.

    UErrorCode status = U_ZERO_ERROR;

    RegexMatcher *matcher = new RegexMatcher(";($)*?a", 0, status);
    assertSuccess(WHERE, status);

    UnicodeString stringToTest = ";\n";
    matcher->reset(stringToTest);

    // if the bug is present, find() will hang
    if (matcher->find()) {
