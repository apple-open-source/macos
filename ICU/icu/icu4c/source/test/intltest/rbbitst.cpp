...
void RBBITest::runIndexedTest( int32_t index, UBool exec, const char* &name, char* params )
{
    ...
    TESTCASE_AUTO(TestThaiBreaks);  // rdar://148084444, rdar://143076921, rdar://143963407, rdar://104468458
...
#if APPLE_ICU_CHANGES
// rdar://
    partition.emplace_back("EB_EastAsian", UnicodeSet(uR"([[[\p{Line_Break=E_Base}][\U0001F46A-\U0001F46D\U0001F46F\U0001F91D\U0001F93C]]&[\p{ea=F}\p{ea=W}\p{ea=H}]])", status));
    partition.emplace_back("EBmEastAsian", UnicodeSet(uR"([[[\p{Line_Break=E_Base}][\U0001F46A-\U0001F46D\U0001F46F\U0001F91D\U0001F93C]]-[\p{ea=F}\p{ea=W}\p{ea=H}]])", status));
#else
...
#if APPLE_ICU_CHANGES
// rdar://
    rules.push_back(std::make_unique<RegexRule>(uR"($PR × ($ID | $EB | $EM))", uR"(\p{Line_Break=Prefix_Numeric})", u'×', uR"((\p{Line_Break=Ideographic} | [[\p{Line_Break=E_Base}][\U0001F46A-\U0001F46D\U0001F46F\U0001F91D\U0001F93C]] | \p{Line_Break=E_Modifier}))"));
    rules.push_back(std::make_unique<RegexRule>(uR"(($ID | $EB | $EM) × $PO)", uR"((\p{Line_Break=Ideographic} | [[\p{Line_Break=E_Base}][\U0001F46A-\U0001F46D\U0001F46F\U0001F91D\U0001F93C]] | \p{Line_Break=E_Modifier}))", u'×', uR"(\p{Line_Break=Postfix_Numeric})"));
#else
...
#if APPLE_ICU_CHANGES
// rdar://
    rules.push_back(std::make_unique<RegexRule>(uR"($EB × $EM)", uR"([[\p{Line_Break=E_Base}][\U0001F46A-\U0001F46D\U0001F46F\U0001F91D\U0001F93C]])", u'×', uR"(\p{Line_Break=E_Modifier})"));
#else
...
// Apple raises the numeric limit to UINT32_MAX-- we have existing tests that use this
// and it works fine.  Also, when Frank added this test, he got the arguments to assertEquals
// in the wrong order
    assertEquals(WHERE, U_BRK_RULE_SYNTAX, ec);
    ec = U_ZERO_ERROR;
    RuleBasedBreakIterator bi2(u"A{4294967296};", pe, ec);
    assertEquals(WHERE, U_BRK_RULE_SYNTAX, ec);
    ec = U_ZERO_ERROR;
    RuleBasedBreakIterator bi3(u"A{4294967295};", pe, ec);
    assertEquals(WHERE, U_ZERO_ERROR, ec);
#else
...
// rdar://148084444, rdar://143076921, rdar://143963407, rdar://104468458

void RBBITest::TestThaiBreaks() {
    UErrorCode status = U_ZERO_ERROR;

    // rdar://148084444 - Thai word "รายการ" breaking incorrectly
    // The problematic Thai text from the radar:
    // "คลิกภาพพื้นหลังเพื่อเลื่อนหน้าต่างออกเพื่อแสดงรายการเดสก์ท็อปและวิดเจ็ตของคุณ"
    // (Click wallpaper to move windows out of the way, revealing your desktop items and widgets.)
    // The word "รายการ" (items) should NOT be broken.
    // The correct break point is between "รายการ" and "เดสก์ท็อป"
    UnicodeString text = u"คลิกภาพพื้นหลังเพื่อเลื่อนหน้าต่างออกเพื่อแสดงรายการเดสก์ท็อปและวิดเจ็ตของคุณ";

    // Expected segments based on Thai QA feedback and ICU dictionary
    // คลิก | ภาพ | พื้นหลัง | เพื่อ | เลื่อน | หน้าต่าง | ออก | เพื่อ | แสดง | รายการ | เดสก์ท็อป | และ | วิดเจ็ต | ของคุณ
    const char16_t* expectedSegments[] = {
        u"คลิก",
        u"ภาพ",
        u"พื้นหลัง",    // Added to dictionary as compound word
        u"เพื่อ",
        u"เลื่อน",
        u"หน้าต่าง",
        u"ออก",
        u"เพื่อ",
        u"แสดง",
        u"รายการ",     // Should NOT be broken as "รา|ยการ"
        u"เดสก์ท็อป",    // Added to dictionary (with tone mark)
        u"และ",
        u"วิดเจ็ต",      // Added to dictionary
        u"ของคุณ"        // Added to dictionary as compound word
    };
    const int32_t expectedCount = UPRV_LENGTHOF(expectedSegments);

    // Create a line break iterator for Thai
    Locale thaiLocale("th_TH");
    LocalPointer<BreakIterator> lineBreak(BreakIterator::createLineInstance(thaiLocale, status));
    if (!assertSuccess(WHERE, status)) {
        return;
    }
    ...
    // Collect actual segments
    UVector32 actualPositions(status);
    int32_t pos = lineBreak->first();
    while ((pos = lineBreak->next()) != BreakIterator::DONE) {
        actualPositions.addElement(pos, status);
    }

    // Extract actual segments as strings
    std::vector<UnicodeString> actualSegments;
    int32_t start = 0;
    for (int32_t i = 0; i < actualPositions.size(); i++) {
        int32_t end = actualPositions.elementAti(i);
        UnicodeString segment;
        text.extractBetween(start, end, segment);
        actualSegments.push_back(segment);
        start = end;
    }

    // Log all segments for debugging
    logln("Expected segments:");
    for (int32_t i = 0; i < expectedCount; i++) {
        logln(UnicodeString("  [") + i + "]: '" + UnicodeString(expectedSegments[i]) + "'");
    }

    logln("\nActual segments:");
    for (size_t i = 0; i < actualSegments.size(); i++) {
        logln(UnicodeString("  [") + (int32_t)i + "]: '" + actualSegments[i] + "'");
    }

    // Compare segment count
    if ((int32_t)actualSegments.size() != expectedCount) {
        errln(UnicodeString(WHERE " - Expected ") + expectedCount + " segments, but got " + (int32_t)actualSegments.size());
    }

    // Compare each segment
    int32_t minCount = (int32_t)actualSegments.size() < expectedCount ? (int32_t)actualSegments.size() : expectedCount;
    for (int32_t i = 0; i < minCount; i++) {
        UnicodeString expectedStr(expectedSegments[i]);
        if (actualSegments[i] != expectedStr) {
            errln(UnicodeString(WHERE " - Segment ") + i + " mismatch:");
            errln(UnicodeString("    Expected: '") + expectedStr + "'");
            errln(UnicodeString("    Actual:   '") + actualSegments[i] + "'");
        }
    }

    // Specifically verify that "รายการ" appears as a complete segment (not broken)
    bool foundRayakan = false;
    UnicodeString rayakan = u"รายการ";
    for (size_t i = 0; i < actualSegments.size(); i++) {
        if (actualSegments[i] == rayakan) {
            foundRayakan = true;
            logln("✓ PASS: 'รายการ' appears as a complete segment");
            break;
        }
    }

    if (!foundRayakan) {
        // Check if it's broken incorrectly
        bool foundBrokenRayakan = false;
        for (size_t i = 0; i < actualSegments.size(); i++) {
            if (actualSegments[i] == u"รา" && i+1 < actualSegments.size() && actualSegments[i+1] == u"ยการ") {
                errln(WHERE " - 'รายการ' is incorrectly broken as 'รา' + 'ยการ'");
                foundBrokenRayakan = true;
                break;
            }
        }
        if (!foundBrokenRayakan) {
            errln(WHERE " - 'รายการ' not found as expected segment");
        }
    }

    // rdar://143076921 - Thai word "คาร์ดิโอ" (Cardio) breaking incorrectly
    {
        // The problematic Thai text: "การแจ้งเตือนคาร์ดิโอฟิตเนส"
        // (Cardio Fitness Notifications)
        // The word "คาร์ดิโอ" (Cardio) should NOT be broken as "คาร์ดิ-โอ"
        UnicodeString text2 = u"การแจ้งเตือนคาร์ดิโอฟิตเนส";

        // Expected segments: การ | แจ้ง | เตือน | คาร์ดิโอ | ฟิตเนส
        const char16_t* expectedSegments2[] = {
            u"การ",
            u"แจ้ง",
            u"เตือน",
            u"คาร์ดิโอ",    // CRITICAL: should NOT be broken as "คาร์ดิ|โอ"
            u"ฟิตเนส"
        };
        const int32_t expectedCount2 = UPRV_LENGTHOF(expectedSegments2);

        lineBreak->setText(text2);

        // Collect actual segments
        UVector32 actualPositions2(status);
        int32_t pos2 = lineBreak->first();
        while ((pos2 = lineBreak->next()) != BreakIterator::DONE) {
            actualPositions2.addElement(pos2, status);
        }

        // Extract actual segments as strings
        std::vector<UnicodeString> actualSegments2;
        int32_t start2 = 0;
        for (int32_t i = 0; i < actualPositions2.size(); i++) {
            int32_t end2 = actualPositions2.elementAti(i);
            UnicodeString segment;
            text2.extractBetween(start2, end2, segment);
            actualSegments2.push_back(segment);
            start2 = end2;
        }

        // Log segments for debugging
        logln("\n[rdar://143076921] Expected segments:");
        for (int32_t i = 0; i < expectedCount2; i++) {
            logln(UnicodeString("  [") + i + "]: '" + UnicodeString(expectedSegments2[i]) + "'");
        }

        logln("\n[rdar://143076921] Actual segments:");
        for (size_t i = 0; i < actualSegments2.size(); i++) {
            logln(UnicodeString("  [") + (int32_t)i + "]: '" + actualSegments2[i] + "'");
        }

        // Compare segment count
        if ((int32_t)actualSegments2.size() != expectedCount2) {
            errln(UnicodeString(WHERE " [rdar://143076921] - Expected ") + expectedCount2 +
                  " segments, but got " + (int32_t)actualSegments2.size());
        }

        // Compare each segment
        int32_t minCount2 = (int32_t)actualSegments2.size() < expectedCount2 ?
                            (int32_t)actualSegments2.size() : expectedCount2;
        for (int32_t i = 0; i < minCount2; i++) {
            UnicodeString expectedStr(expectedSegments2[i]);
            if (actualSegments2[i] != expectedStr) {
                errln(UnicodeString(WHERE " [rdar://143076921] - Segment ") + i + " mismatch:");
                errln(UnicodeString("    Expected: '") + expectedStr + "'");
                errln(UnicodeString("    Actual:   '") + actualSegments2[i] + "'");
            }
        }

        // Specifically verify that "คาร์ดิโอ" appears as a complete segment
        bool foundCardio = false;
        UnicodeString cardio = u"คาร์ดิโอ";
        for (size_t i = 0; i < actualSegments2.size(); i++) {
            if (actualSegments2[i] == cardio) {
                foundCardio = true;
                logln("✓ PASS: 'คาร์ดิโอ' (Cardio) appears as a complete segment");
                break;
            }
        }
        ...
        if (!foundCardio) {
            // Check if it's broken incorrectly
            bool foundBrokenCardio = false;
            for (size_t i = 0; i < actualSegments2.size(); i++) {
                if (actualSegments2[i] == u"คาร์ดิ" && i+1 < actualSegments2.size() &&
                    actualSegments2[i+1] == u"โอ") {
                    errln(WHERE " [rdar://143076921] - 'คาร์ดิโอ' is incorrectly broken as 'คาร์ดิ' + 'โอ'");
                    foundBrokenCardio = true;
                    break;
                }
            }
            if (!foundBrokenCardio) {
                errln(WHERE " [rdar://143076921] - 'คาร์ดิโอ' not found as expected segment");
            }
        }
    }

    // rdar://143963407 - Thai word "เพลย์ลิสต์" (playlist) breaking incorrectly
    {
        UnicodeString text3 = u"เพลย์ลิสต์รออยู่";

        // Current: เพล|ย์|ลิ|สต์
        // Expected: เพลย์|ลิสต์ (syllable breaks acceptable)
        const char16_t* expectedSegments3[] = {
            u"เพลย์",    // play
            u"ลิสต์",     // list
            u"รออยู่"     // is waiting
        };
        int32_t expectedCount3 = UPRV_LENGTHOF(expectedSegments3);

        logln("[rdar://143963407] Testing text: %s", CStr(text3)());

        lineBreak->setText(text3);

        // Collect all segments
        std::vector<UnicodeString> actualSegments3;
        int32_t start3 = 0;
        int32_t end3;
        while ((end3 = lineBreak->next()) != BreakIterator::DONE) {
            UnicodeString segment3;
            text3.extractBetween(start3, end3, segment3);
            actualSegments3.push_back(segment3);
            logln("[rdar://143963407]   Segment %d: '%s'",
                  (int)actualSegments3.size(), CStr(segment3)());
            start3 = end3;
        }

        // Compare against expected segments
        if ((int32_t)actualSegments3.size() != expectedCount3) {
            errln(WHERE " [rdar://143963407] - Segment count mismatch. Expected: %d, Got: %d",
                  expectedCount3, (int)actualSegments3.size());

            logln("[rdar://143963407] Expected segments:");
            for (int32_t i = 0; i < expectedCount3; i++) {
                logln("  [%d] '%s'", i, CStr(UnicodeString(expectedSegments3[i]))());
            }

            logln("[rdar://143963407] Actual segments:");
            for (size_t i = 0; i < actualSegments3.size(); i++) {
                logln("  [%d] '%s'", (int)i, CStr(actualSegments3[i])());
            }
        } else {
            // Check each segment matches
            bool allMatch3 = true;
            for (int32_t i = 0; i < expectedCount3; i++) {
                if (actualSegments3[i] != UnicodeString(expectedSegments3[i])) {
                    errln(WHERE " [rdar://143963407] - Segment %d mismatch. Expected: '%s', Got: '%s'",
                          i, CStr(UnicodeString(expectedSegments3[i]))(), CStr(actualSegments3[i])());
                    allMatch3 = false;
                }
            }

            if (allMatch3) {
                logln("[rdar://143963407] All segments match expected values");
            }

            // Check if playlist is broken incorrectly (as เพล|ย์|ลิ|สต์)
            bool foundBrokenPlaylist = false;
            for (size_t i = 0; i < actualSegments3.size(); i++) {
                if (actualSegments3[i] == u"เพล" || actualSegments3[i] == u"ย์" ||
                    actualSegments3[i] == u"ลิ" || actualSegments3[i] == u"สต์") {
                    errln(WHERE " [rdar://143963407] - 'เพลย์ลิสต์' is incorrectly broken into tiny fragments");
                    foundBrokenPlaylist = true;
                    break;
                }
            }
        }
    ...
    // rdar://104468458 - Thai word "บริการ" (services) breaking incorrectly
    {
        UnicodeString text4 = u"กดค้างไว้เพื่อโทรหาบริการฉุกเฉิน";

        // Current: โทรหาบริ|การฉุกเฉิน (breaks within "บริการ")
        // Expected: โทรหา|บริการ|ฉุกเฉิน (breaks at word boundaries)
        const char16_t* expectedSegments4[] = {
            u"กด",      // press
            u"ค้าง",     // hold
            u"ไว้",      // keep
            u"เพื่อ",     // to
            u"โทรหา",   // call
            u"บริการ",   // services: should NOT be broken
            u"ฉุกเฉิน"    // emergency
        };
        int32_t expectedCount4 = UPRV_LENGTHOF(expectedSegments4);

        logln("[rdar://104468458] Testing text: %s", CStr(text4)());

        lineBreak->setText(text4);

        // Collect all segments
        std::vector<UnicodeString> actualSegments4;
        int32_t start4 = 0;
        int32_t end4;
        while ((end4 = lineBreak->next()) != BreakIterator::DONE) {
            UnicodeString segment4;
            text4.extractBetween(start4, end4, segment4);
            actualSegments4.push_back(segment4);
            logln("[rdar://104468458]   Segment %d: '%s'",
                  (int)actualSegments4.size(), CStr(segment4)());
            start4 = end4;
        }

        // Compare against expected segments
        if ((int32_t)actualSegments4.size() != expectedCount4) {
            errln(WHERE " [rdar://104468458] - Segment count mismatch. Expected: %d, Got: %d",
                  expectedCount4, (int)actualSegments4.size());

            logln("[rdar://104468458] Expected segments:");
            for (int32_t i = 0; i < expectedCount4; i++) {
                logln("  [%d] '%s'", i, CStr(UnicodeString(expectedSegments4[i]))());
            }

            logln("[rdar://104468458] Actual segments:");
            for (size_t i = 0; i < actualSegments4.size(); i++) {
                logln("  [%d] '%s'", (int)i, CStr(actualSegments4[i])());
            }
        } else {
            // Check each segment matches
            bool allMatch4 = true;
            for (int32_t i = 0; i < expectedCount4; i++) {
                if (actualSegments4[i] != UnicodeString(expectedSegments4[i])) {
                    errln(WHERE " [rdar://104468458] - Segment %d mismatch. Expected: '%s', Got: '%s'",
                          i, CStr(UnicodeString(expectedSegments4[i]))(), CStr(actualSegments4[i])());
                    allMatch4 = false;
                }
            }

            if (allMatch4) {
                logln("[rdar://104468458] All segments match expected values");
            }

            // Check if "บริการ" is broken incorrectly
            bool foundBrokenService = false;
            for (size_t i = 0; i < actualSegments4.size(); i++) {
                if (actualSegments4[i] == u"บริ" && i+1 < actualSegments4.size() &&
                    actualSegments4[i+1] == u"การ") {
                    errln(WHERE " [rdar://104468458] - 'บริการ' is incorrectly broken as 'บริ' + 'การ'");
                    foundBrokenService = true;
                    break;
                }
            }
        }
