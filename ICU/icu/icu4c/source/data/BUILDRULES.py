...
def generate(config, io, common_vars):
    ...
    requests += generate_tree(config, io, common_vars,
        ...
        [DepTarget("cnvalias")])  # rdar://155639864
    ...
    requests += generate_tree(config, io, common_vars,
        ...
        [DepTarget("coll_ucadata"), DepTarget("misc_res"), InFile("unidata/UCARules.txt"), DepTarget("cnvalias")])  # rdar://155639864
    ...
    requests += generate_tree(config, io, common_vars,
        ...
        # Never use pool bundle for coll, brkitr, or rbnf
        False,
        [DepTarget("brkitr_brk"), DepTarget("dictionaries"), DepTarget("cnvalias")])  # rdar://155639864
...
def generate_curr_supplemental(config, io, common_vars):
    ...
    return [
        SingleExecutionRequest(
            ...
            dep_targets = [DepTarget("cnvalias")],  # rdar://155639864
            input_files = [input_file],
            output_files = [output_file],
            tool = IcuTool("genrb"),
...
def generate_translit(config, io, common_vars):
    ...
    dep_files.add(DepTarget("cnvalias"))  # rdar://155639864
