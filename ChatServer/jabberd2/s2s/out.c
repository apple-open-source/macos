...
static void _out_dialback(conn_t out, char *rkey) {
    ...
    *c = '\0';
    ...
    /* request auth */
    ns = nad_add_namespace(nad, "jabber:server:dialback", "db");
...
void out_packet(s2s_t s2s, pkt_t pkt) {
    ...
    if (s2s->enable_whitelist > 0 && (_s2s_domain_in_whitelist(s2s, pkt->to->domain) == 0)) {
        ...
        case event_PACKET:
            ...
            if ((out->s->res_version!=NULL) 
				...
				/* If we're not establishing a starttls connection, send dialbacks 
					unless they are diabled for non-TLS connections */
				if (!starttls) {
            ...
            if(NAD_ENS(nad, 0) < 0 || NAD_NURI_L(nad, NAD_ENS(nad, 0)) != 22 || strncmp("jabber:server:dialback", NAD_NURI(nad, NAD_ENS(nad, 0)), 22) != 0) {
