...
static void _authreg_auth_get(c2s_t c2s, sess_t sess, nad_t nad) {
    int ns, elem, ssequence, attr, err;
    ...
    if(!(c2s->ar_mechanisms & (AR_MECH_TRAD_PLAIN | AR_MECH_TRAD_DIGEST | AR_MECH_TRAD_ZEROK | AR_MECH_TRAD_CRAMMD5))) {
	...
	/* Apple: generate CRAM-MD5 challenge */
    if(c2s->ar_mechanisms & AR_MECH_TRAD_CRAMMD5 && c2s->ar->create_challenge != NULL) {
...
static void _authreg_auth_set(c2s_t c2s, sess_t sess, nad_t nad) {
    ...
    if(!authd && c2s->ar_mechanisms & AR_MECH_TRAD_CRAMMD5 && c2s->ar->check_response != NULL)
    {
        elem = nad_find_elem(nad, 1, ns, "crammd5", 1);
        if(elem >= 0)
        {
            snprintf(str, 1024, "%.*s", NAD_CDATA_L(nad, elem), NAD_CDATA(nad, elem));
            if((c2s->ar->check_response)(c2s->ar, username, sess->realm, sess->auth_challenge, str) == 0)
            {
