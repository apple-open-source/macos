            ...
            if(!sess->active && sess->sasl_authd && sess->result == NULL && strcmp(root, "iq") == 0 && nad_find_attr(nad, 0, -1, "type", "set") >= 0) {
                ...
                if(!sess->bound && (ns = nad_find_scoped_namespace(nad, uri_BIND, NULL)) >= 0 && (elem = nad_find_elem(nad, 0, ns, "bind", 1)) >= 0) {
                    ...
                    if(elem >= 0) {
                        ...
                        snprintf(sess->jid->resource, sizeof(sess->jid->resource), "%.*s", NAD_CDATA_L(nad, elem), NAD_CDATA(nad, elem));
                        /* prep the resource */
                        if(jid_prep(sess->jid) != 0) {
                            jid_free(sess->jid);
                            sess->jid = NULL;
                    ...
                    /* generated resource */
                        ...
                        for(i = 0; i < 256; i++) {
                            ...
                            sess->jid->resource[i] = (r >= 0 && r <= 0) ? (r + 48) : (r + 87);
                        }

                        shahash_r(sess->jid->resource, sess->jid->resource);
                ...
                sx_error(s, stream_err_NOT_AUTHORIZED, NULL);
                sx_close(s);
...
static int _c2s_client_mio_callback(mio_t m, mio_action_t a, int fd, void *data, void *arg) {
    ...
    switch(a) {
        ...
        case action_ACCEPT:
            ...
            sess->s->max_stanza_bytes = c2s->max_stanza_bytes;
            sess->s->max_message_bytes = c2s->max_message_bytes;
