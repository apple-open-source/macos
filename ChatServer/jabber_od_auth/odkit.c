...
static int ODKCreateFixedADServerChallenge(ODKSession *session, CFDataRef src, CFDataRef *dst);
static int ODKConfigureADServerChallenge(ODKSession *session);
...
//TODO: CLEANUP use_my_od_record_verify
static int use_my_od_record_verify = 1;
...
ODKConfigureADInfo(ODKSession *session)
{
    ...
    if (adNodeOnSearchPolicy) {
        gADInfo.adNode = ODNodeCreateWithName(kCFAllocatorDefault, kODSessionDefault, CFSTR("/Active Directory/All Domains"), &cfError);
        if (gADInfo.adNode == 0) {
...
int
ODKCreateFixedADServerChallenge(ODKSession *session, CFDataRef src, CFDataRef *dst)
{
    ODKSessionPriv *_session = ODK_SESSION_PRIV(session);
    const char qopString[] = "qop=\"auth\"";
    int retval = -1;
    char *in = 0;
    CFMutableDataRef out = NULL;
    ...
    *dst = NULL;
    ...
    if (sizeof(_session->buffer)-1 <= CFDataGetLength(src)) {
        ODK_LOG(LOG_ERR, "Illegal encoding");
        goto done;
    }

    memcpy(_session->buffer, CFDataGetBytePtr(src), CFDataGetLength(src));
    _session->buffer[CFDataGetLength(src)] = '\0';

    out = CFDataCreateMutable(kCFAllocatorDefault, 0);
    if (out == NULL) {
        ODK_LOG(LOG_ERR, "Failed to allocate data object");
        goto done;
    }

    CFDataAppendBytes(out, (UInt8*)qopString, (CFIndex)strlen(qopString));
    ...
    while (in[0] != '\0') {
        ...
        if (strcasecmp(name, "qop") != 0
         && strcasecmp(name, "cipher") != 0) {
            value_was_quoted = (value[-1] == '"');

            CFDataAppendBytes(out, (UInt8*)",", (CFIndex)1);
            CFDataAppendBytes(out, (UInt8*)name, (CFIndex)strlen(name));
            if (value_was_quoted)
                CFDataAppendBytes(out, (UInt8*)"=\"", (CFIndex)2);
            else
                CFDataAppendBytes(out, (UInt8*)"=", (CFIndex)1);
            CFDataAppendBytes(out, (UInt8*)value, (CFIndex)strlen(value));
            if (value_was_quoted)
                CFDataAppendBytes(out, (UInt8*)"\"", (CFIndex)1);
        }
    }

    *dst = CFDataCreate(kCFAllocatorDefault,
                        CFDataGetMutableBytePtr(out), CFDataGetLength(out));
    if (*dst == NULL) {
        ODK_LOG(LOG_ERR, "Out of memory");
        goto done;
    }
...
int
ODKConfigureADServerChallenge(ODKSession *session)
{
    ODKSessionPriv *_session = ODK_SESSION_PRIV(session);
    int retval = -1;
    int success;
    ...
    if (_session->serverChallenge != NULL) {
        ODK_LOG(LOG_DEBUG, "Server challenge already configured");
        goto done;
    }

    if (ODKHasUserRecord(session)) {
        assert(_session->userName != NULL);
        userName = _session->userName;
    }
    else if (ODKUseActiveDirectory(session)) {
        assert(_session->adNode != NULL);
        userName = CFSTR("anonymous");
    }
    else {
        ODK_LOG(LOG_DEBUG, "Missing username and anonymous capability");
        goto done;
    }
    ...
    CFArrayAppendValue(cfArrayBuf, userName);
    CFArrayAppendValue(cfArrayBuf, _session->authMethod);
    CFArrayAppendValue(cfArrayBuf, CFSTR(""));

    if (ODKHasUserRecord(session)) {
        success = ODRecordVerifyPasswordExtended(_session->userRecord,
                              CFSTR(kDSStdAuthSASLProxy),
                              cfArrayBuf, &cfOutItems, NULL, &cfOutError);
    }
    else {
        success = ODNodeSetCredentialsExtended(_session->adNode,
    ...
    if (!success) {
        if (cfOutError)
            ODK_LOG_CFERROR(LOG_INFO, "Unable to obtain server challenge", cfOutError);
        else
            ODK_LOG(LOG_INFO, "Unable to obtain server challenge");
        goto done;
    }

    CFDataRef cfServerResp = CFArrayGetValueAtIndex(cfOutItems, 0);
    if (cfServerResp == 0) {
        ODK_LOG(LOG_ERR, "Missing server challenge");
        goto done;
    }

    if (ODKCreateFixedADServerChallenge(session, cfServerResp, &_session->serverChallenge) != 0) {
        ODK_LOG(LOG_ERR, "Unable to fix server challenge");
        /* well, just keep what we got and hope for the best */
...
ODKConfigureServerChallenge(ODKSession *session)
{
    ...
    if (ODKHasUserRecord(session) && ODKIsActiveDirectoryRecord(_session->userRecord)) {
        if (ODKConfigureADServerChallenge(session))
            goto done;
    }
    else if ((!ODKHasUserRecord(session)) && ODKUseActiveDirectory(session)) {
        if (ODKConfigureADServerChallenge(session))
            goto done;
    }
    else {
        /* this is where to use a non-AD method to configure the challenge,
         * but the OD API doesn't currently support that */
        goto done;
    }
...
ODKVerifyClientRequestFixed(ODKSession *session, CFStringRef clientRequest)
{
    ...
    if (ODKIsActiveDirectoryRecord(_session->userRecord)) {
        ...
        if (wasAnonymous) {
            success = ODNodeSetCredentialsExtended(_session->adNode,
                                       CFSTR(kDSStdRecordTypeUsers),
                                       CFSTR(kDSStdAuthSASLProxy), cfArrayBuf,
...
// TODO: CLEANUP
#if 1
if (use_my_od_record_verify) {
        success = ODKRecordVerifyPasswordExtended(session, _session->userRecord,
                                       CFSTR(kDSStdAuthDIGEST_MD5), cfArrayBuf,
                                       &cfOutItems, NULL, &cfOutError);
}
else
#endif
