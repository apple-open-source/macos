...
static int _digest_md5_client_step(scod_mech_t mech, scod_t sd, const char *chal, int challen, char **resp, int *resplen) {
    xht attrs, sub;
    char *key, *realm, *nonce, *qop, *charset, *algorithm, *cnonce, *c, *digest_uri;
    md5_state_t md5;
    md5_byte_t hash[16];
    ...
    if(nonce == NULL || qop == NULL || charset == NULL || algorithm == NULL || digest_uri == NULL) {
    ...
    c = _opt_quote(digest_uri);
    spooler(s, "digest-uri=", c, ",", s);
    free(c);

    spooler(s, "nc=00000001,qop=auth,charset=utf-8,algorithm=md5-sess,response=", hrsp, s);
...
//nonce = strdup("WUz/I4ziWN4uB8lwmafiOVGk2h5pvpyloG0PU/37g08=");//XXX
