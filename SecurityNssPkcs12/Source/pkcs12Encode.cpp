	...
	SecNssCoder				&localCdr)
{
	...
	CSSM_DATA &pwd = getEncrPassPhrase();
	...
	SecNssCoder &localCdr)
{
	...
	bool found = pkcsOidToParams(&mStrongEncrAlg,
		...
		padding, mode);
	if(!found) {
		/* app cojnfig error - they gave us bogus algorithm */
		p12ErrorLog("keyBagBuild encrAlg not understood\n");
		CssmError::throwMe(CSSMERR_CSP_INVALID_ALGORITHM);
	}
	CSSM_DATA &encrPhrase = getEncrPassPhrase();
