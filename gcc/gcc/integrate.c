     ...
     rtx loc, copy;
{
  if ((GET_CODE (copy) != REG && GET_CODE (copy) != SUBREG)
      ...
      if (CONSTANT_P (copy) || FIXED_BASE_PLUS_P (copy))
	{
	  ...
	  /*  When generating PIC, don't insert unPIC'ified
	      symbol references into const_equiv_map.  Note
	      that the code below is very similar to that of
	      machopic_indirect_data_reference (), except this
	      function doesn't PIC'ify symbol references if the
	      target processor is i386.  */
	  if (flag_pic && GET_CODE(copy) == SYMBOL_REF)
	      ...
	      if (machopic_name_defined_p (name))
		{
	      ...
	      else	/* NAME was not defined (it's external.)  */
		{
		  /* COPY is the address of a global variable which
		     is external to this file.  We therefore need to
		     explicitly use the var's non-lazy-ptr.  */
...
#ifdef NEXT_SEMANTICS
	/* Need to update REG_EH_RETHROW region number to avoid
	   problems in flow.  */
	if (REG_NOTE_KIND (tem) == REG_EH_RETHROW)
	  {
		...
		/*fprintf (stderr, "#(%s) remapped rethrow rgn %d to %d\n", 
				current_function_name,
				rgn_index, CODE_LABEL_NUMBER (lab));*/
	      }
	  }
#endif	/* NEXT_SEMANTICS  */
