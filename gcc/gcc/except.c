...
/* When inlining/unrolling, we have to map the symbols passed to
   __rethrow as well. This performs the remap. If a symbol isn't found,
...
#ifndef EMIT_EXCEPTION_TABLE_ADDR_ENTRY
#define EMIT_EXCEPTION_TABLE_ADDR_ENTRY(FILE, LABEL_STRING)		\
    do {								\
	rtx sym = gen_rtx_SYMBOL_REF (Pmode, LABEL_STRING);		\
	assemble_integer (sym, POINTER_SIZE / BITS_PER_UNIT, 1);	\
    } while (0)

#endif 

#ifndef EMIT_EXCEPTION_TABLE_TYPE_ENTRY
#define EMIT_EXCEPTION_TABLE_TYPE_ENTRY(TREE, SIZE)			\
				output_constant(TREE, SIZE) 
#endif
  ...
  for ( ; handler != NULL || rethrow != NULL_RTX; handler = handler->next)
    {
      ...
      EMIT_EXCEPTION_TABLE_ADDR_ENTRY(file, buf) ;
      ...
      if (flag_new_exceptions)
        {
          if (handler == NULL || handler->type_info == NULL)
            ...
            if (handler->type_info == CATCH_ALL_TYPE)
              ...
              EMIT_EXCEPTION_TABLE_TYPE_ENTRY ((tree)(handler->type_info), 
