...
;; computations.
(define_attr "type" "integer,load,store,fpload,fpstore,imul,idiv,branch,compare,delayed_compare,fpcompare,mtjmpr,fp,dmul,sdiv,ddiv,ssqrt,dsqrt,jmpreg,vec_simple,vec_complex,vec_fp,vec_perm,vec_easy,vec_load"
...
; AltiVec Units
(define_function_unit "vec_alu1" 1 1
  (eq_attr "type" "vec_simple,vec_fp,vec_complex")
  2 1)

(define_function_unit "vec_alu" 1 0
  (eq_attr "type" "vec_easy")
  2 1)

(define_function_unit "vec_alu" 1 0
  (eq_attr "type" "vec_complex")
  5 1)

(define_function_unit "vec_fp" 1 0
  (eq_attr "type" "vec_fp")
  8 1)

(define_function_unit "vec_perm" 1 1
  (eq_attr "type" "vec_perm")
  2 1)

(define_function_unit "vec_load" 1 0
  (eq_attr "type" "vec_load")
  4 1)
...
(define_insn "addsi3_high"
  ...
  "DEFAULT_ABI == ABI_MACOSX && !TARGET_64BIT"
...
(define_insn "*rlwimi_internal1"
  [(set (match_operand:SI 0 "gpc_reg_operand" "=r")
        (ior:SI (and:SI (match_operand:SI 1 "gpc_reg_operand" "r")
			(match_operand:SI 2 "const_int_operand" "n"))
                (and:SI (match_operand:SI 3 "gpc_reg_operand" "0")
			(match_operand:SI 4 "const_int_operand" "n"))))]
  "(INTVAL( operands[2]) == ~ INTVAL( operands[4]))
   && mask_operand( operands[2], VOIDmode)"
  "*
{
  return \"{rlimi|rlwimi} %0,%1,0,%m2,%M2\" ;
}")
...
(define_insn "*rlwimi_internal2"
  [(set (match_operand:SI 0 "gpc_reg_operand" "=r")
        (ior:SI (and:SI (match_operand:SI 1 "gpc_reg_operand" "0")
			...
			(match_operand:SI 4 "const_int_operand" "n"))))]
  "(INTVAL( operands[2]) == ~ INTVAL( operands[4]))
   && mask_operand( operands[4], VOIDmode)"
  "*
{
  return \"{rlimi|rlwimi} %0,%3,0,%m4,%M4\" ;
}")

;; PPC can handle a constant OR a variable rotate operation (rotate count in a register),
;; but we can't guarantee correct semantics if the shift count is supplied at runtime
;; (e.g. operand 4 below)
;; accordingly, tolerate only constant shifts here

(define_insn "*rlwimi_internal3"
  [(set (match_operand:SI 0 "gpc_reg_operand" "=r")
        (ior:SI (and:SI	(match_operator:SI 5 "shift_operator" [(match_operand:SI 3 "gpc_reg_operand" "r")
							       (match_operand:SI 4 "const_int_operand" "n")])
			(match_operand :SI 6 "const_int_operand" "n"))
                (and:SI (match_operand :SI 1 "gpc_reg_operand" "0")
			(match_operand :SI 2 "const_int_operand" "n"))))]
  "(INTVAL( operands[2]) == ~ INTVAL( operands[6]))
   && rotatable_shift_and_mask( operands[5], operands[6])"
  "*
{
  operands[7] = GEN_INT ( rotate_count( operands[5])) ;
  return \"{rlimi|rlwimi} %0,%3,%h7,%m6,%M6\" ;
}")

(define_insn "*rlwimi_internal4"
  [(set (match_operand:SI 0 "gpc_reg_operand" "=r")
        (ior:SI (and:SI (match_operand :SI 1 "gpc_reg_operand" "0")
			(match_operand :SI 2 "const_int_operand" "n"))
                (and:SI (match_operator:SI 5 "shift_operator" [(match_operand:SI 3 "gpc_reg_operand" "r")
							       (match_operand:SI 4 "const_int_operand" "n")])
			(match_operand :SI 6 "const_int_operand" "n"))))]
  "(INTVAL( operands[2]) == ~ INTVAL( operands[6]))
   && rotatable_shift_and_mask( operands[5], operands[6])"
...
{
  return (WORDS_BIG_ENDIAN)
    ? \"mul hw %0,%1,%2\;mul lw %L0,%1,%2\"
    : \"mul hw %L0,%1,%2\;mul lw %0,%1,%2\";
}"
  [(set_attr "type" "imul")
   (set_attr "length" "8")])
...
{
  return (WORDS_BIG_ENDIAN)
    ? \"mul hwu %0,%1,%2\;mul lw %L0,%1,%2\"
    : \"mul hwu %L0,%1,%2\;mul lw %0,%1,%2\";
}"
  [(set_attr "type" "imul")
   (set_attr "length" "8")])
...
/* APPLE LOCAL: POWERPC expansion for lshrdi3, ashldi3, ashrdi3 added */
/* APPLE LOCAL: there are 6 "subroutines", [a|l]sh[l|r]di3_ppc_blk[1|2] */
/* APPLE LOCAL: and the code that calls them.  Algorithm similar to the */
/* APPLE LOCAL: library functions in libgcc2.c (comments refer to that code */
/* APPLE LOCAL: in places).  Rather bulky, so -Os turns it off. Only shift */
/* APPLE LOCAL: by nonconstant shift count handled here; constants were */
/* APPLE LOCAL: already done elsewhere and well. */

/* APPLE LOCAL  **note** the inital test for 0 in the libgcc2.c implementation */
/* APPLE LOCAL  has been removed.  Correctness of this depends upon shifts */
/* APPLE LOCAL  by 32 doing the right thing!  Correct behavior is guaranteed */
/* APPLE LOCAL  by the slw, srw, sraw instructions.   --dbj */

(define_expand "ashldi3_ppc_blk1"
  [
   (set (match_dup 6) (subreg:SI (match_operand:DI 1 "" "") 0))
   (set (match_dup 7) (subreg:SI (match_dup 1) 1))
   (set (match_operand:SI 5 "" "") 
	    (plus:SI (match_operand:SI 2 "" "") (const_int -32)))
   (set (match_dup 8) (compare:CC (match_dup 5) (const_int 0)))
   (set (pc) (if_then_else (le (match_dup 8) (const_int 0))
			   (label_ref (match_operand:SI 4 "" "")) (pc)))
   (set (subreg:SI (match_operand:DI 0 "" "") 1) (const_int 0))
   (set (subreg:SI (match_dup 0) 0) 
	    (ashift:SI (match_dup 7) (match_dup 5)))
   (set (pc) (label_ref (match_operand:SI 3 "" "")))]
  "TARGET_POWERPC"
  "
{ 
    operands[6] = gen_reg_rtx (SImode);  /* uu.s.high */
    operands[7] = gen_reg_rtx (SImode);  /* uu.s.low */
    operands[8] = gen_reg_rtx (CCmode);
}")

(define_expand "ashldi3_ppc_blk2"
  [
   (use (match_operand:SI 3 "" ""))
   (set (match_dup 6) (subreg:SI (match_operand:DI 1 "" "") 0))
   (set (match_dup 7) (subreg:SI (match_dup 1) 1))
   (set (match_operand:SI 4 "" "") (neg:SI (match_dup 4)))
   (set (subreg:SI (match_operand:DI 0 "" "") 1)
	    (ashift:SI (match_dup 7) (match_operand:SI 2 "" "")))
   (set (match_dup 5) (lshiftrt:SI (match_dup 7) (match_dup 4)))
   (set (match_dup 7) (ashift:SI (match_dup 6) (match_dup 2)))
   (set (subreg:SI (match_dup 0) 0) (ior:SI (match_dup 7) (match_dup 5)))]
  "TARGET_POWERPC"
  "
{ 
    operands[5] = gen_reg_rtx (SImode);  /* carries */
    operands[6] = gen_reg_rtx (SImode);  /* uu.s.high */
    operands[7] = gen_reg_rtx (SImode);  /* uu.s.low */
}")
...
(define_expand "ashldi3"
  [(set (match_operand:DI 0 "gpc_reg_operand" "")
	(ashift:DI (match_operand:DI 1 "gpc_reg_operand" "")
		   (match_operand:SI 2 "reg_or_cint_operand" "")))]
  "TARGET_POWERPC64 || TARGET_POWER || TARGET_POWERPC"
  "
{
  if (TARGET_POWERPC64)
    ;
  ...
  else if (0 && TARGET_POWERPC && !optimize_size &&
	   GET_CODE (operands[2]) != CONST_INT)
    {
      rtx label_end = gen_label_rtx ();
      rtx label_mid = gen_label_rtx ();
      rtx reg_neg_bm = gen_reg_rtx (SImode);  /* -bm */
      rtx temp = gen_reg_rtx (DImode);
      rtx insns_blk1, first, last;
      operands[2] = force_reg( SImode, operands[2]);
      insns_blk1 = gen_ashldi3_ppc_blk1 (temp, operands[1], operands[2],
	    label_end, label_mid, reg_neg_bm);
      first = XVECEXP (insns_blk1, 0, 0);
      emit_insn (insns_blk1);
      emit_label (label_mid);
      emit_insn (gen_ashldi3_ppc_blk2 (temp, operands[1], operands[2], 
	    label_end, reg_neg_bm));
      emit_label (label_end);
      last = emit_insn (gen_movdi (operands[0], temp));
...
(define_expand "lshrdi3_ppc_blk1"
  [
   (set (match_dup 6) (subreg:SI (match_operand:DI 1 "" "") 0))
   (set (match_dup 7) (subreg:SI (match_dup 1) 1))
   (set (match_operand:SI 5 "" "") 
   ...
   (set (pc) (if_then_else (le (match_dup 8) (const_int 0))
			   (label_ref (match_operand:SI 4 "" "")) (pc)))
   (set (subreg:SI (match_operand:DI 0 "" "") 1)
	    (lshiftrt:SI (match_dup 6) (match_dup 5)))
...
(define_expand "lshrdi3_ppc_blk2"
  [
   (use (match_operand:SI 3 "" ""))
   (set (match_dup 6) (subreg:SI (match_operand:DI 1 "" "") 0))
   (set (match_dup 7) (subreg:SI (match_dup 1) 1))
   (set (match_operand:SI 4 "" "") (neg:SI (match_dup 4)))
   (set (subreg:SI (match_operand:DI 0 "" "") 0) 
	    (lshiftrt:SI (match_dup 6) (match_operand:SI 2 "" "")))
   (set (match_dup 5) (ashift:SI (match_dup 6) (match_dup 4)))
   (set (match_dup 7) (lshiftrt:SI (match_dup 7) (match_dup 2)))
   (set (subreg:SI (match_dup 0) 1)
	    (ior:SI (match_dup 7) (match_dup 5)))]
  "TARGET_POWERPC"
  "
{ 
    operands[5] = gen_reg_rtx (SImode);  /* carries */
  ...
  "
{
  if (TARGET_POWERPC64)
	   ...
	   GET_CODE (operands[2]) != CONST_INT)
    {
      ...
      rtx insns_blk1, first, last;
      operands[2] = force_reg( SImode, operands[2]);
      insns_blk1 = gen_lshrdi3_ppc_blk1 (temp, operands[1], operands[2],
	    label_end, label_mid, reg_neg_bm);
      first = XVECEXP (insns_blk1, 0, 0);
      emit_insn (insns_blk1);
      emit_label (label_mid);
      emit_insn (gen_lshrdi3_ppc_blk2 (temp, operands[1], operands[2], 
	    label_end, reg_neg_bm));
      emit_label (label_end);
      last = emit_insn (gen_movdi (operands[0], temp));
...
(define_expand "ashrdi3_ppc_blk1"
  [
   (set (match_dup 6) (subreg:SI (match_operand:DI 1 "" "") 0))
   (set (match_dup 7) (subreg:SI (match_dup 1) 1))
   (set (match_operand:SI 5 "" "") 
   ...
   (set (pc) (if_then_else (le (match_dup 8) (const_int 0))
			   (label_ref (match_operand:SI 4 "" "")) (pc)))
   (set (subreg:SI (match_operand:DI 0 "" "") 1)
	    (ashiftrt:SI (match_dup 6) (match_dup 5)))
   (set (subreg:SI (match_dup 0) 0) 
	    (ashiftrt:SI (match_dup 6) (const_int 31)))
   (set (pc) (label_ref (match_operand:SI 3 "" "")))]
  "TARGET_POWERPC"
  "
{ 
...
(define_expand "ashrdi3_ppc_blk2"
  [
   (use (match_operand:SI 3 "" ""))
   (set (match_dup 6) (subreg:SI (match_operand:DI 1 "" "") 0))
   (set (match_dup 7) (subreg:SI (match_dup 1) 1))
   (set (match_operand:SI 4 "" "") (neg:SI (match_dup 4)))
   (set (subreg:SI (match_operand:DI 0 "" "") 0) 
	    (ashiftrt:SI (match_dup 6) (match_operand:SI 2 "" "")))
   (set (match_dup 5) (ashift:SI (match_dup 6) (match_dup 4)))
   (set (match_dup 7) (lshiftrt:SI (match_dup 7) (match_dup 2)))
   (set (subreg:SI (match_dup 0) 1) (ior:SI (match_dup 7) (match_dup 5)))]
  "TARGET_POWERPC"
  "
{ 
  ...
  "
{
  if (TARGET_POWERPC64)
	   ...
	   GET_CODE (operands[2]) != CONST_INT)
    {
      ...
      rtx insns_blk1, first, last;
      operands[2] = force_reg( SImode, operands[2]);
      insns_blk1 = gen_ashrdi3_ppc_blk1 (temp, operands[1], operands[2],
	    label_end, label_mid, reg_neg_bm);
      first = XVECEXP (insns_blk1, 0, 0);
      emit_insn (insns_blk1);
      emit_label (label_mid);
      emit_insn (gen_ashrdi3_ppc_blk2 (temp, operands[1], operands[2], 
	    label_end, reg_neg_bm));
      emit_label (label_end);
      last = emit_insn (gen_movdi (operands[0], temp));
...
;; The output of this could be r0, but we strongly prefer base
;; registers, since almost all uses of this will need it
;; in a base register shortly.
(define_insn "elf_high"
  [(set (match_operand:SI 0 "gpc_reg_operand" "=b*r")
	(high:SI (match_operand 1 "" "")))]
  "(TARGET_ELF || DEFAULT_ABI == ABI_MACOSX) && ! TARGET_64BIT"
  "{liu|lis} %0,ha16(%1)")
...
;; More ELF-style whackery.  Allow writing to ELF vars with one insn
;; rather than getting the var's address and then writing to that.

(define_insn "elf_storeqi"
  [(set (mem:QI (lo_sum:SI (match_operand:SI 0 "gpc_reg_operand" "b")
                           (match_operand 1 "" "")))
        (match_operand:QI 2 "gpc_reg_operand" "r"))]
   "(DEFAULT_ABI == ABI_MACOSX && !TARGET_64BIT)"
   "stb %2,lo16(%1)(%0)")

(define_insn "elf_storehi"
  [(set (mem:HI (lo_sum:SI (match_operand:SI 0 "gpc_reg_operand" "b")
                           (match_operand 1 "" "")))
        (match_operand:HI 2 "gpc_reg_operand" "r"))]
   "(DEFAULT_ABI == ABI_MACOSX && !TARGET_64BIT)"
   "sth %2,lo16(%1)(%0)")

(define_insn "elf_storesi"
  [(set (mem:SI (lo_sum:SI (match_operand:SI 0 "gpc_reg_operand" "b")
                           (match_operand 1 "" "")))
        (match_operand:SI 2 "gpc_reg_operand" "r"))]
   "(DEFAULT_ABI == ABI_MACOSX && !TARGET_64BIT)"
   "stw %2,lo16(%1)(%0)")

(define_insn "elf_storesf"
  [(set (mem:SF (lo_sum:SI (match_operand:SI 0 "gpc_reg_operand" "b")
                           (match_operand 1 "" "")))
        (match_operand:SF 2 "gpc_reg_operand" "f"))]
   "(TARGET_HARD_FLOAT && DEFAULT_ABI == ABI_MACOSX && !TARGET_64BIT)"
   "stfs %2,lo16(%1)(%0)")

(define_insn "elf_storedf"
  [(set (mem:DF (lo_sum:SI (match_operand:SI 0 "gpc_reg_operand" "b")
                           (match_operand 1 "" "")))
        (match_operand:DF 2 "gpc_reg_operand" "f"))]
   "(TARGET_HARD_FLOAT && DEFAULT_ABI == ABI_MACOSX && !TARGET_64BIT)"
   "stfd %2,lo16(%1)(%0)")
  ...
  "
{
  ...
  if ((TARGET_ELF || DEFAULT_ABI == ABI_MACOSX) && TARGET_NO_TOC && !TARGET_64BIT
      && !flag_pic
...
(define_insn ""
  ...
  "@
   mcrf %0,%1
   mtcrf 128,%1
   {rlinm|rlwinm} %1,%1,%F0,0,31\;mtcrf %R0,%1\;{rlinm|rlwinm} %1,%1,%f0,0,31
   mfcr %0
   mfcr %0\;{rlinm|rlwinm} %0,%0,%f1,0,3
   mr %0,%1
   {l%U1%X1|lwz%U1%X1} %0,%1
...

(define_expand "movsv"
  [(set (match_operand:SV 0 "general_operand" "")
	(match_operand:SV 1 "any_operand" ""))]
  ""
  "
{
  if (GET_CODE (operands[0]) != REG)
    operands[1] = force_reg (SVmode, operands[1]);
  ...
  if (GET_CODE (operands[0]) == REG && INT_REGNO_P (REGNO (operands[0])))
	...
	{
	  rtx stack_slot = assign_stack_temp (SVmode, 16, 0);
  ...
  if (GET_CODE (operands[1]) == CONST_VECTOR)
	...
	{
	case 0:
	  operands[1] = force_const_mem (SVmode, operands[1]);
	  if (! reload_in_progress
	      && ! memory_address_p (SVmode, XEXP (operands[1], 0)))
	    operands[1] = change_address (operands[1], SVmode,
					  XEXP (operands[1], 0));
	  break;
	case 1: /* vzero */
	...
	case 4: /* vspltisw */
	  ...
	  emit_insn (gen_xfA_perm (operands[0],
				   ...
				   gen_rtx (SYMBOL_REF, Pmode, \"vspltisw\")));
	  DONE;
	case 5: /* vspltish */
	  immed = CONST_VECTOR_0 (operands[1]) << 16 >> 16;
	  emit_insn (gen_xfA_perm (operands[0],
				   gen_rtx (CONST_INT, VOIDmode, immed),
				   gen_rtx (SYMBOL_REF, Pmode, \"vspltish\")));
	  DONE;
	case 6: /* vspltisb */
	  immed = CONST_VECTOR_0 (operands[1]) << 24 >> 24;
	  emit_insn (gen_xfA_perm (operands[0],
				   gen_rtx (CONST_INT, VOIDmode, immed),
				   gen_rtx (SYMBOL_REF, Pmode, \"vspltisb\")));
	  DONE;
	case 7: /* lvsr 0,0 */
    ...
    {
      emit_insn (gen_movsv_reg (operands[0], operands[1]));
      DONE;
    }
}")

(define_expand "reload_insv"
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
	(match_operand:SV 1 "any_operand" ""))
...
(define_expand "reload_outsv"
  [(set (match_operand:SV 0 "any_operand" "")
	(match_operand:SV 1 "vector_register_operand" "v"))
   (clobber (match_operand:SI 2 "gpc_reg_operand" "=&b"))]
  ""
  "
{
...
(define_insn ""
  [(set (mem:SV (plus:SI (match_operand:SI 0 "gpc_reg_operand" "b")
	...
	(match_operand:SV 2 "vector_register_operand" "v"))]
  ""
  "stvx %2,%0,%1"
  [(set_attr "type" "store")])

(define_insn ""
  [(set (mem:SV (match_operand:SI 0 "gpc_reg_operand" "b"))
	(match_operand:SV 2 "vector_register_operand" "v"))]
  ""
  "stvx %2,0,%0"
  [(set_attr "type" "store")])
...
(define_split
  [(set (match_operand:SV 0 "vector_register_operand" "")
        (mem:SV (plus:SI (match_operand:SI 1 "gpc_reg_operand" "b")
...
(define_insn ""
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
        (mem:SV (match_operand:SI 1 "gpc_reg_operand" "r")))]
  ""
  "lvx %0,0,%1"
  [(set_attr "type" "vec_load")])

(define_insn "movsv_reg"
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
	(match_operand:SV 1 "vector_register_operand" "v"))]
  ""
...
;; Also check that the RD is not the same as RA.
...
(define_insn "*movsi_update1"
  [(set (match_operand:SI 3 "gpc_reg_operand" "=r,r")
   ...
   (set (match_operand:SI 0 "gpc_reg_operand" "=b,b")
	(plus:SI (match_dup 1) (match_dup 2)))]
  "TARGET_UPDATE && (REGNO (operands[3]) != REGNO (operands[0]))"
  "@
   {lux|lwzux} %3,%0,%2
   {lu|lwzu} %3,%2(%0)"
  [(set_attr "type" "load")])
...
;; APPLE LOCAL peephole to eliminate unneeded computation of FP const
;; APPLE LOCAL address in register
(define_peephole
  [(set (match_operand:SI 0 "gpc_reg_operand" "b")
	(lo_sum:SI (match_dup 0) (match_operand:SI 1 "" "")))
   (set (match_operand:DF 2 "gpc_reg_operand" "=f")
	(mem:DF (match_dup 0)))]
  "TARGET_HARD_FLOAT 
    && find_reg_note (insn, REG_DEAD, 0) 
    && FP_REGNO_P (REGNO (operands[2]))"
  "lfd %2,lo16(%1)(%0)")

(define_peephole
  [(set (match_operand:SI 0 "gpc_reg_operand" "b")
	(lo_sum:SI (match_dup 0) (match_operand:SI 1 "" "")))
   (set (match_operand:SF 2 "gpc_reg_operand" "=f")
	(mem:SF (match_dup 0)))]
  "TARGET_HARD_FLOAT 
    && find_reg_note (insn, REG_DEAD, 0)
    && FP_REGNO_P (REGNO (operands[2]))"
...
(define_insn "nonlocal_goto_receiver"
  ...
  "(TARGET_TOC && TARGET_MINIMAL_TOC)
	|| (DEFAULT_ABI == ABI_MACOSX && flag_pic)"
  "*
{
  rs6000_output_load_toc_table (asm_out_file,
	(DEFAULT_ABI == ABI_MACOSX) ? PIC_OFFSET_TABLE_REGNUM : 30);
  return \"\";
}"
  [(set_attr "type" "load")])

...
(define_insn "call_value_indirect_aix32"
  [(set (match_operand 0 "register_operand" "fgv")
...
;;
;; *****************************************************************
;; *** NOTE!!  AS WRITTEN, THIS NOW ONLY WORKS FOR ABI_MACOSX!!! ***
;; *** SHOULD REALLY CREATE A NEW REG CLASS FOR R12 AND USE THAT ***
;; *****************************************************************
;;
(define_insn "call_indirect_sysv"
  [(call (mem:SI (match_operand:SI 0 "gpc_reg_operand" "r,r"))
	 (match_operand 1 "const_int_operand" "n,n"))
   (use (match_operand 2 "const_int_operand" "O,n"))
   (clobber (match_scratch:SI 3 "=l,l"))
   (clobber (match_scratch:SI 4 "=c,c"))]
  "DEFAULT_ABI == ABI_V4 || DEFAULT_ABI == ABI_SOLARIS || DEFAULT_ABI == ABI_AIX_NODESC || DEFAULT_ABI == ABI_MACOSX"
  "*
{
...
;;
;; DITTO -- THIS HAS BEEN MANGLED FOR MACOSX...
;;
(define_insn "call_value_indirect_sysv"
  [(set (match_operand 0 "register_operand" "=fgv,fgv")
	(call (mem:SI (match_operand:SI 1 "gpc_reg_operand" "r,r"))
	      (match_operand 2 "const_int_operand" "n,n")))
   (use (match_operand 3 "const_int_operand" "O,n"))
   (clobber (match_scratch:SI 4 "=l,l"))
   (clobber (match_scratch:SI 5 "=c,c"))]
  "DEFAULT_ABI == ABI_V4 || DEFAULT_ABI == ABI_SOLARIS || DEFAULT_ABI == ABI_AIX_NODESC || DEFAULT_ABI == ABI_MACOSX"
  "*
{
  ...
  "
{
  ...
  if (GET_CODE (operands[0]) != SYMBOL_REF
	...
	operands[0] = rs6000_longcall_ref (operands[0]);
      ...
      if (DEFAULT_ABI == ABI_V4 || DEFAULT_ABI == ABI_AIX_NODESC || DEFAULT_ABI == ABI_SOLARIS || DEFAULT_ABI == ABI_MACOSX)
...
(define_insn "*call_nonlocal_sysv"
  [(call (mem:SI (match_operand:SI 0 "call_operand" "s,s"))
   ...
   (use (match_operand:SI 2 "immediate_operand" "O,n"))
   (clobber (match_scratch:SI 3 "=l,l"))]
  "(DEFAULT_ABI == ABI_AIX_NODESC || DEFAULT_ABI == ABI_V4 || DEFAULT_ABI == ABI_SOLARIS || DEFAULT_ABI == ABI_MACOSX)
   && (INTVAL (operands[2]) & CALL_LONG) == 0"
  "*
{
  /* Indirect calls should go through call_indirect */
...
(define_expand "j_24_f_fxx_simple"
  [(parallel [(set (reg:CC 74)
		   (unspec:CC [(match_operand:SV 1 "vector_register_operand" "v")
			       (match_operand:SV 2 "vector_register_operand" "v")
			       (match_operand 3 "any_operand" "")] 1))
	      (clobber (match_scratch:SV 4 "=v"))])
...
(define_expand "j_24_f_frxx_simple"
  [(parallel [(set (reg:CC 74)
		   (unspec:CC [(match_operand:SV 2 "vector_register_operand" "v")
			       (match_operand:SV 1 "vector_register_operand" "v")
			       (match_operand 3 "any_operand" "")] 1))
	      (clobber (match_scratch:SV 4 "=v"))])
   (set (match_operand:SI 0 "register_operand" "=r")
...
(define_insn "xfx_perm"
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
	(unspec:SV [(use (match_operand:SV 1 "vector_register_operand" "v"))
		    (match_operand 2 "any_operand" "")] 1))]
  ""
  "%2 %0,%1"
...
(define_insn "xfA_perm"
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
	(unspec:SV [(match_operand:SI 1 "const_int_operand" "A")
...
(define_insn "xfxx_perm"
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
	(unspec:SV [(use (match_operand:SV 1 "vector_register_operand" "v"))
		    (use (match_operand:SV 2 "vector_register_operand" "v"))
		    (match_operand 3 "any_operand" "")] 1))]
  ""
  "%3 %0,%1,%2"
  [(set_attr "type" "vec_perm")])
...
(define_insn "xfxxx_perm"
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
	(unspec:SV [(use (match_operand:SV 1 "vector_register_operand" "v"))
		    (use (match_operand:SV 2 "vector_register_operand" "v"))
		    (use (match_operand:SV 3 "vector_register_operand" "v"))
...
(define_insn "xfii_load"
  [(set (match_operand:SV 0 "vector_register_operand" "=v,v,v")
	(unspec:SV [(use (match_operand:SI 1 "reg_or_cint_operand" "b,O,r"))
		    (use (match_operand:SI 2 "reg_or_cint_operand" "r,r,O"))
...
(define_insn "xlfii_load"
  [(set (match_operand:SV 0 "vector_register_operand" "=v,v,v")
	(mem:SV (unspec:SV [(use (match_operand:SI 1 "reg_or_cint_operand" "b,O,r"))
			    (use (match_operand:SI 2 "reg_or_cint_operand" "r,r,O"))
...
(define_insn "sfxii_store"
  [(set (mem:SV (unspec:SV [(use (match_operand:SI 1 "reg_or_cint_operand" "b,O,r"))
			    (use (match_operand:SI 2 "reg_or_cint_operand" "r,r,O"))
			    (match_operand 3 "any_operand" "")] 10))
	(match_operand:SV 0 "vector_register_operand" "v,v,v"))]
  ""
  "@
   %3 %0,%1,%2
   %3 %0,0,%2
...
(define_insn "vlfiiD_load"
  [(unspec_volatile:SV [(use (match_operand:SI 0 "register_operand" "b"))
...
(define_insn "vlfD_load"
  [(unspec_volatile:SV [(match_operand:SI 0 "const_int_operand" "D")
...
(define_insn "vlf_load"
  [(unspec_volatile:SV [(match_operand 0 "any_operand" "")] 2)]
  ""
  "%0"
  [(set_attr "type" "load")])
...
(define_insn "vxf_fxu"
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
	(unspec_volatile:SV [(match_operand 1 "any_operand" "")] 2))]
  ""
  "%1 %0"
...
(define_insn "vfx_fxu"
  [(unspec_volatile:SV
    [(use (match_operand:SV 0 "vector_register_operand" "v"))
     (match_operand 1 "any_operand" "")] 2)]
  ""
  "%1 %0"
  [(set_attr "type" "integer")])
  ...
  [(set (match_operand:SI 0 "gpc_reg_operand" "=r")
	(match_operator:SI 1 "scc_comparison_operator"
			   [(match_operand 2 "cc_reg_operand" "y")
			    (const_int 0)]))]
  ""
  "%D1mfcr %0\;{rlinm|rlwinm} %0,%0,%J1,31,31"
  [(set_attr "length" "12")])
...
(define_insn ""
  ...
  "! TARGET_POWERPC64"
  "%D1mfcr %3\;{rlinm.|rlwinm.} %3,%3,%J1,31,31"
  [(set_attr "type" "delayed_compare")
   (set_attr "length" "12")])
...
(define_peephole
  [(set (match_operand:SI 0 "gpc_reg_operand" "=r")
   ...
   "%D1%D4mfcr %3\;{rlinm|rlwinm} %0,%3,%J1,31,31\;{rlinm|rlwinm} %3,%3,%J4,31,31"
   [(set_attr "length" "20")])
...
;; For MacOS X, add a variant which takes a register containing the
;; address of the switch table, which avoids us loading the same
;; value twice (reducing the tablejump from 9 to 7 insns.)
;; 0 - index
;; 1 - reg with address of jump_table (i.e., address of operand[2])
;; 2 - label_ref of jump_table

(define_expand "tablejump_labelref"
  [(use (match_operand 0 "" ""))
   (use (match_operand 1 "" ""))
...
{                             
  if (TARGET_32BIT)
    emit_jump_insn (gen_tablejump_labelrefsi (operands[0], operands[1], operands[2]));
  else
    abort ();
  DONE;
}")
(define_expand "tablejump_labelrefsi"
  [(set (match_operand:SI 1 "" "")
        (plus:SI (match_operand:SI 0 "" "")
                 (match_dup 1)))
   (parallel [(set (pc) (match_dup 1))
	      (use (label_ref (match_operand 2 "" "")))])]
  "TARGET_32BIT"
  "
...
;; Zero Overhead Exceptions : Entertaining Times Ahead.
;; operand 0 --> exception context pointer.  Can be ignored as it's unused.
;; operand 1 --> offset added to SP
;; operand 2 --> address to return to
;; To avoid hassles, we place the SP offset into r7 and the return address
;; into R8.  This means that the epilog routines can proceeed as normal
;; and even call out into rest_vec and rest_fp routines without fear of
;; having these parameters overwritten...

(define_expand "eh_epilogue"
  [(use (match_operand:SI 0 "gpc_reg_operand" "r"))
   (use (match_operand:SI 1 "gpc_reg_operand" "r"))
   (use (match_operand:SI 2 "gpc_reg_operand" "r"))]
  "DEFAULT_ABI == ABI_MACOSX"
  "
{
  /* Move the SP Offset into R7.  */
  if (GET_CODE (operands[1]) != REG || REGNO (operands[1]) != 7)
    {
      rtx offs = gen_rtx_REG (SImode, 7);
      emit_move_insn (offs, operands[1]);
      operands[1] = offs;
    }

  if (GET_CODE (operands[2]) != REG || REGNO (operands[2]) != 8)
    {
      rtx ret = gen_rtx_REG (SImode, 8);
      emit_move_insn (ret, operands[2]);
      operands[2] = ret;
    }
  rs6000_eh_epilog_sp_offs = operands[1];
  rs6000_eh_epilog_ret_addr = operands[2];
}")
