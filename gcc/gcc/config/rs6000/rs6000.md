...
; APPLE LOCAL begin gcc3.1 compatibility for fused instructions.
(define_insn ""
  [(set (match_operand:SF 0 "gpc_reg_operand" "=f")
...
(define_insn ""
  ...
  "fnmsubs %0,%1,%2,%3"
  [(set_attr "type" "fp")])
; APPLE LOCAL end gcc3.1 compatibility for fused instructions.
...
;; APPLE LOCAL begin uint8_t or uint16_t->float optimization
(define_expand "floatunshisf2"
  [
   (set (match_dup 7) (zero_extend:SI (match_operand:HI 1 "gpc_reg_operand" "")))
   (set (match_dup 3) (match_dup 2))
   (set (match_dup 6) (match_dup 5))
   (set (match_dup 4) (match_dup 6))
   (set (match_dup 8) (ior:SI (match_dup 7) (match_dup 2)))
   (set (match_dup 9) (match_dup 8))
   (set (match_dup 10) (match_dup 4))
   (set (match_dup 11) (match_operand:SF 0 "reg_or_mem_operand" ""))
   (set (match_dup 12) (minus:SF (match_dup 11) (match_dup 10)))
   (set (match_dup 0) (match_dup 12))]
  "TARGET_HARD_FLOAT && TARGET_FPRS"
  "
{
  operands[2] = force_reg (SImode, GEN_INT(0x4B000000));
  operands[3] = assign_stack_temp (SImode, GET_MODE_SIZE (SImode), 0);  /* ibias */
  operands[4] = assign_stack_temp (SFmode, GET_MODE_SIZE (SFmode), 0);  /* fbias */
  operands[5] = gen_rtx_MEM (SFmode, XEXP(operands[3],0));
  operands[6] = gen_reg_rtx (SFmode);
  operands[7] = gen_reg_rtx (SImode);
  operands[8] = gen_reg_rtx (SImode);
  if (GET_CODE (operands[0]) == REG)
    operands[9] = gen_rtx_SUBREG (SImode, operands[0], 0);
  else
    operands[9] = gen_rtx_MEM (SImode, XEXP(operands[0], 0));
  operands[10] = gen_reg_rtx (SFmode);
  operands[11] = gen_reg_rtx (SFmode);
  operands[12] = gen_reg_rtx (SFmode);
}")
;; APPLE LOCAL end uint8_t or uint16_t->float optimization
...
(define_expand "floatdisf2"
  ...
  "TARGET_POWERPC64 && !TARGET_32BIT && TARGET_HARD_FLOAT && TARGET_FPRS"
  "
{
...
;; APPLE LOCAL begin optimization of DImode >> 32 and then conversion to SImode
  ...
  "*
{
  if (REGNO (operands[0]) == REGNO (operands[1]))
   ...
   [(set_attr "length" "4")])
;; APPLE LOCAL end optimization of DImode >> 32 and then conversion to SImode
...
(define_insn "ashldi3_internal5"
  [(set (match_operand:CC 0 "cc_reg_operand" "=x,?y")
	(compare:CC
	 ...
	 (const_int 0)))
   (clobber (match_scratch:DI 4 "=r,r"))]
  "TARGET_POWERPC64  && !TARGET_32BIT && includes_rldic_lshift_p (operands[2], operands[3])"
  "@
   rldic. %4,%1,%H2,%W3
   #"
  [(set_attr "type" "delayed_compare")
...
(define_insn "ashldi3_internal8"
  [(set (match_operand:CC 0 "cc_reg_operand" "=x,?y")
	(compare:CC
	 ...
	 (const_int 0)))
   (clobber (match_scratch:DI 4 "=r,r"))]
  "TARGET_POWERPC64 && !TARGET_32BIT && includes_rldicr_lshift_p (operands[2], operands[3])"
  "@
   rldicr. %4,%1,%H2,%S3
   #"
  [(set_attr "type" "delayed_compare")
...
(define_insn ""
  [(match_parallel 0 "mov_to_vrsave_operation"
		   [(set (unspec_volatile:SI [(const_int 0)] 117)
  ...
  "
{
  ...
  if (GET_CODE (operands[1]) != SYMBOL_REF
	...
	operands[1] = rs6000_longcall_ref (operands[1]);
	  ...
	  || DEFAULT_ABI == ABI_AIX_NODESC
	  || DEFAULT_ABI == ABI_DARWIN)
/* APPLE LOCAL begin 64bit ptr-to-function return in 32bit mode */
      {
        if (!(GET_CODE (operands[0]) == PARALLEL && GET_MODE(operands[0]) == DImode))
	  operands[0] = force_reg (Pmode, operands[0]);
      }
/* APPLE LOCAL end 64bit ptr-to-function return in 32bit mode */
      else if (DEFAULT_ABI == ABI_AIX)
	{
	  /* AIX function pointers are really pointers to a three word
	     area.  */
...
; APPLE LOCAL separate 'cl' into 'c,*l'; modify attrs appropriately
...
; APPLE LOCAL separate 'cl' into 'c,*l'; modify attr's appropriately
...
(define_insn "xfii_load"
  [(set (match_operand:V4SI 0 "register_operand" "=v,v,v")
...
(define_insn "xlfii_load"
  [(set (match_operand:V4SI 0 "register_operand" "=v,v,v")
	(mem:V4SI (unspec [(use (match_operand:SI 1 "reg_or_zero_operand" "b,O,r"))
			    (use (match_operand:SI 2 "reg_or_zero_operand" "r,r,O"))
...
(define_insn "sfxii_store"
  [(set (mem:V4SI (unspec [(use (match_operand:SI 1 "reg_or_zero_operand" "b,O,r"))
			    (use (match_operand:SI 2 "reg_or_zero_operand" "r,r,O"))
			    (match_operand 3 "any_operand" "")] 10))
	(match_operand:V4SI 0 "register_operand" "v,v,v"))]
  ""
  "@
   %3 %0,%1,%2
   %3 %0,0,%2
