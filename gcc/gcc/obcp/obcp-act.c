...
// FIXME: should arg 4 always be 0 ??? (NO...snaroff fix...3/23/96)
#define xref_tag(code, name) xref_tag (record_type_node, name, 0, 1)
     ...
     char *name;
{
  ...
  if (code == VAR_DECL)
    {
      TREE_STATIC (decl) = 1;
      // required by duplicate_decls/redeclaration_error_message
...
synth_module_prologue ()
{
  ...
  /* snaroff (3/25/96). This fixes the redeclaration errors. */
...
build_module_descriptor ()
{
...
#endif
  {
    ...
    { // snaroff fix (3/25/96)
    tree parmlist; // has the format of the output of get_parm_info

#ifdef OBJCPLUS
    parmlist = build_tree_list (NULL_TREE, void_type_node);
    TREE_PARMLIST (parmlist) = 1; // typically done by get_parm_info
    push_lang_context (lang_name_c); /* extern "C" */
#else
...
build_selector_translation_table ()
{
  ...
  for (chain = sel_ref_chain; chain; chain = TREE_CHAIN (chain))
      ...
      if (! flag_selector_table)
	{
	  ...
	  sc_spec = build_tree_list (NULL_TREE, ridpointers[(int) RID_STATIC]);
	  sc_spec = tree_cons (NULL_TREE, ridpointers[(int) RID_CONST],
			       sc_spec);
...
#ifdef OBJCPLUS
  // inhibits extern/static warnings (snaroff: 3/28/96)
  // Also changed decl.c:warn_extern_redeclared_static().
  // Don't remember how this works in the standard ObjC compiler.
  DECL_ARTIFICIAL (decl) = 1;
#endif
  DECL_EXTERNAL (decl) = 1;
  TREE_PUBLIC (decl) = 0; 
...
/* 
  Generate a FUNCTION_DECL node to be used in build_objc_method_call
  below. 
 */
static tree
  ...
  if (flag_next_runtime)
      ...
      else if (DIRECT_METHOD_FLAG (method_prototype))
	{
	  ...
	  if (! function_decl)
	    {
	      function_decl =
		lookup_name (method_id_for_method (method_prototype));
	      if (! function_decl)
		{
		  function_decl =
		    build_function_decl_for_prototype (method_prototype);
    ...
    tree chain;
{
  ...
  sc_spec = build_tree_list (NULL_TREE, ridpointers[(int) RID_STATIC]);
  sc_spec = tree_cons (NULL_TREE, ridpointers[(int) RID_CONST], sc_spec);
