     ...
     char *filename;
{
  ...
  /* The Objective-C++ protocol arg qualifiers are initially DISABLED, until
...
#ifdef OBJCPLUS
                  /* zlaski 2001-May-04: In the spirit of Turly's hackery, we
                     bail out if we encounter any C++ keywords that really 
                     should be treated as identifiers */
                  if (objc_need_raw_identifier && ptr->rid == RID_CLASS) { 
                    value = IDENTIFIER; 
...
#ifdef OBJCPLUS
              /* zlaski 2001-May-17: I've tried to disable the 'and' and 'or'
                 alternate C++ keywords altogether, but it turns out PowerPlant
                 uses them!  Thing is, they also figure prominently in Cocoa
                 as ObjC selectors. */
              else if ((objc_msg_context || objc_in_method_signature) && 
...
#ifdef OBJCPLUS
  /* zlaski 2001-Mar-28: let the Obj-C++ keywords through, they're not really
     "reserved" */
  if(r && r->rid >= RID_IN && r->rid <= RID_ID) {
      r = 0;
  }       
  /* zlaski 2001-Apr-04: Allow the C++ 'class' keyword to also be used as
