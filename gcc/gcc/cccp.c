...
/* Nonzero means warn if "#pragma once" is encountered.  */
...
#endif /* APPLE_CPP_HEADER_MAPS */

#ifdef APPLE_CPP_INCLUSION_LOGS
/* Number of open() and stat() calls we've done. For performance tuning purposes. */

static unsigned num_opens = 0;
static unsigned num_stats = 0;

/* The open file into which to log header inclusion information, set by the
   '-inclusion-log-file' option, or NULL if none. */
...
static struct default_include {
  ...
  = {
    ...
    /* This should be /usr/local/include/c++ and should come before
       the GNU C++ specific include files.  */
...
enum node_type {
 ...
 T_SPEC_OPTION,  /* special `__option' macro for Metrowerks compatibility */
#endif
...
#define BAILOUT(name) {              \
  ...
  for (i = 1; i < argc; i++) {
	...
	if (!strcmp (argv[i], "-std=iso9899:1990")
	  ...
	  cplusplus = 0, cplusplus_comments = 1, c89 = 0, c9x = 1, objc = 0;
#if defined (NEXT_SEMANTICS) || defined (NEXT_PDO)
	/* cpp is called with the -smart option if cpp-precomp is not found,
	   so just ignore it.  */
  ...
  if (!out_fname || !strcmp (out_fname, ""))
    ...
    /* If flag_dump_symbols is set, let the indexing stuff dump on stdout
       first and then later reopen stdout to out_fname (in write_output()).  */
     ...
     FILE_BUF *op;
{
  ...
  case T_SPEC_OPTION:
    ...
    if (ip->bufp[0] == 'L' && (ip->bufp[1] == '\'' || ip->bufp[1] == '"'))
      goto oops;
#ifdef APPLE_EXTENSION
    if (hp->type == T_SPEC_OPTION) {	/* __option(kwd) handled here */
	U_CHAR *iptr = ip->bufp,
	       *optr;
	U_CHAR *kwd;			/* stores the keyword */

	if (!paren)			/* malformed "__option(keyword)" */
	  goto oops;

	kwd = (char *)alloca(strlen(ip->bufp));	/* too big, but... */
	optr = kwd;
	while (is_idchar[*iptr])	/* copy the keyword */
	  *optr++ = *iptr++;
	*optr = 0;
	SKIP_WHITE_SPACE (iptr);
	if (*iptr != ')')		/* malformed "__option(keyword)" */
	   goto oops;
...
static inline uint32 hmap_hash_string (str_beg, str_end)
    const char *   str_beg;
    const char *   str_end;
    /** Private function that hashes the contents of the string between 'str_beg' and 'str_end' in a case-insensitive way. */
{
...
static inline unsigned hmap_compare_strings (str1_beg, str1_end, str2)
    ...
    const char *   str1_end;
    const char *   str2;
    /** Private function that case-insensitively compares the string bounded by 'str1_beg' and 'str1_end' with the null-terminated string 'str2'. Returns 1 if equal, 0 if not. */
{
...
static struct hmap_header_map * hmap_load_header_map (filename)
    const char *   filename;
    /** Loads a header map from the 'filename', and returns a pointer to it. Returns NULL if for any reason the header map cannot be loaded. Header maps allocated using this function should eventually be deallocated using hmap_free_header_map(). */
{
...
static void hmap_free_header_map (headermap)
    ...
    /** Deallocates a header map that's been allocated using hmap_load_header_map(). */
{
    if (headermap != NULL)
        free(headermap);
}
...
static void hmap_lookup (headermap, filename_beg, filename_end)
    ...
    char **                    filename_beg;
    char **                    filename_end;
    /** Looks up the header named 'filename_beg' in the header map 'headermap'. If it is found, this function places the corresponding null-terminated header path into an internal buffer, and sets 'filename_beg' and 'filename_end' to point to the start and end of it. Otherwise, 'filename_beg' and 'filename_end' are left as they are. Because this function uses an internal buffer for the returned string, the result of one lookup is only guaranteed to be valid until the next lookup. */
{
...
get_filename:
  ...
  case '<':
...
#endif 0
#else /* !NEXT_CPP_SERVER */

               /* If we use "extendedPath" directly here, then we lose when
		  open_include_file squirrels it away (via the "inc") in its 
		  include_hashtab.  So we need to xmalloc a copy.  */
	       {
	...
	/* Now try search for sub frameworks on frameworks
	   in the include stack.  */
	if ( f == -1 )
	  {
	    int i;
	    /* Walk up stack looking at each file that included us.  */
	    for (i=indepth; i >= 0; i--)
	      {
		...
		if (parent_include != NULL)
		  {
		    ...
		    /* Only use if parent include was a framework include.  */
			  ...
			  if (!strncmp (parent_end, ".framework/", flen))
			    {
			      ...
			      /* We now have: /XXX/Frameworks/YYY.framework/ */
			      p = &extendedPath[parent_end - parent_include];
			      strcpy (p, "Frameworks/");
			      p += strlen ("Frameworks/");
			      /* We now have: /XXX/Frameworks/YYY.framework/Frameworks/ */
			      for (src_ptr = fbeg;
				...
				/* Empty loop!  */;
			      strcpy (p, ".framework/");
			      p += strlen (".framework/");
			      /* We now have: /XXX/Frameworks/YYY.framework/Frameworks/ZZZ.framework/ */
			      p_end = p;
				   ...
				   header_dir_index++)
				{
				  ...
				  /* We now have: /XXX/Frameworks/YYY.framework/Frameworks/ZZZ.framework/Headers/ */
				  for (fname_ptr = src_ptr;
				       (*fname_ptr != '\0') && (fname_ptr != fend);
				       fname_ptr++)
				    *p++ = *fname_ptr;
				  *p = '\0';
				  /* We now have: /XXX/Frameworks/YYY.framework/Frameworks/ZZZ.framework/Headers/Foo.h */

				  /* If we use "extendedPath" directly here,
				     then we lose when open_include_file
				     squirrels it away (via the "inc") in its
				     include_hashtab.  So we need to xmalloc
				     a copy.  */
				  {
				    char *xp =
...
#ifdef NEXT_SEMANTICS
  if (inc && importing)
    /* Don't #import this file if it has already been read in
       due to a previously encountered #include.  */
    fd = -2;  /* Note that this is redundant - fd is already -2.  */
  else
#endif
  if (!inc
      || ! inc->control_macro
...
#else
  if (flag_dump_symbols) {
    /* We delayed stdout redirection for indexing. Now is the time to do it.  */
    if (!out_fname || !strcmp (out_fname, ""))
      out_fname = "stdout";
...
#ifdef APPLE_EXTENSION
/* Return whether the "__option" keyword KWDNAME is known.  These __option
   keywords are specified via "-A__option(keyword)", etc.  */
