...
/* Objective-C specific information (from obcp-parse.c) */
      ...
      if (c == 'p')
	{
	  if (getch () == 'r'
		...
		goto skipline;
	      ...
	      /* Fix a problem where we can accidentally skip a line.
		 HANDLE_PRAGMA is quite entitled to expect to read everything
		 after the identifier.  Unfortunately, with the three (count
		 'em!) putback mechanisms in use here, and the LINEMODE stuff,
		 it's all too easy to screw up.
		 Sync up as best we can, and compensate for LINEMODE causing
		 '\n' to be returned as EOF (a character which can't be
		 putback.)  */
		...
		c = getch ();
	      ...
	      /* If LINEMODE is on, then '\n' gets returned as EOF.  This
		 character cannot be putback, so put back a '\n' instead
		 handle_pragma() will read up to that newline.  */
...
#if 0 /* was: #ifdef OBJCPLUS, but we don't want to implicitly push objc */
    if (doing_objc_thang)
...
#if 0
  /* I removed this problematic code because it is gone in the
     newer (post-2.95.2) public CVS version of egcs, and it
     was crashing the compiler (in the my_friendly_abort(61)
     call).  Once we move forward to that newer release, there
     will be a raft of changes mostly due to CPPLIB.
     -- DAL 2000/2/2 */
...
check_identifier:
	...
	if (value == IDENTIFIER)
	  ...
	  identifier:
		...
		/* This is not the right way to determine that we're looking
		   in global scope and something is shadowing the class decl
		   because it doesn't catch the case of name being defined
		   inside class scope.  */
		value = CLASSNAME;
#endif /* OBJCPLUS */
...
#define TOTAL_PARTS ((HOST_BITS_PER_WIDE_INT / HOST_BITS_PER_CHAR) * 2 + 2)
	...
	*p = 0;
	...
	register int num_chars = 0; /* How many bytes of characters have
				       been placed into this constant. */
	int chars_seen = 0;   /* How many input chars have been seen,
				 after backslash escapes translated. */
     ...
     tree t;
{
  ...
  /* This is called from grokdeclarator() in decl.c to check if a
     reserved word is being abused.  Lie for "vector" and "pixel".  */
