	...
	)
{
	...
	*/
    ...
    gettimeofday_cached(base, &start_tv); // ???: Verify success
	...
	)
{
	...
	xctx->sock = sock;
	xctx->spkt = spkt;
	gettimeofday_cached(base, &start_cb); // ???: Verify success
	...
	)
{
	...
	/* Read in the packet */
	rpktl = recvdata(fd, timeout_tv, &sender, &rbuf, sizeof(rbuf));
	if (rpktl < 0) {
	...
	)
{
	...
	case SERVER_UNUSEABLE:
		TRACE(1, ("Server unusable\n"));
		return -1;
		break;

	case PACKET_UNUSEABLE:
		TRACE(1, ("Packet unusable\n"));
		break;

	case SERVER_AUTH_FAIL:
		TRACE(1, ("Server authorization failure\n"));
		break;
