...
/* This is a crude hack, necessary because Apple changed field names between
 * Tiger and Leopard, seemingly due to misunderstanding of struct scoping
 * rules...
 */
#ifdef __i386__
		...
		vm_size_t				stackSize )
{
	...
	*/
	...
	if( !err ) {
		...
		ptrdiff_t fixUpOffset = (ptrdiff_t) ((char*)image - (char*)remoteCode); 
		...
		unsigned int *jumpTableSize )
{
	...
	for( imageIndex = 0; imageIndex < imageCount; imageIndex++ ) {
		...
		const struct section *section = getsectbynamefromheader( header,
																	SEG_TEXT,
																	SECT_TEXT );
		unsigned long start = section->addr + _dyld_get_image_vmaddr_slide( imageIndex );
		unsigned long stop = start + section->size;
		if( p >= start && p <= stop ) {
		...
		ptrdiff_t fixUpOffset)
{
	...
	for (entry = 0; entry < jumpTableCount; entry++) {
		...
		*((unsigned int *)jumpTable) = jmpValue;
		jumpTable = (void*)((char*)jumpTable + 5);
	}
