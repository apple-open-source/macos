/**
 * bootcamp_formatter - Formatting tool for Boot Camp NTFS volumes.
 *
...
#ifndef BOOTCAMP_FORMATTER_ENABLE_WIPEFS
#define BOOTCAMP_FORMATTER_ENABLE_WIPEFS 0
#endif

#if BOOTCAMP_FORMATTER_ENABLE_WIPEFS

/* Prototypes for wipefs functions in libutil.dylib. */
...
#endif /* BOOTCAMP_FORMATTER_ENABLE_WIPEFS */
...
#error "No default device io operations!  Cannot build bootcamp_formatter.  \
...
#ifndef EXEC_NAME
static char EXEC_NAME[] = "newfs_ntfs";
#endif
...
static long long	   g_mft_zone_end	  = 0;		/* Determined from volume_size, in clusters */
...
static const char *const ignore_warn_msg =
	"Refusing to make a filesystem here!\n";

/**
 * struct bootcamp_formatter_options
 */
static struct bootcamp_formatter_options {
	char *dev_name;                 /* Name of the device to use. */
	long long part_start_sect;      /* Start sector of partition on parent
	                                 * device. */
	long sector_size;               /* Sector size in bytes, power of 2.
...
/**
 * bootcamp_formatter_init_options
 */
static void bootcamp_formatter_init_options(
		struct bootcamp_formatter_options *opts2)
{
	if (!opts2)
		return;
...
/**
 * bootcamp_formatter_time
 */
static ntfs_time bootcamp_formatter_time(void)
{
	struct timespec ts;
...
/**
 * bootcamp_formatter_write
 */
static long long bootcamp_formatter_write(struct ntfs_device *dev,
 ...
 *
 * bootcamp_formatter_bitmap_write
 */
static s64 bootcamp_formatter_bitmap_write(struct ntfs_device *dev,
 ...
 *	without overflowing from the allocated buffer
 *
 * bootcamp_formatter_logfile_write
 */
static s64 bootcamp_formatter_logfile_write(struct ntfs_device *dev,
		...
		WRITE_TYPE write_type)
{
	...
	for (i = 0; rl[i].length; i++) {
		...
		do {
			...
			switch (write_type) {
			case WRITE_BITMAP :
				bytes_written = bootcamp_formatter_bitmap_write(
					dev, total, length);
				break;
			case WRITE_LOGFILE :
				bytes_written = 
					bootcamp_formatter_logfile_write(dev,
					total, length);
				break;
			default :
	...
	if (delta) {
		...
		if (!b)
			return -1;
		bytes_written = bootcamp_formatter_write(dev, b, delta);
		eo = errno;
		free(b);
		errno = eo;
		if (bytes_written == -1LL)
 ...
 *			this.
 *	-EINVAL		Can only occur if bootcamp_formatter was compiled with
 *			-DDEBUG. Means the input parameters were faulty.
 */
 ...
 *
 * TODO: We should be returning the size as well, but for bootcamp_formatter
 ...
 */
static int bootcamp_formatter_attr_find(const ATTR_TYPES type,
 ...
 */
static int bootcamp_formatter_attr_lookup(const ATTR_TYPES type,
		const ntfschar *name, const u32 name_len,
		...
		const u32 val_len, ntfs_attr_search_ctx *ctx)
{
	...
	if (!base_ni || !NInoAttrList(base_ni) || type == AT_ATTRIBUTE_LIST)
		return bootcamp_formatter_attr_find(type, name, name_len, ic,
				val, val_len, ctx);
	errno = EOPNOTSUPP;
	return -1;
}
		...
		const runlist *rl, const u8 *val, const s64 val_len)
{
	...
	*/
	...
	if (!bootcamp_formatter_attr_lookup(type, uname, uname_len, ic, 0, NULL,
		0, ctx))
	{
		err = -EEXIST;
		goto err_out;
		...
		const u8 *val, const u32 val_len)
{
	...
	/*
	if (base record)
		bootcamp_formatter_attr_lookup();
	else
	*/
	...
	if (!bootcamp_formatter_attr_lookup(type, uname, uname_len, ic, 0, val,
			val_len, ctx)) {
		err = -EEXIST;
		goto err_out;
	}
		...
		le32 security_id)
{
	...
	si.creation_time = bootcamp_formatter_time();
...
static BOOL non_resident_unnamed_data(MFT_RECORD *m)
{
	...
	ctx = ntfs_attr_get_search_ctx(NULL, m);
	if (ctx && !bootcamp_formatter_attr_find(AT_DATA,
...
static ntfs_time stdinfo_time(MFT_RECORD *m)
{
	...
	ctx = ntfs_attr_get_search_ctx(NULL, m);
	if (ctx && !bootcamp_formatter_attr_find(AT_STANDARD_INFORMATION,
				(const ntfschar*)NULL, 0, CASE_SENSITIVE,
				(u8*)NULL, 0, ctx)) {
		...
		info_time = bootcamp_formatter_time();
	}
	if (ctx)
		ntfs_attr_put_search_ctx(ctx);
	return (info_time);
		...
		const FILE_NAME_TYPE_FLAGS file_name_type)
{
	...
	if (bootcamp_formatter_attr_lookup(AT_STANDARD_INFORMATION, AT_UNNAMED,
		...
		INDEX_ALLOCATION **idx)
{
	...
	err = bootcamp_formatter_attr_lookup(AT_INDEX_ROOT, uname, uname_len,
			ic, 0, NULL, 0, ctx);
	ntfs_ucsfree(uname);
	if (err) {
		err = -ENOTDIR;
 ...
 * simplified search method, works for bootcamp_formatter
 */
		...
		MFT_RECORD *m, ntfschar *name, u32 name_size, ATTR_TYPES type)
{
	...
	if (bootcamp_formatter_attr_lookup(AT_INDEX_ROOT, name, name_size,
...
static int initialize_quota(MFT_RECORD *m)
{
	...
	idx_entry_q1_data->change_time = bootcamp_formatter_time();
	...
	idx_entry_q2_data->change_time = bootcamp_formatter_time();
		...
		FILE_NAME_ATTR *file_name, u32 file_name_size)
{
	 ...
	 * method which is sufficient for bootcamp_formatter but no good
	 * whatsoever in real world scenario. (AIA)
	 */
	...
	while ((char*)ie < index_end && !(ie->ie_flags & INDEX_ENTRY_END)) {
		...
		*/
		 ...
		 * fine for bootcamp_formatter where we don't use POSIX
		 * namespace at all and hence this following code is luxury.
		 * (AIA)
		 */
...
/**
 * bootcamp_formatter_cleanup
 */
static void bootcamp_formatter_cleanup(void)
{
...
/**
 * bootcamp_formatter_open_partition -
 */
static BOOL bootcamp_formatter_open_partition(ntfs_volume *vol)
{
	BOOL result = FALSE;
	int i;
	struct stat sbuf;
	...
	return result;
err:
	ntfs_log_error(ignore_warn_msg);
done:
	return result;
}

/**
 * bootcamp_formatter_get_page_size - detect the system's memory page size.
 */
static long bootcamp_formatter_get_page_size(void)
{
	long page_size;
#ifdef _SC_PAGESIZE
...
/**
 * bootcamp_formatter_override_vol_params -
 */
static BOOL bootcamp_formatter_override_vol_params(ntfs_volume *vol)
{
	s64 volume_size;
	int i;
	BOOL winboot = TRUE;
...
#if BOOTCAMP_FORMATTER_ENABLE_WIPEFS
	{
		/* Note: Assuming unix_io ops where opaque field d_private in
		 * ntfs_device contains an int to be interpreted as a file
		 * descriptor. */
		int err;
		wipefs_ctx handle;

		err = wipefs_alloc(*((int*) vol->dev->d_private), 0, &handle);
		if (err) {
			ntfs_log_error("Error when allocating wipefs handle: "
				"%d (%s)\n",
				err, strerror(err));
		}
		else {
			...
			if(err) {
				ntfs_log_error("Error when wiping device: %d "
					"(%s)\n",
					err, strerror(err));
			}
	...
	opts.num_sectors -= i;
	/* Determine partition start sector. */
	...
	if (!winboot) {
		...
		ntfs_log_warning("To boot from a device, Windows needs the "
				"'partition start sector' to be set.\n");
...
/**
 * bootcamp_formatter_initialize_bitmaps -
 */
static BOOL bootcamp_formatter_initialize_bitmaps(void)
{
	u64 i;
	int mft_bitmap_size;
	...
	g_dynamic_buf_size = bootcamp_formatter_get_page_size();
...
/**
 * bootcamp_formatter_initialize_rl_mft -
 */
static BOOL bootcamp_formatter_initialize_rl_mft(void)
{
	int j;
	BOOL done;

	/* Determine the the mft lcn. */
	if (!g_mft_lcn) {
		/*
	...
	/* Default MFT zone size is 12.5% of the volume. */
	g_mft_zone_end = g_vol->nr_clusters >> 3;
...
/**
 * bootcamp_formatter_initialize_rl_logfile -
 */
static BOOL bootcamp_formatter_initialize_rl_logfile(void)
{
	int j;
	u64 volume_size;
...
/**
 * bootcamp_formatter_initialize_rl_boot -
 */
static BOOL bootcamp_formatter_initialize_rl_boot(void)
{
	int j;
...
/**
 * bootcamp_formatter_initialize_rl_bad -
 */
static BOOL bootcamp_formatter_initialize_rl_bad(void)
{
...
/**
 * bootcamp_formatter_sync_index_record
 *
 * (ERSO) made a function out of this, but the reason for doing that
 * disappeared during coding....
 */
static BOOL bootcamp_formatter_sync_index_record(INDEX_ALLOCATION* idx,
		MFT_RECORD* m, ntfschar* name, u32 name_len)
{
	int i, err;
	ntfs_attr_search_ctx *ctx;
	ATTR_RECORD *a;
	...
	if (bootcamp_formatter_attr_lookup(AT_INDEX_ALLOCATION, name, name_len,
			CASE_SENSITIVE, 0, NULL, 0, ctx)) {
		ntfs_attr_put_search_ctx(ctx);
...
static int create_backup_boot_sector(u8 *buff)
{
	...
	if (g_vol->dev->d_ops->seek(g_vol->dev, (opts.num_sectors + 1) *
			opts.sector_size - size, SEEK_SET) == (s64)-1) {
		ntfs_log_perror("Seek failed");
		goto bb_err;
	}
	if (size > 8192)
		size = 8192;
	bw = bootcamp_formatter_write(g_vol->dev, buff, size);
	if (bw == size)
		return 0;
	e = errno;
	if (bw == -1LL)
...
/**
 * bootcamp_formatter_create_root_structures -
 */
static BOOL bootcamp_formatter_create_root_structures(void)
{
	NTFS_BOOT_SECTOR *bs;
	MFT_RECORD *m;
	leMFT_REF root_ref;
	...
	if (!err) {
		...
		if (bootcamp_formatter_attr_lookup(AT_FILE_NAME, AT_UNNAMED, 0,
				CASE_SENSITIVE, 0, NULL, 0, ctx)) {
			ntfs_attr_put_search_ctx(ctx);
	...
	if (!err)
		err = create_hardlink(g_index_block, root_ref, m,
				...
				FILE_NAME_WIN32_AND_DOS);
	/* mft_bitmap is not modified in bootcamp_formatter; no need to sync it
	 * later. */
	if (!err)
	...
	bs->bpb.sectors_per_track = cpu_to_le16(0);
	bs->bpb.heads = cpu_to_le16(0);
...
/**
 * bootcamp_formatter_redirect
 */
static int bootcamp_formatter_redirect(struct bootcamp_formatter_options *opts2)
{
	int result = 1;
	...
	if (!opts2) {
		ntfs_log_error("Internal error: invalid parameters to "
			"bootcamp_formatter_options.\n");
		goto done;
	}
	/* Initialize the random number generator with the current time. */
	srandom(le64_to_cpu(bootcamp_formatter_time())/10000000);
	...
	/* Open the partition. */
	if (!bootcamp_formatter_open_partition(g_vol))
		goto done;
	/*
	 * Decide on the sector size, cluster size, mft record and index record
	 * sizes as well as the number of sectors/size, etc.
	 */
	if (!bootcamp_formatter_override_vol_params(g_vol))
		goto done;
	/* Initialize $Bitmap and $MFT/$BITMAP related stuff. */
	if (!bootcamp_formatter_initialize_bitmaps())
		goto done;
	/* Initialize MFT & set g_logfile_lcn. */
	if (!bootcamp_formatter_initialize_rl_mft())
		goto done;
	/* Initialize $LogFile. */
	if (!bootcamp_formatter_initialize_rl_logfile())
		goto done;
	/* Initialize $Boot. */
	if (!bootcamp_formatter_initialize_rl_boot())
		goto done;
	...
	if (!g_buf)
		goto done;
	/* Create runlist for $BadClus, $DATA named stream $Bad. */
	if (!bootcamp_formatter_initialize_rl_bad())
		goto done;
	/* Create NTFS volume structures. */
	if (!bootcamp_formatter_create_root_structures())
		goto done;
	/*
	...
	if (!bootcamp_formatter_sync_index_record(g_index_block,
			(MFT_RECORD*)(g_buf + 5 * g_vol->mft_record_size),
			NTFS_INDEX_I30, 4))
		goto done;
	...
	if (bootcamp_formatter_attr_lookup(AT_DATA, AT_UNNAMED, 0,
				CASE_SENSITIVE, 0, NULL, 0, ctx)) {
	...
	for (i = 0; i < g_mft_size / (s32)g_vol->mft_record_size; i++) {
		lw = ntfs_mst_pwrite(g_vol->dev, pos, 1, g_vol->mft_record_size,
...
#ifndef EXEC_NAME
	ntfs_log_quiet("%s completed successfully. Have a nice day.\n", EXEC_NAME);
#endif
	result = 0;
done:
	ntfs_attr_put_search_ctx(ctx);
	bootcamp_formatter_cleanup();	/* Device is unlocked and closed here */
	return result;
}
...
{
	fprintf(stderr, "%s - NTFS formatter for Boot Camp volumes.\n",
			EXEC_NAME);
	fprintf(stderr, "Copyright (C) 2015 Tuxera Inc.\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "usage: %s [-v <volume_name>] <device>\n", EXEC_NAME);
	exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
	...
	bootcamp_formatter_init_options(&opts);
	if(label) {
		opts.label = label;
	}
	...
	if(!ret) {
		fprintf(stderr, "Boot Camp NTFS volume created "
			"successfully.\n");
	}
