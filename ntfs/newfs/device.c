/**
 * device.c - Low level device io functions. 
 *
...
#define DKIOCGETBASE	_IOR('d', 73, uint64_t)
#endif
	{
		...
		if (!dev->d_ops->ioctl(dev, DKIOCGETBASE, &partition_offset)) {
			uint32_t sector_size = 0;

			ntfs_log_debug("DKIOCGETBASE got partition offset: "
					"%llu (0x%llx) bytes\n",
					(unsigned long long) partition_offset,
					(unsigned long long) partition_offset);


			if (!dev->d_ops->ioctl(dev, DKIOCGETBLOCKSIZE,
					&sector_size))
			{
				ntfs_log_debug("DKIOCGETBLOCKSIZE sector size "
						"= %lu bytes\n",
						(unsigned long) sector_size);

				if (sector_size > 0) {
					const uint64_t partition_start_sector =
						partition_offset / sector_size;

					if (partition_offset % sector_size) {
						/* Invalid partition offset. Not
						 * a multiple of the logical
						 * sector size. */
						errno = ERANGE;
					}
					else if (partition_start_sector >
							INT64_MAX)
					{
						/* Partition offset is larger
						 * than what we can return
						 * through this function. */
						errno = ERANGE;
					}
					else {
						ntfs_log_debug("Returning "
							"partition start "
							"sector: %lld "
							"(0x%llx)\n",
							(long long)
				...
				else if (sector_size != -1) {
					/* Unexpected return value from ioctl.
					 * Since it is an unexpected error while
					 * issuing an I/O request we choose EIO.
					 */
					errno = EIO;
				}
			}
