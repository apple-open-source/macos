...
#ifdef __APPLE__
	atf_expect_fail "touch interprets -1 as an error"
#endif
...
#ifdef __APPLE__
atf_test_case rdar70075417
rdar70075417_head() {
	atf_set "descr" "Verifying that touch(1)ing an existing file sets " \
		"its modification time to be later than its creation time."
}
rdar70075417_body() {
	file_name=$(mktemp XXXXXX)
	file_ctime=$(stat -f %Fc ${file_name})

	atf_check touch ${file_name}
	file_mtime=$(stat -f %Fm ${file_name})

	atf_check -o match:1 bc -le "${file_mtime} > ${file_ctime}"
}
#endif /* __APPLE__ */
...
#ifdef __APPLE__
	atf_add_test_case rdar70075417
#endif /* __APPLE__ */
}
