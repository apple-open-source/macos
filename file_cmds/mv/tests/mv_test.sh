...
exec /usr/bin/id "$@"
EOF
	atf_check cp script copy
	atf_check chmod a+rx,u+s script
	atf_check -o inline:"104755\n" stat -f%p script
	atf_check mv script mp/script
	atf_check -o inline:"104755\n" stat -f%p mp/script
	atf_check mv mp/script dir/script
	atf_check cmp -s copy dir/script
