...
struct auto_root_list {
    uint32_t                        count;
    auto_memory_reference_list_t    roots[0];       // variable length structure, size = count * sizeof(auto_memory_reference_list_t) + sum(i, 0, count) { roots[i].count * sizeof(auto_memory_reference_t) }
};
