...
// store thread local and global objects into instance variable using objc_memmove_collectable, test for resurrection warning
...
- (void)finalize {
    if (isGlobal) {
        ...
        if (counter++ == 0) {
            objc_memmove_collectable(&global->string, &self, sizeof(void *));
            globalDidFinalize = 1;
        }
    }
    else  {
