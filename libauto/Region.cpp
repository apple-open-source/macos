...
namespace Auto {
    ...
    Region::Region(Zone *zone, void *address, usword_t size, usword_t nsubzones) {
        ...
        size -= bytes_per_bitmap;
        _pinned.set_address(displace(address, size));
        _pinned.set_size(0);
    ...
    bool Region::add_subzone(Admin *admin) {
        ...
        Subzone *subzone = NULL;
        {
            ...
            usword_t newBitmapSize = Bitmap::bytes_needed(_n_quantum);
            _pending.set_size(newBitmapSize);
            _marks.set_size(newBitmapSize);
            _pinned.set_size(newBitmapSize);
        }
