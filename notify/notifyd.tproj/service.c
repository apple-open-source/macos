 ...
 */
int
service_open_path_private(const char *name, client_t *client, const char *path, uid_t uid, gid_t gid, uint32_t flags)
{
	name_info_t *n;
	svc_info_t *info;
	path_node_t *node;
	...
	node->context32 = client->client_id;
	...
	dispatch_source_set_event_handler(node->src, ^{
		dispatch_async(global.work_q, ^{
			if (0 == dispatch_source_testcancel(node->src))
			{
				daemon_post_client(node->context32);
			}
		});
	});
...
int
service_open_timer_private(const char *name, client_t *client, const char *args)
{
	uint32_t t;
	time_t s, f, e;
	int32_t d;
	...
	if (timer == NULL) return NOTIFY_STATUS_FAILED;
	timer->context32 = client->client_id;
	...
	dispatch_source_set_event_handler(timer->src, ^{
		dispatch_async(global.work_q, ^{
			if (0 == dispatch_source_testcancel(timer->src))
			{
				daemon_post_client(timer->context32);
			}
		});
	});
