...
static errcode_t rw_setup(profile_t profile)
{
	...
	if ((file->data->flags & PROFILE_FILE_SHARED) != 0) {
		...
		retval = pthread_mutex_init(&new_data->lock, NULL);
		if (retval == 0) {
		    new_data->root = NULL;
		    new_data->flags = file->data->flags & ~(PROFILE_FILE_SHARED | PROFILE_FILE_HAVE_DATA);
...
{	
	...
	retval = pthread_mutex_lock(&profile->first_file->data->lock);
	if (retval)
	    return retval;
	...
	for (cpp = names; cpp[1]; cpp++) {
		...
		retval = profile_find_node(section, *cpp, 0, 1,
					   &state, &section);
		if (retval) {
		    pthread_mutex_unlock(&profile->first_file->data->lock);
		    return retval;
		}
	}
