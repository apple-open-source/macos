				 ...
				 gss_OID mech, void **gssCreds)
{
	...
	if (maj != GSS_S_COMPLETE)
		os_log_info(OS_LOG_DEFAULT, "Acquiring NTLM creds for %s\%s failed. GSS returned %d",
			domain, user, maj);
...
smb_release_gss_cred(void *gssCreds, int error)
{
	...
	if (error == 0) {
		...
		(void)gss_destroy_cred(&minor_status, (gss_cred_id_t *)&gssCreds);
	}
}
...
get_realm(void)
{
	...
	if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s: Couldn't initialize kerberos: %d", __FUNCTION__, error);
		return (NULL);
	}		
	error = krb5_get_default_realm(ctx, &realm);
	if (error) {
		msg = krb5_get_error_message(ctx, error);
		os_log_debug(OS_LOG_DEFAULT, "%s: Couldn't get kerberos default realm: %s", __FUNCTION__, msg);
...
smb_gss_get_cred_list(struct smb_gss_cred_list **list, gss_OID mech)
{
	...
	M = gss_iter_creds_f(&minor_status, 0, mech, &ctx, cred_iter);
	
	if (M == GSS_S_COMPLETE) {
		/* Only wait if gss_iter_creds_f did NOT return an error */
		dispatch_semaphore_wait(ctx.s, DISPATCH_TIME_FOREVER);
	} else {
		os_log_debug(OS_LOG_DEFAULT, "%s: gss_iter_creds returned: 0x%x", __FUNCTION__, M);
	}
