				...
				int *fid)
{
	...
	if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s: smb_usr_rq_put_dstring failed syserr = %s",
	...
	*namelenp = htoles((uint16_t)nmlen);
	...
	if (error != 0) {
		os_log_debug(OS_LOG_DEFAULT, "%s: smb_usr_rq_simple failed, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
	}
	...
	if (md_get_uint8(mdp, &wc) != 0 || 
		...
		error = EIO;
		os_log_debug(OS_LOG_DEFAULT, "%s: bad word count, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
 	}
						  ...
						  uint32_t flags2, uint32_t *nt_error)
{
	...
	if (error && (error != ENOENT) && (error != ENOTDIR)) {
		os_log_debug(OS_LOG_DEFAULT, "%s failed, syserr = %s", __FUNCTION__, strerror(error));
	}
	if (nt_error) {
