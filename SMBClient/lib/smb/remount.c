			...
			size_t newlen)
{
	...
	if (sysctlnametomib(sysstr, ctlname, &ctllen) == -1) {
		os_log_error(OS_LOG_DEFAULT, "%s: sysctlnametomib(%s)", __FUNCTION__, 
					 sysstr);
		return -1;
	};
	ctlname[ctllen] = op;
...
SysctlRemountInfo(fsid_t fsid, struct smb_remount_info *info)
{
	...
	if (SysctlByFSID(SMBFS_SYSCTL_REMOUNT_INFO, fsid, info, &size, NULL, 0) == -1) {
		os_log_error(OS_LOG_DEFAULT, "%s: failed for 0x%x:0x%x - %s", 
					   ...
					   uint32_t	maxTimer)
{
	...
	if (!openOptions) {
		os_log_error(OS_LOG_DEFAULT, "%s: Couldn't create open options for %s",
					 __FUNCTION__, url);
		error = ENOMEM;
		goto done;
	}
	...
	while (difftime(time(NULL), start_time) < maxTimer ) {
		...
		os_log_debug(OS_LOG_DEFAULT, "%s: get server info failed %d, sleeping one second, have %d seconds left.",
					  __FUNCTION__, error, 
					 maxTimer - (int)difftime(time(NULL), start_time));
		sleep(1);	/* Wait one second before trying again */
	}
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s: get server info failed from %s with %d", 
	...
	if (authFlags & (SMBV_GUEST_ACCESS | SMBV_SFS_ACCESS | SMBV_PRIV_GUEST_ACCESS)) {
		...
		if (!authInfoDict) {
			os_log_error(OS_LOG_DEFAULT, "%s: Creating authorization dictionary failed for  %s", 
						 __FUNCTION__, url);
			error = ENOMEM;
			goto done;
		}
	...
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s: open session failed from url %s with %d",  
					 __FUNCTION__, url, error);
		goto done;
	}	
	error = smb_share_connect(ctx);
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s: share connect failed from url %s with %d", 
					 __FUNCTION__, url, error);
		goto done;
	}
done:
...
int smb_remount_with_fsid(fsid_t fsid)
{
	...
	os_log_debug(OS_LOG_DEFAULT, "%s: mount url smb:%s", __FUNCTION__,
				 remountInfo.mntURL);
	os_log_debug(OS_LOG_DEFAULT, "%s: client principal name %s", __FUNCTION__,
				 remountInfo.mntClientPrincipalName);
	os_log_debug(OS_LOG_DEFAULT, "%s: client principal name type %d", __FUNCTION__,
				 remountInfo.mntClientPrincipalNameType);
	os_log_debug(OS_LOG_DEFAULT, "%s: authorization flags 0x%x", __FUNCTION__,
				 remountInfo.mntAuthFlags);
	os_log_debug(OS_LOG_DEFAULT, "%s: owner of mount point %d", __FUNCTION__, 
				 remountInfo.mntOwner);
	
	/* Switch to the user that owns the mount */
	error2 = seteuid(remountInfo.mntOwner);
    if (error2) {
		os_log_error(OS_LOG_DEFAULT, "%s: seteuid failed %d for mntOwner",
                     __FUNCTION__, error2);
        goto done;
    }
	...
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s: Could create ctx from url smb:%s", 
					 __FUNCTION__, remountInfo.mntURL);
		error2 = seteuid(rootUID);
        if (error2) {
            os_log_error(OS_LOG_DEFAULT, "%s: seteuid failed %d for rootUID",
                         __FUNCTION__, error2);
        }
	...
	if (smb_tree_conn_fstype(ctx) == SMB_FS_FAT) {
		os_log_error(OS_LOG_DEFAULT, "%s: Could remount url smb:%s found a fat file system", 
					  __FUNCTION__, remountInfo.mntURL);
		error = ENOTSUP;
		goto done;
	}
	...
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s: remount failed for url %s with error = %d", 
					 __FUNCTION__, remountInfo.mntURL, error);
	}
