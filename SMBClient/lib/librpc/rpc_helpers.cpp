...
#if MEMPOOL_DEBUG
        os_log_debug(OS_LOG_DEFAULT, "rpc_mempool(%llu): freeing ptr %p",
...
{
#if MEMPOOL_DEBUG
    os_log_debug(OS_LOG_DEFAULT, "constructing rpc_mempool at %p", this);
#endif
    ptrs.resize(0);
}
...
{
#if MEMPOOL_DEBUG
    os_log_debug(OS_LOG_DEFAULT, "destroying rpc_mempool at %p", this);
#endif
...
#if MEMPOOL_DEBUG
    os_log_debug(OS_LOG_DEFAULT, "rpc_mempool(%llu): allocated ptr %p for %u bytes",
...
#if MEMPOOL_DEBUG
    os_log_debug(OS_LOG_DEFAULT, "rpc_mempool(%llu): freed ptr %p",
			     (unsigned long long)pthread_self(), ptr);
#endif
...
rpc_binding::rpc_binding(const char * string_binding)
{
    ...
    if (status != rpc_s_ok) {
        os_log_error(OS_LOG_DEFAULT, "rpc_binding_from_string_binding failed on <%s> status %#08x",
				 ...
				 const char * EndPoint)
{
    ...
    if (ServerName) {
        ...
        rpc_string_binding_compose(NULL, 
                                   ...
                                   &status);
        if (status != 0) {
            os_log_error(OS_LOG_DEFAULT, "rpc_string_binding_compose failed on <%s>:<%s> status %#08x",
        ...
        rpc_string_binding_compose(NULL, 
                                   ...
                                   &status);
        if (status != 0) {
            os_log_error(OS_LOG_DEFAULT, "rpc_string_binding_compose failed on <%s> status %#08x",
                         EndPoint == NULL ? "nullstr" : EndPoint,
                         status);
    ...
    if (status == 0) {
        ...
        if (binding_string != NULL) {
            ...
            if (status != 0) {
                os_log_error(OS_LOG_DEFAULT, "rpc_string_free failed on <%s> status %#08x",
					 ...
					 dcethread_exc * exc)
{
    ...
    if (status == -1) {
        os_log_debug(OS_LOG_DEFAULT, "unexpected RPC exception: kind=%#x address=%p %s",
