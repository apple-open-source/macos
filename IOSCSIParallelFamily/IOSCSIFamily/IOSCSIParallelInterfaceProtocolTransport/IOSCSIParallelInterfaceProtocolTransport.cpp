...
//--------------------------------------------------------------------------------------
//	е probe	-	Probe for devices we want to match on. This is temporary until Carbon
//				uses SCSITaskLib instead of IOSCSILib.
//--------------------------------------------------------------------------------------


IOService *
IOSCSIParallelInterfaceProtocolTransport::probe ( IOService * provider, SInt32 * score )
{
	
	IOSCSIDevice *		scsiDevice;
	OSNumber *			number;
	UInt8				deviceType = 0xFF;
	
	if ( !super::probe ( provider, score ) )
		return NULL;
	
	scsiDevice = OSDynamicCast ( IOSCSIDevice, provider );
	if ( scsiDevice == NULL )
		return NULL;
	
	number = ( OSNumber * ) provider->getProperty ( kSCSIPropertyDeviceTypeID );
	if ( number != NULL )
	{
		
		deviceType = number->unsigned8BitValue ( ) & kINQUIRY_PERIPHERAL_TYPE_Mask;
		switch ( deviceType )
		{
			
			case 0x00:	// Block
			case 0x01:	// Sequential
			case 0x05:	// MMC
			case 0x07:	// MagnetoOptical
			case 0x08:	// MediumChanger
			case 0x0E:	// RBC
				*score = 10000;
				return this;
				break;
	...
	if ( results.returnCode != kIOReturnSuccess )
	{
		...
		if ( results.scsiStatus == kSCSIStatusCheckCondition )
		{
			...
			if ( results.requestSenseDone )
			{
				...
				SetAutoSenseData ( clientData->scsiTask, ( SCSI_Sense_Data * ) &clientData->senseBytes );
...
IOSCSIParallelInterfaceProtocolTransport::AllocateSCSICommandObjects ( void )
{
	...
	for ( UInt32 index = 0; index < kSCSICommandPoolSize; index++ )
	{
		...
		clientData->senseBuffer = IOMemoryDescriptor::withAddress ( &clientData->senseBytes,
																	sizeof ( SCSISenseData ),
																	kIODirectionIn );
