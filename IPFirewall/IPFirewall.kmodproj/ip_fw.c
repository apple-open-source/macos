...
/*
 * Returns 1 if the tcp flags match.
 */
static int
tcpflg_match(struct tcphdr *tcp, struct ip_fw *f)
{
    ...
    /*
     * We ignore the IP_FW_TCPF_ESTAB bit in fw_tcpf.
     * It is not a tcp flag, it's a bit set to indicate
     * we should allow established connections.
     */
    test_set = f->fw_tcpf & ~IP_FW_TCPF_ESTAB;
...
kern_return_t ipfw_load(struct kmod_info *ki, void *data)
{
    int s;
    int	funnel_state;
...
kern_return_t ipfw_unload(struct kmod_info *ki, void *data)
{
    int s;
    int	funnel_state;
