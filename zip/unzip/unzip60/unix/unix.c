...
#ifdef __APPLE__
/*
 * This function is called by the extractor to ensure the existence of
 * containing directories before extracting each file.  It has multiple
 * modes of operation selected by a portion of the flag parameter.  ROOT
 * mode is called directly from the extractor; the other modes are called
 * indirectly via mapname().
 *
 * ROOT
 *
 * This mode is called prior to extraction if uO.exdir is non-null (which
 * is always the case in Apple code, because we initialize it to "." if
 * the user did not specify an extraction directory).  It checks if the
 * extraction directory exists, creates it if it does not, and stores the
 * path in G.rootpath and its length in G.rootlen.  In Apple code, it
 * stores the resolved real path instead of the original value and does
 * not append a path separator.
 *
 * INIT
 *
 * The argument is the full path of the file being extracted, used only to
 * compute the amount of memory needed.  Allocates G.buildpath with enough
 * space for the root path, a path separator, and the provided file name,
 * and initializes it with a copy of G.rootpath.  In Apple code, the root
 * path does not include a trailing path separator, but one will be added
 * later, so we make sure to allocate space for it.
 *
 * APPEND_DIR
 *
 * The argument is a single path component from the path of the file being
 * extracted.  Appends the component to G.buildpath (preceded by a path
 * separator in Apple code), check if the resulting path exists, and
 * creates it if it does not.  In Apple code, if the directory does exist,
 * we check that it isn't a symlink that points outside the extraction
 * directory.
 *
 * APPEND_NAME
 *
 * Like APPEND_DIR, but for the final component of the name of each file.
...
#elif __APPLE__
        if ((G.buildpath = (char *)malloc(strlen(G.filename)+G.rootlen+2))
#else
...
#ifdef __APPLE__
            G.rootpath = realpath(tmproot, NULL);
            free(tmproot);
            if (G.rootpath == NULL) {
                G.rootlen = 0;
                return MPN_ERR_SKIP;
