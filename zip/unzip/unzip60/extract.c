     ...
     __GDEF
{
    ...
    if (G.extract_flag) {
        ...
        if ((error = checkdir(__G__ uO.exdir ? uO.exdir : ".", ROOT)) > MPN_INF_SKIP) {
...
#ifdef __APPLE__
char *fnfilterp(pfx, raw, space, size)   /* convert name to safely printable form */
    ZCONST char *pfx;
    ZCONST char *raw;
    uch *space;
    extent size;
{
    uch *ret = space;
    if (pfx != NULL && *pfx != '\0') {
        (void)fnfilter(pfx, space, size);
        while (*space)
            space++, size--;
        *space++ = '/';
        size--;
    }
    fnfilter(raw, space, size);
    return ret;
} /* end function fnfilter() */
#endif
