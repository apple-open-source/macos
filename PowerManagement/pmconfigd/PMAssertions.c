...
void checkForAsyncAssertions(void *acknowledgementToken)
{
    ...
    for (long j = 0; (j < cnt) && (procs[j] != NULL); j++) {
        ...
        pid_t pid = pinfo->pid;
        sendCheckAssertionsMsg(pinfo, ^(xpc_object_t reply) {
            processAssertionCheckResp(reply, pid);
        });
    }
...
void sendAssertionActivityUpdateMsg(ProcessInfo *pinfo, bool actives_only, void(^responseHandler)(xpc_object_t))
{
    xpc_object_t msg = NULL;
    msg = xpc_dictionary_create(NULL, NULL, 0);
    if (!msg) {
...
static void displayWakeHandler(assertionType_t *assertType, assertionOps op)
{
    ...
    if (level) {
        if (isDisplayAsleep() || inFlightDimRequest()) {
            if (canSustainFullWake()) {
                INFO_LOG("Turning on display for notification wake");
                unblankDisplay();
            } else {
                ERROR_LOG("Cannot sustain full wake for notification wake");
            }
        }
    } else {
