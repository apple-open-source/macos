...
static void dav_start_handler(void *userdata, const char *name, const char **attrs)
{
    ...
    ascii2ebcdic(elem->name, elem->name, strlen(elem->name));
#endif
...
#ifdef CHARSET_EBCDIC
        ascii2ebcdic(attr->name, attr->name, strlen(attr->name));
        ascii2ebcdic(attr->value, attr->value, strlen(attr->value));
#endif
	attr->next = elem->attr;
	elem->attr = attr;
    }
	...
	*pdoc = NULL;
	...
	while ((len = ap_get_client_block(r, buffer, DAV_READ_BLOCKSIZE)) > 0) {
		...
		goto read_error;
            ...
            ebcdic2ascii(buffer, buffer, len);
#endif
