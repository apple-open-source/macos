...
*******************************************************************************/
      ...
      if ( absOfX > kPiScale42 ) 
            ...
            if ( absOfX == infinity.d ) 
                  ...
                  FESETENVD( OldEnvironment.d );             // restore caller's mode
                  return ( 0 );//nan ( TRIG_NAN ) );               // return NaN
                  }
...
double tan (   double x  )
{
      ...
      FPR_z = 0.0f;			 		tablePointer = ( struct tableEntry * ) ( tanatantable - ( 16 * 14 ) );      
      absOfX = __FABS( x );
      FPR_PiDiv4 = kPiDiv4;		 		FPR_256 = 256.0f;
      FPR_kRint = kRint;		 		FPR_one = 1.0f;
      FPR_PiDiv2 = kPiBy2;
