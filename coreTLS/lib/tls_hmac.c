...
struct HMACContext {
	...
	tls_buffer				    outerHashCtx;
	tls_buffer				    innerHashCtx;
	tls_buffer				    currentHashCtx;
};
	...
	HMACContextRef				*hmacCtx)			// RETURNED
{
	...
	tls_buffer kpad = { digest_block_size, pad };
	...
	if (keyLen <= digest_block_size) {
        ...
        tls_buffer keyBuffer = { keyLen, (uint8_t *)keyPtr };
        tls_buffer outBuffer = { digest->digestSize, tmpkey };
...
static int HMAC_Update(
	...
	size_t          dataLen)
{
	tls_buffer       cdata = { dataLen, (uint8_t *)data };
	if(hmacCtx == NULL) {
        check(0);
		return -1;
    }
	...
	size_t          *hmacLen)		// IN/OUT
{
	...
	tls_buffer       digest = { TLS_HMAC_MAX_SIZE, bytes };
	tls_buffer       cdata;
