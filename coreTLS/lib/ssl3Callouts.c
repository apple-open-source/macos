 ...
 *                              ClientHello.random)) + [...];
 */
static int ssl3GenerateKeyMaterial (
	...
	tls_handshake_t ctx)
{
    ...
    tls_buffer   shaContext, md5Context;
    UInt8       *keyProgress;
    size_t      i,j,remaining, satisfied;
    tls_buffer   leader, masterSecret, serverRandom, clientRandom, shaHash, md5Hash;
 ...
 */
static int ssl3GenerateMasterSecret (
	tls_handshake_t ctx)
{
	int    err;
    tls_buffer   shaState, md5State, clientRandom,
...
ssl3CalculateFinishedMessage(
	tls_handshake_t ctx,
	tls_buffer finished, 		// mallocd by caller
	tls_buffer shaMsgState,		// running total
	tls_buffer md5MsgState, 		// ditto
	UInt32 senderID) 			// optional, nonzero for finished message
{
	int        err;
    tls_buffer       hash, input;
...
static int ssl3ComputeFinishedMac (
	tls_handshake_t ctx,
	tls_buffer finished, 		// output - mallocd by caller
	bool isServer)			// refers to message, not us
{
	int serr;
...
static int ssl3ComputeCertVfyMac (
	tls_handshake_t ctx,
	tls_buffer *finished, 		// output - mallocd by caller
