...
const char *sslGetCipherSuiteString(SSLCipherSuite cs)
{
	...
	switch (cs) {
		...
		default:
			sprintf(noSuite, "Unknown (0x%04X)", (unsigned)cs);
			return noSuite;
	}
}

/*
 * Given a tls_protocol_version - typically from SSLGetProtocolVersion -
...
const char *sslGetSSLErrString(OSStatus err)
{
	...
	switch(err) {
        ...
        case errSSLUnexpectedRecord:        return "errSSLUnexpectedRecord";
        case errSSLWeakPeerEphemeralDHKey:  return "errSSLWeakPeerEphemeralDHKey";
...
const char *sslGetClientAuthTypeString(SSLClientAuthenticationType authType)
{
	...
	switch(authType) {
		...
		case tls_client_auth_type_RSASign:         return "RSASign";
		case tls_client_auth_type_DSSSign:         return "DSSSign";
		case tls_client_auth_type_RSAFixedDH:      return "RSAFixedDH";
		case tls_client_auth_type_DSS_FixedDH:     return "DSS_FixedDH";
		case tls_client_auth_type_ECDSASign:       return "ECDSASign";
		case tls_client_auth_type_RSAFixedECDH:    return "RSAFixedECDH";
		case tls_client_auth_type_ECDSAFixedECDH:  return "ECDSAFixedECDH";
		default:
