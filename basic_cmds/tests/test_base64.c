       ...
       T_META_CHECK_LEAKS(NO))
{
    ...
    int retval = b64_ntop((const u_char *)bin_input, 16, b64_output, 512);
    ...
    dt_stat_time_t s = dt_stat_time_create("b64_ntop");

    T_STAT_MEASURE_LOOP(s) {
        b64_ntop((const u_char *)bin_input, 16, b64_output, 512);
    }
...
T_DECL(b64_pton_basic,
       "Test basic functionality of b64_pton and measure the performance",
       T_META_CHECK_LEAKS(NO))
{
    ...
    int retval = b64_pton((const char *)b64_input, bin_output, 64);
    T_ASSERT_GT(retval, 0, "Successful conversion");
    ...
    dt_stat_time_t s = dt_stat_time_create("b64_pton");

    T_STAT_MEASURE_LOOP(s) {
        b64_pton((const char *)b64_input, bin_output, 64);
    }
