...
/*
 * Portions Copyright 2005-2007,2019 Apple Inc.  All rights reserved.
 *
...
proc_argv0(int pid)
{
	...
	if (argmax == 0) {
	    ...
	    if (ret == -1) {
		(void) fprintf(stderr, "%s: can't get max argument size: %s\n",
		    Pn, strerror(errno));
	        Exit(1);
	    }
	}
	...
	if (argbuf == NULL) {
	    ...
	    if (!argbuf) {
		(void) fprintf(stderr, "%s: can't allocate space for argument buffer: %s\n",
		    Pn, strerror(errno));
	        Exit(1);
	    }
	}
	...
	if (ret == -1) {
	    (void) fprintf(stderr, "%s: PID %d: can't get argument buffer: %s\n",
...
gather_proc_info()
{
	...
	short ckscko;			/* socket file only checking status:
					 ...
					 *	   where supported */
	short ckfdo;			/* file descriptors only (including sockets) */
 ...
 */
	...
	/*
	 * The X option list exclusively the file descriptor table
	 */
	if (Fxopt) {
		cckreg = 0;
		ckscko = 1;
 ...
 */
	 ...
	 */
	    ...
	    if ((cmd == NULL) ||
		((cmdlen != 0) && (CmdLim > cmdlen)) ||
		((cmdlen != 0) && (Cmdl && (Cmdni || Cmdnx)))
	       ) {
		char *argv0;
		/*
	 ...
	 */
	    (void) process_fileports(pid, ckfdo ? 0 : ckscko);
#endif	/* PROC_PIDLISTFILEPORTS */

	/*
	 * Loop through the file descriptors.
	 */
	    (void) process_fds(pid, tai.pbsd.pbi_nfiles, ckfdo ? 0 : ckscko);
	/*
	 * Examine results.
	 */
	    if (examine_lproc())
