...
translator fileinfo_t < struct fileglob *F > {
	fi_name = (F == NULL) ? "<none>" :
		...
		F->fg_type == DTYPE_PSXSHM ? "<shared memory>" :
		F->fg_type == DTYPE_PSXSEM ? "<semaphore>" :
		F->fg_type == DTYPE_KQUEUE ? "<kqueue>" :
		F->fg_type == DTYPE_PIPE ? "<pipe>" :
		F->fg_type == DTYPE_FSEVENTS ? "<fsevents>" : "<unknown (BAD fg_type)>";

	fi_dirname = (F == NULL) ? "<none>" :
		F->fg_type != DTYPE_VNODE ? "<unknown (not a vnode)>" :
