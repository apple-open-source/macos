...
translator csinfo_t < struct inpcb *P > {
	...
	ip_saddr = (P != NULL) ? (((P->inp_vflag & 0x2) != 0) ? 
			...
			inet_ntoa((int32_t *)&P->inp_dependladdr.inp46_local.ia46_addr4.s_addr)) : "<null>";
	ip_daddr = (P != NULL) ? (((P->inp_vflag & 0x2) != 0) ?
			inet_ntoa6(&P->inp_dependfaddr.inp6_foreign) :
			inet_ntoa((int32_t *)&P->inp_dependfaddr.inp46_foreign.ia46_addr4.s_addr)) : "<null>";
	protocol = P->inp_ip_p;
};
...
translator ipinfo_t < struct ip * ip > {
	ip_ver = (ip != NULL) ? (ip->ip_vhltl.ip_x.ip_xv & 0xf0) >> 4: 0;
	ip_plength = (ip != NULL) ? 
		((ip->ip_vhltl.ip_x.ip_xlen) - ((ip->ip_vhltl.ip_x.ip_xhl & 0x0f) << 2)) : 0;
	ip_saddr = (ip != NULL) ? inet_ntoa((int32_t *)&ip->ip_src.s_addr) : "<null>";
	ip_daddr = (ip != NULL) ? inet_ntoa((int32_t *)&ip->ip_dst.s_addr) : "<null>";
};
...
translator ipinfo_t < void_ip_t *i> {
	...
	ip_plength = (i != NULL) ? (((*(uint8_t *)i) >> 4 == 4) ? 
		((((struct ip *)i)->ip_vhltl.ip_w & 0xffff) - 
		((((struct ip *)i)->ip_vhltl.ip_x.ip_xhl & 0x0f) << 2)): 
		(((*(uint8_t *)i) >> 4 == 6) ? 
		ntohs(((struct ip6_hdr *)i)->ip6_ctlun.ip6_un1.ip6_un1_plen) : 0)) : 0;
	ip_saddr = (i != NULL) ? ((((*(uint8_t *)i)) >> 4 == 4) ? 
		inet_ntoa((int32_t *)&(((struct ip *)i)->ip_src.s_addr)) : 
		((((*(uint8_t *)i) >> 4) == 6) ?
		inet_ntoa6(&((struct ip6_hdr *)i)->ip6_src) : "<unknown>")) : "<null>"; 
	ip_daddr = (i != NULL) ? (((*(uint8_t *)i) >> 4 == 4) ? 
		inet_ntoa((int32_t *)&((struct ip*)i)->ip_dst.s_addr) : ((((*(uint8_t *)i) >> 4) == 6) ?
...
translator ipv4info_t < struct ip *ip > {
	ipv4_ver = (ip != NULL) ? (ip->ip_vhltl.ip_x.ip_xv & 0xf0) >> 4: 0;
	ipv4_ihl = (ip != NULL) ? ((ip->ip_vhltl.ip_x.ip_xhl & 0x0f) << 2) : 0;
	ipv4_tos = (ip!= NULL) ? ((ip->ip_vhltl.ip_w & 0x00ff0000) >> 16) : 0;
	ipv4_length = (ip != NULL) ? ((ip->ip_vhltl.ip_w & 0xffff) - 
		((ip->ip_vhltl.ip_x.ip_xhl & 0x0f) << 2)) : 0; 
	ipv4_ident = (ip != NULL) ? ip->ip_id : 0;
	ipv4_flags = (ip != NULL) ? (ntohs(ip->ip_off) & 0xe000) : 0;
	ipv4_offset = (ip != NULL) ? (ip->ip_off & 0x1fff) : 0;
	...
	ipv4_saddr = (ip != NULL) ? inet_ntoa((int32_t *)&ip->ip_src.s_addr) : "<null>";
	ipv4_daddr = (ip != NULL) ? inet_ntoa((int32_t *)&ip->ip_dst.s_addr) : "<null>";
	ipv4_hdr = ip;
};
