...
usage(void)
{
	...
	(void) fprintf(stderr, "\t-C\t\ttrace contention events for mutexes and rwlocks\n");
	(void) fprintf(stderr, "\t-H\t\ttrace hold events for mutexes and rwlocks\n");
	(void) fprintf(stderr, "\t-V\t\tprint the dtrace script to run\n");
	(void) fprintf(stderr, "\t-n count\tdisplay only \'count\' entries for each event type\n");
	(void) fprintf(stderr, "\t-s depth\tshow stack trace upto \'depth\' entries\n");
