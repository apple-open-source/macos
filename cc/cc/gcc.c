...
#ifdef NEXT_SEMANTICS /* Use gcc 2.95.2 by default.  */
static char *spec_version = "2.95.2";
#else
...
#else
static char *standard_exec_prefix_1 = "/usr/local/libexec/";
#endif
#else
     ...
     int *warn;
{
  ...
  if (prefix[len - 1] != '/')
  {
     ...
     temp[len] = '/';
     temp[len+1] = 0;
     add_prefix (pprefix, temp, first, require_machine_suffix, warn);
  }
  else
  {
     add_prefix (pprefix, prefix, first, require_machine_suffix, warn);
...
#ifdef NEXT_SEMANTICS
	      if (install_name && p[0] == 'o' && p[1] == '\0')
		if (i + 1 < argc)
		  /* Name to use in case the -install_name flag is absent.  */
     ...
     char **argv;
{
  ...
  machine_suffix = concat4 (arch_family[0], dir_separator_str,
			   spec_version, dir_separator_str);
  just_machine_suffix = concat (arch_family[0], dir_separator_str);
  ...
  if (verbose_flag)
	...
	fprintf (stderr, "gcc driver version %s executing gcc version %s for Apple PDO\n",
      ...
      if (cp)
	{
	  ...
	  for (current_arch = 0; current_arch < arch_count; current_arch++)
	      ...
	      if (multi_arch)
		{
		  ...
		  machine_suffix = concat4 (arch_family[current_arch],
					    dir_separator_str, spec_version,
					    dir_separator_str);
...
#ifdef REPORT_EVENT
	  if (! strcmp (switches[i].part1, "o"))
	    link_output = *switches[i].args;
#endif
