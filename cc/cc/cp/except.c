...
init_exception_processing ()
{
  ...
  d = build_parse_node (INDIRECT_REF, get_identifier ("__eh_pc_gcc_272"));
  d = start_decl (d, declspecs, 0, NULL_TREE);
  DECL_EXTERNAL (d) = 1;
  cp_finish_decl (d, NULL_TREE, NULL_TREE, 0, 0);
  saved_pc = lookup_name (get_identifier ("__eh_pc_gcc_272"), 0);

  declspecs = tree_cons (NULL_TREE, get_identifier ("void"), NULL_TREE);
  d = build_parse_node (INDIRECT_REF, get_identifier ("__eh_type_gcc_272"));
  d = start_decl (d, declspecs, 0, NULL_TREE);
  DECL_EXTERNAL (d) = 1;
  cp_finish_decl (d, NULL_TREE, NULL_TREE, 0, 0);
  saved_throw_type = lookup_name (get_identifier ("__eh_type_gcc_272"), 0);

  declspecs = tree_cons (NULL_TREE, get_identifier ("void"), NULL_TREE);
  d = build_parse_node (INDIRECT_REF, get_identifier ("__eh_value_gcc_272"));
  d = start_decl (d, declspecs, 0, NULL_TREE);
  DECL_EXTERNAL (d) = 1;
  cp_finish_decl (d, NULL_TREE, NULL_TREE, 0, 0);
  saved_throw_value = lookup_name (get_identifier ("__eh_value_gcc_272"), 0);
}
...
#if defined (MACHO_PIC) && defined (RELOAD_PIC_REGISTER)
      /* Re-establish the PIC base register to be appropriate for
...
#if MACHO_PIC
  /* The "__throw" function always needs the pic table. */
  if (flag_pic)
    current_function_uses_pic_offset_table = 1;
#endif
...
#if defined (MACHO_PIC) && defined (RELOAD_PIC_REGISTER)
  /* Here, we must re-establish the PIC base register, aka
     PIC_OFFSET_TABLE_REGNUM.  This is because the "unwind_first"
     has altered the stack frame, including changing that register
     to be the one appropriate for the caller, rather than for
     the compiler-generated "__throw" code being created here.
     This code needs to access the EH tables, and therefore needs
     its P_O_T_R set right.

     In addition, by the time we branch to any "catch" code, we must have
     the register restored to the value for that function.  Either save
     it somewhere (except that the state of the stack frame is rather
     confusing at this point), or reload it at any "catch".  */
