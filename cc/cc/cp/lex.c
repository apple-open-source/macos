...
init_lex ()
{
  ...
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_ID],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_ID]));
  
  ridpointers[(int) RID_IN] = get_identifier ("in");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_IN],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_IN]));
  
  ridpointers[(int) RID_OUT] = get_identifier ("out");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_OUT],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_OUT]));
  
  ridpointers[(int) RID_INOUT] = get_identifier ("inout");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_INOUT],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_INOUT]));
  
  ridpointers[(int) RID_BYCOPY] = get_identifier ("bycopy");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_BYCOPY],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_BYCOPY]));
  
  ridpointers[(int) RID_ONEWAY] = get_identifier ("oneway");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_ONEWAY],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_ONEWAY]));
			  
  forget_protocol_qualifiers ();
#endif /* OBJCPLUS */
#if defined (WIN32) && defined (NEXT_PDO)
  ridpointers[(int) RID_STDCALL] = get_identifier ("stdcall");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_STDCALL],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_STDCALL]));

  ridpointers[(int) RID_DECLSPEC] = get_identifier ("declspec");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_DECLSPEC],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_DECLSPEC]));

  ridpointers[(int) RID_DLLIMPORT] = get_identifier ("dllimport");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_DLLIMPORT],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_DLLIMPORT]));

  ridpointers[(int) RID_DLLEXPORT] = get_identifier ("dllexport");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_DLLEXPORT],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_DLLEXPORT]));
#endif
...
#ifdef OBJCPLUS
      || yychar == OBJECTNAME || yychar == CLASSNAME
#endif
     )
    {
      yychar = IDENTIFIER;
...
#ifdef OBJCPLUS
	    extern int warn_most;
	    if (current_lang_name != lang_name_objc || !warn_most)
#endif
