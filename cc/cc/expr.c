  ...
  if (code != QUEUED)
	  ...
	  && GET_CODE (XEXP (x, 0)) == QUEUED && !modify)
	{
	  ...
	  /* ??? THIS CODE IS PROBABLY WRONG!!!
		 x should not be modified in place!!!  */
	  register rtx new = x;
	  XEXP (new, 0) = QUEUED_VAR (y);
#else
     ...
     enum expand_modifier modifier;
{
  ...
  switch (code)
		...
		&& CONSTANT_P (XEXP (DECL_RTL (exp), 0))))
	{
	  ...
	  return
#endif
	    change_address (DECL_RTL (exp), mode, addr);
#ifdef WIN32
	  if (DECL_DLLIMPORT (exp))
	  {
		addr = gen_rtx (MEM, Pmode, force_reg (Pmode, addr));
	  }
	  return addr;
#endif
	}
	...
	return
#endif
	  change_address (DECL_RTL (exp), GET_MODE (DECL_RTL (exp)),
			  XEXP (DECL_RTL (exp), 0));
#ifdef WIN32
	  if (DECL_DLLIMPORT (exp))
	    addr = gen_rtx (MEM, Pmode, force_reg (Pmode, addr));
	...
	return
#endif
	  change_address (DECL_RTL (exp), VOIDmode,
...
#ifdef TARGET_TOC
      /* PPC FP regs OVERLAY the GPR's, depends on fn signature */
      for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
	{
	  if (FP_REGNO_P (regno))
	    if (apply_args_reg_offset[regno] >= 24)
	      apply_args_reg_offset[regno] -= 24;
	}
#endif
    }
  return size;
     ...
     rtx function, arguments, argsize;
{
  ...
  /* For now use SAVE_NONLOCAL on PPC architectures as it does not
	have SAVE_BLOCK.  This should possibly be changed to check
	the insn_flag HAVE_save_stack_block? */
#ifdef TARGET_TOC
