     ...
     tree ttl, ttr;
{
  ...
  if (! pedantic)
    {
      /* Ignore pointer qualifiers recursively.
	 This way char ** and const char ** are compatible.  */
     ...
     int require_constant, constructor_constant;
{
  ...
  if (inside_init && TREE_TYPE (inside_init) != 0
	  ...
	  || (code == ARRAY_TYPE
	      && comptypes (type, TREE_TYPE (inside_init)))
	  || (code == POINTER_TYPE
...
/* 
   Answer if DECL is currently not initialized, but will be
   shortly.  That is, if we're in the middle of initializing it.
 */

int
lang_decl_being_initialized (decl)
     tree decl;
{
  struct initializer_stack *p;
  if (!decl)
  ...
  if (decl == constructor_decl)
    return 1;
  for (p = initializer_stack; p; p = p->next)
    if (decl == p->decl)
      return 1;
  return 0;
}
     ...
     tree value;
{
  ...
  /* Sometimes an error-mark will cause an infinite loop */
...
/* Initializers for const declarations should be kept
   longer than to the end of the declaration, so that 
   the optimizer can inline them.  */

void
push_momentary_for_initlist ()
{
  push_momentary ();
  if (   constructor_decl
      && TREE_READONLY (constructor_decl)
      && ! TREE_SIDE_EFFECTS (constructor_decl)
      && TREE_CODE (constructor_decl) == VAR_DECL)
    {
      suspend_momentary ();
    }
}
