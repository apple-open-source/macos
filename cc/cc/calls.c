...
#ifdef _WIN32 
      is_stdcall = TYPE_STDCALL (TREE_TYPE (p));
#endif /* _WIN32 */
    }
...
#ifdef _WIN32
	/* If the tree is marked as stdcall, then append @ and the size of 
	   the parameters to the end of the assembler_name identifier. */
	if( DECL_STDCALL(fndecl) )
	{
		char* newName;
		char parm_size[255];

		_ltoa( args_size.constant, parm_size, 10 );

		// to allow for '@' and \0
		newName = (char*)malloc( strlen( IDENTIFIER_POINTER(DECL_NAME(fndecl)) ) 
			+ strlen( parm_size ) + 2 ); 

		sprintf( newName, "%s@%s", IDENTIFIER_POINTER(DECL_NAME(fndecl)), parm_size );
		DECL_ASSEMBLER_NAME(fndecl) = get_identifier( newName );
		free( newName );
		XSTR( XEXP(DECL_RTL(fndecl), 0), 0 ) = IDENTIFIER_POINTER(DECL_ASSEMBLER_NAME(fndecl));

		DECL_DLLIMPORT(fndecl) = 0;
	}

      /* Get a SYMBOL_REF rtx for the function address.  */
  	  if (DECL_DLLIMPORT (fndecl))
	  {
		rtx loc = XEXP( DECL_RTL (fndecl), 0 );
	  	if (DECL_DLLIMPORT (fndecl))
		{
			rtx addr = copy_rtx (loc);
			PUT_MODE (addr, Pmode);

			funexp = gen_rtx (MEM, GET_MODE (loc), force_reg (Pmode, addr));
	  	}
  	  }
	  else
#endif /* _WIN32 */
...
#ifdef _WIN32
  if (is_stdcall)
    TYPE_STDCALL (TREE_TYPE (funtype)) = 1;
#endif /* _WIN32 */
