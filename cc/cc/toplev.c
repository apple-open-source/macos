...
#ifdef NEXT_SEMANTICS
/* Copied from egcs 1.1.2 - unimplemented, but allows egcs specs file
   to be used.  */
...
#ifdef NEXT_SEMANTICS
  ,
  {"optimize-register-move", &flag_regmove, 1}, /* Copied from egcs 1.1.2.  */
  {"fppc", &flag_fppc, 1},
     ...
     char *name;
{
...

  {
	...
	if (warn_unused
	    && ((TREE_CODE (decl) == FUNCTION_DECL && ! DECL_SAVED_INSNS (decl))
     ...
     char **envp;
{
  ...
  /* krab@next: Sometimes backtraces do not work when
     the stack has been set to its maximum.  This is a
