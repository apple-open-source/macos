  ...
  for (i = 1; i < argc; i++) {
	...
	if (!strcmp (argv[i], "-pedantic"))
	  ...
	  pedantic_errors = 1;
	    ...
	    warning ("-precomp option not supported");
	...
	break;
      ...
      case 'v':
	fprintf (stderr, "GNU CPP version %s", "2.6.1");
...
#ifdef WIN32
{
	...
	if( next_root && *next_root )
	{
		...
		{
			char* new_fname = (char*)xmalloc(strlen(next_root) + strlen(fix_includes[i].fname) + 1);
  ...
  if (!no_standard_includes) {
	...
	{
		char* next_root = getenv( "NEXT_ROOT" );
		char* new_name = (char*)xmalloc(strlen(next_root) + strlen(framework_defaults_array[hdr].fname) + 1);

		if (next_root && next_root[strlen(next_root)-1] == '/')
			next_root[strlen(next_root)-1] = '\0';
		sprintf (new_name, "%s%s", next_root, framework_defaults_array[hdr].fname);
       ...
       if (stat(framework_defaults_array[hdr].fname, &dummy) == 0) {
          // frmaework directory actually exists
          struct file_name_list *new
             = (struct file_name_list *) xmalloc (sizeof (struct file_name_list));
          new->control_macro = 0;
