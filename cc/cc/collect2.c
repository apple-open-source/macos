...
#include <sys/param.h>
#endif /* defined(NEXT_PDO) && defined(hpux) */
...
#if defined(NEXT_PDO) && defined(__svr4__) && defined(sun)
#define COLLECT_ONLY
#define INIT_NAME_FORMAT	"_init"
#define FINI_NAME_FORMAT	"_fini"
#endif
...
#if (defined (LDD_SUFFIX) || SUNOS4_SHARED_LIBRARIES || defined(XCOFF_SCAN_LIBS)) && (!defined (hpux) || !defined (NEXT_PDO))
...
#if defined(NEXT_PDO) && defined(USING_SVR4_H) /* i.e., Solaris */
static int prepends_underscore = 0;
#else
 ...
 */
static char **tmp_fl_filenames = NULL;
static int ntmp_fl_filenames = 0;

/*
 * add_to_tmp_fl_file() takes a pointer to the string which is the -filelist
 ...
 * file.  The temporary file name with is then returned so it can be added to
 * argument list to link as "-c" tmpfile.
 */
static
char *
add_to_tmp_fl_file(
char *filelist_arg)
{
    char *comma, *dirname, *tmppath, filename[MAXPATHLEN + 2], *p;
    int dirsize, filesize;
    int offset;
    FILE *stream, *tmp_fl_file;
    char *tmp_fl_filename;
	...
	if(comma != NULL){
	    *comma = '\0';
	    ...
	    if(dirsize + 2 > MAXPATHLEN){
		*comma = ',';
		fatal("directory name too long for -filelist argument: %s",
		      filelist_arg);
	    } 
	...
	if(stream == NULL)
	    fatal("can't open -filelist argument: %s", filelist_arg);

	if(comma != NULL)
	    *comma = ',';

	tmp_fl_filename = (char *)malloc(L_tmpnam);
	tmpnam(tmp_fl_filename);
	tmp_fl_file = fopen(tmp_fl_filename, "w");
	if(tmp_fl_file == NULL)
	    fatal("can't create temporary file: %s to expand -filelist %s "
		  "argument", tmp_fl_filename, filelist_arg);

	tmp_fl_filenames = (char **)realloc(tmp_fl_filenames,
					    sizeof(char *)
					    * (ntmp_fl_filenames+1));
	tmp_fl_filenames[ntmp_fl_filenames++] = tmp_fl_filename;

	while(fgets(filename+dirsize, MAXPATHLEN+2-dirsize, stream) != NULL){
	    filesize = strlen(filename);
	    if(filesize > MAXPATHLEN &&
	      (filesize != MAXPATHLEN+1 || filename[filesize-1] != '\n'))
		fatal("file name constructed from -filelist %s argument too "
		      "long (%s)", filelist_arg, filename);
	    for(p = filename; *p != '\0'; p++)
		if(*p == '/')
		    *p = DIR_SEPARATOR;
	    if(fputs(filename, tmp_fl_file) == EOF)
		fatal("can't write to temporary file: %s to expand -filelist "
		      "%s argument", tmp_fl_filename, filelist_arg);
	}

	if(fclose (tmp_fl_file) == EOF)
	    fatal("can't close temporary file: %s to expand -filelist %s "
		  "argument", tmp_fl_filename, filelist_arg);
	return(tmp_fl_filename);
}
#endif /* defined(NEXT_PDO) && defined(hpux) */
     ...
     int status;
{
  ...
  for (i = 0; i < ntmp_fl_filenames; i++)
    maybe_unlink (tmp_fl_filenames[i]);
#endif /* defined(NEXT_PDO) && defined(hpux) */
...
#ifdef NEXT_PDO
  if (nm_file_name == 0)
    nm_file_name = find_a_file (&path, "/usr/ccs/bin/nm");
#endif /* NEXT_PDO */
...
#ifdef hpux
	      else if (!strcmp (arg, "-filelist"))
		{
		  ld1--;
		  ld2--;
		  *ld1++ = *ld2++ = "-c";
		  *ld1++ = *ld2++ = add_to_tmp_fl_file(*++argv);
		}
	      break;
#endif /* hpux */
#endif /* NEXT_PDO */
