...
{
  if (INSN_ANNULLED_BRANCH_P (insn))
    return \"ftest\;add,tr 0,0,%%r0\;bl,n %0,0\";
  else
    return \"ftest\;add,tr 0,0,%%r0\;bl%* %0,0\";
}"
  [(set_attr "type" "fbranch")
   (set_attr "length" "12")])
...
(define_expand "movpc"
 [(set (match_operand:SI 0 "register_operand" "=r") 
       (match_operand 1 "code_label_operand" ""))]
 ""
 "{ rtx label = operands[1]; rtx target = operands[0];
     emit_insn (gen_rtx (SET, VOIDmode, pc_rtx, label));
     emit_insn (gen_branch_and_link (label, target));
     emit_label (label);
     LABEL_PRESERVE_P (label) = 1;
     emit_insn (gen_rtx (SET, VOIDmode, target, 
                 gen_rtx (AND, VOIDmode, target, 
       		          gen_rtx (NOT, SImode, gen_rtx (CONST_INT, SImode, 3)))));
     {
	 ...
	 REG_NOTES (insn) = gen_rtx (EXPR_LIST, 
				     REG_EQUAL, label, REG_NOTES (insn));
     }
 }")

(define_insn "branch_and_link"
  [(set (pc) (label_ref (match_operand 0 "" "")))
   (clobber (match_operand 1 "register_operand" "=r"))]
  ""
  "bl %l0,%1%#"
  [(set_attr "type" "uncond_branch")
   (set (attr "length")
  ...
  "*
{
  if (GET_CODE (operands[1]) == CONST_INT)
    {
      operands[1] = GEN_INT (~INTVAL (operands[1]));
      return \"addi,uv %1,%0,%%r0\;blr,n %0,%%r0\;b,n %l3\";
    }
  else
    {
      return \"sub,>> %0,%1,%%r0\;blr,n %0,%%r0\;b,n %l3\";
    }
}"
  [(set_attr "type" "multi")
   (set_attr "length" "12")])
...
{
#ifdef NeXT_ASM
  return \"copy %0,22\;blr %%r0, %%r2\;bv,n 0\(%%r22\)\";
#endif
...
{
#ifdef NeXT_ASM
  return \"copy %r1,%%r22\;blr %%r0, %%r2\;bv,n 0\(%%r22\)\";
#endif
  if (TARGET_FAST_INDIRECT_CALLS)
...
;; These are NeXT PIC code generation
(define_peephole
  [(set (match_operand:SI 0 "register_operand" "=r")
	(lo_sum:SI (match_operand:SI 1 "register_operand" "r")
		   (match_operand:SI 2 "immediate_operand" "i")))
   (set (match_operand:SI 3 "register_operand" "=r")
        (mem:SI (plus:SI (match_dup 0) (match_operand:SI 4 "const_int_operand" ""))))]
 "flag_pic == 2 && (operands[0] == operands[3])"
 "ldw R'%2+%4(%1),%3")
