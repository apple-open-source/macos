...
static enum in_section {in_text} in_section;  /*  for NeXT's ASM_OUTPUT_INT */
#endif
     ...
     HOST_WIDE_INT intval;
{
  ...
  return (VAL_14_BITS_P (intval) || (intval & RIGHT_BITS_MASK) == 0
	  || zdepi_cint_p (intval));
}
...
int
code_label_operand (op, mode)
     rtx op;
     enum machine_mode mode;
{
  return (GET_CODE (op) == CODE_LABEL);
}
#endif
     ...
     enum machine_mode mode;
{
  ...
  if (   GET_CODE (orig) == PLUS
	...
	base = force_reg (SImode, XEXP (orig, 0));
      else
	abort ();

      XEXP (orig, 0) = base;

      if (! arith_operand (XEXP (orig, 1)))
	XEXP (orig, 1) = force_reg (SImode, XEXP (orig, 1));
      return orig;
    }
  ...
  if (current_function_uses_pic_offset_table)
    {
      ...
      rtx target = gen_rtx (REG, Pmode, PIC_OFFSET_TABLE_REGNUM);
      XSTR (label, 4) = machopic_function_base_name ();
      insn = emit_jump_insn_after (gen_branch_and_link (label, target), insn);
      insn = emit_label_after (label, insn);
      LABEL_PRESERVE_P (label) = 1;
      insn = emit_insn_after (gen_andsi3 (target, target, 
					  gen_rtx (CONST_INT, SImode, ~3UL)),
			      insn);
    }
     ...
     enum machine_mode mode;
{
  ...
  if (GET_CODE (x) == PLUS
      ...
      && (GET_CODE (XEXP (x, 1)) == MEM || GET_CODE (XEXP (x, 1)) == PLUS))
    {
      rtx op0 = force_reg (GET_MODE(XEXP (x, 0)), force_operand (XEXP (x, 0), 0));
      rtx op1 = force_reg (GET_MODE(XEXP (x, 1)), force_operand (XEXP (x, 1), 0));
      return force_reg (GET_MODE (x), gen_rtx (PLUS, GET_MODE(x), op0, op1));
    }
     ...
     rtx *operands;
{
  ...
  if (GET_CODE (operands[1]) == CONST_INT)
	...
	return "ldi %1,%0";
      else if ((intval & RIGHT_BITS_MASK) == 0)
#ifdef NEXT_SEMANTICS
	return "ldil L%'%1,%0";
#else
	return "ldil L'%1,%0";
     ...
     int size_is_constant;
{
...
#if 0
  /* PA7100 says fpload/fpstore are more efficient, because
     they use the full 64-bit data cache bandwith */
  if (align >= 8 && TARGET_PA7100)
    align = 8;
  else
#endif
  if (align > 4)
  ...
  if (size_is_constant)
      ...
      if (align >= 8)
	{
	  ...
	  output_asm_insn ("fldds,ma 8(0,%1),%2", operands);

	  for (offset = 8; offset < n_bytes; offset += 8)
	    {
	      if (offset + 8 < n_bytes || offset % 8 == 0)
		output_asm_insn ("fldds,ma 8(0,%1),%3", operands);
	      else
		output_asm_insn ("ldws,ma 0(0,%1),%%r1", operands);
		
	      output_asm_insn ("fstds,ma %2,8(0,%0)", operands);
	  ...
	  if (n_bytes % 8 == 0)
	    /* Store the last double word.  */
	    output_asm_insn ("fstds %2,0(0,%0)", operands);

	  else if (n_bytes % 4 == 0)
	    /* Store the last word.  */
	    output_asm_insn ("stw %%r1,0(0,%0)", operands);

	  else
	    {
	      /* Store the last, partial word.  */
	      operands[4] = gen_rtx (CONST_INT, VOIDmode, n_bytes % 4);
	      output_asm_insn ("stbys,e %%r1,%4(0,%0)", operands);
	    }
	  return "";
	}
#endif
     ...
     int size;
{
  ...
  fprintf (file, "\t.%s \"", STRING_SECTION_NAME);
  ...
  for (i = 0; i < size; i += 4)
      ...
      for (io = 0, co = 0; io < MIN (4, size - i); io++)
	{
	  ...
	  if (c >= ' ' && c < 0177)
	    ...
	    {
	      /* output octal numbers for gas */
	      partial_output[co++] = '\\';
	      partial_output[co++] = (c / 64) % 8 - 0 + '0';
	      partial_output[co++] = (c / 8) % 8 - 0 + '0';
	      partial_output[co++] = c % 8 - 0 + '0';
	    }
#endif
	  else
	    {
...
#ifdef NEXT_SEMANTICS
      sprintf(hp_profile_label_name, "LP%d", profile_label_no);
#else
     ...
     int size;
{
  ...
  for (i = 0; i < n_deferred_plabels; i++)
    {
#ifdef NEXT_SEMANTICS
      in_section = in_text;  /*  for NeXT's ASM_OUTPUT_INT */
#endif
...
#ifdef NEXT_SEMANTICS
      else if (GET_CODE (XEXP (XEXP (x, 0), 0)) == MINUS)
        {
          output_global_address (file, gen_rtx (CONST, VOIDmode, XEXP (XEXP (x, 0), 0)));
        }
#endif
      else abort ();
	...
	(gen_rtx
	 (PARALLEL, VOIDmode,
	  gen_rtvec (
		     ...
		     5,
#endif
			gen_rtx (SET, VOIDmode, gen_rtx (REG, SImode, 29),
...
#ifdef NEXT_SEMANTICS
		     gen_rtx (CLOBBER, VOIDmode, gen_rtx (REG, SImode, 19)),
		     gen_rtx (CLOBBER, VOIDmode, gen_rtx (REG, SImode, 21)),
		     gen_rtx (CLOBBER, VOIDmode, gen_rtx (REG, SImode, 22)),
...
/* Output a local call insn not emitting insns for the delay slot. 
   If -mstub-calls is enabled, we emit the pseudo insn "jbsr", to
   a locally generated stub, and make that stub do the actual call.
...
#ifdef NeXT_ASM
void add_compiler_stub PROTO((tree, tree, int));
void output_compiler_stub PROTO ((void));
int no_previous_def PROTO((tree));
tree get_prev_label PROTO((tree));

#endif

void
output_call_insn (insn, call_dest, return_pointer)
     rtx insn;
     rtx call_dest;
     rtx return_pointer;
...
#ifdef NeXT_ASM
  if(GET_CODE (call_dest) == SYMBOL_REF)
    {
      rtx prev_insn, label_rtx;
      int line_number;
      static char buf[256];
      ...
      if (!flag_pic && !strncmp (IDENTIFIER_POINTER (funname), "$$", 2))
	{
	    {
	      strcpy (temp_buf, "ldil L%'");
	      strcat (temp_buf, IDENTIFIER_POINTER (funname));
	      strcat (temp_buf, ",%%r31\n\tble R%'");
	      strcat (temp_buf, IDENTIFIER_POINTER (funname));
	      strcat (temp_buf, "(4,%%r31)");
	      output_asm_insn (temp_buf, 0);
	    }
	  return;
	}
      else if (!strncmp (IDENTIFIER_POINTER (funname), "$$", 2))
	{
	  strcpy (temp_buf, "*");
	  strcat (temp_buf, XSTR (call_dest, 0));
	  funname = get_identifier (temp_buf);
	}
      if (no_previous_def (funname))
	{
	  label_rtx = gen_label_rtx ();
	  
	  ASM_GENERATE_INTERNAL_LABEL (temp_buf, "L", CODE_LABEL_NUMBER(label_rtx));
	  
	  if (temp_buf[0] == '*')
	    label_buf = temp_buf + 1;
	  else
	    label_buf = temp_buf;
      ...
      else
	{
      ...
      strcpy(buf, "jbsr %0,%r1,");
      strcat(buf, IDENTIFIER_POINTER (labelname));
...
#ifdef NEXT_SEMANTICS
      output_call_insn (insn, call_dest, return_pointer);
      output_asm_insn ("ldo %1-%3-8(%r2),%r2", xoperands);
#endif
    }
