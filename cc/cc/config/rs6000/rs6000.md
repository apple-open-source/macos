...
(define_insn ""
  [(set (zero_extract:SI (match_operand:SI 0 "gpc_reg_operand" "+r")
	...
	(lshiftrt:SI (match_operand:SI 3 "gpc_reg_operand" "r")
		     (match_operand:SI 4 "const_int_operand" "i")))]
  "(32 - (INTVAL (operands[4]) & 31)) >= (INTVAL (operands[1]) & 31)"
  "*
{
  int shift = INTVAL (operands[4]) & 31;
  int start = INTVAL (operands[2]) & 31;
...
;; Do not allow splitting before the registers are allocated, because
;; the scheduler has problems with fixed registers suddenly showing
;; up and getting reg_dead notices.
...
(define_split
  [(set (match_operand:DF 0 "gpc_reg_operand" "")
   ...
   (clobber (reg:DF 76))]
  "reload_completed && ! TARGET_POWERPC64 && TARGET_HARD_FLOAT"
  [(set (match_dup 4)
	(xor:SI (match_dup 1)
		(match_dup 6)))
   (set (match_dup 5)
...
(define_insn "elf_high"
  ...
  "(TARGET_ELF || APPLE_RHAPSODY) && !TARGET_64BIT"
  "{cau|addis} %0,0,ha16(%1)")
  ...
  "
{
  ...
  if ((TARGET_ELF || APPLE_RHAPSODY) && TARGET_NO_TOC && !TARGET_64BIT
      && !flag_pic
...
(define_insn "call_indirect_sysv"
  ...
  "DEFAULT_ABI == ABI_V4 || DEFAULT_ABI == ABI_SOLARIS || DEFAULT_ABI == ABI_AIX_NODESC || DEFAULT_ABI == ABI_RHAPSODY"
  "*
{
  ...
  "
{
  ...
  if (GET_CODE (operands[0]) != SYMBOL_REF
	...
	operands[0] = rs6000_longcall_ref (operands[0]);
      ...
      if (DEFAULT_ABI == ABI_V4 || DEFAULT_ABI == ABI_AIX_NODESC || DEFAULT_ABI == ABI_SOLARIS || DEFAULT_ABI == ABI_RHAPSODY)
...
(define_insn ""
  [(call (mem:SI (match_operand:SI 0 "call_operand" "s,s"))
   ...
   (use (match_operand:SI 2 "immediate_operand" "O,n"))
   (clobber (match_scratch:SI 3 "=l,l"))]
  "(DEFAULT_ABI == ABI_AIX_NODESC || DEFAULT_ABI == ABI_V4 || DEFAULT_ABI == ABI_SOLARIS || DEFAULT_ABI == ABI_RHAPSODY)
   && (INTVAL (operands[2]) & CALL_LONG) == 0"
  "*
{
  /* Indirect calls should go through call_indirect */
