     ...
     int W_len;
{
  ...
  if (rs6000_select[0].string != (char *)0)
    pos = output_option (file, "-mcpu=", rs6000_select[0].string, pos);
  ...
  if (USE_FP_FOR_ARG_P (*cum, mode, type))
	...
	return gen_rtx (REG, mode, cum->fregno);
      ...
      return gen_rtx (EXPR_LIST, VOIDmode,
		      ((align_words >= GP_ARG_NUM_REG)
		       ? NULL_RTX
		       : (align_words + RS6000_ARG_SIZE (mode, type, named)
			  > GP_ARG_NUM_REG
			  /* If this is partially on the stack, then we only
			     include portion actually in registers here.  */
			  ? gen_rtx (REG, SImode, GP_ARG_MIN_REG + align_words)
			  : gen_rtx (REG, mode,
...
rs6000_finalize_pic ()
{
  ...
  if (flag_pic && (DEFAULT_ABI == ABI_V4 || DEFAULT_ABI == ABI_SOLARIS))
      ...
      if (reg)
	{
	  ...
	  /* Generate an instruction to force 'reg' to stay alive */
	  /* for the whole function. */
	  emit_insn(gen_negsi2(reg, reg));
	}
    }
}
  ...
  int labelno;
{
  ...
  fprintf(file, "\tlis 3,%s@h\n", buf);
  fprintf(file, "\tori 3,3,%s@l\n", buf);
...
#if 1 /* I think this should always be 48 - to preserve stack alignment (GDT) */
      ret = 48;
#endif
      break;
