...
(define_expand "mulsf3"
  ...
  "")

;; The next two patterns cause inlining of long long
;; multiplication to happen in expand_binop.
;; However, patterns with the same name seem to already be defined above!
...
;;                    (match_operand:SI 1 "register_operand" "0"))
;;                   (zero_extend:DI
;;                    (match_operand:SI 2 "immediate_operand" "n"))) 0))
;;    (set (match_operand:SI 3 "register_operand" "=d")
;;         (subreg:SI
;;          (mult:DI (zero_extend:DI
;;                    (match_dup 1))
  ...
  "*
{
  ...
  xops[3] = pic_offset_table_rtx;
     ...
     xops[3] = regno_reg_rtx [PIC_OFFSET_TABLE_REGNUM];
...
;; This is how we allocate stack space.  We need to probe the stack
;; even if we are allocating less than 4096 bytes, because we may be
;; allocating several chunks in succession, and we have no way of
;; knowing if the total of those chunks will be less than 4096 bytes.

(define_expand "allocate_stack"
  [(set (reg:SI 1)
	(minus:SI (reg:SI 1) (match_operand:SI 0 "" "")))]
  ""
  "
{ 
#ifdef _WIN32
    rtx eax = gen_rtx (REG, SImode, 0);	       /* Get rtx for eax  */
    emit_move_insn (eax, operands[0]);	       /* Move size to eax  */
    emit_insn (gen_rtx (USE, VOIDmode, eax));  /* Tell compiler eax is live  */
    emit_call_insn (gen_call_value (stack_pointer_rtx,	/* Call _chkstk  */
