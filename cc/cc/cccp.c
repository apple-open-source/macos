  ...
  for (i = 1; i < argc; i++) {
    ...
    if (!strcmp (argv[i], "-header-mapfile")) {
	  if (i + 1 == argc)
	    ...
	    fatal ("more than one `-header-mapfile' option specified; only one is allowed");
	  else
	    header_map = hmap_load_header_map(argv[i+1]), i++;
	}
#endif
    break;
  ...
  case '<':
    ...
    if (*fend == '>' && fend + 1 == limit) {
      ...
      for (bufptr = fbeg; bufptr != fend; bufptr++)
       ...
       if (hasSlash) {
#ifdef APPLE_CPP_HEADER_MAPS
         U_CHAR *oldFBeg = fbeg, *oldFEnd = fend;
         hmap_lookup(header_map, &fbeg, &fend);
#ifdef APPLE_CPP_INCLUSION_LOGS
         if (fbeg != oldFBeg)
	   {
             ilog_printf("      remapping ");
             ilog_write(fbeg, oldFEnd-oldFBeg);
             ilog_printf(" to ");
             ilog_write(fbeg, fend-fbeg);
             ilog_printf("\n");
	   }
...
#ifdef APPLE_CPP_HEADER_MAPS 
       if (header_map != NULL && searchptr == first_bracket_include && !filename_already_mapped)
        {
            U_CHAR *oldFBeg = fbeg, *oldFEnd = fend;
            hmap_lookup(header_map, &fbeg, &fend);
#ifdef APPLE_CPP_INCLUSION_LOGS
     ...
     struct directive *keyword;
{
  ...
  if (flag_dump_symbols)
    printf("+Mh %*.*s %u\n", mdef.symlen, mdef.symlen, mdef.symnam, instack[indepth].lineno);
#ifdef NEXT_CPP_SERVER
  if (hp) {
