...
openfiles()
{
  ...
  if (!tmp_dir)
    tmp_dir = getenv ("TEMP");
  tmp_base = xmalloc (tmp_dir ? strlen (tmp_dir) + 4 : 4);
  sprintf (tmp_base, "%s/b.", tmp_dir ? tmp_dir : "");
#endif
...
#if defined (MSDOS) || (defined (_WIN32) && defined (NeXT_PDO))
      /* File doesn't exist in current directory; try in INIT directory.  */
      cp = getenv("INIT");
#if defined (_WIN32) && defined (NeXT_PDO)
      /* File doesn't exist in current directory;
	 try in NEXT_ROOT directory.  */
      cp = getenv("NEXT_ROOT");
#endif
