...
class OpenConnectionReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.drm_client_key_low, self.drm_client_key_high, self.sarea_handle_low, self.sarea_handle_high, self.bus_id_len,) = unpack_from('xx2x4xIIIII12x', self, count)
        count += 40
...
class xf86driExtension(xcb.Extension):
    ...
    def CreateContext(self, visual, screen, context):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIII', visual, screen, context))
        return self.send_request(xcb.Request(buf.getvalue(), 5, False, True),
                                 CreateContextCookie(),
                                 CreateContextReply)

    def CreateContextUnchecked(self, visual, screen, context):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIII', visual, screen, context))
