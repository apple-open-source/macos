...
#define XTRACE(x, y, z) IrDALogAdd( x, y, z, IrLSAPConnTraceEvents, true)
#else
...
{
    XTRACE(kGetMyId, (int)this >> 16, (short)this);
...
void TLSAPConn::HandleDisconnectedStateEvent(ULong event)
{
	...
	case kIrPutDataRequestEvent:
		...
		XTRACE(kDiscLeftoversReplyEvent, (int)fClient >> 16, (short)fClient);   // testing
...
void TLSAPConn::HandleDisconnectPendingStateEvent(ULong event)
{
    ...
    XTRACE(kLogDiscPendingClient, (int)fClient >> 16, (short)fClient);
	...
	case kIrPutDataReplyEvent:
		...
		XTRACE(kLogDiscPendingConnLstnDone, (int)fPendConnLstn >> 16, (short)fPendConnLstn);
