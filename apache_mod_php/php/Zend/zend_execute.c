...
static void zend_fetch_var_address(zend_op *opline, temp_variable *Ts, int type TSRMLS_DC)
{
	...
	if (opline->op2.u.fetch_type == ZEND_FETCH_LOCAL) {
		FREE_OP(Ts, &opline->op1, free_op1);
	} else if (opline->op2.u.fetch_type == ZEND_FETCH_STATIC) {
...
binary_assign_op_addr: {
			...
			case ZEND_INIT_FCALL_BY_NAME: {
					...
					zend_ptr_stack_n_push(&EG(arg_types_stack), 3, EX(fbc), EX(object).ptr, EX(ce));
					...
					if (EX(opline)->op1.op_type != IS_UNUSED) {
						if (EX(opline)->op1.op_type==IS_CONST) { /* used for class_name::function() */
							...
							if (zend_hash_find(EG(class_table), EX(opline)->op1.u.constant.value.str.val, EX(opline)->op1.u.constant.value.str.len+1, (void **) &EX(ce))==FAILURE) { /* class doesn't exist */
							...
							EX(object).ptr = _get_object_zval_ptr(&EX(opline)->op1, EX(Ts), &EG(free_op1) TSRMLS_CC);

							if ((!EX(object).ptr && EX(Ts)[EX(opline)->op1.u.var].EA.type==IS_OVERLOADED_OBJECT)
								|| ((EX(object).ptr && EX(object).ptr->type==IS_OBJECT) && (EX(object).ptr->value.obj.ce->handle_function_call))) { /* overloaded function call */
								zend_overloaded_element overloaded_element;
							...
							if (!EX(object).ptr->is_ref && EX(object).ptr->refcount > 1) {
								zend_error(E_ERROR, "Bug: Problem in method call");
							}
							EX(object).ptr->is_ref=1;
							EX(object).ptr->refcount++; /* For $this pointer */
							active_function_table = &(EX(object).ptr->value.obj.ce->function_table);
						}
...
do_fcall_common:
				{
					...
					if (EX(function_state).function->type==ZEND_INTERNAL_FUNCTION) {
						...
						if (!zend_execute_internal) {
							/* saves one function call if the zend_execute_internal is not used */
					...
					if (EG(active_op_array)->return_reference == ZEND_RETURN_REF) {
						...
						if (!(*retval_ptr_ptr)->is_ref) {
							if (EX(opline)->extended_value == ZEND_RETURNS_FUNCTION &&
								EX(Ts)[EX(opline)->op1.u.var].var.fcall_returned_reference) {
								/* intentionally left empty */
							} else if (EX(Ts)[EX(opline)->op1.u.var].var.ptr_ptr == &EX(Ts)[EX(opline)->op1.u.var].var.ptr) {
			...
			case ZEND_SEND_VAR_NO_REF:
				...
				}
				{
					...
					if (varptr != &EG(uninitialized_zval) && (PZVAL_IS_REF(varptr) || varptr->refcount == 1)) {
						...
						zend_error(E_NOTICE, "Only variables can be passed by reference");
						ALLOC_ZVAL(valptr);
						*valptr = *varptr;
						if (!EG(free_op1)) {
							zval_copy_ctor(valptr);
...
send_by_ref:
			...
			case ZEND_INCLUDE_OR_EVAL: {
					...
					int return_value_used;
					int free_op1;
					zval *inc_filename = get_zval_ptr(&EX(opline)->op1, EX(Ts), &free_op1, BP_VAR_R);
					...
					FREE_OP(EX(Ts), &EX(opline)->op1, free_op1);
					EG(return_value_ptr_ptr) = original_return_value;
				}
				NEXT_OPCODE();
			case ZEND_UNSET_VAR: {
			...
			case ZEND_JMP_NO_CTOR: {
					...
					object = get_zval_ptr(&EX(opline)->op1, EX(Ts), &EG(free_op1), BP_VAR_R);
			...
			case ZEND_ISSET_ISEMPTY: {
					...
					if (!var) {
						if (EX(Ts)[EX(opline)->op1.u.var].EA.type == IS_STRING_OFFSET) {
							temp_variable *T = &EX(Ts)[EX(opline)->op1.u.var];
							...
							PZVAL_UNLOCK(EX(Ts)[EX(opline)->op1.u.var].EA.data.str_offset.str);
							T->tmp_var.refcount=1;
							T->tmp_var.is_ref=1;
							T->tmp_var.type = IS_STRING;
