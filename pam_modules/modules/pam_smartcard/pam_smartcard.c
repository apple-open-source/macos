...
int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv)
{
    ...
    keychain = copySmartCardKeychainForUser(odRecord, user);
    if (keychain == NULL)
    {
    ...
    {
        openpam_log(PAM_LOG_NOTICE, "Smart card already unlocked, allowing access.");
        retval = PAM_SUCCESS;
        goto cleanup;
    }
    ...
    for(int try = 0; try < MAX_PIN_RETRY; ++try)
    {
        ...
        {
            openpam_log(PAM_LOG_NOTICE, "Smart card successfully unlocked, allowing access.");
            retval = PAM_SUCCESS;
            break;
        }
        else
