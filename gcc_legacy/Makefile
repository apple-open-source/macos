...
# DO_SYMLINKS set to 'yes' will all the full gcc2 installation which
...
# gcc3.  We may not want to clobber gcc3's sym links.  Thus setting
...
install: installhdrs build install_no_src
	if [ "$(RC_RELEASE)" = Flask -a "$(DO_SYMLINKS)" = yes ]; then \
	...
	if [ "$(TARGET_OS)" = macos -a "$(DO_SYMLINKS)" = yes ]; then \
...
installhdrs: DSTROOT cplusplus_hdrs
	if [ "$(TARGET_OS)" = teflon -o "$(TARGET_OS)" = macos \
	  ...
	  if [ "$(DO_HDR_SYMLINKS)" = yes ]; then \
	    ln -s ../gcc/$$os_name/default/machine/limits.h  $$std_include_dir/machine; \
	  fi && \
	  $(RM) -f /tmp/limits.$$$$; \
	fi
