     ...
     int nop;			/* suppress parens */
{
  switch (TREE_CODE (t))
    ...
    case COMPOUND_EXPR:
      /* Not all compound expressions have two exprs!  A case in point is a
	 "E GrowableArray<T>::at_grow(int, E = (E)(0))", where the default
	 argument is a cast expression with only one expr (the "0").  */
