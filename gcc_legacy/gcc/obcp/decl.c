...
/* LANG_xxx stuff appears to be needed for -faltivec.  */
tree
...
/* Dump function definition information for the indexing symbols.  */
void 
     ...
     int flags;
{
  ...
  if (TREE_CODE (decl) == FIELD_DECL)
#ifdef OBJCPLUS
      /* zlaski 2001-Apr-23: Obj-C++ classes may not be statically allocated
         since C++ would not know how to initialize them properly (esp. the
  ...
  if (TREE_CODE (decl) == VAR_DECL || TREE_CODE (decl) == FUNCTION_DECL
	...
	{
#ifdef NEXT_SEMANTICS
	  /* The purpose of this "if" is to prevent static const
	     members from being placed in readonly memory. Because
	     of a collision in the common part of the compiler code
	     with Objective C/C++, a constructor type initializer
	     does not guarantee that the static const member will 
	     not be put in readonly memory. By setting the 
	     side-effects bit on the decl tree, the member,
	     in a target independent way, is prevented from 
	     being placed in readonly memory. 
     ...
     tree attrlist;
{
     ...
     and get it as a string, for an error message.  */
  {
	...
	switch (TREE_CODE (decl))
	  ...
	  case TREE_LIST:
#ifdef OBJCPLUS          
            /* zlaski 2001-Apr-03: In the 2.7.2.1 version of this routine, 0
  ...
  if (RIDBIT_SETP (RID_LONG, specbits)
      && TYPE_MAIN_VARIANT (type) == double_type_node)
    {
	  ...
	  /* c-decl.c has the the shabby story.  */
	  && ! in_system_header
  ...
  stdcallp = !! (RIDBIT_SETP (RID_STDCALL, specbits))/* + TYPE_STDCALL (type)*/;
#endif
	...
	if (decl == NULL_TREE)
	  {
	    ...
	    /* 9.2p13 [class.mem] */
#ifdef OBJCPLUS
            /* zlaski 2001-Mar-29: Obj-C++ classes have no constructors or destructors! */
...
#if defined (_WIN32) && defined (NEXT_PDO)
    /* If this function is a stdcall function, set the stdcall_flag using the
       TYPE_STDCALL macro.  */
    if (stdcallp)
      TYPE_STDCALL (decl) = 1;
...
#ifdef OBJCPLUS
/* snaroff (3/30/96): this is a total "COPOUT" fix for a problem briefly
     ...
     tree name;
{
  ...
  /* If the name is anonymous, then it cannot be a forward reference.  */
     ...
     int pre_parsed_p;
{
  ...
  /* If a virtual inline destructor/constructor, mark it coalesced since
     we'll most likely end up with multiple copies.  */
...
#ifdef MODERN_OBJC_SYNTAX /* These are required to avoid undefined symbols!  */
