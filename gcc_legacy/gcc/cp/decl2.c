  ...
  if (!strcmp (p, "-ftraditional") || !strcmp (p, "-traditional"))
...
#endif
	{
	  ...
	  warn_unknown_pragmas = 1;       
	    ...
	    {
	      warn_most = 1;
	      /* Change sign so we can tell it was turned on by -Wmost.  */
...
/* Finish processing a builtin type TYPE.  Its name is NAME,
     ...
     tree decl;
{
  ...
  if (DECL_TEMPLATE_INSTANTIATION (decl)
	...
	DECL_NOT_REALLY_EXTERN (decl) = 0;
      ...
      /* Global template functions are good candidates for coalescing.  */
      if (TREE_PUBLIC (decl))
...
#if defined(NEXT_SEMANTICS) && !defined(MACOSX)
  /* If this is a destructor, remember it for atexit queueing by the
     subsequent constructor.  $$$ WHAT IF THERE'S NO SUCH CONSTRUCTOR???  */
...
#if defined(NEXT_SEMANTICS) && !defined(MACOSX)
  /* If this is a constructor, and there's a queued destructor function,
     generate a call to atexit calling said queued destructor function. 

     NOTE THAT THIS MIGHT HAVE INCORRECT SEMANTICS AS IT'S COARSE-GRAINED
     i.e., destructors are not queued on a per-object basis: everything
     gets constructed, and then the destructor for everything is queued.  */

  if (constructor_p && DYNAMIC_DESTRUCTORS && queued_destructor_fn)
    {
      /* As the last action in the constructor-caller, make it call
	 atexit to schedule the call of the corresponding dtor-caller.
	 Note that the compiler has already generated the dtor-caller
	 by the time we're here doing the ctor-caller.  */
...
finish_file ()
{
  ...
  /* If no code generation expected then quit.  */
  if (flag_syntax_only)
    return;
...
go_next:
      ...
      for (i = 0; i < pending_statics_used; ++i) 
	{
	  ...
	  import_export_decl (decl);
#ifdef NEXT_SEMANTICS
	  /* if (dynamic code gen) then we can't have any commons... */
	  if (flag_pic)
	    DECL_COMMON (decl) = 0;
#endif
...
/* This is something of the form `int ((int)(int)(int)1)' that has turned
   ...
   In the above example, DECL is the `(int)(int)(int)', and EXPR is the
   `1'.  EXPR may also be a comma expression that has not been reduced.
   Parse the expression as a vector constant as well.  */
     ...
     tree type;
{
  switch (TREE_CODE (type))
    ...
    case TEMPLATE_TEMPLATE_PARM:
      return 0;
    case VECTOR_TYPE:		/* <tur 08May99> Handle __vectors  */
      return arg_assoc_type(k, TREE_TYPE (type));
    case LANG_TYPE:
