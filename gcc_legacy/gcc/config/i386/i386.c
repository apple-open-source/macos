...
load_pic_register (int do_rtl, int is_picreg_reload) 
{
  ...
  if (TARGET_DEEP_BRANCH_PREDICTION)
	...
	abort();
      ...
      /* If PIC_OFFSET_TABLE_RTX is a *real* register, use it.  */
      ...
      /* Otherwise, check out the REG_RENUMBER array, if available.  */

      else if (reg_renumber && pic_reg >= 0 && reg_renumber[pic_reg] >= 0)
	xops[0] = gen_rtx_REG (SImode, reg_renumber[pic_reg]);

      if (! xops[0])	/* since we can POP MEM, allow mem also.  */
      {
      ...
      else
	{
	  ...
	  /* skip the '*' if there is one in the name.  */
	  if (*pic_label == '*')
	    ++pic_label;
          ...
          if (is_picreg_reload)
            {
              ...
              /* subtract the difference between the label we just emitted
                 and the function's pic_label emitted in the prologue.  */
     ...
     int do_rtl;
{
  ...
  if (tsize >= 4096)
    {
      ...
      yops[1] = GEN_INT( tsize );			// Get rtx for the size
  ...
  for (regno = limit - 1; regno >= 0; regno--)
    if ((regs_ever_live[regno] && ! call_used_regs[regno])
#if !defined(MACHO_PIC) || /*it's defined, but: */ defined(FIXED_PIC_REG)
	|| (regno == PIC_OFFSET_TABLE_REGNUM && pic_reg_used))
#else
       )
#endif
  ...
  if (pic_reg_used)
    load_pic_register (do_rtl, /*is_picreg_reload:*/ 0);
