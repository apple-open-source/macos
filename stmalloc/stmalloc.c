...
#define VM_COPY_THRESHOLD		(10 * vm_page_size)
...
#define KILL_THRESHOLD			(8 * vm_page_size)

#define LARGE_THRESHOLD			(5 * vm_page_size) // at or above this use "large"
...
// GrP stmalloc mods
// 1.5 GB for small objects?
#define GrP_small_buckets 1
// short free list for large objects?
#define GrP_large_death_row 1
// 0: replace malloc() (compile-time only)
// 1: replace szone_malloc() (DYLD_INSERT_LIBRARIES)
#define GrP_replace_scalable_malloc 0

#if GrP_small_buckets
// Size of each bucket. Bucket distribution is hardcoded as:
// (0..512] bytes in 16-byte increments
// (512..2560] bytes in 64-byte increments
// (2560..18944] bytes in 512-byte increments
#define SMALL_BUCKET_SIZE (16*1024*1024)

#define GrP_small_bucket_lazy_alloc 0
#define GrP_small_bucket_lazy_init 0

#endif


#if GrP_large_death_row
// Number of entries kept on death row
#define LARGE_DEATH_ROW_COUNT 2
#endif


#if ! GrP_replace_scalable_malloc
/* leave szone intact, so don't use its symbol names */
#define scalable_zone_info st_zone_info
#define szone_check_counter stzone_check_counter
#define szone_check_modulo stzone_check_modulo
#define szone_check_start stzone_check_start
#define create_scalable_zone create_st_zone
#endif
...
#if GrP_small_buckets
// required: sizeof(small_bucket_t) % sizeof(void *) == 0
typedef struct {
    uint32_t size; // of each object, in bytes
    uint32_t freeCount; // number of objects currently in free list
    char *next; // next place to allocate a fresh object
    int overallocatedCount; // number of overallocations sent to small regions
    void *freeList[0];
} small_bucket_t;
#endif
...
#if GrP_small_buckets
    /* Small objects quantized and segregated into fast buckets */
    /* Requires 3 blocks of 0.5GB each */
    /* Overflow sent to small regions */
    vm_address_t bucket16; // (0..512] bytes in 16-byte increments
    vm_address_t bucket64; // (512..2560] bytes in 64-byte increments
    vm_address_t bucket512; // (2560..18944] bytes in 512-byte increments
#endif
...
#if GrP_large_death_row
    /* Death row holding on to recently-freed large objects */
    vm_range_t		large_death_row[LARGE_DEATH_ROW_COUNT];
    unsigned		large_death_row_used;
#endif
...
static INLINE void init_bucket(small_bucket_t *b, size_t object_size)
{
    b->size = object_size;
    b->next = SMALL_BUCKET_SIZE/(object_size+4)*4 + (char *)(b+1);
    if ((vm_address_t)b->next & (QUANTUM-1)) {
        b->next += QUANTUM - ((vm_address_t)b->next & (QUANTUM-1));
    }
}

static vm_address_t allocate_bucket(szone_t *szone, size_t first, size_t last, size_t object_size) 
{
    vm_address_t addr;
    vm_address_t target;
    kern_return_t err;
    ...
    do {
        addr = target;
        err = vm_allocate(mach_task_self(), &addr, 32 * SMALL_BUCKET_SIZE, 0);
        if (!err) break;
        target += 0x20000000;
    } while (target != 0);

    if (err) {
	malloc_printf("*** malloc: bucket allocation failed (vm_allocate returned %p\n", err);
        szone_error(szone, "Can't allocate region", NULL);
        return 0;
    }

#if ! GrP_small_bucket_lazy_init
    for (i = 0, s = first; s <= last; s += object_size, i++) {
        init_bucket((small_bucket_t *)(addr + i*SMALL_BUCKET_SIZE), s);
    }
#endif
...
static INLINE small_bucket_t *small_bucket_for_size(szone_t *szone, size_t s)
{
    small_bucket_t *bucket;
    if (s == 0) s = 1;
    if (s <= 512) {
#if GrP_small_bucket_lazy_alloc
        if (!szone->bucket16) {
            // malloc_printf("allocating bucket16\n");
            szone->bucket16 = allocate_bucket(szone, 16, 12, 16);
        }
#endif
        bucket = (small_bucket_t *)(szone->bucket16 + (s - 1) / 16 * SMALL_BUCKET_SIZE);
#if GrP_small_bucket_lazy_init
        if (!bucket->size) {
            if (s & 15) s += 16 - (s & 15);
            init_bucket(bucket, s);
        }
#endif
        return bucket;
    }
...
#if GrP_small_bucket_lazy_alloc
        if (!szone->bucket64) {
            // malloc_printf("allocating bucket64\n");
            szone->bucket64 = allocate_bucket(szone, 512+64, 2560, 64);
        }
#endif
        bucket = (small_bucket_t *)(szone->bucket64 + (s - 512 - 1) / 64 * SMALL_BUCKET_SIZE);
#if GrP_small_bucket_lazy_init
        if (!bucket->size) {
            if (s & 63) s += 64 - (s & 63);
            init_bucket(bucket, s);
...
#if GrP_small_bucket_lazy_alloc
        if (!szone->bucket512) {
            // malloc_printf("allocating bucket512\n");
            szone->bucket512 = allocate_bucket(szone, 2560+512, 18944, 512);
        }
#endif
        bucket = (small_bucket_t *)(szone->bucket512 + (s - 2560 - 1) / 512 * SMALL_BUCKET_SIZE);
#if GrP_small_bucket_lazy_init
        if (!bucket->size) {
            if (s & 511) s += 512 - (s & 511);
            init_bucket(bucket, s);
        }
#endif
        return bucket;
...
static INLINE small_bucket_t *small_bucket_for_ptr(szone_t *szone, const void *ptr)
{
    vm_address_t addr = (vm_address_t)ptr;
    if ((szone->bucket16  &&  addr >= szone->bucket16  &&  addr < szone->bucket16 + 32*SMALL_BUCKET_SIZE)  ||
        (szone->bucket64  &&  addr >= szone->bucket64  &&  addr < szone->bucket64 + 32*SMALL_BUCKET_SIZE)  ||
        (szone->bucket512  &&  addr >= szone->bucket512  &&  addr < szone->bucket512 + 32*SMALL_BUCKET_SIZE)) 
    {
        small_bucket_t *b = (small_bucket_t *)(addr & ~(SMALL_BUCKET_SIZE-1));
        if (!b->size) szone_error(szone, "small bucket not initialized", ptr);
        return b;
    } else {
        return NULL;
    }
...
static INLINE void *small_bucket_try_malloc_should_clear(szone_t *szone, small_bucket_t *bucket, size_t size, boolean_t cleared_requested)
{
    void *result;
    if (bucket->freeCount) {
        // allocate from free list
        result = (void *)bucket->freeList[--bucket->freeCount];
        if (cleared_requested) {
            bzero(result, bucket->size);
        }
        return result;
    } else if (bucket->next + bucket->size <= SMALL_BUCKET_SIZE + (char *)bucket) {
        // allocate with pointer bump
        result = (void *)bucket->next;
        bucket->next += bucket->size;
        // guaranteed already cleared
        return result;
    } else {
        return NULL;
    }
...
static INLINE void small_bucket_free(szone_t *szone, small_bucket_t *bucket, void *ptr)
{
    bucket->freeList[bucket->freeCount++] = ptr;
}
...
static void *large_and_huge_malloc(szone_t *szone, unsigned num_pages, boolean_t cleared_requested) {
    ...
    if (num_pages >= (1 << vm_page_shift)) {
        ...
        large_entry_t		entry = {0};
#if GrP_large_death_row
        if (!cleared_requested) {
            SZONE_LOCK(szone);
            // best-fit scan across death row
            int i;
            int bestIndex = -1;
            vm_range_t bestRange = {0xffffffff, 0xffffffff};
            for (i = 0; i < szone->large_death_row_used; i++) {
                vm_range_t r = szone->large_death_row[i];
                if (r.size == size) {
                    // perfect fit - use it
                    bestIndex = i;
                    bestRange = r;
                ...
                if (r.size > size  &&  r.size < bestRange.size) {
                    // r is better than bestRange
                    bestIndex = i;
                    bestRange = r;
                }
            }
            if (bestIndex >= 0) {
                // resurrect bestIndex/bestRange
                entry.address_and_num_pages = bestRange.address | (bestRange.size>>vm_page_shift);
                addr = bestRange.address;
                szone->large_death_row[bestIndex] = szone->large_death_row[--szone->large_death_row_used];
                // malloc_printf("malloc large recycled %p (requested %p bytes, got %p)\n", LARGE_ENTRY_ADDRESS(entry), size, LARGE_ENTRY_SIZE(entry));
            }
            SZONE_UNLOCK(szone);
        }
#endif
        if (!entry.address_and_num_pages) {
            // failed to allocate from death row - allocate fresh pages
...
static void szone_free(szone_t *szone, void *ptr) {
    ...
    {
        // try a small bucket
        small_bucket_t *bucket = small_bucket_for_ptr(szone, ptr);
        if (bucket) {
            SZONE_LOCK(szone);
            small_bucket_free(szone, bucket, ptr);
            SZONE_UNLOCK(szone);
            return;
        }
    }
...
#if GrP_large_death_row
        if (szone->large_death_row_used < LARGE_DEATH_ROW_COUNT) {
            vm_range_to_deallocate.address = vm_range_to_deallocate.size = 0;
            szone->large_death_row[szone->large_death_row_used++] = large_free_no_lock(szone, entry);
        } else 
#endif
        {
            // too small for death row, or death row full
...
#if GrP_small_buckets
        // try small buckets
        small_bucket_t *bucket = small_bucket_for_size(szone, size);
        ptr = NULL;
        if (bucket) {
            ptr = small_bucket_try_malloc_should_clear(szone, bucket, size, cleared_requested);
        } 
        if (!ptr) 
#endif
        {
            // no small buckets or small bucket full - use small regions
...
#if GrP_small_buckets
    // Try small buckets before MSIZE_FLAGS check
    {
        small_bucket_t *bucket = small_bucket_for_ptr(szone, ptr);
        if (bucket) return bucket->size;
    }
#endif
...
static void *szone_realloc(szone_t *szone, void *ptr, size_t new_size) {
	...
	if (try_realloc_large_in_place(szone, ptr, old_size, new_size)) return ptr;
        new_ptr = large_and_huge_malloc(szone, num_pages, 0);
...
#if GrP_small_buckets
    if (szone->bucket16)  deallocate_pages(szone, szone->bucket16,  32*SMALL_BUCKET_SIZE, 0);
    if (szone->bucket64)  deallocate_pages(szone, szone->bucket64,  32*SMALL_BUCKET_SIZE, 0);
    if (szone->bucket512) deallocate_pages(szone, szone->bucket512, 32*SMALL_BUCKET_SIZE, 0);
#endif
...
#if GrP_small_buckets
static vm_address_t init_small_buckets(vm_address_t addr, size_t first, size_t last, size_t step)
{
    int i;
    size_t s;
    
    for (i = 0, s = first;  s <= last;  i++, s += step) {
        small_bucket_t *b = (small_bucket_t *)(addr + i * SMALL_BUCKET_SIZE);
        b->size = s;
        b->next = SMALL_BUCKET_SIZE/(s+4)*4 + (char *)(b+1);
        if ((vm_address_t)b->next & (QUANTUM-1)) {
            b->next += QUANTUM - ((vm_address_t)b->next & (QUANTUM-1));
        }
    }
...
malloc_zone_t *create_scalable_zone(size_t initial_size, unsigned debug_flags) 
{
    ...
    size_t		msize;
    size_t		msize_used = 0;
    if (getenv("MallocPrintInfo")) {
        malloc_printf("=== create_scalable_zone(%d,%d); small_buckets %s, large_death_row %s\n", initial_size, debug_flags, GrP_small_buckets ? "YES" : "NO", GrP_large_death_row ? "YES" : "NO");
    }
    if (firstTime) {
        firstTime = 0;
...
#if GrP_small_buckets
#if ! GrP_small_bucket_lazy_alloc
    szone->bucket16 = allocate_bucket(szone, 16, 512, 16);
    szone->bucket64 = allocate_bucket(szone, 512+64, 2560, 64);
    szone->bucket512 = allocate_bucket(szone, 2560+512, 18944, 512);
#endif
#endif
...
{
    if (!sz) sz = (szone_t *)create_st_zone(0, 0);
    return szone_malloc(sz, s);
}
void *calloc(size_t s, size_t t)
{
    if (!sz) sz = (szone_t *)create_st_zone(0, 0);
    return szone_calloc(sz, s, t);
}
void *realloc(void *ptr, size_t s)
{
    if (!sz) sz = (szone_t *)create_st_zone(0, 0);
    return szone_realloc(sz, ptr, s);
}
void free(void *ptr)
{
    if (!sz) sz = (szone_t *)create_st_zone(0, 0);
