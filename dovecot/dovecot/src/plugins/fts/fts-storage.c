...
struct fts_storage_build_context {
	...
	/* APPLE - for asynchronous indexing of mail parts */
	...
	unsigned int skip_body:1;
	unsigned int body_part:1;
	unsigned int mid_message:1;
	unsigned int continue_headers:1;
	unsigned int continue_block:1;
};
...
static int fts_build_mail_flush_headers(struct fts_storage_build_context *ctx)
{
	...
	/* APPLE */
	ret = fts_backend_build_more(ctx->build, str_data(ctx->headers),
				     str_len(ctx->headers));
	if (ret <= 0)
		return ret;
...
/* APPLE split parts of fts_build_mail into fts_build_mail_next_mail so
   building can be asynchronous within a single mail */
static int fts_build_mail_next_mail(struct fts_storage_build_context *ctx,
				    uint32_t uid)
{
	...
	ctx->continue_headers = FALSE;
	ctx->continue_block = FALSE;

	if (mail_get_stream(ctx->mail, NULL, NULL, &ctx->input) < 0)
		return -1;

	ctx->prev_part = NULL;
	ctx->pool = pool_alloconly_create("fts parser", 8192);
	ctx->parser = message_parser_init(ctx->pool, ctx->input,
...
/* APPLE split parts of fts_build_mail into fts_build_mail_next_part so
   building can be asynchronous within a single mail */
static int fts_build_mail_next_part(struct fts_storage_build_context *ctx)
{
	struct message_block raw_block;
	int ret;

	for (;;) {
		ret = message_parser_parse_next_block(ctx->parser, &raw_block);
		i_assert(ret != 0);
		if (ret < 0) {
		...
		if (raw_block.part != ctx->prev_part) {
			...
			if (!ctx->skip_body && ctx->prev_part != NULL) {
				i_assert(ctx->body_part);
		...
		if (raw_block.hdr != NULL) {
			...
			ctx->skip_body =
				!fts_backend_build_body_begin(ctx->build,
		...
		if (ctx->block.hdr != NULL) {
			fts_parse_mail_header(ctx, &raw_block);
			fts_build_mail_header(ctx, &ctx->block);
		} else if (ctx->block.size == 0) {
			/* end of headers */
			str_append_c(ctx->headers, '\n');
		} else {
			i_assert(ctx->body_part);
			ret = fts_backend_build_more(ctx->build,
						     ctx->block.data,
						     ctx->block.size);
			if (ret < 0) {
				ret = -1;
...
/* APPLE split parts of fts_build_mail into fts_build_mail_next_mail and
   fts_build_mail_next_part so building can be asynchronous within a
   single mail */
static int fts_build_mail(struct fts_storage_build_context *ctx)
{
	...
	ctx->build->fts_ctx = ctx;
	if (ctx->continue_headers) {
	...
	if (ctx->continue_block) {
		i_assert(!ctx->skip_body);
		i_assert(ctx->body_part);
		ret = fts_backend_build_more(ctx->build, ctx->block.data,
					     ctx->block.size);
		if (ret == 0)
			return 0;
		ctx->continue_block = FALSE;
	...
	if (ret > 0)
		ret = fts_build_mail_next_part(ctx);
	if (ret == 0)
		return 0;

	if (!ctx->skip_body && ctx->body_part)
		fts_backend_build_body_end(ctx->build);
	if (message_parser_deinit(&ctx->parser, &parts) < 0)
...
static int fts_build_more(struct fts_storage_build_context *ctx)
{
	...
	while (ctx->mid_message ||				/* APPLE */
	       mailbox_search_next(ctx->search_ctx, ctx->mail)) {
		/* APPLE */
		if (!ctx->mid_message) {
			ret = fts_build_mail_next_mail(ctx, ctx->mail->uid);
			if (ret < 0)
				return -1;
		}

		T_BEGIN {
			ret = fts_build_mail(ctx);		/* APPLE */
		} T_END;

		/* APPLE */
		if (ret == 0) {
			ctx->mid_message = TRUE;
			return 0;
		}
		ctx->mid_message = FALSE;
			       ...
			       struct fts_search_context *fctx)
{
	...
	if (fctx->build_ctx == NULL) {
		...
		fctx->build_ctx->mail_ctx = ctx;		/* APPLE */
	}
	return TRUE;
}
...
#ifdef APPLE_OS_X_SERVER
static void fts_escape(string_t *str, const char *orig)
{
	static const char *hexchars = "0123456789abcdef";

	while (*orig != '\0') {
		unsigned char c = *orig;
		if ((c >= 'A' && c <= 'Z') ||
		    (c >= 'a' && c <= 'z') ||
		    (c >= '0' && c <= '9'))
			str_append_c(str, c);
			...
			str_append_c(str, '%');
			str_append_c(str, hexchars[(c >> 4) & 0xf]);
			str_append_c(str, hexchars[c & 0xf]);
		}
...
static void fts_enqueue_update(struct mailbox *box)
{
	static const char *queue_dir = "/var/db/dovecot.fts.update";
	string_t *path;
	int fd;

	/* creating a file in the queue_dir kicks off a separate process 
	   which updates the search index.  the file is named user.mailbox
	   with all non-alphanum chars hex-escaped.  this keeps the number
	   of files in the queue_dir reasonable. */
	path = t_str_new(128);
	str_printfa(path, "%s/", queue_dir);
	fts_escape(path, box->storage->user->username);
	str_append_c(path, '.');
	fts_escape(path, box->name);

	fd = open(str_c(path), O_WRONLY | O_CREAT | O_EXCL, 0600);
	if (fd < 0 && errno != EEXIST) {
		i_error("fts: open(%s, O_CREAT) failed: %m", str_c(path));
		return;
	}
	close(fd);
}
		       ...
		       bool committed)
{
	...
	if (ft->expunges) {
		/* APPLE */
		if (fbox->backend_substr != NULL) {
			fts_backend_expunge_finish(fbox->backend_substr,
						   box, committed);
		}
...
#ifdef APPLE_OS_X_SERVER
	if ((ft->expunges || ft->saves) && committed)
...
static int fts_save_finish(struct mail_save_context *ctx)
{
	...
	ft->saves = TRUE;
	return fbox->module_ctx.super.save_finish(ctx);
}
...
static int fts_copy(struct mail_save_context *ctx, struct mail *mail)
{
	...
	ft->saves = TRUE;
	return fbox->module_ctx.super.copy(ctx, mail);
}
#endif
...
/* APPLE */
void fts_compact(struct mailbox *box)
{
	struct fts_mailbox *fbox = FTS_CONTEXT(box);

	if (fbox->backend_substr != NULL)
		fts_backend_compact(fbox->backend_substr);
	if (fbox->backend_fast != NULL)
		fts_backend_compact(fbox->backend_fast);
}

/* APPLE */
struct mail_search_context *
fts_build_get_mail_context(struct fts_storage_build_context *ctx)
{
	return ctx->mail_ctx;
}
