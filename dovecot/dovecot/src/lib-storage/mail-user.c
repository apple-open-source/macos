...
/* APPLE */
enum mail_user_dir_type { MAIL_USER_HOME, MAIL_USER_SIEVE_DIR };
static int mail_user_get_dir(struct mail_user *user, const char **dir_r,
			     enum mail_user_dir_type type)
{
	...
	case MAIL_USER_HOME:
		...
		break;
	case MAIL_USER_SIEVE_DIR:
		if (user->sieve_dir_looked_up) {
			*dir_r = user->_sieve_dir;
			return user->_sieve_dir != NULL ? 1 : 0;
		}
		break;
	default:
		i_unreached();
	...
	*dir_r = NULL;
	...
	if (ret >= 0) {
		...
		/* APPLE */
		user->_sieve_dir = NULL;
		for (fp = fields; ret != 0 && *fp != NULL; fp++) {
			if (strncasecmp(*fp, "sieve_dir=", 10) == 0) {
				user->_sieve_dir = p_strdup(user->pool,
							    *fp + 10);
				break;
			}
		}
		user->sieve_dir_looked_up = TRUE;
...
{
	return mail_user_get_dir(user, home_r, MAIL_USER_HOME);
}

/* APPLE */
int mail_user_get_sieve_dir(struct mail_user *user, const char **dir_r)
{
	return mail_user_get_dir(user, dir_r, MAIL_USER_SIEVE_DIR);
}
