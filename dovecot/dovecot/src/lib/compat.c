...
/* APPLE */
int time_t_max_bits(void)
{
	/* When cross-compiling, TIME_T_MAX_BITS may be wrong.
	   Determine actual size at runtime.  Moved from configure.in. */
	static int bits = 0;

	if (bits == 0) {
		for (bits = 1; bits < (int) sizeof(time_t)*8; bits++) {
		...
		if (bits > 40) {
			/* Solaris 9 breaks after 55 bits. Perhaps other
			   systems break earlier.  Let's just do the
			   same as Cyrus folks and limit it to 40 bits. */
			bits = 40;
		}
	}
...
/* APPLE */
bool time_t_signed(void)
{
	/* When cross-compiling, TIME_T_SIGNED may be wrong.
	   Determine actual signedness at runtime.  Moved from configure.in. */
	return (time_t)(int)-1 <= 0 ? TRUE : FALSE;
}
