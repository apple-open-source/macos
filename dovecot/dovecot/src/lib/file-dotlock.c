...
/* APPLE - PID alone is insufficient when sharing mail processes.  Also need
   to check client ID.  Scattered changes throughout this file to support
   reading, writing, and checking both PID and CID. */

#define DEFAULT_LOCK_SUFFIX ".lock"

/* 0.1 .. 0.2msec */					/* APPLE */
#define LOCK_RANDOM_USLEEP_TIME (100000 + (unsigned int)random() % 100000)
			    ...
			    uintmax_t *client_id)		/* APPLE */
{
	...
	*host++ = '\0';
	...
	/* APPLE - it may contain pid:cid:host */
	cid = strchr(host, ':');
	if (cid != NULL) {
		char *tmp = cid;
		cid = host;
	...
	/* APPLE - if there is a client ID it must be valid */
	if (cid != NULL) {
		if (str_to_uintmax(cid, client_id) < 0)
			return -1;
	} else
		*client_id = 0;
		      ...
		      uintmax_t client_id)			/* APPLE */
{
	...
	pid_t pid = -1;
	uintmax_t locking_client_id = 0;			/* APPLE */
	bool changed;
	int ret;
	...
	if (lock_info->have_pid) {
		...
		if (kill(pid, 0) == 0 || errno != ESRCH) {
			...
			/* APPLE */
			if (locking_client_id != 0 && client_id != 0 &&
			    locking_client_id != client_id)
				return 0;
			  ...
			  uintmax_t client_id)			/* APPLE */
{
	...
	/* APPLE - write pid:cid:host, if possible */
	if (client_id != 0)
		str = t_strdup_printf("%s:%llu:%s", my_pid,
	       ...
	       uintmax_t client_id)				/* APPLE */
{
	...
	do {
		...
		ret = check_lock(now, &lock_info, client_id);	/* APPLE */
		if (ret < 0)
			break;
...
{
	/* APPLE */
	return file_dotlock_create_multiclient(set, path, flags, dotlock_r, 0);
}

/* APPLE */
int file_dotlock_create_multiclient(const struct dotlock_settings *set,
				    const char *path,
				    enum dotlock_create_flags flags,
				    struct dotlock **dotlock_r,
				    uintmax_t client_id)
...
/* APPLE */
int file_dotlock_delete_multiclient(struct dotlock **dotlock_p,
				    uintmax_t client_id ATTR_UNUSED)
{
	/* only one dotlocker at a time, so nothing special here */
	return file_dotlock_delete(dotlock_p);
}
