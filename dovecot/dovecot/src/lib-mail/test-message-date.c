...
static void test_message_date_parse(void)
{
	...
	for (i = 0; i < N_ELEMENTS(input); i++) {
		/* APPLE - determine time_t bounds at runtime.  This may cause
		   compiler warnings about constant overflows in the arrays
		   but it skips those when run. */
		if (i == 0 && !time_t_signed())
			continue;
		if (i == 1 && !time_t_signed())
			continue;
		if (i == 2 && !(time_t_max_bits() > 32 || !time_t_signed()))
			continue;
