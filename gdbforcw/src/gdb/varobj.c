  ...
  if (cv != NULL)
    {
      int len;
      char *buffer;
      warning ("Duplicate variable object name");
      len = strlen (var->obj_name);
      buffer = (char *) xmalloc (len + 32);
      sprintf (buffer, "%s#dup#%d", var->obj_name, auto_number++);
      xfree (var->obj_name);
      var->obj_name = buffer;
      /* We need to recompute the index.  We don't then see if the name
       is unique, because we are using auto_number to make a unique
       name.  */
      i = 1;
      index = 0;
      for (chp = var->obj_name; *chp; chp++)
