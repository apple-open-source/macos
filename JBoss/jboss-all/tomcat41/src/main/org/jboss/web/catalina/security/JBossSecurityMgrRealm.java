...
public class JBossSecurityMgrRealm extends RealmBase implements Realm, Valve
{
       ...
       throws IOException, ServletException
   {
    ...
    {
       boolean trace = category.isEnabledFor(XLevel.TRACE);
       if( trace )
          category.log(XLevel.TRACE, "Begin authenticate, username="+username);
        SimplePrincipal principal = null;
        Context securityCtx = getSecurityContext();
        if( securityCtx == null )
        {
       ...
       if( trace )
          category.log(XLevel.TRACE, "End authenticate, principal="+principal);
        return principal;
    }
    ...
    public boolean hasRole(Principal principal, String role)
    {
       boolean trace = category.isEnabledFor(XLevel.TRACE);
       if( trace )
          category.log(XLevel.TRACE, "Begin hasRole, principal="+principal+", role="+role);
       boolean hasRole = false;
        try
        {
            ...
            if( hasRole )
            {
                category.log(XLevel.TRACE, "User: "+principal+" is authorized");
            }
            else
            {
                RealmMapping securityMgr = (RealmMapping) securityCtx.lookup("realmMapping");
                Set userRoles = securityMgr.getUserRoles(principal);
                category.log(XLevel.TRACE, "User: "+principal+" is NOT authorized, requiredRoles="+requiredRoles+", userRoles="+userRoles);
            }
        }
        catch(NamingException e)
        {
       ...
       if( trace )
          category.log(XLevel.TRACE, "End hasRole, principal="+principal+", role="+role+", hasRole="+hasRole);
       return hasRole;
    }
