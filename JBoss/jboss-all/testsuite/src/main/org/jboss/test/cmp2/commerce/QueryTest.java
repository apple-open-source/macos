...
public class QueryTest extends net.sourceforge.junitejb.EJBTestCase
{
   ...
   public void testJBossQL() throws Exception
   {
      ...
      assertEquals(expected,
                   compileJBossQL("SELECT OBJECT(o1) " +
   ...
   public void testEJBQL() throws Exception
   {
      ...
      String expected = "1.4".equals(javaVersion) ?
            "SELECT t0_o.ORDER_NUMBER FROM ORDER_DATA t0_o, LINEITEMEJB t2_l, PRODUCTCATEGORYEJB t1_pc, PRODUCT_PRODUCT_CATEGORY t3_l_product_productCategories_R, PRODUCT t4_l_product WHERE (t0_o.ORDER_NUMBER = ? AND t1_pc.name = ?) AND (t4_l_product.id=t3_l_product_productCategories_R.PRODUCT_ID AND t1_pc.id=t3_l_product_productCategories_R.PRODUCT_CATEGORY_ID AND t2_l.product=t4_l_product.id AND t0_o.ORDER_NUMBER=t2_l.ORDER_NUMBER)" :
            "SELECT t0_o.ORDER_NUMBER FROM ORDER_DATA t0_o, LINEITEMEJB t2_l, PRODUCTCATEGORYEJB t1_pc, PRODUCT_PRODUCT_CATEGORY t3_l_product_productCategories_R, PRODUCT t4_l_product WHERE (t0_o.ORDER_NUMBER = ? AND t1_pc.name = ?) AND (t0_o.ORDER_NUMBER=t2_l.ORDER_NUMBER AND t4_l_product.id=t3_l_product_productCategories_R.PRODUCT_ID AND t1_pc.id=t3_l_product_productCategories_R.PRODUCT_CATEGORY_ID AND t2_l.product=t4_l_product.id)";
      assertEquals(expected,
                   compileEJBQL("SELECT OBJECT(o) FROM OrderX o, " +
                                "IN(o.lineItems) l, " +
      ...
      assertEquals("SELECT DISTINCT t0_o.ORDER_NUMBER " +
                   "FROM ORDER_DATA t0_o, LINEITEMEJB t2_l " +
                   "WHERE t0_o.ORDER_NUMBER = ? OR EXISTS (SELECT t1_o_lineItems.id FROM LINEITEMEJB t1_o_lineItems " +
                   "WHERE t0_o.ORDER_NUMBER=t1_o_lineItems.ORDER_NUMBER AND t1_o_lineItems.id = t2_l.id)",
                   compileEJBQL("SELECT OBJECT(o) FROM OrderX o, LineItem l WHERE o.ordernumber = ?1 OR l MEMBER o.lineItems",
                                Set.class, new Class[]{Long.class}));

      assertEquals("SELECT DISTINCT t0_o.ORDER_NUMBER " +
                   "FROM ORDER_DATA t0_o, LINEITEMEJB t2_l " +
                   "WHERE t0_o.ORDER_NUMBER = ? OR NOT EXISTS (SELECT t1_o_lineItems.id FROM LINEITEMEJB t1_o_lineItems " +
                   "WHERE t0_o.ORDER_NUMBER=t1_o_lineItems.ORDER_NUMBER AND t1_o_lineItems.id = t2_l.id)",
      ...
      assertEquals("SELECT DISTINCT t0_p.id " +
                   "FROM PRODUCT t0_p, PRODUCTCATEGORYEJB t3_pc " +
                   "WHERE t0_p.id = ? OR EXISTS (SELECT t2_p_productCategories_RELATION_.PRODUCT_CATEGORY_ID " +
                   "FROM PRODUCT_PRODUCT_CATEGORY t2_p_productCategories_RELATION_ " +
                   "WHERE t0_p.id=t2_p_productCategories_RELATION_.PRODUCT_ID " +
                   "AND t3_pc.id=t2_p_productCategories_RELATION_.PRODUCT_CATEGORY_ID)",
                   compileEJBQL("SELECT OBJECT(p) FROM Product p, ProductCategory pc WHERE p.id = ?1 OR pc MEMBER p.productCategories",
                                Set.class, new Class[]{Long.class}));

      assertEquals("SELECT DISTINCT t0_p.id " +
                   "FROM PRODUCT t0_p, PRODUCTCATEGORYEJB t3_pc " +
                   "WHERE t0_p.id = ? OR NOT EXISTS (SELECT t2_p_productCategories_RELATION_.PRODUCT_CATEGORY_ID " +
                   "FROM PRODUCT_PRODUCT_CATEGORY t2_p_productCategories_RELATION_ " +
                   "WHERE t0_p.id=t2_p_productCategories_RELATION_.PRODUCT_ID " +
                   "AND t3_pc.id=t2_p_productCategories_RELATION_.PRODUCT_CATEGORY_ID)",
      ...
      assertEquals("SELECT DISTINCT t0_o.ORDER_NUMBER " +
                   "FROM ORDER_DATA t0_o " +
                   "WHERE t0_o.ORDER_NUMBER = ? OR EXISTS (SELECT t1_o_lineItems.id " +
                   "FROM LINEITEMEJB t1_o_lineItems " +
                   "WHERE t0_o.ORDER_NUMBER=t1_o_lineItems.ORDER_NUMBER)",
                   compileEJBQL("SELECT OBJECT(o) FROM OrderX o WHERE o.ordernumber = ?1 OR o.lineItems IS NOT EMPTY",
                                Set.class, new Class[]{Long.class}));

      assertEquals("SELECT t0_l.id FROM CUSTOMEREJB t1_c, ORDER_DATA t2_o, LINEITEMEJB t0_l WHERE (t1_c.id = 1) AND (t1_c.id=t2_o.customer AND t2_o.ORDER_NUMBER=t0_l.ORDER_NUMBER)",
