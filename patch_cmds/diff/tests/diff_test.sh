...
{
	# Trailing $ in each for regex.
	atf_check -o save:c1.mtime \
	    stat -f '%Sm' -t '%Y-%m-%d %H:%M:%S$' "$(atf_get_srcdir)/input_c1.in"
	atf_check -o save:c2.mtime \
	    stat -f '%Sm' -t '%Y-%m-%d %H:%M:%S$' "$(atf_get_srcdir)/input_c2.in"

	atf_check -o save:unified_p.out -s exit:1 \
	    diff -up "$(atf_get_srcdir)/input_c1.in" "$(atf_get_srcdir)/input_c2.in"

	(echo -n "^--- $(atf_get_srcdir)/input_c1.in\t"; cat c1.mtime) > c1.match
	(echo -n "^+++ $(atf_get_srcdir)/input_c2.in\t"; cat c2.mtime) > c2.match
