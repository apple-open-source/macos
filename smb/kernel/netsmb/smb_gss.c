		...
		switch (AvId) {
			case kMsvAvFlags:
				if (((startptr+sizeof(uint32_t)) > endptr) || (AvLen != sizeof(uint32_t))) {
					...
					uint32_t GuestAccessRequired = htolel(*(uint32_t *)startptr);
...
{	
	...
	if (gp->gss_major != GSS_S_CONTINUE_NEEDED) {
...
skip_optional:
	...
	/*
	 * They didn't send us a domain name, thats ok just skip it. Likewise will 
	 * send a NULL domain name.
	 */
	if (domain.length == 0) {
		SMBWARNING("Server sent us a NULL domain name: length = %d allocated = %d offset = %d\n", 
			domain.length, domain.allocated, domain.offset);
	    goto skip_domain_name;
	 }
	 ...
	 * we will now use the one supplied by the server.
	 *
	 * NOTE: All Mac OS X Samba Servers require a domain.
	 */
	if ((vcp->vc_domain == NULL) || (vcp->vc_domain[0] == 0)) {
		...
		if (*ntlmflags & NTLMSSP_NEGOTIATE_UNICODE) {
			...
			if (domain_str)
				smb_unitostr(domain_str, (const uint16_t *)domain_offset_ptr, domain.length, maxlen, 0);
		} else {
		...
		if (domain_str) {
			...
			SMB_LOG_AUTH("Using servers domain %s, use the use_server_domain option to turn this off.\n", domain_str);
		} 
	}
...
skip_domain_name:
	...
	if ((address_list.length == 0) || (address_list.allocated == 0)) {
		...
		/* 
		 * No target info we need to return an empty address, this is used to 
		 * mark the last element. This code should be removed in the future, but
		 * just in case lets correct it here for now. The SMBX server returns a 
		 * empty list.
		 */
		*target_info = malloc(sizeof(address_list), M_SMBTEMP, M_WAITOK | M_ZERO);
		*target_len = (uint16_t)sizeof(address_list);
		error = 0;
		goto done;
	} 
...
{	
	...
	uint16_t *domain_str = NULL;
	uint64_t server_nonce = *(uint64_t *)vcp->vc_ch;
	...
	uint8_t *keyExchange = NULL;
	uint16_t length;
	
	DBG_ASSERT(gp->gss_token == NULL);

	/* If we are doing anonymous then we shouldn't have a username or domain */
	...
	if (domain_len) {		
		...
		domain_len = smb_strtouni(domain_str, (const char *)vcp->vc_domain, domain_len, 0);
	}
	...
	if (username_len) {
		...
		username_len = smb_strtouni(username, (const char *)vcp->vc_username, username_len, 0);		
	}
	...
	if (hostname_len) {
		...
		hostname_len = smb_strtouni(hostname, (const char *)vcp->vc_localname, hostname_len, 0);
	}
	/*
	...
	if (vcp->vc_flags & SMBV_ANONYMOUS_ACCESS) {
		...
		read_random((void *)&client_nonce, (unsigned)sizeof(client_nonce));
	...
	*ntlmssp++ = letohl(NTLMSSP_TypeThreeMessage);
	...
	length = (uint16_t)keyExchange_len;
	secbuffer.length = letohs(length);
	secbuffer.allocated = secbuffer.length;
	...
	*ntlmssp = letohl(ntlmflags);
	...
	/* We only send the version field if we are building in debug mode */
	{
		...
		ntlmssp++;	/* skip the flags field from above */
		version = (uint8_t *)ntlmssp;
	...
	SMB_FREE(target_info, M_SMBTEMP);		
	SMB_FREE(lmv2, M_SMBTEMP);
	SMB_FREE(ntlmv2, M_SMBTEMP);
	SMB_FREE(username, M_SMBTEMP);
	SMB_FREE(hostname, M_SMBTEMP);
	SMB_FREE(domain_str, M_SMBTEMP);
	if (error) {
		SMB_FREE(gp->gss_token, M_SMBTEMP);
		gp->gss_tokenlen = 0;		
	}
...
static int create_ntlmssp_negotiate_message(struct smb_vc *vcp)
{
	...
	*ntlmssp = letohl(ntlmflags);
	...
	gp->gss_major = GSS_S_CONTINUE_NEEDED;	/* We need more processing */
...
retry:
	*display_name = '\0';
	...
	if (keylen > 0) {
		...
		MALLOC(vcp->vc_mackey, uint8_t *, vcp->vc_mackeylen, M_SMBTEMP, M_WAITOK);
	...
	if (otokenlen > 0) {
		MALLOC(cp->gss_token, uint8_t *, otokenlen, M_SMBTEMP, M_WAITOK);
	...
	if (cp->gss_cpn_display == NULL && display_name[0]) {
		...
		MALLOC(cp->gss_cpn_display, char *, len, M_SMBTEMP, M_WAITOK);
 ...
 */
static int
smb_gss_ssandx(struct smb_vc *vcp, vfs_context_t context, uint32_t caps, uint16_t *action)
{
...
smb_gss_ssnsetup(struct smb_vc *vcp, vfs_context_t context)
{
	...
	do {
		...
		if ((vcp->vc_flags & SMBV_KERBEROS_ACCESS) || (smbfs_kern_ntlmssp == 0))
		...
		if (error || (SMB_GSS_ERROR(&vcp->vc_gss))) {
			SMB_LOG_AUTH("%s extended security error = %d gss_major = %d gss_minor = %d\n", 
					   (smbfs_kern_ntlmssp) ? "Kernel NTLMSSP" : "GSSD", 
	...
	if ((error == 0) && !SMBV_HAS_GUEST_ACCESS(vcp) && (action & SMB_ACT_GUEST)) {
		 ...
		 * case. If we are using the kernel NTLMSSP then we should have a password
		 * and can tell if they gave us a blank password, otherwise we will force
		 * them to log in as guest if there password is empty.
		 *
		 * The user wanted authenticated access but got guest access, log them 
		 * out and return EAUTH.
		 */
		if ((smbfs_kern_ntlmssp == 0) || (vcp->vc_pass && vcp->vc_pass[0])) {
