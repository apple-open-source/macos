...
static pthread_mutex_t ndr_handle_lock = PTHREAD_MUTEX_INITIALIZER;
    ...
    ndr_uuid_t *ts_uuid, int ts_vers)
{
	...
	if (as_uuid)
		mlrpc_uuid_to_str(as_uuid, abstract_syntax, sizeof(abstract_syntax));

	if (ts_uuid)
		mlrpc_uuid_to_str(ts_uuid, transfer_syntax, sizeof(transfer_syntax));
...
#if 0
/* First off, the whole concept of a static registry of open service handles
 * is really ugly and horrid. Second, if we must have this, then listing them
 * as a bunch of strings is really only useful for one problem - we should
 * have a general traversal if we want this.
 *
 * Note that OpenSolaris never uses this API.
 *
 * -- jpeach
 */
int
...
ndr_hdalloc(const ndr_xa_t *xa, const void *data)
{
	...
	if (uuid.data[1] == 0) {
		...
		uuid.data[0] = 0;
		uuid.data[1] = 0;
		uuid.data[2] = sid->sid_authorities[1];
		uuid.data[3] = sid->sid_authorities[2];
		uuid.data[4] = sid->sid_authorities[3];
	}

	++uuid.data[1];
#endif

	OSAtomicIncrement32((volatile int32_t *)&(uuid.data[1]));

	/* AFAICT, the handle value just has to be unique, not meaningful. */
	hd->nh_id.data[0] = 0;
	hd->nh_id.data[1] = uuid.data[1];
	hd->nh_id.data[2] = (uint32_t)time(NULL);
	hd->nh_id.data[3] = (uint32_t)clock();
	hd->nh_id.data[4] = 0;
	...
	(void) pthread_mutex_lock(&ndr_handle_lock);
	hd->nh_next = ndr_handle_list;
	ndr_handle_list = hd;
	(void) pthread_mutex_unlock(&ndr_handle_lock);
...
/*
 * XXX Shouldn't we be using the following instead:
 *		void uuid_unparse(uuid_t uu, char * out);
 */
void
mlrpc_uuid_to_str(ndr_uuid_t *uuid, char *str, size_t maxlen)
{
	(void) snprintf(str, maxlen, "%08x-%04x-%04x-%02x%02x%02x%02x%02x%02x%02x%02x",
