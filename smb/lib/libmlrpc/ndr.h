 ...
 *		Think of this as a strlcpy() of a variable length string
...
struct ndr_reference {
	...
	uint32_t		pdu_offset;	/* referent in stub data */
	char			*datum;		/* referent in local memory */
	char			**backptr;	/* referer to set */
	uint16_t		outer_flags;	/* XXX_is() from top level */
	uint16_t		inner_flags;	/* XXX_is() in encapsulated */
	uint16_t		type_flags;	/* "requires" */
	uint16_t		packed_alignment;
	uint32_t		size_is;	/* conforming constructs */
	uint32_t		strlen_is;	/* strings */
	uint32_t		switch_is;	/* union arg selector */
	uint32_t		dimension_is;	/* fixed-len array size */
	uint32_t		pdu_end_offset;	/* offset for limit of PDU */
};
