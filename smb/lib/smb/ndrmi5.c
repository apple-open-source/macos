...
)
{
    ...
    else    /* Conformant or open */
    {
        ...
        {
            l_storage_len = rpc_ss_get_typed_integer(sz_type,
                                     IDL_msp->IDL_param_vec[sz_index]);
        }
        ...
        || (array_type == IDL_DT_OPEN_ARRAY))
    {
        ...
        {
            l_data_len = rpc_ss_get_typed_integer(ln_type,
                                     IDL_msp->IDL_param_vec[ln_index]);
        }
...
)
{
    ...
    do {
        ...
        switch(type_byte)
        {
            ...
            default:
#ifdef DEBUG_INTERP
                asl_log(NULL, NULL, ASL_LEVEL_ERR, "rpc_ss_ndr_m_struct_cs_shadow:unrecognized type %d\n", type_byte);
                exit(0);
#endif
    ...
    *p_defn_vec_ptr = defn_vec_ptr;
        ...
        || (array_type == IDL_DT_OPEN_ARRAY))
    {
        ...
        range_pair.upper = rpc_ss_get_typed_integer(ln_type,
                            (rpc_void_p_t)&cs_shadow[ln_index].IDL_data); /* B */
    ...
    for (i = 0; ; i++)
    {
        ...
        do {
            ...
            switch(type_byte)
            {
                ...
                default:
#ifdef DEBUG_INTERP
                    asl_log(NULL, NULL, ASL_LEVEL_ERR, "rpc_ss_ndr_m_param_cs_shadow:unrecognized type %d\n", type_byte);
                    exit(0);
#endif
                    RAISE(rpc_x_coding_error);
            }
