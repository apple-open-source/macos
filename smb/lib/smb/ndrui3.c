...
)
{
    ...
    switch(type_byte)
    {
        ...
        case IDL_DT_FIXED_ARRAY:
            ...
            IDL_GET_LONG_FROM_VECTOR(defn_index, arm_type_ptr);
            rpc_ss_ndr_unmar_fixed_arr(defn_index, body_addr, IDL_msp);
            break;
        ...
        case IDL_DT_UNIQUE_PTR:
            /*
             * If the node number is 0, unmarshall a null pointer into
             * the space for the pointer, otherwise marshall IDL_NEW_NODE.
             * XXX - is that the right thing to do?
             */
            rpc_ss_ndr_unmar_scalar(IDL_DT_ULONG, &node_number, IDL_msp);
            *(rpc_void_p_t *)body_addr = node_number ? IDL_NEW_NODE : 0;
            break;
        case IDL_DT_STRING:
            IDL_DISCARD_LONG_FROM_VECTOR(arm_type_ptr);
        ...
        case IDL_DT_V1_STRING:
            rpc_ss_ndr_unmar_v1_string(body_addr, IDL_msp);
            break;
    ...
    switch(type_byte)
    {
        ...
        case IDL_DT_ENC_UNION:
            ...
            rpc_ss_ndr_u_enc_union_or_ptees(body_addr, defn_index, idl_true,
                                                                      IDL_msp);
            break;
        case IDL_DT_UNIQUE_PTR: /* XXX - handle these like full pointers */
        case IDL_DT_FULL_PTR:
            IDL_GET_LONG_FROM_VECTOR(defn_index, arm_type_ptr);
                                                 /* Will skip properties byte */
...
)
{
    ...
    rpc_ss_ndr_u_fix_or_conf_arr( 1, IDL_ecount_p,
                           ...
                           IDL_chunk_array, p_pipe_state->IDL_msp );
...
)
{
    ...
    switch(transmitted_type)
    {
        ...
        case IDL_DT_REPRESENT_AS:
            ...
            break;
        case IDL_DT_V1_STRING:
            rpc_ss_ndr_unmar_v1_string(transmitted_data, IDL_msp);
            break;
        default:
#ifdef DEBUG_INTERP
...
)
{
    ...
    if (v1_count == 0)
    {
        ...
        {
            rpc_ss_ndr_arr_align_and_opt( IDL_unmarshalling_k, &base_type,
