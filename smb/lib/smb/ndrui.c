    ...
    do {
		...
		/* XXX idl_ulong_int is 32 bits this is a problem for another day! */
            ...
            case IDL_DT_CS_RLSE_SHADOW:
                rpc_ss_ndr_u_struct_cs_shadow(struct_addr,
    ...
    for (i=0; i<element_count; i++)
    {
        switch (base_type)
        {
            ...
            case IDL_DT_REPRESENT_AS:
                ...
                break;
            case IDL_DT_V1_STRING:
                rpc_ss_ndr_unmar_v1_string(array_addr, IDL_msp);
                array_addr = (rpc_void_p_t)
                                      ((idl_byte *)(array_addr) + element_size);
                break;
            default:
...
)
{
    ...
    rpc_ss_ndr_arr_align_and_opt( IDL_unmarshalling_k,
...
)
{
    ...
    if (Z_values != NULL)   /* NULL possible for transmit_as case */
    {
        for (i=0; i<dimensionality; i++)
        {
            if ((idl_ulong_int)(range_list[i].upper - range_list[i].lower) > Z_values[i])
            {
        ...
        do {
			...
			if (IDL_msp->IDL_type_vec[TVEC_INT_REP_OFFSET] != 
			  ...
			  rpc_ss_mem_item_free(&IDL_msp->IDL_mem_handle,
                    ...
                    rpc_ss_ndr_unmar_fixed_arr(defn_index, param_addr, IDL_msp);
                    if (type_has_pointers)
                    {
                ...
                case IDL_DT_VARYING_ARRAY:
                    ...
                    rpc_ss_ndr_unmar_varying_arr( array_defn_ptr,
                             type_has_pointers, param_addr, IDL_msp );
                    is_string = idl_false;
                    break;
                case IDL_DT_CONF_ARRAY:
                    /* Properties byte */
                ...
                case IDL_DT_V1_STRING:
                    ...
                    IDL_DISCARD_LONG_FROM_VECTOR(type_vec_ptr);
                                                      /* Flattened array defn */
                    rpc_ss_ndr_unmar_v1_string(param_addr, IDL_msp);
                    break;
