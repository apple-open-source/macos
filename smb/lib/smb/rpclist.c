...
/*
 * Sigh.  We compile with "-W" (or "-Wextra"), so GCC warns about
 * structures not completely initialized, and we compile with "-Werror",
 * so warnings cause the compilation to fail, and this structure
 * contains some elements whose contents depend on a #define, and
 * that include an opaque type (a pthread mutex), so we *can't*
 * statically initialize it.
 *
 * We initialize it the first time we use it.
 */
GLOBAL rpc_lookaside_rcb_t rpc_g_lookaside_rcb;
/* XXX - need a mutex to protect this? */
INTERNAL boolean rpc_g_lookaside_rcb_initialized;

/*
**++
**
...
{
    if (!rpc_g_lookaside_rcb_initialized)
    {
        rpc_g_lookaside_rcb_initialized = true;

        rpc_g_lookaside_rcb.res_id = RPC_C_LOOKASIDE_RES;
        rpc_g_lookaside_rcb.waiter_cnt = 0;
        rpc_g_lookaside_rcb.max_wait_times = RPC_C_LOOKASIDE_RES_MAX_WAIT;
        rpc_g_lookaside_rcb.wait_time = RPC_C_LOOKASIDE_RES_WAIT;
    }
