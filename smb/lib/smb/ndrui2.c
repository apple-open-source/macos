    ...
    switch(pointee_type)
    {
        ...
        case IDL_DT_FIXED_ARRAY:
            ...
            IDL_DISCARD_LONG_FROM_VECTOR( defn_vec_ptr );
                                                    /* Full array definition */
            IDL_GET_LONG_FROM_VECTOR(pointee_defn_index, defn_vec_ptr);
            rpc_ss_ndr_unmar_fixed_arr( pointee_defn_index, p_node, IDL_msp );
            if (type_has_pointers)
            {
...
)
{
    ...
    do {
        ...
        for (ui=0; ui<dimensionality; ui++)
        {
            inner_slice_address += control_data[ui].index_value
                                        * control_data[ui].subslice_size;
        }
        /* XXX - can these be negative? */
        ...
        while (dim >= 0)
        {
            ...
            /* XXX - can upper or lower be negative? */
            if (control_data[dim].index_value < (idl_ulong_int)range_list[dim].upper)
                break;
