...

INTERNAL void dealloc_acb
(
  void                    *arg
)
{
    rpc_cn_assoc_p_t    assoc = arg;

    rpc__cn_assoc_acb_dealloc (assoc);
}

INTERNAL boolean close_connection
(
  rpc_cn_assoc_p_t        assoc,
  boolean                 from_cleanup
)
{
...
("(rpc__cn_network_receiver) assoc->%x desc->%x rpc__socket_close failed, error = %d\n", 
    ...
    if (!from_cleanup)
    {
         ...
         */
        pthread_cleanup_push (dealloc_acb, assoc);
        pthread_testcancel();
...
/*
 * We don't map cancels to pthread_cancel_e - POSIX doesn't guarantee
 * that you can longjmp out of cancel cleanup handlers.
 */
INTERNAL void rcvr_cleanup
(
  void                    *arg
)
{
...
)
{
    ...
    while (!done && !assoc->cn_ctlblk.exit_rcvr)
    {
        ...
        TRY
        {
            ...
            pthread_cleanup_push (rcvr_cleanup, assoc);
            TRY
            {
		...
		RPC_DCE_SVC_PRINTF ((
            ...
            RPC_DBG_PRINTF (rpc_e_dbg_general, RPC_C_CN_DBG_GENERAL,
                                ("CN: assoc->%x call_rep->none No longer receiving...Close socket\n",
                                 assoc));
            done = close_connection(assoc, false);
        }
        CATCH_ALL
        {
        }
...
("CN: call_rep->%x assoc->%x desc->%x receiver canceled, caught in receive_dispatch()\n",
	 ...
	 */        
        ...
        /*
         * Catch any cancels that might have been posted.
         */
        pthread_testcancel();
...
)
{
	...
	*/
	    ...
	    pthread_testcancel();
            ...
            RPC_SOCKET_RECEIVE (assoc->cn_ctlblk.cn_sock,
                                (byte_p_t)((unsigned8 *)(fbp->data_p) + fbp->data_size),
                                need_bytes,
                                &bytes_rcvd,
                                &serr);
...
("CN: call_rep->%x assoc->%x desc->%x receiver canceled, caught in receive_packet()\n",
            ...
            (assoc->assoc_status != rpc_s_ok))
        {
            ...
            if (rpc__naf_is_connect_closed (assoc->cn_ctlblk.cn_sock, st))
            {
                ...
                *st = rpc_s_connection_closed;
                RPC_SOCKET_RECEIVE_DONE (assoc->cn_ctlblk.cn_sock, &serr);
                return;
            }
        }
     ...
     *
     * XXX - lkcl says "don't do that" on p. 36.
     */
     ...
     * Point to the packet header.
     */
    pktp = (rpc_cn_packet_p_t) fbp->data_p;

    /*
     * If this is the last frag, indicate to the socket layer that
     * we've received all of the packet, in case it needs to wait
     * until we send another packet before trying to read a reply.
     */
    if (RPC_CN_PKT_FLAGS (pktp) & RPC_C_CN_FLAGS_LAST_FRAG)
        RPC_SOCKET_RECEIVE_DONE (assoc->cn_ctlblk.cn_sock, &serr);
