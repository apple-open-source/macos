...
static void generate_member_macro(const char *, const char *, ndr_member_t *,
    ndr_typeinfo_t *);
static void generate_member_macro_with_arg(const char *, const char *,
    ndr_member_t *, ndr_typeinfo_t *, ndr_node_t *);
...
static void
generate_member_macro(const char *memkind, const char *macro, ndr_member_t *mem,
    ndr_typeinfo_t *ti)
{
	char	fname_type[NDLBUFSZ];
...
static void
generate_member_macro_with_arg(const char *memkind, const char *macro,
...
generate_typeinfo_typeinfo(ndr_typeinfo_t *ti, int is_static, char *fname_type)
{
	...
	*flags = 0;
	...
	type_null_decl(ti, cast_buf, NDLBUFSZ);
	...
	(void) printf("\t.name = \"%s\",\n", fname_type);
	(void) printf("\t.version = 1,		/* NDR version */\n");
	(void) printf("\t.alignment = %d,		/* alignment */\n", ti->alignment);
	(void) printf("\t.type_flags = %s,	/* flags */\n", flags);
	(void) printf("\t.ndr_func = ndr_%s,	/* ndr_func */\n", fname_type);
	(void) printf("\t.pdu_size_fixed_part = %d,		/* pdu_size_fixed_part */\n",
	    ti->size_fixed_part);
	(void) printf("\t.pdu_size_variable_part = %d,		/* pdu_size_variable_part */\n",
	    ti->size_variable_part);

	(void) printf("\t.c_size_fixed_part = sizeof%s, /* c_size_fixed_part */\n",
	    cast_buf);
	(void) printf("\t.c_size_variable_part = %d,		/* c_size_variable_part */\n",
	    ti->size_variable_part);
	(void) printf("};\n\n");
}
