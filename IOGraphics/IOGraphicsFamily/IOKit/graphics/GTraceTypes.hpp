...
#define GTRACE_ARCHAIC_CPP ((KERNEL || _KERNEL_) && __cplusplus < 201103L)
#endif
...
#else
// kGTRACE_ARGUMENT_POINTER is undefined in user land. DO NOT record pointers
// in either a trace entry nor anywhere in the breadcrumb buffer. The problem
// is that pointers will be published in a sysdiagnose and would make the
// randomisation slide public which defeats the purpose of address-space
// randomisation.
#endif
...
// Use this to encode tag0, for automatic decoding and start/end pairing
#define GPACKFUNCTAG(funcid, functype, tag0) \
    MAKEGTRACETAG((((functype) & 0x3) << 10) | ((funcid) & 0x03FF) | (tag0))
#define GUNPACKFUNCTYPE(tag0) static_cast<uint8_t>((tag0 >> 10) & 0x3)
#define GUNPACKFUNCCODE(tag0) static_cast<uint16_t>(tag0 & 0x03FF)
#define GUNPACKTAG0(tag0) \
    static_cast<uint16_t>(tag0 & kGTRACE_ARGUMENT_MASK)
...
typedef struct GTraceEntry {
#if !__cplusplus
    uint64_t       fEntry[8];  // Pure C interface
#else // C++ interface
    // Internal structures
    struct ID {
        uint64_t fID;
