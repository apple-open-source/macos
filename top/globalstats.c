...
enum {
	GLOBALSTAT_PROCESSES,
	GLOBALSTAT_TIME,
	GLOBALSTAT_LOAD,
	GLOBALSTAT_CPU,
	GLOBALSTAT_SHAREDLIBS,
	GLOBALSTAT_MEMREGIONS,
	GLOBALSTAT_PHYSMEM,
	GLOBALSTAT_VM,
	GLOBALSTAT_SWAP,
	GLOBALSTAT_PURGEABLE,
	GLOBALSTAT_NETWORKS,
	GLOBALSTAT_DISKS
};
...
static int
humanize_globalstat(char *out, size_t s, int64_t n)
{
	return humanize_number(out, s, n, "", HN_AUTOSCALE, GLOBALSTAT_HN_FORMAT);
}

static bool
init_globalstat(WINDOW *parent, struct globalstat *gs, int type)
{
...
static void
update_max(struct globalstat *gs)
{
...
static int
get_globalstat_reqwidth(struct globalstat *gs)
{
	return gs->length + 1;
}

static void
set_globalstat_geometry(struct globalstat *gs, int x, int y, int width, int height)
{
	gs->x = x;
	gs->y = y;
	gs->size.width = width;
...
void *
top_globalstats_create(WINDOW *parent)
{
...
static void
reset_globalstat(struct globalstat *gs)
{
	gs->data[0] = '\0';
	gs->length = 0;
}
...
static void
update_time(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	struct timeval tval;
	struct tm tm;
	...
	if (STATMODE_ACCUM == top_prefs_get_mode()) {
		unsigned int sec = (unsigned int)((tsamp->timens - tsamp->b_timens) / NSEC_PER_SEC);
		unsigned int min = sec / 60;
		unsigned int hour = min / 60;

		gs->length = snprintf(gs->data, sizeof(gs->data), "%u:%02u:%02u", hour, min % 60, sec % 60);
...
static void
update_processes(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	int i, offset, chunksize;

	gs->length = snprintf(
			gs->data, sizeof(gs->data), "Processes: %" PRIu32 " total, ", (uint32_t)tsamp->nprocs);
	...
	for (i = 0; i < LIBTOP_NSTATES; ++i) {
		if (tsamp->state_breakdown[i]) {
			chunksize = snprintf(gs->data + offset, sizeof(gs->data) - offset, "%d %s, ",
					tsamp->state_breakdown[i], libtop_state_str(i));
			if (chunksize < 0)
				break;
	...
	chunksize = snprintf(gs->data + gs->length, sizeof(gs->data) - gs->length,
...
static void
update_load(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	gs->length = snprintf(gs->data, sizeof(gs->data), "Load Avg: %.2f, %.2f, %.2f ",
			tsamp->loadavg[0], tsamp->loadavg[1], tsamp->loadavg[2]);
...
static void
cpu_percent(unsigned long long ticks, unsigned long long totalticks, unsigned long long *whole,
...
static void
update_cpu(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	...
	gs->length = snprintf(gs->data, sizeof(gs->data),
			...
			"%" PRIu64 "."
			"%" PRIu64 "%% idle ",
			userwhole, userpart, syswhole, syspart, idlewhole, idlepart);
...
static void
update_sharedlibs(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	char code[6];
	char data[6];
	char linkedit[6];
	...
	if ((-1
				== humanize_number(
						code, sizeof(code), tsamp->fw_code, "", HN_AUTOSCALE, GLOBALSTAT_HN_FORMAT))
			|| (-1
					== humanize_number(data, sizeof(data), tsamp->fw_data, "", HN_AUTOSCALE,
							GLOBALSTAT_HN_FORMAT))
			|| (-1
					== humanize_number(linkedit, sizeof(linkedit), tsamp->fw_linkedit, "",
...
static void
update_memregions(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	char resident[6];
	char priv[6];
	char shared[6];
	...
	gs->length = snprintf(gs->data, sizeof(gs->data),
			...
			"%s private, "
			"%s shared.",
			tsamp->reg, resident, priv, shared);
	if (gs->length < 0) {
		reset_globalstat(gs);
		return;
	}
...
static void
update_physmem(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	char wired[6];
	char used[6];
	char physfree[6];
	...
	struct top_uinteger wiredresult, usedresult, physfreeresult, compressorresult;
	...
	wiredresult = top_init_uinteger(tsamp->vm_stat.wire_count * tsamp->pagesize, false);
	...
	gs->length = snprintf(gs->data, sizeof(gs->data),
			...
			"%s unused.",
			used, wired, compressor, physfree);
...
static void
update_vm(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	char vsize[6];
	char fwvsize[6];
	...
	gs->length = snprintf(gs->data, sizeof(gs->data),
			...
			vsize, fwvsize, (unsigned long long)tsamp->vm_stat.swapins,
			(unsigned long long)(tsamp->vm_stat.swapins - tsamp->p_vm_stat.swapins),
			(unsigned long long)tsamp->vm_stat.swapouts,
			(unsigned long long)(tsamp->vm_stat.swapouts - tsamp->p_vm_stat.swapouts));
...
static void
update_swap(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	...
	if (!tsamp->xsu_is_valid) {
		gs->length = snprintf(gs->data, sizeof(gs->data), "Swap: N/A");
	...
	gs->length = snprintf(gs->data, sizeof(gs->data), "Swap: %s + %s free.", used, avail);
...
static void
update_purgeable(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	struct top_uinteger purgeable_result, purges_result, prev_purges, purges_delta;
	char purgeable[6];
	char purges[6];
	char pdelta[6];
	...
	if (!tsamp->purgeable_is_valid) {
		gs->length = snprintf(gs->data, sizeof(gs->data), "Purgeable: N/A");
	...
	gs->length = snprintf(gs->data, sizeof(gs->data), "Purgeable: %s %s(%s) pages purged.",
			purgeable, purges, pdelta);
...
static bool
update_networks(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
	...
	gs->length = snprintf(gs->data, sizeof(gs->data),
			...
			"%s/%s in, "
			"%s/%s out.",
			inpbuf, inbytesbuf, outpbuf, outbytesbuf);
	if (-1 == gs->length) {
		reset_globalstat(gs);
		return true;
	}
...
static bool
update_disks(struct globalstat *gs, const libtop_tsamp_t *tsamp)
{
...
bool
top_globalstats_update(void *ptr, const void *sample)
{
	...
	if (!top_prefs_get_logging_mode()) {
		for (i = 0; i < GLOBALSTAT_TOTAL; ++i) {
			// MWW: If we've disabled framework logging one of these windows isn't valid and needs
			// skipping
			if (!top_prefs_get_frameworks() && (i == GLOBALSTAT_SHAREDLIBS))
				continue;
...
static void
setup_globalstats_geometry(
		struct globalstat_controller *c, int availwidth, int availheight, int *outheight)
{
	...
	*outheight = 0;
	...
	if (remaining >= gswidth) {
		...
		set_globalstat_geometry(gs, availwidth - gswidth, y, gswidth, /*height*/ 1);
...
bool
top_globalstats_resize(void *ptr, int width, int height, int *consumed_height)
{
	struct globalstat_controller *c = ptr;
	int i;
...
void
top_globalstats_iterate(void *ptr, bool (*iter)(char *, void *), void *dataptr)
{
	struct globalstat_controller *c = ptr;
	int i;
...
void
top_globalstats_reset(void *ptr)
{
	struct globalstat_controller *c = ptr;
	int i;
