...
static bool
pgrp_insert_cell(struct statistic *s, const void *sample)
{
	const libtop_psamp_t *psamp = sample;
	char buf[GENERIC_INT_SIZE];

	snprintf(buf, sizeof(buf), "%u", (unsigned int)psamp->pgrp);
...
static struct statistic_callbacks callbacks = { .draw = generic_draw,
	...
	.insert_cell = pgrp_insert_cell,
	.reset_insertion = generic_reset_insertion };

struct statistic *
top_pgrp_create(WINDOW *parent, const char *name)
{
	return create_statistic(STATISTIC_PGRP, parent, NULL, &callbacks, name);
}
