...
static void
reset_state(struct user_input_state *s)
{
	s->buf[0] = '\0';
	s->offset = 0;
}
...
static void
completion(void *tinst, struct user_input_state *s)
{
	reset_state(s);
	current_state = NULL;
}
...
static void
error_draw(void *tinst, struct user_input_state *s, WINDOW *win, int row, int column)
{
	char display[60];
...
static struct user_input_state error_state
		= { .offset = 0, .completion = completion, .draw = error_draw };

void
user_input_set_error_state(const char *err)
{
...
static void
status_draw(void *tinst, struct user_input_state *s, WINDOW *win, int row, int column)
{
	if (s->misc > 1) {
		s->completion(tinst, s);
		return;
...
static struct user_input_state status_state
		= { .offset = 0, .completion = completion, .draw = status_draw };

void
user_input_set_status_state(const char *status)
{
...
void
user_input_set_state(struct user_input_state *state)
{
	current_state = state;
}
...
void
user_input_set_position(int r, int c)
{
	row = r;
	column = c;
}
...
void
user_input_draw(void *tinst, WINDOW *win)
{
