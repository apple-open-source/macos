...
struct service {
	...
	/* Cellular context */
	CFMachPortRef			cellularPort;
	CFRunLoopSourceRef		cellularRLS;
...
CFStringRef		gBundleDir;
extern CFURLRef 	gIconURLRef;
CFStringRef		gIconDir;
...
int scnc_sendmsg(struct service *serv, uint32_t msg_type, CFDataRef cfdata, uid_t uid, gid_t gid, int pid, mach_port_t bootstrap);
