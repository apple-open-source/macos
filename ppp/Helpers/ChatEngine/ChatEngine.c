...
#ifndef lint
static const char rcsid[] = "$Id: ChatEngine.c,v 1.4 2000/12/15 21:29:13 callie Exp $";
#endif
...
#if 0
    // temporary, until configd handles SIGCHLD
    {
        int pid;
...
#define isalnumx(chr)	((((chr) >= '0') && ((chr) <= '9')) \
	...
	case 'd':
	    ...
	    *s1++ = cur_chr;
        ...
        case 'A':
            if (sending && username) {
                for (phchar = username; *phchar != '\0'; phchar++)
                    *s1++ = *phchar;
            }
            else {
                *s1++ = '\\';
        ...
        case 'P':
            if (sending && password) {
                for (phchar = password; *phchar != '\0'; phchar++)
                    *s1++ = *phchar;
            }
            else {
                *s1++ = '\\';
...
int c;
{
    ...
    //status = write(1, &ch, 1);
    sys_send_confd(csockfd, PPP_WRITEFD, &ch, 1, ppplink, 0);
    status = 1;
...
#ifdef APPLE
                if (ppplink != -1) {
                    sys_send_confd(csockfd, PPP_CCLNOTE, report_buffer, strlen(report_buffer), ppplink, 0);
                }
#endif
            }
	}
