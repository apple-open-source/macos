 ...
 */
extern OSStatus NSS_SMIMEUtil_EnableCipher(long which, int on);
 ...
 *    it is disabled.
 */
extern OSStatus NSS_SMIMEUtils_AllowCipher(long which, int on);

/*
 * Does the current policy allow S/MIME decryption of this particular
 * algorithm and keysize?
 */
extern PRBool NSS_SMIMEUtil_DecryptionAllowed(SECAlgorithmID *algid, SecSymmetricKeyRef key);
 ...
 */
extern OSStatus NSS_SMIMEUtil_CreateSMIMECapabilities(PLArenaPool *poolp, CSSM_DATA *dest, PRBool includeFortezzaCiphers);

/*
 * NSS_SMIMEUtil_CreateSMIMEEncKeyPrefs - create S/MIME encryption key preferences attr value
 */
extern OSStatus NSS_SMIMEUtil_CreateSMIMEEncKeyPrefs(PLArenaPool *poolp, CSSM_DATA *dest, SecCertificateRef cert);

/*
 * NSS_SMIMEUtil_CreateMSSMIMEEncKeyPrefs - create S/MIME encryption key preferences attr value using MS oid
 */
extern OSStatus NSS_SMIMEUtil_CreateMSSMIMEEncKeyPrefs(PLArenaPool *poolp, CSSM_DATA *dest, SecCertificateRef cert);

/*
 * NSS_SMIMEUtil_GetCertFromEncryptionKeyPreference - find cert marked by EncryptionKeyPreference
 *          attribute
 */
extern SecCertificateRef NSS_SMIMEUtil_GetCertFromEncryptionKeyPreference(SecKeychainRef keychainOrArray, CSSM_DATA *DERekp);

/*
 * NSS_SMIMEUtil_FindBulkAlgForRecipients - find bulk algorithm suitable for all recipients
 */
extern OSStatus
NSS_SMIMEUtil_FindBulkAlgForRecipients(SecCertificateRef *rcerts, SECOidTag *bulkalgtag, int *keysize);
