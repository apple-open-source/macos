...
SecCmsEncryptedDataCreate(SecCmsMessage *cmsg, SECOidTag algorithm, int keysize)
{
    ...
    encd = (SecCmsEncryptedData *)PORT_ArenaZAlloc(poolp, sizeof(SecCmsEncryptedData));
    if (encd == NULL)
	goto loser;
	...
	rv = SecCmsContentInfoSetContentEncAlg(poolp, &(encd->contentInfo), algorithm, NULL, keysize);
	break;
    default:
	...
	rv = SecCmsContentInfoSetContentEncAlgID(poolp, &(encd->contentInfo), pbe_algid, keysize);
...
SecCmsEncryptedDataEncodeBeforeStart(SecCmsEncryptedData *encd)
{
    ...
    CSSM_DATA *dummy;
    SecCmsContentInfo *cinfo = &(encd->contentInfo);
