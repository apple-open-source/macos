...
SecCmsDigestContextStartMultiple(SECAlgorithmID **digestalgs)
{
    ...
    cmsdigcx = (SecCmsDigestContext *)PORT_Alloc(sizeof(SecCmsDigestContext));
    if (cmsdigcx == NULL)
	return NULL;
			    ...
			    CSSM_DATA ***digestsp)
{
    ...
    digests = (CSSM_DATA **)PORT_ArenaAlloc(poolp, (cmsdigcx->digcnt+1) * sizeof(CSSM_DATA *));
    digest = (CSSM_DATA *)PORT_ArenaZAlloc(poolp, cmsdigcx->digcnt * sizeof(CSSM_DATA));
