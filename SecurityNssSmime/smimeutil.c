...
/*
 * XXX Would like the "parameters" field to be a CSSM_DATA *, but the
...
extern const SEC_ASN1Template SecCmsRecipientKeyIdentifierTemplate[];
...
PRBool
NSS_SMIMEUtil_DecryptionAllowed(SECAlgorithmID *algid, SecSymmetricKeyRef key)
{
    unsigned long which;
...
smime_choose_cipher(SecCertificateRef scert, SecCertificateRef *rcerts)
{
    ...
    PRBool scert_is_fortezza  = PR_FALSE;
#else
 ...
 */
OSStatus
NSS_SMIMEUtil_CreateSMIMECapabilities(PLArenaPool *poolp, CSSM_DATA *dest, PRBool includeFortezzaCiphers)
{
 ...
 */
OSStatus
NSS_SMIMEUtil_CreateSMIMEEncKeyPrefs(PLArenaPool *poolp, CSSM_DATA *dest, SecCertificateRef cert)
{
    NSSSMIMEEncryptionKeyPreference ekp;
    CSSM_DATA *dummy = NULL;
 ...
 */
OSStatus
NSS_SMIMEUtil_CreateMSSMIMEEncKeyPrefs(PLArenaPool *poolp, CSSM_DATA *dest, SecCertificateRef cert)
{
    CSSM_DATA *dummy = NULL;
    PLArenaPool *tmppoolp = NULL;
 ...
 */
SecCertificateRef 
NSS_SMIMEUtil_GetCertFromEncryptionKeyPreference(SecKeychainRef keychainOrArray, CSSM_DATA *DERekp)
{
    PLArenaPool *tmppoolp = NULL;
