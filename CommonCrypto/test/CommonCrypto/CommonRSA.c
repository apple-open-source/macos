...
static int roundTripCrypt(CCRSACryptorRef pubKey, CCRSACryptorRef privKey) {
    ...
    ok(bcmp(clear, decrypted, moved) == 0, "Results are what we started with");
    return bcmp(clear, decrypted, moved);
}
                 ...
                 char *pstr, char *qstr, char *mstr, char *dstr)
{
    ...
    byteBuffer xp1 = hexStringToBytes(xp1str);
    byteBuffer xp2 = hexStringToBytes(xp2str);
    byteBuffer xp = hexStringToBytes(xpstr);
    byteBuffer xq1 = hexStringToBytes(xq1str);
    byteBuffer xq2 = hexStringToBytes(xq2str);
    byteBuffer xq = hexStringToBytes(xqstr);
    byteBuffer p = hexStringToBytes(pstr);
    byteBuffer q = hexStringToBytes(qstr);
    byteBuffer m = hexStringToBytes(mstr);
    byteBuffer d = hexStringToBytes(dstr);
...
static void *bytesFromDecStr(char *input, size_t *len) {
    CCStatus status;
    CCBigNumRef bignum = CCBigNumFromDecimalString(&status, input);
    ok(status == kCCSuccess, "Converted to bignum from dec string");
    *len = (size_t) CCBigNumByteCount(bignum);
    ok(*len != 0, "Found length of output string");
    if(*len == 0) return NULL;
    uint8_t *retbuf = malloc(*len);
...
static int RSAKnownImport() {
    CCStatus status;
    CCRSACryptorRef cryptor = NULL;
    static char *exponentStr = "35";
    static char *modulusStr  = "16429076410450161535073874736106803575662859484726828345799060085155997358602738873383604680599890265682092905047970272950471894188202923717671050897685451030686968629320267204343975174846513660437543";
    
    size_t exponent_size    = 0;
    size_t modulus_size     = 0;
    uint8_t *exponent = bytesFromDecStr(exponentStr, &exponent_size);
    uint8_t *modulus  = bytesFromDecStr(modulusStr, &modulus_size);
    if(exponent && modulus) {
...
int CommonRSA (int argc, char *const *argv) {
    ...
    if(stdgen) {
        if(verbose) diag("Starting to generate keys stepping %lu\n", keystep);
        for(size_t keysize = 1024; keysize < 4097; keysize+=keystep) {
            if(verbose) diag("Generating %lu bit keypair\n", keysize);
    ...
    if(build931) {
        ...
        if(verbose) diag("Starting to build key from data\n");
