...
static int RSAKnownImport() {
    ...
    static uint8_t exp[] = {0x23};
    static uint8_t mod[] = {0x36, 0x2C, 0xC3, 0xDA, 0xC3, 0x0D, 0x03, 0x16, 0x3B, 0x1D, 0x70, 0x66, 0x6E, 0xF8, 0x0D, 0x76, 0xFC, 0x25, 0x2D, 0x4B, 0x0D, 0xE4, 0x79, 0x54, 0xBB, 0xF6, 0x65, 0xBC, 0x33, 0x92, 0x68, 0xB5, 0x30, 0x01, 0x99, 0x6F, 0x92, 0x9E, 0x58, 0x16, 0x2E, 0x27,};

    status = CCRSACryptorCreateFromData(ccRSAKeyPublic, mod, sizeof(mod), exp, 1, NULL, 0, NULL, 0, &cryptor);
...
int CommonRSA (int __unused argc, char *const * __unused argv) {
    ...
    if(build931) {
        ...
        if(verbose) diag("Build FIPS 186 key pair %d", i++);
