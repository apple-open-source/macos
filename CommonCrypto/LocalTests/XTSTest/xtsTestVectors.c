...
{
	char keyString[300], twkString[300];
    int ckLen, keyLength;
    uint8_t *key, *tweak, *iv;
    uint8_t *pt, *ct;
    ...
    if((retval = CCCryptorCreateWithMode(0, kCCModeXTS, kCCAlgorithmAES128, ccDefaultPadding, NULL, key, keyLength, tweak, keyLength, 0, 0,  &encCryptorRef)) == kCCSuccess) {
        if(direction == ENCRYPT) {
            if((retval = CCCryptorEncryptDataBlock(encCryptorRef, iv, pt, dataLen, dataOut)) == kCCSuccess) {
                if (memcmp(ct, dataOut, dataLen)) printf("Output %s\nExpect %s\n", bytesToHexString((uint8_t *) dataOut, dataLen), /*bytesToHexString(ct, dataLen)*/ cipherText);
                else passed = 1;
            } else  printf("Failed to encrypt\n", retval);
        } else {
            if((retval = CCCryptorDecryptDataBlock(encCryptorRef, iv, ct, dataLen, dataOut)) == kCCSuccess) {
                if (memcmp(pt, dataOut, dataLen)) printf("Output %s\nExpect %s\n", bytesToHexString((uint8_t *) dataOut, dataLen), /* bytesToHexString(pt, dataLen)*/ plainText);
                else passed = 1;
    ...
    printf("KEY %s\n", keyString);
    printf("key %s\n", bytesToHexString(key, keyLength));
    printf("TWK %s\n", twkString);
    printf("twk %s\n", bytesToHexString(tweak, keyLength));
    printf("IV %s\n", ivStr);
    printf("iv %s\n", bytesToHexString(iv, strlen(ivStr)/2));
#endif
    free(pt);
    free(ct);
    free(key);
