...
bool KeyLargo::start(IOService *provider)
{
	...
	if (keyLargoDeviceId == (kPangeaDeviceId25 | kIntrepidDeviceId3e)) 
...
void KeyLargo::turnOffIntrepidIO(bool restart)
{
    UInt32 				regTemp;
	IOInterruptState	intState;
	bool				usingSCCA;
...
#define PRINTNOT(n) (n) ? "" : "NOT"
	DLOG ("KeyLargo::setReferenceCounts - chooseSCCA %s and %s enabled\n", 
		PRINTBOOL(chooseSCCA), PRINTNOT(fcr0 & kKeyLargoFCR0SccAEnable));
	DLOG ("KeyLargo::setReferenceCounts - chooseSCCB %s and %s enabled\n", 
		PRINTBOOL(chooseSCCB), PRINTNOT(fcr0 & kKeyLargoFCR0SccBEnable));
	DLOG ("KeyLargo::setReferenceCounts - chooseI2S0 %s and %s enabled\n", 
		PRINTBOOL(chooseI2S0), PRINTNOT(fcr1 & kKeyLargoFCR1I2S0Enable));
	DLOG ("KeyLargo::setReferenceCounts - chooseI2S1 %s and %s enabled\n", 
		PRINTBOOL(chooseI2S1), PRINTNOT(fcr1 & kKeyLargoFCR1I2S1Enable));
	DLOG ("KeyLargo::setReferenceCounts - chooseAudio %s and %s enabled\n", 
		PRINTBOOL(chooseAudio), PRINTNOT(fcr1 & kKeyLargoFCR1AudioCellEnable));
	DLOG ("KeyLargo::setReferenceCounts - VIA %s enabled\n", 
		PRINTNOT(fcr3 & kKeyLargoFCR3ViaClk16Enable));
	DLOG ("KeyLargo::setReferenceCounts - clk31RefCount = %d, clk45RefCount = %d, clk49RefCount = %d\n",
...
void KeyLargo::saveVIAState(void)
{
    ...
    UInt8* savedKeyLargoViaState = savedKeyLargoState.savedVIAState;

    // Save VIA state.  These registers don't seem to get restored to any known state.
    savedKeyLargoViaState[0] = *(UInt8*)(viaBase + kKeyLargovBufA);
    savedKeyLargoViaState[1] = *(UInt8*)(viaBase + kKeyLargovDIRA);
    savedKeyLargoViaState[2] = *(UInt8*)(viaBase + kKeyLargovBufB);
    savedKeyLargoViaState[3] = *(UInt8*)(viaBase + kKeyLargovDIRB);
    savedKeyLargoViaState[4] = *(UInt8*)(viaBase + kKeyLargovPCR);
    savedKeyLargoViaState[5] = *(UInt8*)(viaBase + kKeyLargovACR);
    savedKeyLargoViaState[6] = *(UInt8*)(viaBase + kKeyLargovIER);
    savedKeyLargoViaState[7] = *(UInt8*)(viaBase + kKeyLargovT1C);
    savedKeyLargoViaState[8] = *(UInt8*)(viaBase + kKeyLargovT1CH);
...
void KeyLargo::restoreVIAState(void)
{
    ...
    *(UInt8*)(viaBase + kKeyLargovBufA) = savedKeyLargoViaState[0];
    eieio();
    *(UInt8*)(viaBase + kKeyLargovDIRA) = savedKeyLargoViaState[1];
    eieio();
    *(UInt8*)(viaBase + kKeyLargovBufB) = savedKeyLargoViaState[2];
    eieio();
    *(UInt8*)(viaBase + kKeyLargovDIRB) = savedKeyLargoViaState[3];
    eieio();
    *(UInt8*)(viaBase + kKeyLargovPCR) = savedKeyLargoViaState[4];
    eieio();
    *(UInt8*)(viaBase + kKeyLargovACR) = savedKeyLargoViaState[5];
    eieio();
    *(UInt8*)(viaBase + kKeyLargovIER) = savedKeyLargoViaState[6];
    eieio();
    *(UInt8*)(viaBase + kKeyLargovT1C) = savedKeyLargoViaState[7];
    eieio();
    *(UInt8*)(viaBase + kKeyLargovT1CH) = savedKeyLargoViaState[8];
    eieio();
...
IOReturn KeyLargo::callPlatformFunction( const char * functionName,
					  bool waitForFunction,
					  void *param1, void *param2,
					  void *param3, void *param4 )
{
...
#if DEBUGREFCOUNTS
		DLOG ("KeyLargo::EnableSCC (enable) - clk31RefCount = %d, clk45RefCount = %d, clk49RefCount = %d\n",
			clk31RefCount, clk45RefCount, clk49RefCount);
#endif
    } else {	// Powering down
        if(device == 0)
...
#if DEBUGREFCOUNTS
		DLOG ("KeyLargo::EnableSCC (disable) - clk31RefCount = %d, clk45RefCount = %d, clk49RefCount = %d\n",
			clk31RefCount, clk45RefCount, clk49RefCount);
#endif
    }
